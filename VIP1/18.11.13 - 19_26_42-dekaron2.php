...
function LoadVIPPrediction()
	...
	tpQ = wp:AddProdictionObject(_Q, rangeQ, 2500, 0.25, nil, myHero, 
	        function(unit, pos, castspell) 
                if GetDistance(pos) < rangeQ and myHero:CanUseSpell(_Q) == READY then 
					CastSpell(_Q, pos.x, pos.z)
                end 
            end)
	tpQC = wp:AddProdictionObject(_Q, rangeW, 2500, 0.25, nil, obj, 
	        function(unit, pos, castspell) 
                if GetDistance(pos, obj) < rangeQ and myHero:CanUseSpell(_Q) == READY then 
					CastSpell(_Q, pos.x, pos.z)
                end 
            end)
end
...
function OnDraw()
	if markedTarget then
		DrawCircle(markedTarget.x, markedTarget.y, markedTarget.z, 100, ARGB(244,66,155,255))
	end
	if not myHero.dead then
