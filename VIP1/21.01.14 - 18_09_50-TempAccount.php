...
function Var()
	...
	qRange, eRange, rRange = 425, 540, 460
	passivedmg = nil
end
...
function Menu()
	...
	DariusMenu.combo:addParam("t", "Use Tiamat as AA Animation Cancel", SCRIPT_PARAM_ONOFF, true)
	DariusMenu.combo:addParam("h", "Use Tiamat as AA Animation Cancel", SCRIPT_PARAM_ONOFF, true)
		...
		for i = 1, heroManager.iCount, 1 do
			...
			if ValidTarget(target) then
				...
				if target ~= nil and target.team ~= myHero.team and not target.dead and target.visible and GetDistance(target) < 600 and target.health+10 < iDmg then
					if MorganaMenu.ks.ignite and not TargetHaveBuff("SummonerDot", target) then
...
function OnTick()
	...
	--if DariusMenu.combo.fullcombo then W() end
	...
	if DariusMenu.combo.fullcombo and DariusMenu.combo.e then
		if EREADY and ts.target ~= nil and ValidTarget(ts.target) and ts.target.visible == true and myHero.dead == false then
			if GetDistance(ts.target) < eRange and GetDistance(ts.target) > 150 then
