...
function MenuDraw()
    ...
    menu.draw:addParam("targeteng", "(Q) Engage Range", SCRIPT_PARAM_ONOFF, false)
...
--   dp:containsFloat(Target.networkID) and p.header == 0xFE and menu.combo.key then -- 0xFE --0x64 -- 0xC3
       ...
       -- if MainTargetHitChance >= qChance_ then
         ...
         --  end
          if MainTargetHitChance >= qChance_ and GetTickCount() + GetLatency()/2 - qTick > (1000/myPlayer.attackSpeed) - GetLatency()/2 then
           CastSpell(_Q, AOECastPosition.x, AOECastPosition.z)
          elseif MainTargetHitChance >= 1 then
            CastSpell(_Q, AOECastPosition.x, AOECastPosition.z)
          end
       -- end

      elseif menu.comboextra.range.useNew == 1 and MainTargetHitChance >= qChance_ and GetTickCount() + GetLatency()/2 - qTick > (1000/myPlayer.attackSpeed) - GetLatency()/2 then
          CastSpell(_Q, AOECastPosition.x, AOECastPosition.z)
      elseif MainTargetHitChance >= 1 then
            CastSpell(_Q, AOECastPosition.x, AOECastPosition.z)
...
function OnApplyParticle(unit, particle)
  if ValidTarget(unit) and ValidTarget(Target) then 
    if unit.networkID == Target.networkID then
      ...
      if particle.name == "Riven_Base_P_Buff.troy" then myPlayer:Attack(Target) end
    end
  end 
  --if unit.isMe then print(particle.name) end
end
  ...
  for i=1, #championsT do
    if myTarget.charName:find(championsT[i]) and GetDistance(myTarget) <= GetCustomRange(_Q) then
      ...
      if championsT[i] == "Jax" then
        ...
        --if JaxRcd > 1 then Combo(myTarget) end
        ...
        if ComboON then
            ...
            --if ThisIsReal(Target) <= myPlayer.range and Passive then myPlayer:Attack(Target) end
              if qr_ and user then
                ...
                if qe_ and usee and not lockE then
                  CastE(Target)
                end
...
function LineFarm()  
    ...
    if ValidTarget(minionLane) then
     if mode_ == 2 then myPlayer:Attack(minionLane) end                  
        ...
        if eLane_ and not UnderTurret(Minion) then
...
function F5Target()
  ...
  if Selector_menu and _G.Selector_Enabled then return Selector.GetTarget() end
...
if ValidTarget(myTarget) and TargetIsValid(myTarget) and GetDistance(myTarget) <= rangeToTarget then
  ...
  --end
	...
	if qDmg+wDmg+rDmg+iDmg < myTarget.health then KillText = "Can't Kill" end
...
function DamageCombo(myTarget)
  ...
  CastE(myTarget)
    ...
    -- CastE(myTarget)
    -- CastW(myTarget) 
  elseif qDmg == 0 or qDmg*3 < myTarget.health then
    lockUltimate = false 
   -- lockPacket = false
    lockIgnite = false  
	  CastQ(myTarget)
