...
botBought=(_G["linker"] or devMode or _G.C_UNIT_INVISIBLE)
Load_Items=(_G.Load_Items or devMode or _G.C_UNIT_INVISIBLE) and FileExist(dominionCfg_PATH)
if _G.CHECKDC then
	CHECK_DC = _G.CHECKDC
else
	CHECK_DC = false
...
--if GetGame().map.index ~= 8 then PrintChat("KevinBot - wrong map -"..tostring(GetGame().map.index)) PrintChat("<font color='#FFFFFF'>KBOT INACTIVE</font>") return else PrintChat("<font color='#FFFFFF'>KBOT ACTIVE</font>") end
...
--Map Check
	...
	function initializeCombos()
		function moveToIdeal() -- Removes derping when mouse is in one position instead of myHero:MoveTo mousePos
			...
			if myRange > 300 and ((tsHPP > 0.5 and GetDistance(ts.target,myHero) < (2/3)*myRange) or myHero.health/myHero.maxHealth < tsHPP) then 
				...
				heroMove(makeDistance(myHero,ts.target,1100))
	...
	--C
		function CaitlynCombo()
            ...
            if EReady and myHero.health < ts.target.health then CastSpell(_E,ts.target.x,ts.target.z) end		
            if RReady and getDmg("R",ts.target,myHero) > ts.target.health + 20 then CastSpell(_R,ts.target) end			
            orbwalk()
		end
	--	
	...
	--	
		...
		function DrMundoCombo()
            ...
            WReady = (myHero:CanUseSpell(_W) == READY)
            EReady = (myHero:CanUseSpell(_E) == READY and distanceToEnemy < eRange)
            RReady = (myHero:CanUseSpell(_R) == READY and myHero.health < (myHero.maxHealth * 0.2))
	...
	--R
		...
		function RyzeCombo()
			...
			if not myHero:CanUseSpell(_Q) == READY and not myHero:CanUseSpell(_W) == READY and not myHero:CanUseSpell(_E) == READY then
			orbwalk()
			end
		end
	--S	
	...
	--W	
		function WarwickCombo()
			if not myHero.dead and ts.target ~= nil then
				...
				if RReady and not QReady and getDmg("R",ts.target,myHero) then CastSpell(_R) end
	...
	function LoadChampionDetails() 
		...
		local AD_CARRY = {"Berserker's Greaves", "Sanguine Blade","Infinity Edge","Frozen Mallet","Last Whisper","Runaan's Hurricane"}
		...
		champions = {
			...
			levelSequence = getSkillOrder("WQE"),
			items = {"Berserker's Greaves","Sanguine Blade","Infinity Edge","Phantom Dancer","Frozen Mallet","Guardian Angel"},
			tsRange = 1150,
			damageType = DAMAGE_PHYSICAL,
			...
			levelSequence = getSkillOrder("QWE"),
			items = {"Berserker's Greaves", "Sanguine Blade", "Phantom Dancer", "Infinity Edge", "Black Cleaver", "Runaan's Hurricane"},
			tsRange = 1150,
			damageType = DAMAGE_PHYSICAL,
			action = "SivirCombo",
			extra = "SivirObject"
			...
			levelSequence = getSkillOrder("WQE"),
			items = {"Berserker's Greaves","Phantom Dancer","Infinity Edge","Sanguine Blade","Zephyr","Frozen Mallet"},
			tsRange = 1150,
			damageType = DAMAGE_PHYSICAL,
			action = "VayneCombo",
			extra = "VayneObject"
	...
	function battle_retreat()
		...
		if math.sqrt((ts.target.x-6920)^2+(ts.target.z-6800)^2)<1000 then
			heroMove(heroBase.x,heroBase.z) 
		else
	...
	function SmartMove(x,z)
		...
		if baseDistance > heroDistance or baseDistance > heroShrineDistance then
			...
			if heroDistance < heroShrineDistance or hasSpeedBuff then
				if captureCount() >= 3 and farmMinions() then return else
	...
	function Cast(key,spellType,x,z)
	...
	function BuyItems()
        local buyPot = true
        if getFreeSlots() <= 2 then buyPot = false SellItemId(2003) end
	...
	itemTable = {
		...
		--Tier1 stuff (except vamp scepter but cmon...)
		...
		{name = "B. F. Sword", id = 1038, requires = {}},
		{name = "Hunter's Machete", id = 1039, requires = {}},
		...
		{name = "Doran's Blade", id = 1055, requires = {}},
		{name = "Doran's Ring", id = 1056, requires = {}},
		{name = "Negatron Cloak", id = 1057, requires = {}},
		{name = "Needlessly Large Rod", id = 1058, requires = {}},
		{name = "Prospector's Blade", id = 1062, requires = {}},
		{name = "Prospector's Ring", id = 1063, requires = {}},

		--Tier higher Stuff that needs requirements

		{name = "Abyssal Scepter", id = 3001, requires = {"Blasting Wand", "Negatron Cloak"}},
		{name = "Archangel's Staff", id = 3007, requires = {"Blasting Wand", "Tear of the Goddess"}},
		{name = "Manamune", id = 3008, requires = {"Long Sword", "Tear of the Goddess"}},
		...
		{name = "Catalyst the Protector", id = 3010, requires = {"Sapphire Crystal", "Ruby Crystal"}},
		{name = "Sorcerer's Shoes", id = 3020, requires = {"Boots of Speed"}},
		{name = "Frozen Mallet", id = 3022, requires = {"Giant's Belt", "Ruby Crystal","Pickaxe"}},
		{name = "Twin Shadows", id = 3023, requires = {"Kage's Lucky Pick", "Null-Magic Mantle"}},
		{name = "Glacial Shroud", id = 3024, requires = {"Chain Vest", "Sapphire Crystal"}},
		{name = "Iceborn Gauntlet", id = 3025, requires = {"Sheen","Glacial Shroud"}},
		{name = "Guardian Angel", id = 3026, requires = {"Chain Vest", "Negatron Cloak"}},
		{name = "Rod of Ages", id = 3029, requires = {"Catalyst the Protector", "Blasting Wand"}},
		{name = "Chalice of Harmony", id = 3028, requires = {"Faerie Charm","Faerie Charm", "Null-Magic Mantle"}},
		{name = "Infinity Edge", id = 3031, requires = {"B. F. Sword", "Pickaxe", "Cloak of Agility"}},
		{name = "Last Whisper", id = 3035, requires = {"Pickaxe", "Long Sword"}},
		{name = "Mana Manipulator", id = 3037, requires = {"Faerie Charm", "Faerie Charm"}},
		{name = "Mejai's Soulstealer", id = 3041, requires = {"Amplifying Tome"}},
		{name = "Phage", id = 3044, requires = {"Ruby Crystal", "Long Sword"}},
		{name = "Phantom Dancer", id = 3046, requires = {"Zeal", "Dagger","Cloak of Agility"}},
		...
		{name = "Ohmwrecker", id = 3056, requires = {"Ruby Crystal", "Blasting Wand","Philosopher's Stone"}},
		{name = "Sheen", id = 3057, requires = {"Sapphire Crystal","Amplifying Tome"}},
		{name = "Banner of Command", id = 3060, requires = {"Fiendish Codex", "Emblem of Valor"}},
        {name = "Spectre's Cowl", id = 3211, requires = {"Ruby Crystal", "Negatron Cloak"}},
		{name = "Spirit Visage", id = 3065, requires = {"Kindlegem", "Spectre's Cowl"}},
		{name = "Kindlegem", id = 3067, requires = {"Ruby Crystal"}},
		{name = "Sunfire Cape", id = 3068, requires = {"Giant's Belt", "Chain Vest"}},
		{name = "Shurelya's Reverie", id = 3069, requires = {"Philosopher's Stone", "Kindlegem"}},
		...
		{name = "Bloodthirster", id = 3072, requires = {"B. F. Sword", "Vampiric Scepter"}},
		{name = "Ravenous Hydra", id = 3074, requires = {"Vampiric Scepter", "Tiamat"}},
		{name = "Thornmail", id = 3075, requires = {"Chain Vest", "Cloth Armor"}},
		{name = "Tiamat", id = 3077, requires = {"Pickaxe", "Long Sword", "Rejuvenation Bead", "Rejuvenation Bead"}},
		{name = "Trinity Force", id = 3078, requires = {"Phage","Sheen","Zeal"}},
		{name = "Warden's Mail", id = 3082, requires = {"Cloth Armor", "Cloth Armor"}},
		{name = "Warmog's Armor", id = 3083, requires = {"Giant's Belt", "Ruby Crystal", "Rejuvenation Bead"}},
		{name = "Overlord's Bloodmail", id = 3084, requires = {"Giant's Belt", "Ruby Crystal"}},
		{name = "Runaan's Hurricane", id = 3085, requires = {"Recurve Bow", "Dagger", "Dagger"}},
		{name = "Zeal", id = 3086, requires = {"Dagger","Brawler's Gloves"}},
		{name = "Statikk Shiv", id = 3087, requires = {"Zeal", "Avarice Blade",}},
		{name = "Rabadon's Deathcap", id = 3089, requires = {"Needlessly Large Rod", "Blasting Wand"}},
		{name = "Wooglet's Witchcap", id = 3090, requires = {"Blasting Wand", "Seeker's Armguard", "Amplifying Tome"}},
		{name = "Seeker's Armguard", id = 3191, requires = {"Amplifying Tome", "Cloth Armor", "Cloth Armor"}},
		{name = "Wit's End", id = 3091, requires = {"Recurve Bow", "Null-Magic Mantle","Dagger"}},
		{name = "Shard of True Ice", id = 3092, requires = {"Kage's Lucky Pick", "Mana Manipulator", }},
		{name = "Avarice Blade", id = 3093, requires = {"Brawler's Gloves"}},
		{name = "Philosopher's Stone", id = 3096, requires = {"Rejuvenation Bead", "Faerie Charm"}},
		{name = "Emblem of Valor", id = 3097, requires = {"Cloth Armor", "Rejuvenation Bead"}},
		{name = "Kage's Lucky Pick", id = 3098, requires = {"Amplifying Tome"}},
		{name = "Lich Bane", id = 3100, requires = {"Sheen", "Blasting Wand"}},
		{name = "Stinger", id = 3101, requires = {"Dagger", "Dagger"}},
		{name = "Banshee's Veil", id = 3102, requires = {"Ruby Crystal", "Spectre's Cowl"}},
		{name = "Lord Van Damm's Pillager", id = 3104, requires = {"Heart of Gold", "The Brutalizer"}},
		{name = "Aegis of the Legion", id = 3105, requires = {"Emblem of Valor", "Ruby Crystal", "Null-Magic Mantle"}},
		{name = "Madred's Razors", id = 3106, requires = {"Hunter's Machete", "Cloth Armor"}},
		{name = "Runic Bulwark", id = 3107, requires = {"Null-Magic Mantle", "Aegis of the Legion"}},
		{name = "Fiendish Codex", id = 3108, requires = {"Amplifying Tome"}},
		{name = "Frozen Heart", id = 3110, requires = {"Warden's Mail", "Glacial Shroud"}},
		{name = "Mercury's Treads", id = 3111, requires = {"Null-Magic Mantle", "Boots of Speed"}},
		{name = "Malady", id = 3114, requires = {"Dagger", "Dagger", "Amplifying Tome"}},
		{name = "Nashor's Tooth", id = 3115, requires = {"Fiendish Codex", "Stinger"}},
		{name = "Rylai's Crystal Scepter", id = 3116, requires = {"Amplifying Tome", "Blasting Wand", "Giant's Belt"}},
		{name = "Boots of Mobility", id = 3117, requires = {"Boots of Speed"}},
		{name = "Wicked Hatchet", id = 3122, requires = {"Cloak of Agility", "Long Sword"}},
		...
		{name = "Madred's Bloodrazor", id = 3126, requires = {"Cloth Armor", "Pickaxe", "Recurve Bow"}},
		{name = "Deathfire Grasp", id = 3128, requires = {"Needlessly Large Rod", "Fiendish Codex"}},
		{name = "Sword of the Divine", id = 3131, requires = {"Recurve Bow", "Dagger"}},
		{name = "Brutalizer", id = 3134, requires = {"Long Sword", "Long Sword"}},
		{name = "Void Staff", id = 3135, requires = {"Amplifying Tome", "Blasting Wand"}},
		{name = "Haunting Guise", id = 3136, requires = {"Amplifying Tome", "Ruby Crystal"}},
		{name = "Mercurial Scimitar", id = 3139, requires = {"Quicksilver Sash", "B. F. Sword"}},
		{name = "Quicksilver Sash", id = 3140, requires = {"Negatron Cloak"}},
		{name = "Sword of the Occult", id = 3141, requires = {"Long Sword"}},
		{name = "Youmuu's Ghostblade", id = 3142, requires = {"Avarice Blade", "The Brutalizer"}},
		...
		{name = "Liandry's Torment", id = 3151, requires = {"Amplifying Tome", "Haunting Guise"}},
		{name = "Will of the Ancients", id = 3152, requires = {"Hextech Revolver", "Kage's Lucky Pick"}},
		{name = "Blade of the Ruined King", id = 3153, requires = {"Dagger","Bilgewater Cutlass","Dagger"}},
		{name = "Wriggle's Lantern", id = 3154, requires = {"Madred's Razors", "Vampiric Scepter"}},
		{name = "Hexdrinker", id = 3155, requires = {"Long Sword", "Null-Magic Mantle"}},
		{name = "Maw of Malmortius", id = 3156, requires = {"Hexdrinker", "Pickaxe"}},
		{name = "Zhonya's Hourglass", id = 3157, requires = {"Needlessly Large Rod", "Chain Vest"}},
		...
		{name = "Morellonomicon", id = 3165, requires = {"Faerie Charm", "Fiendish Codex", "Kage's Lucky Pick"}},
		{name = "Bonetooth Necklace", id = 3166, requires = {}},
		{name = "Zephyr", id = 3172, requires = {"Stinger", "Pickaxe"}},
		{name = "Eleisa's Miracle", id = 3173, requires = {"Philosopher's Stone"}},
		...
		{name = "Sanguine Blade", id = 3181, requires = {"Vampiric Scepter", "Pickaxe"}},
		{name = "Entropy", id = 3184, requires = {"Phage", "B. F. Sword"}},
		{name = "Lightbringer", id = 3185, requires = {"Grez's Spectral Lantern", "Pickaxe"}},
		{name = "Kitae's Bloodrazor", id = 3186, requires = {"Pickaxe", "Recurve Bow"}},
		{name = "Hextech Sweeper", id = 3187, requires = {"Amplifying Tome", "Amplifying Tome", "Kindlegem"}},
		...
		{name = "Locket of the Iron Solari", id = 3190, requires = {"Kindlegem", "Cloth Armor", "Rejuvenation Bead"}},
		{name = "Augment: Power", id = 3196, requires = {"The Hex Core"}},
		{name = "Augment: Gravity", id = 3197, requires = {"The Hex Core"}},
		{name = "Augment: Death", id = 3198, requires = {"The Hex Core"}},
		{name = "Hex Core", id = 3200, requires = {}},
		{name = "Spirit of the Spectral Wraith", id = 3206, requires = {"Spirit Stone", "Pickaxe"}},
		{name = "Spirit of the Ancient Golem", id = 3207, requires = {"Kindlegem", "Spirit Stone"}},
		{name = "Spirit of the Elder Lizard", id = 3209, requires = {"Spirit Stone", "Long Sword","Long Sword"}},
		{name = "Mikael's Crucible", id = 3222, requires = {"Chalice of Harmony", "Sapphire Crystal"}},
	...
	function LoadItemss()
        ...
        if Load_Items == true then
            ...
            for k,v in pairs(config.SKILLS) do
				n=n+1
				champion_skills_ff[n]={k:gsub(' ',''):gsub("'",''):gsub("Wukong","MonkeyKing"):gsub("Dr.Mundo","DrMundo"),k}
			end
            ]]
            
            
			table.sort(champion_names_from_file[1])
            --table.sort(champion_skills_ff[1])
	...
	function OnLoad()
		...
		PrintChat("KBot["..mapName.."]_"..isBeta().."_ Loaded <" .. myHero.charName .. ">")
		dom_load = true
    ...
    function dominionLogic()
		...
		--Old farm minion position
		if captureCount() > 4 then state = "winning" end
