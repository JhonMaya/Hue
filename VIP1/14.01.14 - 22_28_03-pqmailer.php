<?php exit() ?>--by pqmailer 217.82.54.68
local Version = 0.12
local Config
local ts
local SpellData = {}
...
function OnLoad()
	...
	SpellData = {
		...
		E = {Range = 900, Speed = 900, Delay = 1, Width = 200},
...
function OnProcessSpell(unit, spell)
	if unit.isMe and spell.name:lower():find("attack") then
		...
		if Target then
			...
			if (Config.combo.useW and Config.combo.teamfight and GetDistance(Target) >= Config.combo.useWrange) or (Config.harass.useW and Config.harass.poke and ManaCheck()) and myHero:CanUseSpell(_W) == READY and GetDistance(Target) <= SpellData.W.Range then
				DelayAction(function()
					tpW:GetPredictionCallBack(Target, CastW)
				end, spell.windUpTime-GetLatency()/1000)
			end
...
function CastW(unit, pos, spell)
	if ValidTarget(unit) and pos ~= nil and GetDistance(unit) <= SpellData.W.Range and myHero:CanUseSpell(_W) == READY and not wCol:GetMinionCollision(pos, myHero) then
