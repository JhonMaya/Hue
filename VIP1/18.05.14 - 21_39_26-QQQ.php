...
---------------------------------------------------------------------
local UPDATE_FILE_PATH = SCRIPT_PATH.."Evelynn - The Devil wears Prada.lua"
...
if _G.EVEAUTOUPDATE then
    ...
    if ServerData then
        local ServerVersion = string.match(ServerData, "_G.EvelynnVersion = \"%d+.%d+\"")
        ...
        if ServerVersion then
            ...
            if tonumber(_G.EvelynnVersion) < ServerVersion then
				...
				AutoupdaterMsg("Successfully updated! (".._G.EvelynnVersion.." -> "..ServerVersion.."), Please reload (double [F9]) for the updated version!") end) end, 3)
            else
...
function OnLoad()
	...
	PrintChat("<font color=\"#FF7373\">[".._G.IsLoaded.."]:</font><font color=\"#FFDFBF\"> Sucessfully loaded! Version: [<u><b>".._G.EvelynnVersion.."</b></u>]</font>")
end
---------------------------------------------------------------------
--- Menu ------------------------------------------------------------
---------------------------------------------------------------------
function AddMenu()
	...
	-- Other --
	EvelynnMenu:addParam("Version", "Version", SCRIPT_PARAM_INFO, _G.EvelynnVersion)
	EvelynnMenu:addParam("Author", "Author", SCRIPT_PARAM_INFO, _G.EvelynnAuthor)
end
---------------------------------------------------------------------
--- On Tick ---------------------------------------------------------
---------------------------------------------------------------------
...
function Check()
	...
	-- Check if items are ready -- 
		...
		bftReady		= (bftSlot		~= nil and myHero:CanUseSpell(bftSlot)		== READY) -- Blackfiretorch
		odvReady		= (odvSlot		~= nil and myHero:CanUseSpell(odvSlot)		== READY) -- Odyns Veil
		bftReady		= (bftSlot		~= nil and myHero:CanUseSpell(bftSlot)		== READY) -- Blackfiretorch
		entReady		= (entSlot		~= nil and myHero:CanUseSpell(entSlot)		== READY) -- Entropy
	...
	-- Set the slots for item --
		...
		odvSlot			= GetInventorySlotItem(3180)
...
function UseItems()
	...
	if ValidTarget(enemy) then
		...
		if bftReady		and GetDistance(enemy) <= 750 then CastSpell(bftSlot, enemy) end
		if odvReady		and GetDistance(enemy) <= 525 then CastSpell(odvSlot) end
		if entReady		and GetDistance(enemy) <= 350 then CastSpell(entSlot) end
...
function AimTheR(enemy)
	local AoeCastPosition, MainTargetHitChance = VP:GetCircularAOECastPosition(enemy, rDelay, rWidth, rRange, rSpeed, myHero)
	if MainTargetHitChance >= 2 and GetDistance(enemy) <= rRange and RREADY
...
function AimTheRonX(enemy)
	if RREADY then
		...
		if MainTargetHitChance >= 2 and GetDistance(enemy) <= rRange and nTargets >= Mintargets
			then CastSpell(_R, AoeCastPosition.x, AoeCastPosition.z)
		end
	end
end
...
function CheckHitEnemys()
	...
	for i, enemy in ipairs(GetEnemyHeroes()) do
		local position = VP:GetPredictedPos(enemy, rDelay, rSpeed)
		if ValidTarget(enemy) and GetDistance(enemy, position) <= rWidth and GetDistance(enemy.visionPos, myHero) <= rRange-100 then
