<?php exit() ?>--by dekaron2 86.161.53.40
local QRange = 600 -- (actually 575)
local WRange = 500 -- (actually 475)
...
function OnLoad()
	...
	FHConfig.itemsettings:addParam("woo", "Use Zhonya/Wooglets on ult", SCRIPT_PARAM_ONOFF, false)
...
function OnTick()
	...
	edamage = math.floor(((((((myHero:GetSpellData(_Q).level-1) * 20) + 65) + .45 * myHero.ap))))
	wooslot = GetInventorySlotItem(3090)
	wooready = (wooslot ~= nil and myHero:CanUseSpell(wooslot) == READY)
	zhoslot = GetInventorySlotItem(3157)
	zhoready = (zhoslot ~= nil and myHero:CanUseSpell(zhoslot) == READY)
	...
	if ultActive and ts.target ~= nil then
		...
		if FHConfig.itemsettings.woo and (wooready or zhoready) then
			if wooready then CastSpell(wooslot) end
			if zhoready then CastSpell(zhoslot) end
		end
	end
...
function OnSendPacket(packet)
	if packet.header == 113 then 
   		local AApacket = {}
   		AApacket.dwArg1 = packet.dwArg1
   		AApacket.dwArg2 = packet.dwArg2
   		AApacket.sourceNetworkId = packet:DecodeF()
   		AApacket.type = packet:Decode1()

   		if AApacket.type == 2 and FHConfig.combosettings.comboActive and Draining then
   			packet:Block()
   		elseif AApacket.type == 3 and FHConfig.combosettings.comboActive and Draining then
   			packet:Block()
   		end
   end
end
...
function AutoKS()
	if FHConfig.kssettings.autoKS and ts.target ~= nil and eready and ValidTarget(ts.target, 750) then
...
function Combo()
	if FHConfig.combosettings.comboActive and ts.target ~= nil then
		if qready and ValidTarget(ts.target, 575) and not Draining then 
			CastExploit(_Q, ts.target)
		end

		if Draining then
		elseif wready and not Draining and ValidTarget(ts.target, 475) then
