...
function OnDraw()
	if ts.target then
		...
		local closestPoint, cUnit = findClosestPoint(focalPoint)
		if cUnit and dashOut then
			DrawCircle(cUnit.x, cUnit.y, cUnit.z, 50, 0xFF0000)
			DrawCircle(cUnit.x, cUnit.y, cUnit.z, 55, 0xFF0000)
			DrawCircle(cUnit.x, cUnit.y, cUnit.z, 60, 0xFF0000)
			DrawCircle(cUnit.x, cUnit.y, cUnit.z, 65, 0xFF0000)
			DrawCircle(cUnit.x, cUnit.y, cUnit.z, 70, 0xFF0000)
		end
		if minion and minion.valid and not minion.dead and GetDistance(minion) <= eRange then
			local wPred = myHero + (Vector(minion) - myHero):normalized()*eRange
			if wPred then
...
function OnDash(unit, dash)
	if unit.isMe and YasuoConfig.Basic.basicCombo and not knockUpReady then
...
--[[
			if YasuoConfig.Basic.basicCombo and YasuoConfig.Basic.useR and ((ts.target and unit == ts.target) or (es.target and unit == es.target)) then
				CastSpell(_R)
			elseif YasuoConfig.Auto.autoR and countKnockedUp(unit, 400) >= YasuoConfig.Auto.autoREnemies then
				CastSpell(_R)
			elseif YasuoConfig.Kill.rKS and unit.health < getDamage(unit) then
				CastSpell(_R)
			end]]
