...
local function UltimateKS()
    ...
    for i, Enemys in pairs(GetEnemyHeroes()) do
        if ValidTarget(Enemys) and GetDistance(Enemys) < 900 and TargetIsValid(Enemys) then 
...
function OrbWalk(myTarget)
    if ValidTarget(myTarget) and TargetIsValid(myTarget) and ThisIsReal(myTarget) <= myPlayer.range then
        if timeToShoot() then
            myPlayer:Attack(myTarget)
