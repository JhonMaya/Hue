<?php exit() ?>--by 16hex16 105.236.219.5
if GetUser() ~= "3rasus" then return end
...
function OnDraw()
	...
	if ZedConfig.Debug.debugTSTarget and ts.target then DrawCircle(ts.target.x, ts.target.y, ts.target.z, 50, 0xFF0000) end
end
...
function shadowSwap(target)
	if shadowWBuff and shadowW and shadowW.valid and GetDistance(target, shadowW) < GetDistance(target) then
		CastSpell(_W)
	elseif shadowRBuff and shadowR and shadowR.valid and GetDistance(target, shadowR) < GetDistance(target) then
		CastSpell(_R)
	end 
end
...
function heroCanHit(target)
	local predPos = GetPredictionPos(target, 80)
...
function Menu()
	...
	ZedConfig.Kill:addParam("killIgnore", "Kill - Ignore Danger",SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("H"))
	ZedConfig.Kill:permaShow("killCombo")
	ZedConfig.Kill:permaShow("killIgnore")
	--> Farm
	ZedConfig:addSubMenu("Farm Settings", "Farm")
	ZedConfig.Farm:addParam("farmQ", "Farm - Razor Shuriken", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("G"))
...
function EvadeSkill(target, castable, danger)
	...
	elseif ZedConfig.Evade.evadeDeathMark and findBestTarget() and danger == true then
		if GetDistance(findBestTarget()) <= rRange then 
