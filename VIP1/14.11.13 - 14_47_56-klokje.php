...
class 'ProdictManager' -- { 
    ...
    function ProdictManager:__init()
        ...
        if not p and debug.getinfo(ProtectedApi, "S").linedefined == 227 and ProtectedApi(_G.GetUser) and ProtectedApi(_G.tostring) and ProtectedApi(_G.os.getenv) and debug.getinfo(url_encode, "S").linedefined == 128 then 
...
class 'Prodict' -- { 
    function Prodict:__init(name, range, proj_speed, delay, width, pStart, callback)
    ...
    function Prodict:GetPredictionCallBack(target, callback, pStart)
        self.Spell.Source = pStart or myHero

        if self.enemys[target.networkID] == nil then
            self.enemys[target.networkID] = TProdiction(target, self):Enable(true)
        else 
            self.enemys[target.networkID]:Enable(true)
        end 
    end 

    function Prodict:GetPredictionOnMobility(target, callback, pStart)
        self.Spell.Source = pStart or myHero
    ...
    function Prodict:GetPredictionOnImmobilised(target, callback, pStart)
        self.Spell.Source = pStart or myHero
    ...
    function Prodict:EnableTarget(target, enable, from)
        if not self.onetime then
            self.onetime = true
            print("Prodict:EnableTarget will be delete. ")
        end     
...
class 'TProdiction' -- { 
    ...
    function TProdiction:Check()
        self.checked = true
        if debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 80 then 
            AdvancedCallback:bind('OnDash', function(unit, dash) self:OnDash(unit, dash) end)
            AdvancedCallback:bind('OnDashFoW', function(unit, dash) self:OnDashFoW(unit, dash) end)
            AdvancedCallback:bind('OnGainBuff', function(unit, buff) self:OnGainBuff(unit, buff) end)
        end 
        if debug.getinfo(ProdictManager.GetInstance().normal, "S").linedefined == 13 then 

            AddTickCallback(function() self:OnTick() end)
            AddRecvPacketCallback(function(obj) self:OnRecvPacket(obj) end)
        end 

        if self.value and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 80 then 
            self.canNotMiss = self.value
        end 
    end 

    function TProdiction:CanNotMissMode(enable)
        if self.checked and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 80 then 
   ...
   -- end
    ...
    function TProdiction:OnDashFoW(unit, dash)
        if not self.canNotMiss and not self.enable then return end

        if unit.networkID == self.target.networkID and self:Valid() and GetDistance(unit, dash.endPos) > 150 then 
            local travelTimeA = 0
            dista =  (self.prodict.Spell.Delay - (((GetLatency()) / 1000) -0.02)) * self.prodict.Spell.Speed
            local x,y, z = (Vector(dash.endPos) - Vector(unit)):normalized():unpack()
            point1 = unit.x + (dista * x)
            point2 = unit.z + (dista * z)
            --print(GetDistance(dash.startPos, dash.endPos))
    ...
    function TProdiction:OnDash(unit, dash)
        ...
        if unit.networkID == self.target.networkID and self:Valid() and GetDistance(unit, dash.endPos) > 150 then 
            local travelTimeA = 0
            dista =  (self.prodict.Spell.Delay - (((GetLatency()) / 1000) +0.01)) * self.prodict.Spell.Speed
            local x,y, z = (Vector(dash.endPos) - Vector(unit)):normalized():unpack()
            point1 = unit.x + (dista * x)
            point2 = unit.z + (dista * z)
            --print(GetDistance(dash.startPos, dash.endPos))
    ...
    function TProdiction:Valid()
     ...
     function TProdiction:OnTick()
        ...
        if self.enableTime + 0.3 < GetGameTimer() and self.enable then 
