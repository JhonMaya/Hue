...
if ValidTarget(myTarget) and TargetIsValid(myTarget) and GetDistance(myTarget) <= rangeToTarget then
  ...
  --end
	...
	if myPlayer:CanUseSpell(_Q) ~= READY and myPlayer:CanUseSpell(_W) ~= READY and myPlayer:CanUseSpell(_R) ~= READY and myPlayer:CanUseSpell(_E) ~= READY then KillText = "Spells in CD" end
  if iDmg >= myTarget.health and myPlayer:CanUseSpell(_Q) ~= READY and myPlayer:CanUseSpell(_W) ~= READY and myPlayer:CanUseSpell(_R) ~= READY and myPlayer:CanUseSpell(_E) ~= READY then lockIgnite = false KillText = "(I) can kill" BurnYouShit(myTarget) end
...
--   local qDamage                       = {30, 90, 150, 210, 270}
--   local qRatio                        = {0.4, 0.45, 0.5, 0.55, 0.6}
--   local wDamage                       = {50, 80, 110, 140, 170}
--   local wRatio                        = myPlayer.ad -- 100% of attack damage
--   local r1DamageRatio                 = 0.2*myPlayer.addDamage -- 20% of attack damage
...
--       if canUseAbility[a] == _Q then
--         local firstDamage = myPlayer:CalcDamage(myTarget, qDamage[myPlayer:GetSpellData(canUseAbility[a]).lvl])
--         qDmg = firstDamage + (firstDamage*r1DamageRatio)
--         finalDamage = qDmg
--       end
...
function OnDraw()
    ...
    if ValidTarget(Target) and TargetIsValid(Target) and GetDistance(Target) <= rangeToTarget then
      ...
      -- if damageWithR_ then
      --   DrawText3D(tostring(TextTest), Target.x - 150, Target.y, Target.z, 26, ARGB(255, 000, 000, 255), true)
      -- end
    end
-- x-200, y*600
    if passive_ then       
