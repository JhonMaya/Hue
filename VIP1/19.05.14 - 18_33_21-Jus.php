...
function OnProcessSpell(unit, spell)
    ...
    if useEvadeee_ and _G.Evadeee or _G.Evadeee_impossibleToEvade then
  ...
  if integration_ then
    ...
    if selectedTarget == nil then
        ...
        for i, Targets in pairs(GetEnemyHeroes()) do
            if ValidTarget(Targets, range_) then
...
if ValidTarget(myTarget) and TargetIsValid(myTarget) and GetDistance(myTarget) <= rangeToTarget then
  ...
  --end
	...
	if myPlayer:CanUseSpell(_Q) ~= READY and myPlayer:CanUseSpell(_W) ~= READY and myPlayer:CanUseSpell(_R) ~= READY and iDmg < myTarget.health then KillText = "Spell in CD" end
...
function heroCanMove()
    return ( GetTickCount() + GetLatency() / 2 > lastAttack + lastWindUpTime + 0.2)
end 
 
function timeToShoot()
  local ComboON = menu.combo.key
  if ComboON and not lockPacket or myPlayer:CanUseSpell(_W) ~= READY then return (GetTickCount() + GetLatency() / 2 > (lastWindUpTime - GetLatency()/2) ) end
