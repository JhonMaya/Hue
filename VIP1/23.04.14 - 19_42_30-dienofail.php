<?php exit() ?>--by dienofail 137.187.163.95
...
function OnLoad()
	menu = scriptConfig("Riven by Jus", "rivenbyjus")
	...
	menu.combo.gap:addParam("eg", "Range to start with (E)", SCRIPT_PARAM_SLICE, 385, 125, 550, 0)
	menu.combo.gap:addParam("eeg", "Range to stop with (Q)", SCRIPT_PARAM_SLICE, 126, 126, 550, 0)
...
function _OrbWalk(myTarget)	
	if ValidTarget(myTarget, myTrueRange - 10) then		
		if timeToShoot() then
...
function moveToCursor()
	...
	if GetDistance(mousePos) > orbsmooth_ + 50 then
...
function CastQ(myTarget)
	...
	if valid and qReady and useq_ then
		...
		if GetDistance(myTarget) >= qgapStop_ and GetDistance(myTarget) <= qgap_ then
			CastSpell(_Q, myTarget.x, myTarget.z)
		end
	end
end
...
function KillEverybodyMothaFucker(myTarget)
	...
	local enemyHealth = (myTarget.health / myTarget.maxHealth * 100)		
		...
		if eReady and usee_ then
			if GetDistance(myTarget) <= eg1 and not aaboost then
...
function Harass_Key(myTarget)
	...
	local orb_ 		= 	menu.system.orbwalker
	if valid then
		if useq_ and qReady then
			Packet('S_CAST', { spellId = _Q, x=myTarget.x, y=myTarget.z }):send()
		end
		if usee_ and eReady then
			Packet('S_CAST', { spellId = _E, x=mousePos.x, y=mousePos.z }):send()
		end
		if usew_ and wReady then
			Packet('S_CAST', { spellId = _W, }):send()
		end		
	end
...
function OnProcessSpell(unit, spell)	
	...
	if unit.IsMe then
		...
		if spell.name == "RivenTriCleave" then		
			if valid then        
        		if qCancel then        
        			local movePos = Target + (Vector(myPlayer) - Target):normalized()*(GetDistance(Target)+55)
            		if movePos then
   		...
   		if spell.name == "RivenFeint" and valid then
   			...
   			if qReady and usee_ and eq_ and GetDistance(Target) >= qgapStop_ and GetDistance(Target) <= qgap_ then
				CastSpell(_Q, Target.x, Target.z)
			end
		
   			if wReady and usew_ and ew_ and GetDistance(Target) <= 260 then
        		CastSpell(_W)
    		end		
   		end

   		if spell.name == "RivenMartyr" and valid then
   			if items_0 then
   				CastCommonItem()
   			end
   		end
