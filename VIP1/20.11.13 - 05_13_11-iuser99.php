...
class 'Caster' -- {
    ...
    _G.pManager = ProdictManager.GetInstance() 
	...
	function Caster:__init(spell, range, spellType, speed, delay, width, useCollisionLib)
		...
		if spellType == SPELL_LINEAR or spellType == SPELL_CIRCLE or spellType == SPELL_LINEAR_COL then
			--if type(range) == "number" and (not speed or type(speed) == "number") and (not delay type(delay) == "number" and (type(width) == "number" or not width) then
				...
				self.pred = pManager:AddProdictionObject(self.spell, self.range, self.speed, self.delay, self.width)
				self.predFunc = function(unit, pos, castspell)
	...
	function Caster:Cast(target, minHitChance)
		...
		elseif self.spellType == SPELL_LINEAR or self.spellType == SPELL_CIRCLE or self.spellType == SPELL_LINEAR_COL then
			if self.pred and ValidTarget(target) then
				self.pred:GetPredictionCallBack(target, function(unit, pos, spell)
					self.predFunc(unit, pos, spell) 
				end)
				return true
...
class 'AutoUpdate' -- {
	...
	AutoUpdate.base = "iuser.com"

	function AutoUpdate:__init(SCRIPT_VERSION, SCRIPT_NAME, VERSION_URL, URL, PATH)
		...
		PrintAlert(self.SCRIPT_NAME .. " :  Checking version...", 30, 255, 255, 255)
	end 

	function AutoUpdate:CheckVersion(data)
		local online_version = tonumber(data)
		if type(online_version) ~= "number" then return end 
		if online_version and online_version > self.SCRIPT_VERSION then 
			PrintAlert(self.SCRIPT_NAME .. " : There is a new version. Updating (Do not F9).", 30, 0, 255, 255)
			self.need_update = true
		else 
			PrintAlert(self.SCRIPT_NAME .. " : You have the most recent version.", 30, 255, 255, 255)
		end 
	end 

	function AutoUpdate:Tick() 
		if self.need_run then 
			self.need_run = false 
			if _G.UseUpdater == nil or _G.UseUpdater == true then 
				GetAsyncWebResult(AutoUpdate.base, self.VERSION_URL, function(data) 
					self:CheckVersion(data)
				end)
			else
				PrintChat(self.SCRIPT_NAME .. " : AutoUpdate off.", 30, 255, 255, 255)
			end
		end 

		if self.need_update then 
			self.need_update = false 
			DownloadFile(self.URL, self.PATH, function()
                if FileExist(self.PATH) then
                   PrintAlert(self.SCRIPT_NAME .. " : Updated. Please reload.", 30, 255, 255, 255)
                end
			end)
		end 
	end 
