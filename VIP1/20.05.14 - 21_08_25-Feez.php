...
local devname = r({102,101,101,122})
local scriptname = 'annie the unbearable'
local scriptver = 4.45
...
function Kek11()
  ...
  if (g1 == true) and (g2 == true) and (g3 == true) then
    PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,39,35,68,66,48,48,48,52,39,62,65,110,110,105,101,58,32,65,117,116,104,32,83,101,114,118,101,114,32,79,102,102,108,105,110,101,60,47,102,111,110,116,62})) -- Set below to true if you want to allow everyone access if all servers are down
    fkgoud = false
  end
end
...
function Kek4(authCheck)
  ...
  if (dePack[r({115,116,97,116,117,115})]) then
    if (dePack[r({115,116,97,116,117,115})] == r({76,111,103,105,110,32,83,117,99,101,115,115,102,117,108})) then
      PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,39,35,68,66,48,48,48,52,39,62,65,110,110,105,101,58,32,65,117,116,104,111,114,105,122,101,100,60,47,102,111,110,116,62}))
      fkgoud = true
    else
      reason = dePack[r({114,101,97,115,111,110})]
      PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,39,35,68,66,48,48,48,52,39,62,65,110,110,105,101,58,32,65,99,99,101,115,115,32,68,101,110,105,101,100,58,32})..reason)
    end
  end
  if not dePack[r({115,116,97,116,117,115})] then
    PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,39,35,68,66,48,48,48,52,39,62,65,110,110,105,101,58,32,65,99,99,101,115,115,32,68,101,110,105,101,100,60,47,102,111,110,116,62}))
  end
end
...
function OnLoad()
	if FileExist(HOSTSFILE) then
		...
		if file ~= nil then
			...
			if stringff or stringfg or stringfh or stringfi then PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,39,35,68,66,48,48,48,52,39,62,65,110,110,105,101,58,32,65,99,99,101,115,115,32,68,101,110,105,101,100,32,40,72,70,77,41,60,47,102,111,110,116,62})) return end
		end
	end
	...
	Kek9()
	Kek10()
	PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,39,35,68,66,48,48,48,52,39,62,65,110,110,105,101,58,32,86,97,108,105,100,97,116,105,110,103,32,65,99,99,101,115,115,60,47,102,111,110,116,62})) -- Validating Access
	DelayAction(Kek11,4)
end
...
function OnTick()
	if not ldCnfig and fkgoud then LoadConfig(); ldCnfig = true end
	if not fkgoud then return end
	ts:update()
	...
	if farmTurnedOff and not AUConfig.combosettings.comboActive then farmTurnedOff = false; AUConfig.farmsettings.qfarm = true end
	--Stuff
	Combo()
	AutoQFarm()
	Harass()
...
function MinionPredict(minion)
	local distanceM = GetDistanceSqr(minion.visionPos, myHero.visionPos)
	local time = .25 + distanceM / 1400 - 0.07
	local PredictedHealth = VP:GetPredictedHealth(minion, time)
	if qready and minion ~= nil and PredictedHealth <= myHero:CalcMagicDamage(minion, qdamage) and distanceM <= 390625 then
