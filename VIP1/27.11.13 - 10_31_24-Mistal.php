<?php exit() ?>--by Mistal 85.94.224.21
...
function OnLoad()
  ...
  if VIP_USER and CassConfig.PRODICTION then require 'Prodiction' end
  ...
  if VIP_USER and CassConfig.PRODICTION then
...
function OnTick()
  ...
  allyMinions:update()
    ...
    if brandfire and GetDistance(brandfire) <= 335 then CastItem(ZHONYA) end
      ...
      if CassConfig.Spam then AutoE() end
...
function AutoE()
	...
	if myHero:CanUseSpell(_E) == READY then
		...
		if ValidTarget(tstarget, 700) then
			...
			if (CassConfig.Spam and TargetPoisoned(tstarget)) or (getDmg("E", tstarget, myHero) > tstarget.health and inRange <= 2) then
        ...
        CastSpell(_E, tstarget)
			...
			for _, enemy in pairs(enemies) do
				...
				if ValidTarget(enemy, 700) then
					...
					if (CassConfig.Spam and TargetPoisoned(enemy)) or (getDmg("E", enemy, myHero) > enemy.health and inRange <= 2) then
