...
return {
	...
	["Janna"] = {charName = "Janna", skillshots = {
		["HowlingGale"] = { spellKey = _Q, type = "LINE", spellName = "HowlingGale", name = "HowlingGale", range = 1700, radius = 120, projectileName = "HowlingGale_mis.troy", },
	}},
...
function GetSpellRadius(spell)
	return spell.info.radius + Menu.SkillSettings[spell.info.charName .. "SkillSettings"][spell.info.spellName .. "buffer"]
end
...
function GetBestPosition()
	...
	if lastBestPosition and not isDangerousPos(lastBestPosition) then
	bestPosition = lastBestPosition
	closestPosition = lastMovePos ~= nil and lastMovePos:distance(bestPosition)
	end
...
function CheckUnitCollision(pos)
	...
	for i = 1, heroManager.iCount do
	local hero = heroManager:GetHero(i)		
		if hero.name ~= myHero.name and pos:distance(Point(hero.x,hero.z)) < 75  < 75 then
			return true
		end		
	end	
...
function OnDraw()
	...
	if Menu.DrawSkillShots then		
		...
		if lastBestPosition then		
		for i=1,5,1 do
		DrawCircle(lastBestPosition.x, myHero.y, lastBestPosition.y, 50+i, 0xFFFFFF)
		end		
		end
	end	
end
...
function OnSendPacket(p)
	...
	if Menu.DangerArea and packetName == 'S_MOVE' and isDodging == false and packet:get('type') == 2 then
		...
		for spellName, spell in pairs(skillShots) do
			...
			if not playerInSkillShot(spell) then --and inSkillShot(spell, Point(predPos.x, predPos.z), Menu.DodgeBuffer) then
				packet:block()
				local pos = GetBestLastMove()
				if pos then
				lastMoveCommand = GetTick()
...
function OnLoad()
	...
	Menu = scriptConfig("ezEvade v" .. sVersion, "ezEvade")
		...
		if hero.team ~= player.team then
			...
			if championInfo[hero.charName] and championInfo[hero.charName].skillshots then
				...
				for spellName, spell in pairs(championInfo[hero.charName].skillshots) do
					...
					Menu.SkillSettings[hero.charName .. "SkillSettings"]:addParam(spell.spellName .. "buffer", "Extra Radius", SCRIPT_PARAM_SLICE, 0, 0, 150, 0)
				end
			end						
		else
