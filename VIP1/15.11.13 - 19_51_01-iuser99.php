...
Menu.Get():addParam("title", "				uLissandra",  SCRIPT_PARAM_INFO, "")
	...
	Menu.Get():addSubMenu("Combo", "Combo")
		...
		Menu.Get().Combo:addParam("AA", "Use Auto-Attacks", SCRIPT_PARAM_ONOFF, true)
...
function OnTick()
	c:OnTick()
	Target = c:GetTarget()
	if Menu.Get().Combo.Active or Menu.Get().Farm.LaneClear.Active or Menu.Get().Farm.Active or Menu.Get("Harass").Active then 
	...
	if Target then 
		...
		if Menu.Get("Combo").AA and GetDistance(Target) <= myHero.range then 
