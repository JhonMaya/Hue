...
function OnTick()
	...
	for _, Enemy in pairs(RecallTable) do
		...
		if myHero:CanUseSpell(_R) == READY and TravelTime >= Enemy.rTime and TravelTime < Enemy.rTime + 20 then
			if getDmg("R", Unit, myHero) >= Unit.health and unit.visible then
...
function OnRecvPacket(p)
	if p.header == 0xD7 then
		...
		if RecallTable[__source] and __type == 4 then
			local String = (GetTickCount() > RecallTable[__source].finish and "End" or "Abort")
			PrintChat("Recall "..String.." : "..__hero.charName)
		...
		elseif RecallTable[__source] == nil and __type == 6 then
			...
			PrintChat("Recall: "..__hero.charName.." - "..tostring(RecallTypes[__hero:GetSpellData(RECALL).name]))
		end
	end
end
