...
if isDev == true then
	if Menu4.Dev2 then Weave:DoWeave(Target)
	end
end
if isDev == true then
...
function HandleR()
	if Menu.UseR then 
		if Target and ValidTarget(Target) and GetDistance(Target) <= 870 and AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target, GetEDashPos(Target)) then
			if not HasUlt then
				CastSpell(_R, Target.x, Target.z)
				CastSpell(_E, Target.x, Target.z)
			end
...
function HandleRCast()
	if Target and ValidTarget(Target) and GetDistance(Target) <= 800 and GetDistance(Target) >= 450 and HasUlt then
		if getDmg("R", Target, myHero) >= Target.health then
			CastSpell(_R, Target.x, Target.z)
			--SkillRC:Cast(Target)  --prodiction is broken as fuck
		end	
	end	
	if Target and ValidTarget(Target) and HasUlt then
		if GetDistance(Target) >= 450 and not myHero:CanUseSpell(_E) == READY then
			CastSpell(_R, Target.x, Target.z)
		end
	end	
	if Target and ValidTarget(Target) and GetDistance(Target) <= 850 and HasUlt then
		if not myHero:CanUseSpell(_Q) == READY and NextQ - GetTickCount() < 3000 and not myHero:CanUseSpell(_W) == READY then
			CastSpell(_R, Target.x, Target.z)
		end
	end	
	if Target and ValidTarget(Target) and GetDistance(Target) >= 650 and HasUlt then
	 	if not AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target, GetEDashPos(Target)) or not myHero:CanUseSpell(_E) then
	 		CastSpell(_R, Target.x, Target.z)
	 	end
	end
	if Target and ValidTarget(Target) and GetDistance(Target) <= 700 then 
		if not myHero:CanUseSpell(_Q) == READY and NextQ - GetTickCount() < 3000 and not myHero:CanUseSpell(_E) and HasUlt then
			CastSpell(_R, Target.x, Target.z)
		end
	end
	if Target and ValidTarget(Target) and GetDistance(Target) <= 700 then
