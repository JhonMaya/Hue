...
function playerDistanceToSpell(spell)
...
function GetBestPosition()
	...
	closestPosition = lastMovePos and lastMovePos:distance(bestPosition)
	end
	...
	if not inDanger and canHeroWalkToPos(pos) and (Menu.UnitCollision == false or CheckUnitCollision(pos) == false) then
		...
		elseif lastMovePos == nil and canHeroWalkToPos(pos) then
			return pos
		end
...
function canHeroWalkToPos2(pos)
	local heroPoint = Point(myHero.x, myHero.z)
	...
	for i=1, checks, 1 do
		...
		for spellName, spell in pairs(skillShots) do
			if spell.startPos and spell.endPos then
				local timeElapsed = 1000 * heroPoint:distance(pos)/myHero.ms + GetLatency() --add number
...
function GetSpellCollisionTimeToPos (spell, pos)
	...
	DrawCircle(heroPos.x, myHero.y, heroPos.z, 50, 0x00FF00)
		...
		local timeToCollision = GetTimeToCollision2 (heroPoint, spell.endPos, walkDir * myHero.ms, Point(0,0), Menu.DodgeBuffer, spell.info.radius)
		local spellHitTime = spell.startTime + spell.info.spellDelay
		
		if timeToCollision and timeToCollision > 0 and timeToCollision * 1000 > spellHitTime then
			return true
		end		
	end
	end
...
--https://code.google.com/p/xna-circle-collision-detection/downloads/detail?name=Circle%20Collision%20Example.zip&can=2&q=
    local Pab = VectorObj(Pa - Pb) --no need?
    local Vab = VectorObj(Va - Vb)
...
function GetLongestPoint(pos)
	...
	local dir = (pos - Point(myHero.x,myHero.z)):normalized()
	local checkDist = 50
	local checkIndex = 0
...
function checkHeroDirection()
	...
	if waypoints and #waypoints > 1 then
		...
		local predPos = Vector(myHero) + dir * myHero.ms * (GetLatency()/1000)
...
function DodgeSkillShots()
	...
	if Menu.DangerArea and checkHeroDirection() and isDodging == false then
		...
		if lastMovePos and not canHeroWalkToPos(lastMovePos) then
...
function OnTick()
	if Menu.StopDodging == false and Menu.DodgeSkillShots and GetTick() - lastTickCount > Menu.TickLimiter  then
...
function OnDraw()
	...
	if Menu.DrawSkillShots then		
		...
		local waypoints = wayPointManager:GetWayPoints(myHero)	
		if #waypoints < 2 then
			canHeroWalkToPos2(Point(myHero.x,myHero.z))
		else
			canHeroWalkToPos2(waypoints[2])
		end		
		printTime = GetTick()
		end]]
	end
...
function OnSendPacket(p)
	...
	if isDodging then
		if packetName == 'S_MOVE' then
			local movePos = Point(packet:get('x'),packet:get('y'))
			if lastBestPosition and lastBestPosition:distance(movePos) > 50 then --test 50
			packet:block()
	...
	if Menu.DangerArea and packetName == 'S_MOVE' and isDodging == false and packet:get('type') == 2 then
		...
		if not canHeroWalkToPos(movePos) then
		...
		for spellName, spell in pairs(skillShots) do
			...
			local dir = (movePos-Point(myHero.x,myHero.z)):normalized()
			dir = Vector(dir.x,0,dir.y)
						
			local predPos = Vector(myHero) + dir * 50
						
			if not playerInSkillShot(spell) and playerDistanceToSpell(spell) < Menu.DodgeBuffer + 100 then
				packet:block()
