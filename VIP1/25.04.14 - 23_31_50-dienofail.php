...
function OnLoad()
        ...
        menu.combo.cancel:addParam("advsmooth", "Auxiliary Smoothness", SCRIPT_PARAM_SLICE, 1, 0, 20, 2)
...
function CastQ(myTarget)
        ...
        if valid and qReady and useq_ then
                ...
                elseif GetDistance(myTarget) >= qgapStop_ and GetDistance(myTarget) == qgap_ then
                        --Packet('S_CAST', { spellId = _Q, x=myTarget.x, y=myTarget.z }):send()
                        CastSpell(_Q, myTarget.x, myTarget.z)
                end
        end
...
function KillEverybodyMothaFucker(myTarget)     
        ...
        if valid then                
                ...
                if rReady and user_ then 
                    ...
                    if GetDistance(myTarget) <= rmin_ and aaboost and not UnderTurret(myPlayer, true) then                          
                        CastSpell(_R)                                   
                    end             
                end
                ...
                --end
                    if GetDistance(myTarget) >= egStop_ and GetDistance(myTarget) <= eg1 or not aaboost then
                        --Packet('S_CAST', { spellId = _E, x=myTarget.x, y=myTarget.z }):send()
                        CastSpell(_E, myTarget.x, myTarget.z)
                    end
        ...
        if unit.IsMe then
                ...
                if spell.name:lower():find("attack") then                       
                        ...
                        if qReady and valid and advanced_ and aaboost then
                                DelayAction( CastQ(Target) , GetLatency() /2000 + adv_smooth_)
                        end     
                end 
        end

        if unit.IsMe and valid then
