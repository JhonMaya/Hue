<?php exit() ?>--by Jus 177.9.138.217
if myHero.charName ~= "Riven" or not VIP_USER then return end

--------------------------

function _AutoupdaterMsg(msg)
    print("<font color=\"#D4A017\"><b>Riven, Boost My Elo:</b></font> <font color=\"#FFFFFF\">"..msg..".</font>") 
end
...
function Others()   
  ...
  --GetTargetTable() 
    PrintChat("<font color=\"#FF0000\"><b>Riven, Boost My Elo by</b></font><font color=\"#FFFFFF\"><b>  Jus</b></font>")
end
...
local function PredCastQ(myTarget)
  ...
  if useq and qpred_ then      
  	local del   =   DelayCalc()
  	if mode_ == 1 then delayType = del else  delayType = ( (lastWindUpTime/1000) - ((GetLatency())/2000) - 0.1 ) end
    -- print("Negative: "..tostring(( (lastWindUpTime/1000) - ((GetLatency())/2000) - 0.05 )))
    --  print("normal: "..tostring(( (lastWindUpTime/1000) - ((GetLatency())/2000) )))
  if delayType < 1 then delayType = 0.21 end

    if GetDistance(myTarget) <= GetCustomRange(_Q) then

    	 local AOECastPosition, MainTargetHitChance, nTargets = vp:GetCircularAOECastPosition(myTarget, delayType, 120, GetCustomRange(_Q), math.huge, myPlayer)
    	 
       if MainTargetHitChance >= qChance_ and not Passive then
    		qTick = GetTickCount() + GetLatency() / 2
    		CastSpell(_Q, AOECastPosition.x, AOECastPosition.z)    		
    	 end
    	 
       if MainTargetHitChance >= qChance_ and GetTickCount() + GetLatency() / 2 - qTick > del*1000 then
...
function CastQ(myTarget)  
    local useq      =   menu.combo.skills.q
    local mode_     =   menu.comboextra.mode
    local range_1 	=	  GetCustomRange(_Q)
    ...
    if useq then
        local del   =   DelayCalc()
        if mode_ == 1 then delayType = del else  delayType = ( (lastWindUpTime/1000) - ((GetLatency())/2000) -0.1 ) end 
        if GetDistance(myTarget) <= range_1 and not Passive then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, myTarget.x, myTarget.z)
        end
        if GetDistance(myTarget) <= range_1 and GetTickCount() + GetLatency() / 2 - qTick > del*1000 then
        	if delayType < 1 then delayType = 0.19 end
            DelayAction(function()
        ...
        if GetDistance(myTarget) <= GetCustomRange(_Q) and GetTickCount() + GetLatency() / 2 - qTick > buffdelay then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, myTarget.x, myTarget.z)
        end 
    end	
...
local function CastW(myTarget)   
    ...
    if usew then       
        if GetDistance(myTarget) <= range_2 then            
        	CastSpell(_W)            
        end 
    end
...
local function AutoStun()
    ...
    if CountEnemyHeroInRange(range_3) >= autow_ then
         CastSpell(_W)           
    end
    --if ThisIsReal(myTarget) <= myPlayer.range and Passive then myPlayer:Attack(myTarget) end
end
...
function NewCastR(myTarget)
    ...
    if user then
		...
		if rPred and temUltimate then
			...
			if mainHitChance >= rChance and GetDistance(myTarget) <= 900 and myTarget.health <= DamageR then
				CastSpell(_R, mainCastPosition.x, mainCastPosition.z)
        		if not myTarget.dead then lockIgnite = false end
			-- elseif GetDistance(myTarget) >= myPlayer.range and GetDistance(myTarget) <= 900 and (myTarget.health/myTarget.maxHealth)*100 <= DamageR then
			-- 	CastSpell(_R, mainCastPosition.x, mainCastPosition.z)
			-- 	if not myTarget.dead then lockIgnite = false end
      elseif GetTickCount() + GetLatency() / 2 - rTick > GetBuffDelay(_R) then
        ...
        if not myTarget.dead then lockIgnite = false end
			...
			-- elseif GetDistance(myTarget) <= 900 and myTarget.health <= DamageR then
   --      CastSpell(_R, mainCastPosition.x, mainCastPosition.z)
   --      if not myTarget.dead then lockIgnite = false end
   --    elseif GetTickCount() + GetLatency() / 2 - rTick > GetBuffDelay(_R) then
   --      CastSpell(_R, mainCastPosition.x, mainCastPosition.z)
   --    end
			...
			elseif GetDistance(myTarget) >= myPlayer.range and GetDistance(myTarget) <= 900 and (myTarget.health/myTarget.maxHealth)*100 <= DamageR then
				CastSpell(_R, myTarget.x, myTarget.z)
				if not myTarget.dead then lockIgnite = false end
      elseif GetDistance(myTarget) <= 900 and GetTickCount() + GetLatency() / 2 - rTick > GetBuffDelay(_R) then
        CastSpell(_R, myTarget.x, myTarget.z)
        if not myTarget.dead then lockIgnite = false end
      end
			-- elseif GetDistance(myTarget) <= 900 and myTarget.health <= DamageR then
         ...
         if animationname:lower():find("attack") then
            local del   =   DelayCalc()       
             if GetDistance(myPlayer, Target) <= GetCustomRange(_Q) then -- (GetTickCount() + GetLatency() / 2) - qTick > del then                      
                 if mode_ == 1 then delayType = ( (del/1000) ) else  delayType = ( (lastWindUpTime/1000) - ((GetLatency())/2000) - 0.025 ) end           
                 DelayAction(function()
...
function OnProcessSpell(unit, spell)
    ...
    if unit.isMe then      
        ...
        if ComboON and Target ~= nil and not Target.dead and Target.visible then
	        if spell.name:lower():find("riventricleave") then
	        	qTick = GetTickCount() + GetLatency() / 2
	        	if speed_ and ThisIsReal(Target) <= myPlayer.range then lastAttack = 0 myPlayer:Attack(Target) end          
...
function LineFarm()  
    ...
    if minionLane ~= nil and not minionLane.dead then                  
        ...
        elseif GetTickCount() + GetLatency()/2 - tock > 2500 then
            tock = GetTickCount() + GetLatency()/2
            CastSpell(_Q, minionLane.x, minionLane.z)
        elseif not UnderTurret(Minion) then
            CastSpell(_E, minionLane.x, minionLane.z)
        end
        if GetDistance(minionLane, myPlayer) <= 380 then CastSpell(_W) end
  ...
  if not KillMobs then return end
    ...
    if Minion ~= nil and not Minion.dead then
        if useq and not Passive then
           DelayAction(function() CastSpell(_Q, Minion.x, Minion.z) end, qDelay_)
        end
        if usee and Minion ~= nil and not Minion.dead then
            DelayAction(function() CastSpell(_E, Minion.x, Minion.z) end, eDelay_)
        end
        if usew and GetDistance(Minion, myPlayer) <= 282 and not Passive and Minion ~= nil and not Minion.dead then
    ...
    if selectedTarget == nil then
        ...
        for i, Targets in pairs(GetEnemyHeroes()) do
            if ValidTarget(Targets) then
              if GetDistance(Targets) <= range_ then
                local basedmg   = 80
...
function TextDamage(myTarget)
  ...
  if GetTickCount() - GetLatency()/2 - TextTick < (refres_*1000)/100 then return end
  ...
  if qDmg or wDmg or rDmg ~= 0 then
    ...
    elseif iDmg >= myTarget.health then KillText = "(I) can kill"
	...
	if myPlayer:CanUseSpell(_Q) ~= READY and myPlayer:CanUseSpell(_W) ~= READY and myPlayer:CanUseSpell(_R) ~= READY and iDmg < myTarget.health then KillText = "Can't Kill" end
...
local function DamageCombo(myTarget)
  if not ValidTarget(myTarget) and not TargetIsValid(myTarget) then return end
	...
	if qDmg*3 >= myTarget.health and GetDistance(myTarget) <= GetCustomRange(_Q) then
		...
		if qPacket then
			qTick = GetTickCount() + GetLatency() / 2
			Packet('S_CAST', {spellId = _Q, fromX = myTarget.x, fromY = myTarget.z, toX = myTarget.x, toY = myTarget.z, targetNetworkId = myTarget.networkID}):send()
		else
  ...
  if qDmg*3 < myTarget.health then 
	  CastQ(myTarget)
  end
  NewCastR(myTarget)
  CastE(myTarget)
...
function timeToShoot()
    return (GetTickCount() + GetLatency() / 2 > qTick or lastAttack + lastAttackCD)
end 
...
-- x-200, y*600
    if passive_ then       
        ...
        --end
          ...
          DrawText(PassiveTxt, 20, x/2 - (string.len(PassiveTxt)/2), (y2/8)*y, ARGB(255,255,255,255)) --x/2 - 65, y+850
    ...
    if minionJ_ and clearjungle_ and Minion ~= nil and not Minion.dead then
      DrawCircle2(Minion.x, Minion.y, Minion.z, 85, ARGB(255, 000, 255, 000))
    end

    if minionL_ and lastHitkey_ and minion ~= nil and not minion.dead then
      DrawCircle2(minion.x, minion.y, minion.z, 65, ARGB(255, 000, 255, 000))
    end

    if laneClear_ and laneclearkey_ and minionLane ~= nil and not minionLane.dead then
