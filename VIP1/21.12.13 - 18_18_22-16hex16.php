...
function OnTick()
	...
	if not YasuoConfig.Basic.basicCombo and not YasuoConfig.Basic.basicHarass then
		if QREADY and YasuoConfig.Auto.autoQ then
			if (myHero.health > myHero.maxHealth*0.2 and countEnemies(ts.target, 600) <= 1) or ts.target.health < getDmg("Q", ts.target, myHero)+getDmg("AD", ts.target, myHero) then
	...
	--> Farm
	if YasuoConfig.Farm.Farm and not YasuoConfig.Basic.basicCombo and not YasuoConfig.Basic.basicHarass then
...
function OnDash(unit, dash)
	if (unit.isMe and not knockUpReady) and (YasuoConfig.Basic.basicCombo or YasuoConfig.Basic.basicHarass) then
		if ts.target and GetDistance(ts.target, dash.endPos) < 275 then 
