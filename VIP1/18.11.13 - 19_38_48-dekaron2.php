...
function LoadVIPPrediction()
	...
	tpQ = wp:AddProdictionObject(_Q, rangeQ, 1700, 0.535, nil, myHero, 
...
function OnGainBuff (unit, buff)
	if unit.isMe and unit.valid then
		if buff.name == "KennenLightningRush" then
			usingE = true
		end
		--print(buff.name)
	end
...
function CastE(target)
	...
	if ValidTarget(target) then
		if GetDistance(target) <= rangeE and EREADY and usingE == false then
			CastSpell(_E)
		end
	else
		return
...
function CastItems(target, allItems)
	if not ValidTarget(target) or not Config.useItems then 
		return
	else
