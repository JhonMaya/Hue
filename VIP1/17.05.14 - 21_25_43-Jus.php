...
function MenuExtraCombo()
	...
	menu.comboextra:addParam("delay", "Manual Mode Delay:", SCRIPT_PARAM_SLICE, 2, 0, 4, 0)
...
local function PredCastQ(myTarget)
    ...
    if useq and qpred_ then      
    	local del   =   DelayCalc()
    	if mode_ == 1 then delayType = del else  delayType = ( (lastWindUpTime/1000) - ((GetLatency())/2000) - 0.09 ) end
      if delayType < 1 then delayType = 0.19 end
        if GetDistance(myTarget) <= GetCustomRange(_Q) then    	
    	   ...
    	   if GetDistance(myTarget) <= GetCustomRange(_Q) and GetTickCount() + GetLatency() / 2 - qTick > GetBuffDelay(_Q) then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, AOECastPosition.x, AOECastPosition.z)
         end
      end
...
function CastQ(myTarget)  
    ...
    if useq then
        local del   =   DelayCalc()
        if mode_ == 1 then delayType = del else  delayType = ( (lastWindUpTime/1000) - ((GetLatency())/2000) - 0.86 ) end 
        ...
        for i, Targets in pairs(GetEnemyHeroes()) do
            if ValidTarget(Targets) then
              if GetDistance(Targets) <= range_ then
                ...
                if ValidTarget(Targets) then
                  local distancMouse = GetDistance(mousePos, Targets)
              	  if distancMouse <= 150 then
                    Enemy = Targets
                  end
                end
              end
...
--         local x, y = 0 ,0
--         if x or y == 0 then
--           x, y = GetPositionToText()
--         end
--         DrawText3D("Passive Counter: "..tostring(PassiveIndicator), 200*x, 200*y, 0, 22, ARGB(255,255,255,255), true)
--     end
-- end
