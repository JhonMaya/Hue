...
function MenuExtraComboRange()
    ...
    menu.comboextra.range:addParam('starte', "Range to Engage", SCRIPT_PARAM_SLICE, 410, 250, 500, 0)
end
...
local function PredCastQ(myTarget)
    ...
    if useq and qpred_ then
    	...
    	if GetDistance(myTarget) <= range_ and MainTargetHitChance >= qChance_ and GetTickCount() + GetLatency() / 2 - qTick > del*1000 then
    		if delayType < 1 then delayType = 0.225 end
    		DelayAction(function()
                        	if ValidTarget(myTarget) then
                        		qTick = GetTickCount() + GetLatency() / 2 
                                CastSpell(_Q, AOECastPosition.x, AOECastPosition.z)                                 
    	...
    	if GetDistance(myTarget) <= range_ and GetAArange(myTarget) > myPlayer.range and GetTickCount() + GetLatency() / 2 - qTick > buffdelay then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, AOECastPosition.x, AOECastPosition.z)
        end
    end
...
function CastQ(myTarget)   
    ...
    if useq then
        ...
        if GetDistance(myTarget) <= GetCustomRange(_Q) and GetAArange(myTarget) > myPlayer.range and GetTickCount() + GetLatency() / 2 - qTick > buffdelay then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, myTarget.x, myTarget.z)
        end 
    end	
...
local function DamageCombo(myTarget)	
	...
	if IgniteSpell.slot ~= nil then
		skillTable	=	{ "Q", "R", "IGNITE"}
		skills 		=	{ _Q, _R, IgniteSpell.slot}
	else
		skillTable	=	{ "Q", "R"}
		skills 		=	{ _Q, _R}
	end
	...
	for b=1, #possible do
		if possible[b] == "Q" then
			...
			if qDmg > myTarget.health then KillText = "3x(Q) can kill" else KillText = "Harass" end
		end
		if possible[b] == "R" then
			--rDmg = getDmg(possible[b], myTarget, myPlayer, 1)
			rDmg = getDmg(possible[b], myTarget, myPlayer, 2)
			if rDmg > myTarget.health then KillText = "(R) can kill" else KillText = "Harass" end	
		end		
		if possible[b] == "IGNITE" then
			iDmg = getDmg(possible[b], myTarget, myPlayer)
			if iDmg > myTarget.health and rDmg < myTarget.health then KillText = "(I) can kill" else KillText = "Harass" end
		end
	...
	for i=1, #needTable do		
		if needTable[i] ~= 0 then			
			if needTable[i] == qDmg then
                if qDmg >= myTarget.health and GetDistance(myTarget) <= 550 and iDmg < myTarget.health and pDmg < myTarget.health then
                	lockIgnite = true
				    CastSpell(_Q, myTarget.x, myTarget.z)
                else
                	lockIgnite = false
			...
			if needTable[i] == iDmg then
				if iDmg < myTarget.health and pDmg >= myTarget.health then
					lockIgnite = true
				else
					if iDmg > myTarget.health and pDmg < myTarget.health then
						lockIgnite = false
			...
			if needTable[i] == rDmg then
				if rDmg > iDmg and rDmg >= myTarget.health and pDmg < myTarget.health then
					lockIgnite = true
					NewCastR(myTarget)
				else
					if rDmg < iDmg and myTarget.health > rDmg and myTarget.health < iDmg and pDmg < myTarget.health then
						lockIgnite = false
