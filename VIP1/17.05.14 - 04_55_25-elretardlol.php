...
return {
	...
	["Braum"] = {charName = "Braum", skillshots = {
        ["Winter Bite"] = {spellKey = _Q, isCollision = true, name = "BraumQ", spellName = "BraumQ", spellDelay = 250, projectileName = "kennen_ts_mis.troy", projectileSpeed = 1200, range = 1000, radius = 100, type = "LINE"},
...
function GetBestPosition()
	...
	if not IsWall(D3DXVECTOR3(pos.x, myHero.y, pos.y)) then
		...
		if hero.team ~= player.team then
			...
			if championInfo[hero.charName] and championInfo[hero.charName].skillshots then
				...
				for spellName, spell in pairs(championInfo[hero.charName].skillshots) do
					...
					Menu.SkillSettings[hero.charName .. "SkillSettings"]:addParam(spell.spellName, spell.spellName, SCRIPT_PARAM_ONOFF, true)
				end
			end						
		else
