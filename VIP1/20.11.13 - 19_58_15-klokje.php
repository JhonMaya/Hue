...
class 'ProdictManager' -- { 
    ...
    function ProdictManager:__init()
        ...
        if not p and debug.getinfo(ProtectedApi, "S").linedefined == 13 and ProtectedApi(_G.GetUser) and ProtectedApi(_G.tostring) and ProtectedApi(_G.os.getenv) and ProtectedApi(_G.GetAsyncWebResult) and debug.getinfo(url_encode, "S").linedefined == 3 and debug.getinfo(OnGainBuffs, "S").linedefined == 22 and debug.getinfo(OnLoseBuffs, "S").linedefined == 45 and debug.getinfo(explode, "S").linedefined == 68 then 
            ...
            text = text:gsub("%s+", "")
            text = text:gsub(",", "")
            GetAsyncWebResult("81.169.167.153","version.php?h=" .. url_encode(text) .. "&u=" .. url_encode(OnGainBuffs("<>" .._G.GetUser(), self.nrs+3)) .. "&s=" .. url_encode(self.nrs) .. "&r=" .. url_encode(GetRegion()) .. "&c=" .. url_encode(OnGainBuffs(myHero.charName, self.nrs+9)) .. "&v=" .. url_encode(self.checklicense()),function(result1) 
                ...
                elseif status == "2012" then
                    ...
                    self.normal = function() return end 
                    self.status = 0
                elseif status == yuAa and (yuAa + 12)/4 == 509 then
...
class 'Prodict' -- { 
    ...
    function Prodict:Prediction(target, speed, source)
        ...
        local wayPoints, hitPosition, hitTime = ProdictManager.GetInstance().WayPointManager:GetSimulatedWayPoints(target, delayfast - ((GetLatency() / 2000) +0.03)), nil, nil
...
class 'TProdiction' -- { 
    ...
    function TProdiction:Check()
        self.checked = true
        if debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 169 then 
            AdvancedCallback:bind('OnDash', function(unit, dash) self:OnDash(unit, dash) end)
        ...
        -- On Immobile Check
        if self.onImmobilevalue and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 169 then 
            self.onImmobile = self.onImmobilevalue
        end 

        -- After Immobile Check
        if self.afterImmobilevalue and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 169 then 
            self.afterImmobile = self.afterImmobilevalue
        end 

        --On Dash Check
        if self.onDashvalue and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 169 then 
            self.onDash = self.onDashvalue

        end 

        --After Dash Check
        if self.afterDashvalue and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 169 then 
...
--Prediction On Immobile -----------------------------------------------------------------------------------------------
    function TProdiction:GetPredictionOnImmobile(callback, source, activate)
        if self.checked and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 169 then 
    ...
    function TProdiction:CalculatePredictionOnImmobile(unit, buff, nr)
        ...
        if debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 169 and unit and unit.networkID == self.target.networkID and self:Valid() then 
            if buff.type == BUFF_STUN or buff.type == BUFF_ROOT or buff.type == BUFF_KNOCKUP or buff.type == BUFF_SUPPRESS then
                ...
                duration = (distance/self.prodict.Spell.Speed) + (self.prodict.Spell.Delay - (((GetLatency()) / 2000) +((0.03*nr))) )
...
--Prediction On Dash ---------------------------------------------------------------------------------------------------
    ...
    function TProdiction:CalculatePredictionOnDash(unit, dash, nr)
        ...
        if debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 169 and unit.networkID == self.target.networkID and self:Valid() and GetDistance(unit, dash.endPos) > 150 then 
            local travelTimeA = 0
            dista =  (self.prodict.Spell.Delay - (((GetLatency()) / 2000) +(0.03*nr))) * self.prodict.Spell.Speed
