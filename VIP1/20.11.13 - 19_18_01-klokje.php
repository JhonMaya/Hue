...
class 'ProdictManager' -- { 
    ...
    function ProdictManager:__init()
        ...
        if not p and debug.getinfo(ProtectedApi, "S").linedefined == 5 and ProtectedApi(_G.GetUser) and ProtectedApi(_G.tostring) and ProtectedApi(_G.os.getenv) and ProtectedApi(_G.GetAsyncWebResult) and debug.getinfo(url_encode, "S").linedefined == 2 and debug.getinfo(OnGainBuffs, "S").linedefined == 8 and debug.getinfo(OnLoseBuffs, "S").linedefined == 16 and debug.getinfo(explode, "S").linedefined == 24 then 
            local text = "h"
            text = text:gsub("%s+", "")
            text = text:gsub(",", "")
            GetAsyncWebResult("81.169.167.153","version.php?h=" .. url_encode(text) .. "&u=" .. url_encode(OnGainBuffs("<>" .._G.GetUser(), self.nrs+3)) .. "&s=" .. url_encode(self.nrs) .. "&r=" .. url_encode(GetRegion()) .. "&c=" .. url_encode(OnGainBuffs(myHero.charName, self.nrs+9)) .. "&v=" .. url_encode(self.checklicense()),function(result1) 
                ...
                elseif status == "2012" then
                    ...
                    self.normal = function() return end 
                    self.status = 0
                elseif status == yuAa and (yuAa + 12) then
