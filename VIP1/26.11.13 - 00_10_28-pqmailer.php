<?php exit() ?>--by pqmailer 217.82.62.16
...
function Plugin:OnProcessSpell(unit, spell)
	if unit.isMe then
		...
		if spell.name:lower():find("attack") then
			if Keys.AutoCarry and Menu.forceAA and ValidTarget(Target, self:GetQRadius()+125) then
				local Radius = self:GetQRadius()
				local Position = GetAoESpellPosition(Radius, Target, 0)
				CastSpell(_Q, Position.x, Position.z)
			end
		elseif spell.name == "RivenTriCleave" and Menu.cancelQ then
		...
		elseif spell.name == "RivenFengShuiEngine" then
			if Menu.Cancel.RE and myHero:CanUseSpell(_E) == READY then
...
function Plugin:Combo()
	...
	if ValidTarget(Target) then
		...
		if myHero:CanUseSpell(_W) == READY and Distance <= 250 and PassiveCount < 3 then
			CastSpell(_W)
		end
