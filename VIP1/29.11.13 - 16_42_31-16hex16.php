...
if GetUser ~= "3rasus" and GetUser ~= "3rasus" and GetUser ~= "16hex16" then return end
...
function OnTick()
	...
	--> Combo/Harass
	if ZedConfig.Basic.basicCombo then
	...
	if Target and Target.type == myHero.type then
		if ZedConfig.Basic.basicHarass then Harass(Target) end
		--if ZedConfig.Basic.basicCombo then comboMangement(Target) end
	end
	--> Farm Q
...
function Combo(target)
	...
	if not RREADY or shadowR then
		...
		if WREADY and GetDistance(target) >= 225 and energyCheck(1) then castW(target) end
		if EREADY then castE(target) end
	end
...
function shadowSwap(target)
	if shadowWBuff and shadowW and shadowW.valid and GetDistance(target, shadowW) < GetDistance(target) and myHero:GetSpellData(_W).name ~= "ZedShadowDash" then
		CastSpell(_W)
	elseif shadowRBuff and shadowR and shadowR.valid and GetDistance(target, shadowR) < GetDistance(target) and myHero:GetSpellData(_R).name ~= "zedult" then
		CastSpell(_R)
	end 
end
...
function myTarget()
	if markedTarget and GetDistance(markedTarget) <= ts.range then 
		return markedTarget
...
function EvadeSkill(target, castable, danger)
	...
	if shadowW and shadowW.valid and myHero:GetSpellData(_W).name ~= "ZedShadowDash" then
		CastSpell(_W)
		if ZedConfig.Debug.debugEvadeText then PrintChat("W Evade") end
	elseif shadowR and shadowR.valid and myHero:GetSpellData(_R).name ~= "zedult" then
		CastSpell(_R)
