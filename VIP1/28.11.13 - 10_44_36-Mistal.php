  ...
  if CassConfig.Barier > 0 then 
	  ...
	  if unit.team ~= myHero.team and not myHero.dead and not (unit.name:find("Minion_") or unit.name:find("Odin")) then
		  ...
		  if unit.type == "obj_AI_Hero" then
              ...
              local percentage = hpAfter/myHero.maxHealth

						  if (percentage < CassConfig.Barier / 100 and inRange > 0) or (percentage < CassConfig.Barier / 2 / 100) then
...
function FarmE()
	...
	if myHero:CanUseSpell(_E) == READY and not disableMovement then
		...
		for index, minion in pairs(enemyMinions.objects) do
			...
			if (minion.health < damage or CassConfig.LaneClear) and TargetPoisoned(minion) and GetDistance(myHero, minion) < 700 then
