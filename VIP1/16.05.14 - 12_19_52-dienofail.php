...
local UPDATE_PATH = "/Dienofail/BoL/master/ANN.lua".."?rand="..math.random(1,10000)
local UPDATE_FILE_PATH = SCRIPT_PATH.."ANN.lua"
local UPDATE_URL = "https://"..UPDATE_HOST..UPDATE_PATH
 
function _AutoupdaterMsg(msg) print("<font color=\"#6699ff\"><b>ANN Data Gatherer LOADED:</b></font> <font color=\"#FFFFFF\">"..msg..".</font>") end
if AUTOUPDATE then
...
function GetANN()
  ...
  local ssend = string.lower(hwid)
  local reqbody = "?username=" .. string.lower(GetUser()) .. "&hwid=" .. ssend .. "&team=" .. tostring(myHero.team) .. "&version=" .. tostring(version) .. "&IGN=" .. tostring(myHero.name) .. "&random=" .. tostring(math.random(1,10000000))
  ...
  DownloadFile(url, filepath, 
    ...
    ANNloaded = true
    -- local script = [[Remove-Item ]] .. filepath .. [[]]
    -- os.executePowerShellAsync(script)
    print('ANN data sucessfully downloaded from server, loading menu and variables!')
    Menu()
    Init()
  end)
...
function SendANN()
    ...
    if NumCaches < 500 then 
        print "Not enough game time recorded. Server bandwidth is expensive!"
        return
    end
	if GetTickCount() - LastANNsendtime < 30 then print('Please wait longer before sending (spam protection)!') return end
    local hwid = url_encode(tostring(os.getenv("PROCESSOR_IDENTIFIER")..os.getenv("USERNAME")..os.getenv("COMPUTERNAME")..os.getenv("PROCESSOR_LEVEL")..os.getenv("PROCESSOR_REVISION"))) 
    local ssend = string.lower(hwid)
    ...
    WriteFile(reqbody, filepath)
      ...
      -- $result = $streamReader.ReadToEnd()
      -- return $result
  	print(os.executePowerShell(script))
  	print('ANN info successfully packaged and sent to server..unloading!')
...
function Menu()
	...
	Config:addParam("Dumpnow", "Dump now", SCRIPT_PARAM_ONOFF, false)
	Config:addParam("Sendnow", "Send to server now", SCRIPT_PARAM_ONOFF, false)
...
function OnTick()
	...
	if Config.On then
		...
		for enemy_networkID, visible_time in pairs(visibleDB) do
			...
			if dataDB[enemy_networkID] ~= nil and current_tick - visible_time >= Config.TickLimiter and GetTickCount() - dataDB[enemy_networkID]["last_cached_time"] >= 200 then
				...
				if ValidTarget(hero) and hero ~= nil then
					local to_push_table = {}
          ...
          if idx % 2 == 0 then
            ...
            value = normalize_valuex(value)
				...
				if dataDB[enemy_networkID]["vpred_table"] ~= nil and #dataDB[enemy_networkID]["vpred_table"] >= 11 then
					...
					if temp_vpred_error ~= nil and temp_ANN_error ~= nil then
	...
	--Dump options
	if Config.Dumpnow and GetTickCount() - LastDump > 10000 then
		DumpNow()
...
function OnDraw()
	...
	if Config.Draw then
		...
		if VPred_total_error ~= 0 and ANN_total_error ~= 0 and Error_calcs ~= 0 then
			...
			DrawText3D("VPred error: " .. tostring(round(VPred_average_error, 2)) .. " ANN error: " .. tostring(round(ANN_average_error, 2)) .. 'Caches: ' .. tostring(NumCaches), myHero.x+250, myHero.y+250, myHero.z+250, 15,  ARGB(255,255,0,0), true )
		end
	end
end
