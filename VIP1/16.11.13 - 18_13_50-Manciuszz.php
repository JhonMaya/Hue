...
local showClose = false -- Show box locations that are close to you
local showCloseRange = 800

local drawboxSpots = false
--------------------------------------------

local red, yellow, green, blue, purple = 0x990000, 0x993300, 0x00FF00, 0x000099, 0x660066
...
function OnWndMsg(msg,key)
    if msg == KEY_DOWN and key == GetKey("W") then
        drawboxSpots = (myHero:CanUseSpell(_W) == READY)
    elseif msg == WM_RBUTTONDOWN and drawboxSpots then
        drawboxSpots = false
    elseif msg == WM_LBUTTONDOWN and drawboxSpots then
        drawboxSpots = false
        myHero:MoveTo(mousePos.x, mousePos.z)
    elseif IsKeyDown(16) and key == GetKey("W") then
        drawboxSpots = false
    end
end
...
function drawCircles(x,y,z,colour)
    ...
    if colour == red or colour == blue or colour == purple or colour == yellow then
            ...
            if drawboxSpots then
                for x, boxSpot in pairs(group.Locations) do
                    if GetDistance(boxSpot) < showLocationsInRange then
                        boxColour = (GetDistance(boxSpot, mousePos) <= 399 and 0xFFFFFF) or group.Colour
...
local boxSnapRadius = 200
local posCorrected = false
function OnSendPacket(p)
    local packet = Packet(p)
    if packet:get('name') == 'S_CAST' then
        if SPELL_2 == packet:get('spellId') and GetInGameTimer() > 55 then
            local pos = {x = packet:get('fromX'), y = myHero.y, z = packet:get('fromY') }
            for i,group in pairs(boxSpots) do
                if group.Enabled == true then
                    for x, boxSpot in pairs(group.Locations) do
                        if boxSpot ~= nil and GetDistance(pos, boxSpot) <= boxSnapRadius and not posCorrected then
                            packet:block()
                            posCorrected = true
                            CastSpell(packet:get('spellId'), boxSpot.x, boxSpot.z)
                            posCorrected = false
                            return
                        end
                    end
