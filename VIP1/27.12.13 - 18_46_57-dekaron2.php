<?php exit() ?>--by dekaron2 87.114.47.70
...
function LoadVIPPrediction()
        ...
        tpQ = ProdictManager.GetInstance():AddProdictionObject(_Q, 902, 1500, 0.5, 45, myHero)
        tpQC = ProdictManager.GetInstance():AddProdictionObject(_Q, 902, 1500, 0.5, 45, obj)
end
...
function CastE(target)
        ...
        if ValidTarget(target) then
                ...
                if next(clones)~=nil then
                for i, obj in pairs(clones) do
                        if obj:GetDistance(target) <= rangeE and EREADY then
                                CastSpell(_E)
                        end
                end
        end
...
function OnCreateObj(obj)      
        if obj ~= nil and obj.name:find("Zed_Base_W_tar.troy") then
