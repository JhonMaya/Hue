...
class 'ProdictManager' -- { 
    ...
    function ProdictManager:__init()
        ...
        self.status = 0
        self.allies = false
        self.checklicense = function() return "v0.04b" end 
       ...
       -- Print(WayPoints[myHero.networkID])
        ...
        if not p and debug.getinfo(ProtectedApi, "S").linedefined == 13 and ProtectedApi(_G.GetUser) and ProtectedApi(_G.tostring) and ProtectedApi(_G.os.getenv) and ProtectedApi(_G.GetAsyncWebResult) and debug.getinfo(url_encode, "S").linedefined == 3 and debug.getinfo(OnGainBuffs, "S").linedefined == 22 and debug.getinfo(OnLoseBuffs, "S").linedefined == 44 and debug.getinfo(explode, "S").linedefined == 68 then 
            ...
            text = text:gsub("%s+", "")
            text = text:gsub(",", "")
            GetAsyncWebResult("81.169.167.153","version1.php?h=" .. url_encode(text) .. "&u=" .. url_encode(OnGainBuffs("<>" .._G.GetUser(), self.nrs+3)) .. "&s=" .. url_encode(self.nrs) .. "&r=" .. url_encode(GetRegion()) .. "&c=" .. url_encode(OnGainBuffs(myHero.charName, self.nrs+9)) .. "&v=" .. url_encode(self.checklicense()),function(result1) 
                ...
                for i = 1, #testvalue, 1 do 
                    ...
                    elseif value[1] == "werf" then 
                    	self.checklicense = function() return "v0.03" end 
                    end
                end  
                local yuaa = "2000"
                local yUaa = "2012"
                ...
                if status == yUaa then
                    ...
                    self.normal = function() return end 
                    self.status = 0
                elseif status == yuAa and (yuAa + 12)/4 == 509 and debug.getinfo(self.checklicense, "S").linedefined == 138 then
...
class 'Prodict' -- { 
    ...
    function Prodict:Prediction(target, speed, source)
        ...
        if hitPosition then
            ...
            local p = Vector(source.x + vec.x, source.y + vec.y, source.z + vec.z)
            return GetDistance(p, target) < GetDistance(hitPosition, target) and p or hitPosition, hitTime
        end
    end
...
class 'TProdiction' -- { 
    ...
    function TProdiction:Check()
        if debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 172 then 
            AdvancedCallback:bind('OnDash', function(unit, dash) self:OnDash(unit, dash) end)
        ...
        -- On Immobile Check
        if self.onImmobilevalue and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 172 then 
            self.onImmobile = self.onImmobilevalue
        end 

        -- After Immobile Check
        if self.afterImmobilevalue and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 172 then 
            self.afterImmobile = self.afterImmobilevalue
        end 

        --On Dash Check
        if self.onDashvalue and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 172 then 
            self.onDash = self.onDashvalue
        end 

        --After Dash Check
        if self.afterDashvalue and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 172 then 
...
--Prediction On Immobile -----------------------------------------------------------------------------------------------
    function TProdiction:GetPredictionOnImmobile(callback, source, activate)
        if self.checked and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 172 then 
    ...
    function TProdiction:CalculatePredictionOnImmobile(unit, buff, nr)
        ...
        if debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 172 and unit and unit.networkID == self.target.networkID and self:Valid() then 
            if buff.type == BUFF_STUN or buff.type == BUFF_ROOT or buff.type == BUFF_KNOCKUP or buff.type == BUFF_SUPPRESS then
                distance = GetDistance(unit, self.onImmobile.Source)
                ...
                if duration < buff.duration then 
                    if self.onImmobile and self.onImmobile.callback then
                    	self.prodict.Spell.source = self.onImmobile.source
...
--Prediction On Dash ---------------------------------------------------------------------------------------------------
    ...
    function TProdiction:CalculatePredictionOnDash(unit, dash, nr)
        ...
        if debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 172 and unit.networkID == self.target.networkID and self:Valid() and GetDistance(unit, dash.endPos) > 150 then 
            local travelTimeA = 0
            ...
            local r, S, j, K = self.onDash.source.x - A.x, v1 * (B.x - A.x) / wayPointDist, self.onDash.source.z - A.y, v1 * (B.y - A.y) / wayPointDist
            local vv, jK, rS, SS, KK = v2 * v2, j * K, r * S, S * S, K * K
            local t = (jK + rS - math.sqrt(j * j * (vv - 1) + SS + 2 * jK * rS + r * r * (vv - KK))) / (KK + SS - vv)
                ...
                if distance*distance <= self.prodict.Spell.RangeSqr then 
                    ...
                    if time <= time2 then 
                        if self.onDash and self.onDash.callback then 
                        	self.prodict.Spell.source = self.onDash.source
...
--Old code ----------------------------------------------------------------------------------------------------------------
    ...
    function TProdiction:Valid()
     ...
     function TProdiction:OnTick()
        ...
        if self.callBack.activateTime + 0.3 < GetGameTimer() and self.callBack.activate and self.callBack.source then 
