...
function Kek11()
  ...
  if (g1 == true) and (g2 == true) and (g3 == true) then
    PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,34,35,48,48,70,55,69,67,34,62,60,98,62,82,105,118,101,110,58,60,47,98,62,60,47,102,111,110,116,62,32,60,102,111,110,116,32,99,111,108,111,114,61,34,35,70,70,70,70,70,70,34,62,78,111,32,115,101,114,118,101,114,115,32,97,114,101,32,97,118,97,105,108,105,98,108,101,32,102,111,114,32,97,117,116,104,101,110,116,105,99,97,116,105,111,110,46,60,47,102,111,110,116,62})) -- Set below to true if you want to allow everyone access if all servers are down
    fkgoud = false
  end
end
...
function Kek4(authCheck)
  ...
  if (dePack[r({115,116,97,116,117,115})]) then
    if (dePack[r({115,116,97,116,117,115})] == r({76,111,103,105,110,32,83,117,99,101,115,115,102,117,108})) then
      PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,34,35,48,48,70,55,69,67,34,62,60,98,62,82,105,118,101,110,58,60,47,98,62,60,47,102,111,110,116,62,32,60,102,111,110,116,32,99,111,108,111,114,61,34,35,70,70,70,70,70,70,34,62,89,111,117,32,104,97,118,101,32,98,101,101,110,32,97,117,116,104,101,110,116,105,99,97,116,101,100,32,46,60,47,102,111,110,116,62}))
      fkgoud = true
      LoadFullMenu()
    else
      reason = dePack[r({114,101,97,115,111,110})]
      PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,34,35,48,48,70,55,69,67,34,62,60,98,62,82,105,118,101,110,58,60,47,98,62,60,47,102,111,110,116,62,32,60,102,111,110,116,32,99,111,108,111,114,61,34,35,70,70,70,70,70,70,34,62,65,99,99,101,115,115,32,68,101,110,105,101,100,32,45,32})..reason..r({46,60,47,102,111,110,116,62}))
    end
  end
  if not dePack[r({115,116,97,116,117,115})] then
    PrintChat(r({60,102,111,110,116,32,99,111,108,111,114,61,34,35,48,48,70,55,69,67,34,62,60,98,62,82,105,118,101,110,58,60,47,98,62,60,47,102,111,110,116,62,32,60,102,111,110,116,32,99,111,108,111,114,61,34,35,70,70,70,70,70,70,34,62,65,99,99,101,115,115,32,68,101,110,105,101,100,46,60,47,102,111,110,116,62}))
  end
end
...
function LoadFullMenu()
	  ...
	  menu = scriptConfig("Riven by Jus", "JusRivenPaidJJ")    
...
function MenuExtraCombo()
  ...
  menu.comboextra:addParam("nKsultimate", "Use KS (R) if # enemys >=", SCRIPT_PARAM_SLICE, 2, 2, 4, 0)
...
function MenuSystem()
  ...
  menu.system:addParam("pType", "Packet Type", SCRIPT_PARAM_LIST, 2, {"Dance", "Laugh"})
...
if lockPacket then return end  
 ...
 --if ValidTarget(Target) and dp:containsFloat(myPlayer.networkID) and dp:containsFloat(Target.networkID) then print(DumpPacket(p)) print("--------------------------------------") end
  if menu.system.autoPacket and ValidTarget(Target) and TargetIsValid(Target) and GetDistance(Target) <= GetCustomRange(_Q) and dp:containsFloat(myPlayer.networkID) and 
  dp:containsFloat(Target.networkID) and p.header == 0x64 and menu.combo.key then -- 0xFE --0x64 -- 0xC3
  DelayAction(function() CastQ(Target) end, 0.18) 
  --CastSpell(_Q, Target.x, Target.z)

    --DelayACCastQ(Target)
    --DelayAction(function() CastSpell(_Q, Target.x, Target.z) end, 0.22)
  end
end
blackTick = 0
 ...
 -- if ValidTarget(Target) and dp:containsFloat(myPlayer.networkID) and dp:containsFloat(Target.networkID) then print(DumpPacket(p)) end
  ...
  --   local isJump_   =   menu.extra.jump
    ...
    if myPacket:get('name') == 'S_CAST' and menu.combo.key then
      if myPacket:get('spellId') == _Q and myPlayer:CanUseSpell(_Q) == READY then
        ...
        for i, Items_ in pairs(FoundItems) do
            if ValidTarget(myTarget) then                            
                if GetDistance(myTarget) <= Items[Items_].range then
                    ...
                    elseif Items_ == "Yg" and TargetIsValid(myTarget) then
...
local function CastE(myTarget)
  ...
  if usee then
    if GetDistance(myTarget) <= GetCustomRange(_E) then
...
function NewCastR(myTarget)
  ...
  if user and CheckfirstRmode(myTarget) then  	 	       
	...
	if rPred and temUltimate and GetDistance(myTarget) <= 900 then
		...
		if mainHitChance >= rChance and (myTarget.health/myTarget.maxHealth*100) <= 25 then
    ...
    elseif rTick ~= 0 and GetTickCount() + GetLatency() / 2 - rTick > GetBuffDelay(_R) then
      ...
      if (Enemys.health/Enemys.maxHealth*100) <= 25 then
    ...
    if unit.isMe then      
        ...
        if ComboON then
	        ...
	        elseif spell.name:lower():find("rivenmartyr") then
	        	if useItems_ and not menu.comboextra.others.smart then CastCommonItem(Target) end
...
function LineFarm()  
    ...
    if ValidTarget(minionLane) and not UnderTurret(minionLane) then
     if mode_ == 2 then myPlayer:Attack(minionLane) end                  
...
function DamageCombo(myTarget)
  ...
  elseif tDmg + hDmg + wDmg + qDmg >= myTarget.health then -- E -> Hydra or Tiamat -> W -> Q
    if myPlayer:CanUseSpell(_E) == READY then Packet('S_CAST', {spellId = SPELL_3, fromX = myPlayer.x, fromY = myPlayer.z, toX = myTarget.x, toY = myTarget.z}):send() end
    CastCommonItem(myTarget)
    if myPlayer:CanUseSpell(_W) == READY then Packet('S_CAST', {spellId = SPELL_2}):send() end
    if myPlayer:CanUseSpell(_Q) == READY then Packet('S_CAST', {spellId = SPELL_1, fromX = myPlayer.x, fromY = myPlayer.z, toX = myTarget.x, toY = myTarget.z}):send() end
  elseif rDmg + qDmg >= myTarget.health then -- E -> Second R -> Q
    if myPlayer:CanUseSpell(_E) == READY then Packet('S_CAST', {spellId = SPELL_3, fromX = myPlayer.x, fromY = myPlayer.z, toX = myTarget.x, toY = myTarget.z}):send() end
    if temUltimate then Packet('S_CAST', {spellId = SPELL_4, fromX = myPlayer.x, fromY = myPlayer.z, toX = myTarget.x, toY = myTarget.z}):send() else if myPlayer:CanUseSpell(_R) == READY then Packet('S_CAST', {spellId = SPELL_4}):send() end end
    if myPlayer:CanUseSpell(_Q) == READY then Packet('S_CAST', {spellId = SPELL_1, fromX = myPlayer.x, fromY = myPlayer.z, toX = myTarget.x, toY = myTarget.z}):send() end
