...
function CastW(pos)
	if pos ~= nil then
		local willCollide = ProdWCol:GetMinionCollision(pos, myHero)
...
function Harass(Target)
	...
	if ValidTarget(Target) then
		if Menu.harasssettings.W then
			if GetDistanceBBox(Target) < RangeW then
				CastW(WSpellPos)
			end
		end
...
function AutoRWKS()
	for i = 1, heroManager.iCount do
        ...
        if RReady and ValidTarget(Enemy, RangeW) and Enemy.health < dmg+dmg*0.8 + 25 then
        	local KSPos = ProdW:GetPrediction(Enemy)
        	local minioncollision = ProdWCol:GetMinionCollision(KSPos, myHero)
			if not minioncollision then
				CastSpell(_R)
				if HeimerRReady then
...
function Menu()
    ...
    Menu:addSubMenu("["..myHero.charName.." - Combo Settings]", "combosettings")
    	...
    	Menu.combosettings:addParam("UseR", "Use R in Combo", SCRIPT_PARAM_ONOFF, true, string.byte("G"))
