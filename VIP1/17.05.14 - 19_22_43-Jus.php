...
local function CastQ(myTarget)   
    ...
    if useq then
        ...
        if GetDistance(myTarget) <= 550 and GetTickCount() + GetLatency() / 2 - qTick > del then -- (GetTickCount() + GetLatency() / 2) - qTick > del then                      
            if mode_ == 1 then delayType = ( (del/1000) + 0.22 ) else  delayType = ( (lastWindUpTime/1000) - ((GetLatency())/2000) - 0.02 ) end
...
function OnDraw()
    ...
    if targettext_ and ValidTarget(Target) and KillText ~= nil and TargetIsValid(Target) then
