...
local i101001011011101001000 = 0.961
...
function OnTick()
	...
	if i101001011111110100100 then
		if i101001011111111100100 and i000101011111111100100 then
			...
			ts.name = "Ahri TS"
			Menu:addTS(ts)
			QProdict = Prodict:AddProdictionObject(_Q, 1000, 1700, 0.24, 50)--880
			EProdict = Prodict:AddProdictionObject(_E, 1000, 1550, 0.25, 60)--975
		...
		if i000101011111111100100 then
			if not i101001011111111100100 then
				if not i000001011111111100100 then
					...
					--if not p then PrintChat("<font color='#00BFFF'>Unauthorized user! Get license for: "..GetUser()..".</font>") end
				end
			end
		end
		if i101001011111111100100 and i000001011111111100100 and not i000101011111111100100 then
			...
			if ts.target ~= nil then
				PrintChat(""..getDmg("Q", ts.target, myHero))
				local Qdmg = 0
				local Wdmg = 0
				local Edmg = 0
				local Rdmg = 0
				local DFGdmg = 0
				local IGNdmg = 0
				--if CanQ then Qdmg = getDmg("Q", ts.target, myHero)*1 end
				if CanQ then
...
function aOnDeleteObj()
	local i31230193012331291203910 = {"spudgy"}
	local p = false
		...
		if Menu.DrawHelperMenu.drawHPtext then
			for n, DrawTarget in pairs(Enemies) do
				if DrawTarget ~= nil and DrawTarget.valid and not DrawTarget.dead and DrawTarget.team == TEAM_ENEMY then
					...
					local Wdmg = 0
					local Edmg = 0
					if CanQ then Qdmg = getDmg("Q", DrawTarget, myHero) end
