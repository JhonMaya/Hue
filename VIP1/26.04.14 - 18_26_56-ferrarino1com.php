...
function OnLoad()
        ...
        menu.combo:addParam("mode", "Combo Mode: ", SCRIPT_PARAM_LIST, 1 , {"Standard", "Custom"})
        ...
        menu.combo.gap:addParam("qqg", "Range to stop with (Q)", SCRIPT_PARAM_SLICE, 127, 125, 500, 0)
        ...
        menu.combo.cancel:addParam("advsmooth", "Auxiliary Smoothness", SCRIPT_PARAM_SLICE, 1, 0, 200, 2)
        ...
        menu.system:addParam("versionN", "Version info: ", SCRIPT_PARAM_INFO, version)
        --[[spells]]
        ...
        --[[permaShow]]
        menu.system:permaShow("versionN")
...
local Target                                                                            = nil
local aaboost, UsandoHP, UsandoRecall, temUltimate, qPassive                            = false, false, false, false, false
local lastAttack, lastWindUpTime, lastAttackCD                                          = 0, 0, 0
local myTrueRange                                                                       = myPlayer.range + GetDistance(myPlayer.minBBox)
local rTick, qTick                                                                      = 0, 0

function _OrbWalk(myTarget)
        local orbsmooth_        =       menu.system.orbsmooth 
        local rangewith			=		myPlayer.range + GetDistance(myPlayer.minBBox)
        if temUltimate then
        	rangewith = rangewith + 75
        else
        	rangewith = rangewith
       	end  
        if ValidTarget(myTarget) and GetDistance(myTarget) <= rangewith then             
                if timeToShoot() then
...
function OnSendPacket(p)
        ...
        local orbsmooth_        =       menu.system.orbsmooth
        local valid             =       ValidTarget(Target, orbsmooth_ + 10)
...
function StandardCombo(myTarget)
        ...
        if valid then                
                ...
                if rReady and user_ then 
                    ...
                    elseif GetDistance(myTarget) <= rmin_ then                          
                        CastSpell(_R)                                   
                    end             
                end

                if wReady and usew_ and GetDistance(myTarget) <= 260 then
                    Packet('S_CAST', { spellId = _W }):send()                              
                end    

                if qReady and useq_ and GetDistance(myTarget) <= 550 and not aaboost then
                    Packet('S_CAST', { spellId = _Q, x=myTarget.x, y=myTarget.z }):send()
                    qTick = os.clock() + GetLatency() / 2 * 1000
                elseif os.clock() + GetLatency() / 2 * 1000 >= qTick + 2 and qReady and useq_ and GetDistance(myTarget) <= 550 then
                    	Packet('S_CAST', { spellId = _Q, x=myTarget.x, y=myTarget.z }):send()
                    	qTick = os.clock() + GetLatency() / 2 * 1000                   
                elseif eReady and usee_ and GetDistance(myTarget) <= 380 then
                    Packet('S_CAST', { spellId = _E, x=myTarget.x, y=myTarget.z }):send()
                elseif wReady and usew_ and GetDistance(myTarget) <= 260 then
                    Packet('S_CAST', { spellId = _W }):send()                              
                end     
...
function CastQ(myTarget)
        ...
        if valid and qReady and useq_ then
        	--qTick = os.clock()
        		if GetDistance(myTarget) >= qgapStop_ and GetDistance(myTarget) <= qgap_ and qPassive and not aaboost then
                        --Packet('S_CAST', { spellId = _Q, x=myTarget.x, y=myTarget.z }):send()
                        CastSpell(_Q, myTarget.x, myTarget.z)
                        qTick = os.clock() + GetLatency() / 2 * 1000
                
                elseif aaboost and os.clock() + GetLatency() / 2 * 1000 >= qTick + 2 then
                    	Packet('S_CAST', { spellId = _Q, x=myTarget.x, y=myTarget.z }):send()
                        qTick = os.clock() + GetLatency() / 2 * 1000
                elseif GetDistance(myTarget) >= qgapStop_ and GetDistance(myTarget) <= qgap_ and not aaboost then
                	Packet('S_CAST', { spellId = _Q, x=myTarget.x, y=myTarget.z }):send()
                        qTick = os.clock() + GetLatency() / 2 * 1000
                end
        end
...
function KillEverybodyMothaFucker(myTarget)     
        ...
        if valid then
                ...
                if rReady and user_ then 
                    ...
                    if GetDistance(myTarget) <= rmin_ and aaboost then                          
                        CastSpell(_R)                                   
                    end
                end             
        ...
        if unit.isMe then                             
                for i=1, #BuffNames do
                        if buff.name:lower():find(BuffNames[i]) then
                                ...
                                if BuffNames[i] == "riventricleavesoundone" or "riventricleavesoundtwo" or "riventricleavesoundthree" then
                                	qPassive = true end     
                        end     
                end
        end
...
function LineFarm()     
        ...
        for i, Minion in pairs(MinionsInimigos.objects) do              
                if Minion ~= nil and GetDistance(Minion) <= lineR_ then                         
                        ...
                        if eReady and GetDistance(Minion) <= 380 and not aaboost then
                                Packet('S_CAST', { spellId = _E, x=Minion.x, y=Minion.z }):send()
                        end
                        if qReady and GetDistance(Minion) <= 550 and not aaboost then
                                Packet('S_CAST', { spellId = _Q, x=Minion.x, y=Minion.z }):send()
                        end     
                        if wReady and GetDistance(Minion) <= 260 and not aaboost then
                                Packet('S_CAST', { spellId = _W, }):send()                              
                        end     
                        if items_0 then
                                CastCommonItem()
...
function OnProcessSpell(unit, spell)    
        ...
        if unit.IsMe then
                ...
                if spell.name:lower():find("attack") then                       
                        ...
                        if qReady and valid and advanced_ and aaboost then
                        	qTick = os.clock()
                        	delayTime 	= 0
                        	s_adv_		= ((GetLatency() / 2 + 6) /  adv_smooth_)
                        		if s_adv_ > lastWindUpTime then
                        			delayTime = s_adv_
                        		else
                        			delayTime = spell.windUpTime
                        		end
                        		if mode_ == 2 then	
                                	DelayAction( CastQ(Target) , delayTime)
                                else
                                	DelayAction( CastSpell(_Q, Target.x, Target.z) , delayTime) 
                                end                             
                        end     
                end 
        end
        ...
        if unit.IsMe then
                ...
                if spell.name:lower():find("RivenTriCleave") then  --q     
                      qTick = os.clock() + GetLatency() / 2 * 1000
                        ...
                        if qCancel then        
                            ...
                            if movePos then
                                ...
                                elseif wReady and usew_ and ew_ and GetDistance(Target) <= wRange_ then
                        --Packet('S_CAST', { spellId = _W }):send()
                            		CastSpell(_W)
                            	end
                              -- end 
                ...
                if spell.name:lower():find("RivenFeint") and valid then --e
                        ...
                        if qReady and eq_ and GetDistance(Target) >= qgapStop_ and GetDistance(Target) <= qgap_ and not aaboost then
                ...
                if spell.name:lower():find("RivenMartyr") and valid then --w
                        if items_0 then
                                CastCommonItem()
                        end
                end

                if spell.name:lower():find("RivenFengShuiEngine") then --r
                        rTick = os.clock()
                end
