...
local function MenuComboUltimate()
	...
	menu.combo.ult:addParam("ultStop", "(R) to avoid big Damage", SCRIPT_PARAM_ONOFF, true)
	...
	menu.combo.ult:addParam("qextra", "Check (Q) extra range", SCRIPT_PARAM_ONOFF, true)
...
local function MenuFarm()
	...
	menu.farm:addSubMenu("[Last Hit Settings]", "extra")
	menu.farm.extra:addParam("delay", "Extra Delay to Last Hit", SCRIPT_PARAM_SLICE, 360, -600, 600, 0)
	menu.farm.extra:addParam("arcane", "Use Arcane Balde Masterie", SCRIPT_PARAM_ONOFF, true)
	menu.farm.extra:addParam("", "[Extra Settings]", SCRIPT_PARAM_INFO, "")
	menu.farm.extra:addParam("q", "Use (Q) with Last Hit", SCRIPT_PARAM_ONOFF, true)
	menu.farm.extra:addParam("qpassive", "Only (Q) with passive", SCRIPT_PARAM_ONOFF, true)
	menu.farm.extra:addParam("qbehind", "Only (Q) if have target behind", SCRIPT_PARAM_ONOFF, true)
	menu.farm:addParam("lasthit", "Last Hit Key", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("V"))	
...
Bind('Animation', function(unit, animation)
    if unit.isMe then
        Game.Chat.Print('Animation: '..animation.name)
    end
end)
...
function OnProcessSpell(unit, spell)
	...
	if unit.isMe then
		--print(spell.name)
		--if spell.name == "LissandraQ" then	 print(tostring(GetDistance(spell.endPos) - GetDistance(spell.startPos))) end
		...
		if spell.name:lower():find("lissandrae") and ValidTarget(Target) then
			local e_final = GetDistance(spell.endPos)
			local e_pos   = Vector(secondE):normalized()
			if e_ and secondE ~= nil and CountEnemyHeroInRange(gap_2, secondE) <= gap_1 and GetDistance(Target, sencondE) <= e_final or GetDistance(Target, secondE) <= gap_2 or GetDistance(spell.endPos) >= GetDistance(Target) then
				if not prot then CastSpell(_E)
				elseif prot and not UnderTurret(secondE) then CastSpell(_E)	end		
			end
		end
	end

	if menu.combo.ult.ultStop then
		if unti ~= nil and unit.type == myPlayer.type and spell.target.networkID == myPlayer.networkID and GetDistance(spell.endPos) <= 100 then
...
local function CheckRangeMinion(myTarget)
	local comboq			=	menu.combo.ult.qextra	
	local enemyMinions    	=   minionManager(MINION_ENEMY, 855, myPlayer, MINION_SORT_HEALTH_ASC) 
    enemyMinions:update()   
    Minion = enemyMinions.objects[1]
    if comboq and ValidTarget(Minion) and ValidTarget(myTarget) and GetDistance(Minion, myTarget) <= 155 then
    	return true
    else
    	return false
    end
...
function CastQ(myTarget)
	...
	local ComboON	=	menu.combo.key
	local extraR	=	menu.harass.qextra
	if extraR and CheckRangeMinion(myTarget) then
		if q_ and ComboON and GetDistance(myTarget) <= 855 then
			CastSpell(_Q, myTarget.x, myTarget.z)
		end
		if q_2 and GetDistance(myTarget) <= 855 then
			CastSpell(_Q, myTarget.x, myTarget.z)
		end
	else	
		if q_ and ComboON and GetDistance(myTarget) <= 725 then
			CastSpell(_Q, myTarget.x, myTarget.z)
		end
...
local function CastE(myTarget)
	...
	if e_ and ComboON and GetDistance(myTarget) <= 1200 and CountEnemyHeroInRange(1200) >= gap_ and secondE == nil then
...
local function CastEh(myTarget)
	...
	if e_ and GetDistance(myTarget) <= 1200 then
		CastSpell(_E, myTarget.x, myTarget.z)
	end
end
...
local function CastEVp(myTarget)
	...
	if e_ and GetDistance(myTarget) <= 1200 and secondE == nil and CountEnemyHeroInRange(1200) >= gap_ then
		local CastPosition, HitChance, Position = vp:GetLineCastPosition(myTarget,
		0.25, 110, 1200, 850, myPlayer)
...
local TickMinions	=	0
function LastHitMinions(myTarget)
    local delay_    		=   menu.farm.extra.delay
    local myOrb				=	menu.system.walk
    local arcane_			=	menu.farm.extra.arcane
    local q_ 				=	menu.farm.extra.q
    local qpassive_			=	menu.farm.extra.qpassive
    local qbehind_			=	menu.farm.extra.qbehind
    local minions    		= 	minionManager(MINION_ENEMY, 1000, myPlayer, MINION_SORT_HEALTH_ASC)
    minions:update()
    if GetTickCount() + GetLatency() / 2 > TickMinions and myOrb == true then
        myPlayer:MoveTo(mousePos.x, mousePos.z)
    end
    local minion = minions.objects[1]
    if ValidTarget(minion) then
        local aDmg = getDmg("AD", minion, myPlayer)
        if arcane_ then 
         	local pDmg = (myPlayer.ap)*0.05
         	aDmg = aDmg + pDmg 
        end
        if ValidTarget(minion) and minion.health <= aDmg and ThisIsReal(minion) <= myPlayer.range and GetTickCount() + GetLatency() / 2 > TickMinions then
            if q_ then
            	if qpassive_ and passive then
            		if qbehind_ and ValidTarget(myTarget) and GetDistance(minion, myTarget) <= 100 then
            ...
            TickMinions = GetTickCount() + GetLatency() / 2 + 360 + delay_
        end
    end
end
...
function OnTick()
	...
	local lasthit_	=	menu.farm.lasthit
	local realTargt	=	nil
	Target = GetTarget()
	...
	if lasthit_ then LastHitMinions(realTarget) end
