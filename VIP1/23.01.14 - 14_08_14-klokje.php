...
--Prediction CallBack --------------------------------------------------------------------------------------------------
    function TProdiction:GetPredictionCallBack(callback, source)
        if self.checked and debug.getinfo(ProdictManager.GetInstance().normal, "S").linedefined == 93 then 
            ...
            --print("added")
            self.callBack.callback = function(unit, pos , spell) callback(unit, pos , spell) end 
...
--Old code ----------------------------------------------------------------------------------------------------------------
            ...
            if rate22 > 2 then 
                ...
                if #waypoints > 1 then
                    ...
                    speed = speed + (0.317* rate)
                        ...
                        if (GetGameTimer() - self.memory.memory[#self.memory.memory].startT) < 0.3 + (100-ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance)*0.02 then
        ...
        if self.callBack.activate and self:IsValid() and self.callBack.source then 
            ...
            --if ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance <= 85 then
                if GetDistance(self.target, self.pref) <= 0 then 
                    ...
                    waittime =  0.6 + (100 - ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance)*0.008
                    ...
                    --end 
                    if waittime <= 0.8 then waittime = 0.8 end
                   -- print(GetGameTimer() - self.laststandstill)
                    if (GetGameTimer() - self.laststandstill> 0.6 and  GetGameTimer() - self.laststandstill< waittime) or GetGameTimer() - self.laststandstill>= 3 then 

                        local add = 0.07 * waittime
    ...
    --
        ...
        if self.callBack.activate and self.callBack.source then
            time = (0.01 * ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance) - 0.3
