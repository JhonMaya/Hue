...
class 'ProdictManager' -- { 
    ...
    function ProdictManager:__init()
        ...
        self.status = 0
        self.allies = false
        self.checklicense = function() return _A_'G_]_b: ?:89E=J' end 
...
--
    ...
    function Prodict:Prediction(target, speed, source, waypoin)
       ...
       --     p = 4 
       -- end 
       -- self.Spell.Delay = self.Spell.Delay + 0.0003
        ...
        tempspe = speed 
        local kk = self.Spell.Delay/(speed/1.9 ) --- ((self.Spell.Delay/10) * tempspe)
        if kk <= 0 then kk = 0 end 
        --if fast >= 20000 or fast == math.huge then
        --kk = kk + 0.2
        --end
        local delayfast = kk
        if delayfast < 0 then print("d " .. delayfast) delayfast = 0 end
        if fast < 0 then print(fast) fast = 0  end
...
--Prediction CallBack --------------------------------------------------------------------------------------------------
    function TProdiction:GetPredictionCallBack(callback, source)
        if self.checked and debug.getinfo(ProdictManager.GetInstance().normal, "S").linedefined == 93 then 
            ...
            --print("added")
            self.callBack.callback = function(unit, pos , spell, k) callback(unit, pos , spell) end 
...
--Old code ----------------------------------------------------------------------------------------------------------------
    ...
    function TProdiction:GetPrediction(pStart)
        ...
        if pStart and debug.getinfo(ProdictManager.GetInstance().normal, "S").linedefined == 93 then 
            ...
            --if rate22 > 1 then 
                  --  rate = rate22
                    rate = #self.memory.memory
                    local speed = 1 
                    speed = speed + (0.2* rate)--0.327
    ...
    function TProdiction:OnNewWayPoints(waypoints)
        ...
        if self.callBack.source ~= nil then 
            local twe = 0.2
            if self.prodict.Spell.Speed >= 20000 or self.prodict.Spell.Speed == math.huge then
                    twe = 0.2
            end         
            ...
            if rate22 > 2 then 
               ...
               -- if #waypoints > 1 then
                    local p = (Point(self.target.visionPos.x,self.target.visionPos.z) - Point(self.target.x,self.target.z)):normalize()
                    --print(GetDistance())
                    ...
                    if #self.memory.memory > 0 then 
                        ...
                        if octant ~= data then
                            ...
                            if h > 4 then 
                                ...
                                if h > 8 then
                                    ti = (#self.memory.memory * 0.2) + 1
                                     local pos, hitTime = self.prodict:Prediction(self.target, ti, self.callBack.source)
                                        if pos ~= nil and not IsWall(D3DXVECTOR3(pos.x, self.target.visionPos.y, pos.z)) and self:IsValid() then 
                                           ...
                                           -- self.prodict.Spell.source = self.callBack.source
                                            --self.callBack.callback(self.target, pos,self.prodict.Spell, 2)
                                                   --     print("1")
                                           -- self.callBack.activate = false
                                           -- return
                                                 --end
                                ...
                                if pos ~= nil and not IsWall(D3DXVECTOR3(pos.x, self.target.y, pos.z)) and self:IsValid() then
                                    newpos = Vector(pos.x, self.target.y, pos.z)
                                    local distance = GetDistance(self.target, newpos) - (GetDistance(self.target.minBBox, self.target.maxBBox)/4)
                                    local p = Vector(self.target) + Vector(newpos - Vector(self.target)):normalized() * distance
                                    self.prodict.Spell.source = self.callBack.source
                                    self.callBack.callback(self.target, p,self.prodict.Spell, 2)
                                                   --     print("1")
                                   self.callBack.activate = false
                                    return
                                end
                    ...
                    local cou = 0 
                    for i, momor in ipairs(self.memory.memory) do
                        cou = cou + momor.data.octant
                    end
                    cou = cou/#self.memory.memory
                    j = octant - cou
                   ...
                   -- print(j)
                    ...
                    local pos, hitTime = self.prodict:Prediction(self.target, j, self.callBack.source)
                                if pos ~= nil and not IsWall(D3DXVECTOR3(pos.x, self.target.y, pos.z)) and self:IsValid() then
                                    newpos = Vector(pos.x, self.target.y, pos.z)
                                    local distance = GetDistance(self.target, newpos) - (GetDistance(self.target.minBBox, self.target.maxBBox)/4)
                                    local p = Vector(self.target) + Vector(newpos - Vector(self.target)):normalized() * distance
                    ...
                    --print("Rate : " .. rate)
                    local speed = 1 
                    speed = speed + (rate*twe)

                        if ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].Targets[self.target.charName] ~= nil and ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].Targets[self.target.charName] == false then return end

                        --if speed == 1 then speed = speed/1.5  return end
                        --if (GetGameTimer() - self.memory.memory[#self.memory.memory].startT) < 0.03 + (100-ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance)*0.03 then
                        speed = speed-- - ((GetGameTimer() - self.memory.memory[#self.memory.memory].startT)*6)
                        if speed < 1 then speed = 1 end
                       -- print(speed)
    ...
    function TProdiction:OnTick()
        if self.callBack.activate and self:IsValid() and self.callBack.source then 
            ...
            for slot, time in pairs(info.root) do
                if time  < GetGameTimer() then
                    ...
                    if GetGameTimer() + (GetDistance(self.target, self.callBack.source)/self.prodict.Spell.Speed)<time and GetDistanceSqr(self.target,self.callBack.source) <= self.prodict.Spell.RangeSqr then
                        ...
                        self.prodict.Spell.source = self.callBack.source
                        self.callBack.callback(self.target, self.target, self.prodict.Spell, 3) 
                        --print("5")
            ...
            --if ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance <= 85 then
                if GetDistance(self.target, self.pref) <= 0 then 
                    ...
                    waittime =  0.4 + (100 - ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance)*0.008
                    --if ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance < 60 then 
                    --    waittime = waittime + (60 - ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance)*0.026
                    --end 
                    if waittime <= 0.6 then waittime = 0.6 end
                   -- print(GetGameTimer() - self.laststandstill)
                    if (GetGameTimer() - self.laststandstill> 0.4 and  GetGameTimer() - self.laststandstill< waittime) or GetGameTimer() - self.laststandstill>= 3 then 
                        ...
                        local add = 0--0.05 * self.standstil

                        local p, t, c = self.prodict:Prediction(self.target, 1 + add, self.callBack.source, self.lastwaypoint)
                        if p ~= nil and self:IsValid() and GetDistance(p) > 300 then 

                            if ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].Targets[self.target.charName] ~= nil and ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].Targets[self.target.charName] == false then return end
                                 newpos = Vector(p.x, self.target.y, p.z)
                                    local distance = GetDistance(self.target, newpos) - (GetDistance(self.target.minBBox, self.target.maxBBox)/4)
                                    local pos = Vector(self.target) + Vector(newpos - Vector(self.target)):normalized() * distance
                                    self.prodict.Spell.source = self.callBack.source
                                    self.callBack.callback(self.target, pos,self.prodict.Spell, 2)
                                                   --     print("1")
                                    self.callBack.activate = false
                                    return
                          --  self.prodict.Spell.source = self.callBack.source
                          --  self.callBack.callback(self.target, p, self.prodict.Spell , 1) 
                          -- self.callBack.activate = false
                          -- return 
                        end                
                    end 
            ...
            if self.standstil > 40 then 
                if ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].Targets[self.target.charName] ~= nil and ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].Targets[self.target.charName] == false then return end
                self.prodict.Spell.source = self.callBack.source
                self.callBack.callback(self.target, self.target, self.prodict.Spell, 4) 
                self.callBack.activate = false 
                return
            end 
        ...
        if self.callBack.activate and self.callBack.source then
           -- time = (0.01 * ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].HitChance) - 0.2
          ---  if time < 0 then time = 0 end
            if self.callBack.activateTime + 0.18 < GetGameTimer()  then 
                if ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].Targets[self.target.charName] ~= nil and ProdictManager.GetInstance().ProdictionConfig[self.prodict.name].Targets[self.target.charName] == false then return end
