<?php exit() ?>--by ahmedzarga23 197.0.98.4
...
local RequireI = Require("SourceLib")
  RequireI:Add("vPrediction", "https://github.com/AWABoL150/BoL/blob/master/Honda7-Scripts/common/VPrediction.lua")
  RequireI:Add("SOW", "https://github.com/AWABoL150/BoL/blob/master/Honda7-Scripts/common/SOW.lua")
  RequireI:Check()
if RequireI.downloadNeeded == true then return end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local latestVersion=nil
local updateCheck = false
local SCRIPT_NAME = "Legendary Yasuo"

function PrintMessage(msg) 
print("<font color=\"#FF7373\">["..SCRIPT_NAME.."]:</font> <font color=\"#FFDFBF\">"..msg..".</font>") 
end
...
function getVersion()
 GetAsyncWebResult("dl.dropboxusercontent.com","/s/9si6k62d5yfncsk/Yasuo.txt",getDownloadVersion)
end 

function update()
    if updateCheck == false then
        local PATH = SCRIPT_PATH.."/Yasuo2.lua"
        local URL = "http://dl.dropboxusercontent.com/s/zr4m9h4spg5dcc5/Yasuo2.lua"
        if latestVersion~=nil and latestVersion ~= VERSION then
            updateCheck = true
            PrintMessage("UPDATING Yasuo - "..SCRIPT_PATH..GetCurrentEnv().FILE_NAME)
            DownloadFile(URL, PATH,function ()
                PrintMessage("UPDATED - reload please (F9 twice)")
            end)            
        elseif latestVersion == VERSION then
            updateCheck = true
            PrintMessage("is up to date")        
        end
    end
end
...
local BlockableProjectiles = {
  ...
  ['LeblancSoulShackle'] = {charName = "Leblanc", spellSlot = "E", range = 925, width = 70, speed = 1600, delay = .5, SpellType = "skillshot",collision = true, Blockable = true ,  riskLevel = "extreme", cc = true, hitLineCheck = true},
...
local QDamage = {[1]= 20, [2] = 40,[3]= 60,[4] =80 ,[5] =100 }
--((Dodgin and Stuff))--
local DataTable = {}
local Sexpired = true
--((Other Options))--
...
function ScriptSetUp1()
  ...
  for i,enemy in pairs (Enemies) do
    ...
    Menu:addParam(tostring(enemy:GetSpellData(spell).name),"Block "..tostring(enemy.charName).." Spell "..tostring(Spells2[j]),SCRIPT_PARAM_ONOFF,true)
    end 
  end 
 end 
end 
...
--((Damage))--
DamageCalculator:RegisterDamageSource(_Q, _PHYSICAL,0,20, _PHYSICAL, _AD, 1.0, function() return (player:CanUseSpell(_Q) == READY) end)

--((MENU))--
Config = scriptConfig("Yasuo", "Yasuo")
Config:addParam("Combo", "Combo", SCRIPT_PARAM_ONKEYDOWN, false, 32)
Config:addParam("Harass", "Harass", SCRIPT_PARAM_ONKEYDOWN, false, string.byte('T'))
Config:addParam("Farm", "Farm", SCRIPT_PARAM_ONKEYDOWN, false, string.byte('X'))
...
Config.FSub:addParam("LFarm", "Lane clear mode Farm", SCRIPT_PARAM_ONKEYDOWN, false, string.byte('V'))
...
Config.Ultimate:addParam("AutoUlti","Auto-Ulti if x targets", SCRIPT_PARAM_SLICE, 1, 1, 5)
...
Config.OOSub:addParam("AutoLevel","AutoLevel Spells(require reload)",SCRIPT_PARAM_ONOFF,true)
...
function Combo()
  ...
  if itarget then
    ...
    if RDQ == false  then
        if (Qfound and  Q:IsReady() and Config.ComboSub.ComboSub1.useQ and not Config.ComboSub.Packet.useP and not isCastingE) or (isCastingE and TS:GetTarget(300)) then
      Q:Cast(Qfound)
      elseif (Qfound and  Q:IsReady() and Config.ComboSub.ComboSub1.useQ and Config.ComboSub.Packet.useP and not isCastingE) or (isCastingE and TS:GetTarget(300)) then
        CastQPacket(Qfound)
        end
    end
    if RDQ == true then
      if (QEfound and  Q:IsReady() and Config.ComboSub.ComboSub1.useQ and not Config.ComboSub.Packet.useP and not isCastingE) or (isCastingE and TS:GetTarget(300)) then
        Q:Cast(QEfound)
        elseif (QEfound and  Q:IsReady() and Config.ComboSub.ComboSub1.useQ and  Config.ComboSub.Packet.useP and not isCastingE) or (isCastingE and TS:GetTarget(300)) then
        CastQPacket(QEfound)
        end
    end
...
--((MMA))--
    ...
    if MMATarget ~= nil then
        if (Q:IsReady() and Config.ComboSub.ComboSub1.useQ and not Config.ComboSub.Packet.useP and not isCastingE) or (isCastingE and TS:GetTarget(300)) then
        Q:Cast(MMATarget)
        elseif (Q:IsReady() and Config.ComboSub.ComboSub1.useQ and  Config.ComboSub.Packet.useP and not isCastingE) or (isCastingE and TS:GetTarget(300)) then
...
function AutoIgnite()
  local Gfound = TS:GetTarget(600)
	if Gfound then
	   if iReady and getDmg("IGNITE",Gfound, myHero) >= Gfound.health then CastSpell(ignite,Gfound) end
    end
  end
...
function GetMMATarget()
  ...
  elseif not _G.MMA_Loaded  then 
    print('Failed TO detect MMA please use SOW or Use MMA at your OWN Risk ')
    end
end
    ...
    Orbwalker:EnableAttacks()
      ...
      if ValidTarget(minion) then
        ...
        if GetDistance(minion, myHero) < SpellQ.Range then
				   --print('farm5')
          if DamageCalculator:CalcSpellDamage(minion,_Q) >= minion.health then
					    --print(tostring(DamageCalculator:CalcSpellDamage(minion,_Q)))
              CastSpell(_Q, minion.x, minion.z)
            end
          end
          elseif Config.FSub.LFarm and GetDistance(minion, myHero) < SpellQ.Range then 
					  CastSpell(_Q, minion.x, minion.z)
          end
        end
      end
...
function FarmE()
  if Config.FSub.useE then
    Orbwalker:EnableAttacks()
    for _, minion in pairs(EnemyMinions.objects) do
      local EdmgMinion = getDmg("E", minion, myHero)
      if ValidTarget(minion) then
...
--[[function GetDamage(spell)
  if spell == _Q then 
	 QDamage2 = QDamage[myHero:GetSpellData(_Q).level] + myHero.damage * 1 
	  return QDamage2
   end 
end]]
...
--((OnUltimate))--
function OnUltimate()
  local TargetsRequired = Config.Ultimate.AutoUlti
  if unitsknocked and  unitsknocked > 1 then 
	  if unitsknocked >= Config.Ultimate.AutoUlti and R:IsReady() then 
      R:Cast()
    end 
  end 
end 
--((Calculate amount of enemies knocked up))--
--[[function Calculate_Knocked(spell)
	local Enemies = GetEnemyHeroes()
	local unitsknocked = 0 
	for i, enemy in pairs(Enemies) do 
		if ValidTarget(enemy,SpellR.Range) and not enemy.dead and GetDistance(enemy) <= SpellR.Range and TargetHaveBuff("yasuoq3mis",enemy) then 
			unitsknocked = unitsknocked + 1 
			 elseif CountEnemyHeroInRange(SpellR.Range,myHero) == 0 then 
				unitsknocked = 0
			  end 
			return unitsknocked
		end
...
function OnTick()
    ...
    if Config.Ultimate.AutoUlti > 1 then OnUltimate() end
    if Config.WASub.AutoWA then CastDatWall() end  
    end 
    EnemyMinions:update()
    JungleMinions:update()
...
function CastDatWall()
if not Sexpired and (GetTickCount() - DataTable.Stick) <= (DataTable.Srange/DataTable.Sspeed)*1000 then
            local spellDirection     = (DataTable.Send - DataTable.Sstart):normalized()
            local spellStartPosition = DataTable.Sstart + spellDirection
            local spellEndPosition   = DataTable.Sstart + spellDirection * DataTable.Srange
            ...
            if lineSegment:distance(heroPosition) <= 200 and W:IsReady() then
                --print('Block Dangerous Spelll with W')
                CastSpell(_W,SpellStartos,SpellEndos)
            end
      ...
      if BlockableProjectiles[spell.name]
        ...
        if  BlockableProjectiles[spell.name].Blockable then 
          if BlockableProjectiles[spell.name].SpellType == "skillshot" then 
          	  if Config.WASub.UseEva and _G.Evadeee and not _G.Evadeee_impossibleToEvade then return end 
          	     Sexpired = false
                    DataTable = {
                        SpellCaster = unit,
                        Stick = GetTickCount(),
                        Sstart = Point(spell.startPos.x, spell.startPos.z),
                        Send = Point(spell.endPos.x, spell.endPos.z),
                        Srange = BlockableProjectiles[spell.name].range,
                        Sspeed = BlockableProjectiles[spell.name].speed
												
                    }
										SpellStartos = spell.startPos.x
										SpellEndos = spell.startPos.z
                end 
             
elseif BlockableProjectiles[spell.name].SpellType == 'enemyCast' and spell.target ~= nil and  GetDistance(unit) > 400 and Menu[spell.name]  then 
