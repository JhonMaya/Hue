...
function UpdateLoad()
	--[[versionGOE = 0.001
	Update = UpdateLib("AIOCombo")
	Update.LocalVersion = versionGOE
	Update.SCRIPT_NAME = "AIOCombo"
	Update.SCRIPT_URL = "https://bitbucket.org/BoLPain/private-scripts/raw/master/AIOCombo.lua"
	Update.PATH = BOL_PATH.."Scripts\\".."AIOCombo.lua"
	Update.HOST = "painenterprises.cuccfree.com"
	Update.URL_PATH = "/Scripts/Revisions/AIOCombo.lua"
	Update:Run()	]]
	PrintChat("AIOCombo: Checking updates.")
end
PrintChat("AIOCombo: Gathering Data, please do not press f9.")
GetAsyncWebResult(
"painenterprises.cuccfree.com",
"/pain/aiocombo/auth.php",
"aioc="..GetUser(),
function(msg) 
	PrintChat("AIOCombo: Data has been gathered.")
	if msg == "authed" then 
		FakeLoad = true
		PissOff = false
...
function Load()
	...
	Menu.Skills:addParam("PacketsInfo", "Overide Spell Packet Blocking?", SCRIPT_PARAM_INFO, "")
...
function Thresh()
	...
	if Menu.General.Protect then
		if WREADY then
			if closestAlly ~= nil then 
			if(GetDistance(closestAlly) < wRange) then
...
function AFKLoader()
	...
	TimerDelay = GetTickCount()
	AFKTimer = 15000
	AFKTimerDelay = GetTickCount()
	AFKVar = false
end
function AFKDrawer()
	if FakeLoad == false then
		if CurrentTime > 0 then
			DrawText("AFK Timer: "..((CurrentTime/1000)).."(s)",25,200,200,RGBA(15,175,11,255))
		else
			DrawText("AFK Timer: Time is Up",25,200,200,RGBA(15,175,11,255))
		end
		if AFKVar ~= nil then
			DrawText("AFK: "..tostring(AFKVar),25,200,250,RGBA(15,175,11,255))
		end
	end
end
function AFKTicker()
	if FakeLoad == false then
		CurrentPosition = Vector(myHero.x,myHero.y,myHero.z)
		if GetTickCount() - AFKTimerDelay > (5000) then
			LastPosition = Vector(myHero.x,myHero.y,myHero.z)
		end
		if GetTickCount() - AFKTimerDelay > AFKTimer then
		...
		if GetTickCount() - TimerDelay > Time then
