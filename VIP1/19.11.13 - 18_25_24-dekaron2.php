...
function LoadVIPPrediction()
	...
	tpQ = wp:AddProdictionObject(_Q, rangeQ, 20000, 0.5)
	tpW = wp:AddProdictionObject(_W, rangeW, 20000, 0.375)
end
...
function CastQ(target)
	if not ValidTarget(target) or not QREADY or GetDistance(target) > rangeQ then return end
	if ValidTarget(target) then
		if GetDistance(target) <= rangeQ and QREADY then
			tpW:GetPredictionCallBack(target, QProdiction)
		end
	else
		return
	end
end
function QProdiction(target, pos)
	if pos == nil then return end 
    if GetDistance(pos) <= rangeQ and ValidTarget(target) then
        CastSpell(_Q, pos.x, pos.z)
    end
end
function CastW(target)
	if not ValidTarget(target) or not WREADY or GetDistance(target) > rangeW then return end
	if ValidTarget(target) then
		if GetDistance(target) <= rangeW and WREADY then
			tpW:GetPredictionCallBack(target, WProdiction)
		end
	else
		return
	end
end
function WProdiction(target, pos)
	if pos == nil then return end 
    if GetDistance(pos) <= rangeW and ValidTarget(target) then
...
function MoveToMouse()
	...
	if GetDistance(mousePos)>200 then
		local movePos = HeroPos +(HeroPos -pos)*(-200/GetDistance(mousePos))
