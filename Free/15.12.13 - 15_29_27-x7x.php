...
local URL_GOE = "http://noobkillerpl.cuccfree.org/aKatarina.lua"
...
function UpdateScriptGOE()
	if needRun_GOE then
		needRun_GOE = false
		if _G.UseUpdater == nil or _G.UseUpdater == true then GetAsyncWebResult("noobkillerpl.cuccfree.org", "aKatarinaVer.lua", CheckVersionGOE) end
	end
	...
	if Menu.ComboHelperMenu.ComboHelper and NextComboTick < GetTickCount() and not ultActive and not CastR then
		if ts.target ~= nil then
			...
			elseif ts.target.health < Qdmg+Wdmg+Edmg and CanQ and CanW and CanE then
				...
				if CanR and not CanE and not CanW and not CanQ and GetDistance(ts.target) < 250 then
					--[[CastR = true
					CastRtick = GetTickCount() + 5]]
...
--pointAtMaxRange = myHeroPos + directionVector:normalized() * maxRange
...
function WardJump(x,y, maxrange)
