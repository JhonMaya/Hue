...
function _AutoUpdate:GetCheckFile()
	...
	DelayAction(function() DownloadFile("https://autocarryreborn.googlecode.com/svn/Scripts/Common/conn.check", self.ConCheckPath, function() self:ConCheckDownloaded() end) end, 0.5)
end

function _AutoUpdate:ConCheckDownloaded()
	return FileExist(self.ConCheckPath)
end
...
function _AutoUpdate:_OnTick()
	...
	elseif GetTickCount() < self.StartedCheckFile + 5000 and not self:ConCheckDownloaded() then
		return
	elseif GetTickCount() > self.StartedCheckFile and not self.CancelledDownload then
		self:CannotDownload()
		return
	end


	if not self.RequiredFilesDownloadComplete then
...
function _AutoUpdate:CannotDownload()
	...
	for i, File in pairs(self.Files) do
		if not FileExist(File.Path..File.Name) and File.Required then
			table.insert(self.required, File)
		end
	end
