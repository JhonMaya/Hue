...
function _Menu()
	menu = scriptConfig("Irelia: Main Menu", "Better Nerf Irelia")
		...
		menu:addParam("Debugg", "Debugg", SCRIPT_PARAM_ONOFF, false)
		...
		menu:addSubMenu("Irelia: Combo", "combo")
			...
			menu.combo:addParam("useE","Use (E)", SCRIPT_PARAM_LIST, 1, { "ALWAYS", "STUN", "NEVER" })
		...
		menu:addSubMenu("Irelia: Harass", "harass")
			...
			menu.harass:addParam("useE","Use (E)", SCRIPT_PARAM_LIST, 1, { "ALWAYS", "STUN", "NEVER" })
		-------------------------------------------------------------------------------------------
...
if menu.combo.combokey and ts.target ~= nil then
	...
	if menu.combo.useQ2 then
		for _, minion in pairs(EnemyMinionManager.objects) do 
		  if minion and ts.target then 
		    if GetDistance(minion,ts.target) < GetDistance(ts.target,myHero) and (getDmg("Q",minion,myHero)+getDmg("AD",minion,myHero)) > minion.health then
		      print("CastSpell(_Q,minion) in Combo")
		    end 
	    end 
	  end  
	end		
		if menu.combo.useE == 2 then
			...
			(myHero.health * 100 /myHero.maxHealth) < (ts.target.health * 100 /ts.target.maxHealth) 		 then
				print("ONLY STUN!")
				print("CastSpell(_E, target) in Combo")
			end
		end 
	...
	if menu.harass.harasskey and ts.target ~= nil then
		if Qready and menu.harass.useQ and GetDistance(ts.target) < Qrange then
			print("CastSpell(_Q, target) in harass")
		end
		if Wready and menu.harass.useW and GetDistance(ts.target) < Wrange then
			print("CastSpell(_W) in harass")
		end
		if menu.harass.useE == 2 then
			if Eready and	GetDistance(ts.target) < Erange and
			(myHero.health * 100 /myHero.maxHealth) < (ts.target.health * 100 /ts.target.maxHealth) 		 then
				print("ONLY STUN!")
				print("CastSpell(_E, target) in harass") 
			end
		end 
	...
	if menu.Debugg then
		for i, enemy in pairs(Enemies) do
			...
			menu.killsteal.killstealQ then 
				print("CastSpell(_Q, ts.target) on killsteal!")
			end
		end
	end
end
...
function _lclear()
  EnemyMinionManager:update()
	...
	local clear = menu.lclear.clear
	local useQ = menu.lclear.useQ
	local useW = menu.lclear.useW
	local useR = menu.lclear.useR
	
	if Minions and GetDistance(Minions) < 160 and clear and useW and _ManaFarm() then
		CastSpell(_W)
	end
		_itemslots3()
	for i, minion in pairs(EnemyMinionManager.objects) do
		...
		clear and useQ and _ManaFarm() then
			CastSpell(_Q, minion)
		end
		if Rready and	clear and useR then
