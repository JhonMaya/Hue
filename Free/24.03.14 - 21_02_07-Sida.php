...
function _Minions:__init()
    ...
    AddTickCallback(function() self:_OnTick() end)
 -- AddCreateObjCallback(function(o) self:OnCreateObj(o) end)
...
function _Minions:OnCreateObj(Object)
	-- if Object and Object.charName and Object.team ~= TEAM_ENEMY and Object.type == "obj_AI_Minion" and Data:IsCannonMinion(Object) then
	-- 	table.insert(self.Cannons, Object)
	-- end
end

function _Minions:OnProcessSpell(Unit, Spell)
	if Unit and Spell and Spell.target and Unit.team ~= myHero.team and Unit.type == "obj_AI_Turret"  then
...
function _Minions:GetTowerMinion()
	if Orbwalker:CanOrbwalkTarget(self.TowerTarget) and self:ContainsTowerAttack(self.TowerTarget) then
		...
		local damage = VP:GetPredictedHealth2(self.TowerTarget, time)
		local myDamage = VP:CalcDamageOfAttack(myHero, self.TowerTarget, {name = "Basic"}, 0)

		-- 1 tower 1 me
		local pdamage = damage
		if self.TowerTarget.health - pdamage > -5 and self.TowerTarget.health - pdamage < myDamage then
			return nil
		end

		-- 1 tower 2 me
		pdamage = myDamage + damage
		if self.TowerTarget.health - pdamage > -5 and self.TowerTarget.health - pdamage < myDamage then
			return self.TowerTarget
		end
	end
end
...
function _Minions:WaitForCannon()
	...
	for _, min in pairs(self.EnemyMinions.objects) do
		if Data:IsCannonMinion(min) then
	...
	for i, Cannon in pairs(cans) do
		if Orbwalker:CanOrbwalkTargetCustomRange(Cannon, self.AttackRangeBuffer) then
			local time = (Orbwalker:GetWindUp() * 2) + Orbwalker:GetAnimationTime() + GetDistance(Cannon.visionPos, myHero.visionPos) / MyHero.ProjectileSpeed - 0.07
			if VP:GetPredictedHealth2(Cannon, time) < (VP:CalcDamageOfAttack(myHero, Cannon, {name = "Basic"}, 0)) + self:BonusDamage(Cannon)  then
				return Cannon
			end
		end
	end
...
function _Minions:FindKillable()
	...
	if cannon then
		local time = Orbwalker:GetWindUp() + GetDistance(cannon.visionPos, myHero.visionPos) / MyHero.ProjectileSpeed - 0.07
		local predhealth = VP:GetPredictedHealth(cannon, time)
		if Orbwalker:CanOrbwalkTargetCustomRange(cannon, self.AttackRangeBuffer) and predhealth < VP:CalcDamageOfAttack(myHero, cannon, {name = "Basic"}, 0) + self:BonusDamage(cannon)  then
			if self.TowerTarget and cannon == self.TowerTarget and predhealth < -10 then
				return
			end
			return cannon
		end
	...
	for i, minion in ipairs(self.EnemyMinions.objects) do
		...
		local predhealth = VP:GetPredictedHealth(minion, time)
		if Orbwalker:CanOrbwalkTargetCustomRange(minion, self.AttackRangeBuffer) and predhealth < VP:CalcDamageOfAttack(myHero, minion, {name = "Basic"}, 0) + self:BonusDamage(minion)  then
			if self.TowerTarget and minion == self.TowerTarget and predhealth < -10 then
				break
			end
			result = minion
			break
...
function _Minions:ShouldWait()
	for i, minion in ipairs(self.EnemyMinions.objects) do
		local time = (Orbwalker:GetWindUp() * 2) + Orbwalker:GetAnimationTime() + GetDistance(minion.visionPos, myHero.visionPos) / MyHero.ProjectileSpeed - 0.07
		if Orbwalker:CanOrbwalkTargetCustomRange(minion, self.AttackRangeBuffer) and VP:GetPredictedHealth2(minion, time) < (VP:CalcDamageOfAttack(myHero, minion, {name = "Basic"}, 0)) + self:BonusDamage(minion)  then
...
function _Minions:LaneClear()
	...
	elseif not self:ShouldWait() and Helper:GetTime() > self.LastWait + 500 then
		Orbwalker:Orbwalk(self:GetLaneClearTarget())
	elseif Helper:GetTime() > self.LastMove then
		self.LastMove = Helper:GetTime()
		MyHero:Move()
	end
end
