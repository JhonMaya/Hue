...
function OnLoad() print("Pimpin ain't easy -Doublelift 2013") end
local target
_G.evading = false
...
local autoAttackAwareness = true
local targetSelection = true
local priorty2Selected = true
...
function bestTarget()
    ...
    if priorty2Selected and ValidTarget(GetTarget(), myHero.range + HitBox, true) and GetTarget().type == myHero.type then
        return datTarget
    else
...
function OnTick()
    ...
    if testAPI.BaseWindUpTime == nil or myHero.dead then
        if IsKeyDown(_G.hotkey123) and not myHero.dead then
    ...
    if IsKeyDown(_G.hotkey123) then
...
function OnDraw()
    ...
    DrawCircle(myHero.x, myHero.y, myHero.z, myHero.range + HitBox, colors.green)
    if autoAttackAwareness then
        for i, enemy in pairs(enemyTable) do
