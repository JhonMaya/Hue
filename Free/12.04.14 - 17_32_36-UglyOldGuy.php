...
local UPDATE_TMP_FILE = LIB_PATH.."ExtraHudTemp.txt"
local ServerData
...
function checkupdate()
	if _G.ExtraHudUpdateEnabled then
    	DownloadFile(UPDATE_URL, UPDATE_TMP_FILE, UpdateCallback)
	end
...
function UpdateCallback()
    ...
    if file ~= nil then
        ...
        if content then
            ...
            if (Version ~= nil) and (Version > tonumber(_G.ExtraHudVersion)) then
                file = io.open(SELF, "w")
                    if file then
                        file:write(content)
                        file:flush()
                        file:close()
                        PrintChat("<font color=\"#00FF00\" >ExtraHud: Successfully updated to: <b>"..Version.."</b> Reload script by pressing F9 twice</font>")
                    else
                        PrintChat("<font color=\"#00FF00\" >ExtraHud: Error updating to new version (<b>"..Version.."</b>) Please manually update(check thread)</font>")
                    end
            elseif (Version ~= nil) and (Version == tonumber(_G.ExtraHudVersion)) then
                 PrintChat("<font color=\"#00FF00\" >ExtraHud: No updates found, you've got the latest version: <b>"..Version.."</b> </font>")
            end
        end
    end
end