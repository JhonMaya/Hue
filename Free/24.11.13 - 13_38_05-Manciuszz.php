...
function OnTick()
    ...
    target = (ValidTarget(ts.target, ts.range, true) and ts.target or nil) or (ts.targetSelected and ValidTarget(GetTarget(), ts.range, true) and GetTarget() or nil)
    ...
    if OrbConfig.scriptActive then
        ...
        elseif target == nil or GetTickCount() > lastAttack + lastWindUp then
            if not _G.evading and updateTargets then myHero:MoveTo(mousePos.x, mousePos.z) end
            updateTargets = true
        end
    end
end
