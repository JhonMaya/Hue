...
function _AutoUpdate:GetRequiredFiles()
	...
	-- Sprites
	table.insert(Files, {Name="bar_green.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/trunk/Sprites/SidasAutoCarry/bar_green.dds", Type = "Sprite"})
	table.insert(Files, {Name="bar_red.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/trunk/Sprites/SidasAutoCarry/bar_red.dds", Type = "Sprite"})
	table.insert(Files, {Name="updater_no.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/trunk/Sprites/SidasAutoCarry/updater_no.dds", Type = "Sprite"})
	table.insert(Files, {Name="updater_yes.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/trunk/Sprites/SidasAutoCarry/updater_yes.dds", Type = "Sprite"})
	table.insert(Files, {Name="updater_viewchanges.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/trunk/Sprites/SidasAutoCarry/updater_viewchanges.dds", Type = "Sprite"})
	table.insert(Files, {Name="updater_unselected.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/trunk/Sprites/SidasAutoCarry/updater_unselected.dds", Type = "Sprite"})

	-- Libraries
	table.insert(Files, {Name="Prodiction.lua", Path = LIB_PATH, URL = "https://autocarryreborn.googlecode.com/svn/trunk/Scripts/Common/Prodiction.lua", Type = "Library"})
	table.insert(Files, {Name="Collision.lua", Path = LIB_PATH, URL = "https://autocarryreborn.googlecode.com/svn/trunk/Scripts/Common/Collision.lua", Type = "Library"})
...
function _AutoUpdate:DownloadRequiredFiles()
	...
	for i, File in pairs(self.Files) do
		if not FileExist(File.Path..File.Name) then
			DownloadFile(File.URL, File.Path..File.Name, function() PrintSystemMessage("Downloaded "..File.Type.." "..File.Name) end)
		end
	end
end
...
function _AutoUpdate:_OnWndMsg(Msg, Key)
	if self.UpdateBoxVisible then
		if Msg == WM_LBUTTONDOWN then
			...
			elseif CursorIsUnder(WINDOW_W / 2 - 88, WINDOW_H / 2 + 32, 135, 20) then
				RunCmdCommand("start /max https://code.google.com/p/autocarryreborn/source/list?path=/trunk/Scripts/Sida%27s+Auto+Carry+-+Reborn.lua")
			end
		end
	end
end
...
function _AutoUpdate:GetLatestVersion()
	DownloadFile("https://autocarryreborn.googlecode.com/svn/trunk/Scripts/Common/Reborn.version", self.VersionPath, function() self:VersionDownloaded() end)
end

function _AutoUpdate:DownloadLatestScript()
	self:PrintDownloadingUpdate()
	self.UpdateBoxVisible = false
	DownloadFile("https://autocarryreborn.googlecode.com/svn/trunk/Scripts/Sida's%20Auto%20Carry%20-%20Reborn.lua", self.ScriptPath, function() self:ScriptDownloaded() end)
end
...
function _AutoUpdate:VersionDownloaded()
	...
	if Version then
		if tonumber(Version) > self.CurrentVersion then
			...
			PrintSystemMessage("Version "..Version.." is available!")
		else
