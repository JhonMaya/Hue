...
function _AutoUpdate:__init()
	...
	self.ScriptPath = SCRIPT_PATH .. "Sida's Auto Carry - Reborn.lua"
...
function _AutoUpdate:_OnDraw()
	if self.UpdateBoxVisible then
		...
		local x,y = (WINDOW_W/2)-(width/2), (WINDOW_H/2)-(height-2)
		local textXOffset, textYOffset = 20, 20
		local boxXOffset, boxYOffset = 30, 80

		DrawRectangle(x, y, width, height, 4294967280 )
		DrawText(tostring("Sida's Auto Carry: Reborn"), 15, x + textXOffset, y + 10, 0xFF000000)
		DrawText(tostring("An update is available"), 15, x + 30, y + 10 + textYOffset, 0xFF000000)
		DrawText(tostring("Do you want to download it?"), 15, x + 20, y + 10 + textYOffset * 2, 0xFF000000)
		DrawRectangle(x + 10 + boxXOffset, y + 20 + boxYOffset, 50, 30, 0xFF00FF00)
		DrawRectangle(x + 20 + boxXOffset * 3, y + 20 + boxYOffset, 50, 30, 0xFFFF0000)
		DrawText(tostring("Yes"), 15, x + 25 + boxXOffset, y + 30 + boxYOffset, 0xFF000000)
		DrawText(tostring("No"), 15, x + 35 + boxXOffset * 3, y + 30 + boxYOffset, 0xFF000000)
	end
end
...
function _AutoUpdate:_OnWndMsg(Msg, Key)
	if self.UpdateBoxVisible then
		if Msg == WM_LBUTTONDOWN then
			if CursorIsUnder((WINDOW_W/2)-(200/2) + 10 + 30, (WINDOW_H/2)-(150-2) + 30 + 80, 50, 30) then
				self:DownloadLatestScript()
			elseif CursorIsUnder((WINDOW_W/2)-(200/2) + 20 + 90, (WINDOW_H/2)-(150-2) + 30 + 80, 50, 30) then
				self.UpdateBoxVisible = false
			end
		end
	end
...
function _AutoUpdate:DownloadLatestScript()
	self.UpdateBoxVisible = false
	DownloadFile("https://autocarryreborn.googlecode.com/svn/trunk/Scripts/Sida's%20Auto%20Carry%20-%20Reborn.lua", self.ScriptPath, function() print("Downloaded reborn") end)
end
