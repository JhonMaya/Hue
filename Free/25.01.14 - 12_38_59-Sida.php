...
function _Minions:_OnTick()
	...
	if self.AlmostKillable and not self.AlmostKillableCheck then
		if self:GetNumberOfAttackingMinions(self.AlmostKillable) == 0 then
			self.AlmostKillableCheck = {Time = GetTickCount(), Minion = self.AlmostKillable}
		end
	elseif self.AlmostKillableCheck and self.AlmostKillable and self.AlmostKillable == self.AlmostKillableCheck.Minion then
		if GetTickCount() > self.AlmostKillableCheck.Time + 800 and self:GetNumberOfAttackingMinions(self.AlmostKillableCheck) == 0 then
			self.AlmostKillableCheck = nil
			self.AlmostKillable = nil
		end
	elseif self.AlmostKillableCheck then
		self.AlmostKillableCheck = nil
	end
...
function _Minions:DoMinionCalc(Minion, Freeze)
	if Orbwalker:CanOrbwalkTarget(Minion) then
		...
		if TotalDamage * 2.1 + MyDamage > Minion.health then
			Almost = Minion
		end
