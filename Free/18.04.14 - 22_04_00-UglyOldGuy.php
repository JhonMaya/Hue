...
function Variables()
	Spells = {
		...
		["Q"] = {key = _Q, name = "Sigil of Silence", range = 700,  ready = false, mana = 0, dmg = 0, last = false, data = myHero:GetSpellData(_Q), pdmg = 0},
...
function CastSkill(Skill, enemy)
	if Skill == _Q then
		if GetDistanceSqr(enemy) > Spells.Q.range*Spells.Q.range then
			return false
		end
		if VIP_USER then
	...
	elseif Skill == _E then
		if GetDistanceSqr(enemy) > Spells.E.range*Spells.E.range then
			return false
		end
		if VIP_USER then
			if LeblancMenu.predType == 1 then
...
function DamageCalculation()
	for i = 1, heroManager.iCount do
		...
		if ValidTarget(enemy) then
			...
			elseif enemy.health <= (Spells.Q.dmg + Spells.Q.pdmg + Spells.W.dmg + Spells.R.rqdmg + Spells.E.dmg + itemsDmg) then
