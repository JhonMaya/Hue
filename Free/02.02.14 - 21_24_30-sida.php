...
function _AutoUpdate:GetRequiredDownloads()
	if GetTickCount() > self.LastFileCheck + 5000 then
		if not self:HasRequiredFiles() then
			...
			return true
			PrintSystemMessage("Req complete")
		end
	else
		PrintSystemMessage("Failed")
		return false
	end
end
...
function _AutoUpdate:_OnTick()
	...
	elseif not self.DoneLoad and self.CanInit then
		PrintSystemMessage("Running init")
		RunLoad()
		self.DoneLoad = true
	end
PrintSystemMessage("lololol")
...
function _AutoUpdate:DownloadRequiredFiles()
	PrintSystemMessage("Creating required")
	...
	PrintSystemMessage("Create done")
end
