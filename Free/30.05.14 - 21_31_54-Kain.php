...
function OrbwalkDetection()
	...
	-- Disables SOW's inbuild autoreset 
...
function AddMenu()
	...
	RengarMenu.Extra:addParam("extraInfo", "", SCRIPT_PARAM_INFO, "")
	...
	RengarMenu.Extra:addParam("extraInfo", "", SCRIPT_PARAM_INFO, "")
	RengarMenu.Extra:addParam("extraInfo", "--- Tripple-Q Settings ---", SCRIPT_PARAM_INFO, "")
	RengarMenu.Extra:addParam("switchTargetMode", "Autoswitch SOW Targetmode while Tripple-Q: ", SCRIPT_PARAM_ONOFF, true)
	RengarMenu.Extra:addParam("trippleQDelay", "Delay for Itemusage: ", SCRIPT_PARAM_SLICE, 0.1, 0, 1, 1)
	RengarMenu.Extra:addParam("extraInfo", "", SCRIPT_PARAM_INFO, "")
	RengarMenu.Extra:addParam("extraInfo", "--- Q Settings ---", SCRIPT_PARAM_INFO, "")
	RengarMenu.Extra:addParam("qModeSwitch", "Switch Q-AttackMode: ", SCRIPT_PARAM_LIST, 1, { "Cast before AA", "Cast after AA"})
	RengarMenu.Extra:addParam("bushQ", "AA-reset with Q after you jump from a bush: ", SCRIPT_PARAM_ONOFF, true)
...
function KeyBindings()
	...
	if TrippleQKey and RengarMenu.Extra.switchTargetMode and rSOW.Menu.Mode == 1 and trippleQtargetChange
		then
	...
	if not TrippleQKey and RengarMenu.Extra.switchTargetMode and rSOW.Menu.Mode == 2 and targetModeSOW == 1
		then
			rSOW.Menu.Mode = 1
			targetModeSOW = 2
	end
...
function CastTheQ()
	if SiOrbWalker
		then
			if RengarMenu.Extra.qModeSwitch == 1
				then rSOW:RegisterBeforeAttackCallback(function() CastSpell(_Q) end)
			else rSOW:resetAA() rSOW:RegisterAfterAttackCallback(function() CastSpell(_Q) end)
		end
	end
	if MMA
		then CastSpell(_Q)
...
function CastTheW(enemy)
	if (not WREADY or (GetDistanceSqr(enemy, myHero) > wRange*wRange))
		...
		if not RengarMenu.Extra.packetUsage
			then CastSpell(_W)
		elseif RengarMenu.Extra.packetUsage
	...
	for i=1, heroManager.iCount do
		local enemy = heroManager:GetHero(i)
			if ValidTarget(enemy) and enemy ~= nil
				...
				local qDmg	= qDmgPre + aaDmg
				local qDmgE = qDmgEPre + aaDmg
	...
	if TrippleQCombo == 5
		then
			if GetDistance(Target) <= qRange and SecondQUsed == true
				then
					CastSpell(_Q)
					rSOW:RegisterAfterAttackCallback(function() trippleQtargetChange = false end)
			end
	end
else rSOW:OrbWalk()
	end
		...
		then
			if spell.name == 'RengarQ' and TrippleQCombo >= 2
				then
					...
					local delay = RengarMenu.Extra.trippleQDelay
...
function OnLoseBuff(unit, buff)
	if RengarMenu.Extra.bushQ then
		if ValidTarget(Target) and GetDistanceSqr(Target, myHero) <= 600*600 and QREADY 
			then
				if unit.isMe and buff.name == 'rengarpassivebuff'
					then
						DelayAction(function()
						if GetDistanceSqr(Target, myHero) <= (qRange*qRange)*2 --150*150
							then rSOW:resetAA() Packet("S_CAST", {spellId = _Q}):send()
						end
						end, 0.3)
				end 
		end
