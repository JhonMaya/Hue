...
function _Minions:FindKillable()
	...
	for i, minion in ipairs(self.EnemyMinions.objects) do
		...
		if Orbwalker:CanOrbwalkTargetCustomRange(minion, self.AttackRangeBuffer) and VP:GetPredictedHealth(minion, time) < VP:CalcDamageOfAttack(myHero, minion, {name = "Basic"}, 0) + self:BonusDamage(minion)  then
			result = minion
			break
		end
	end
