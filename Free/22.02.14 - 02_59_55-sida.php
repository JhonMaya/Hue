<?php exit() ?>--by sida 87.242.140.96
...
function RunLoad()
	...
	if debug.getinfo(_G.GetUser).what == "C" then
		cBa = _G.GetUser
...
function CheckID(User,Pass,Serial,Register)
   ...
   -- PrintChat(Host..authPath.."auth.php".."?a=login&user="..User.."&pass="..Pass.."&hwid="..hwid)
	...
	if Register then
		...
		for _, row in pairs(ism) do
			if row[6] == Serial then
				print("Found user "..row[7])
			end
		end
...
function checkOff()
	PrintSystemMessage("Checking for non-auth mode...")
	local data = "~sida/ocheck.php?a=ena&epoc=211965&cvar=eH56jaCjd3uushdg7659h&ua="..lTick.."&hap=f"
	--print(url_encode("/~sida/oCheck.php?a=ena&epoc=211965&cvar=eH56jaCjd3uushdg7659h&ua="..lTick.."&hap=f"))
	GetAsyncWebResult("sida2.botoflegends.com",data, CheckedOff)
end
...
function CheckedOff(result)
	if result and tostring(result):trim() == tostring(math.floor(lTick * 5 + 100 / 1257 - 2 * 4)):trim() then
		PrintSystemMessage("Non-auth mode detected! Script temporarily available for everyone.")
		AddTickCallback(MainTick)
	else
		RunLoad()
	end
