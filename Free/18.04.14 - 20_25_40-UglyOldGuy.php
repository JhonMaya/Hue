...
function FindSprite(file)
	...
	if FileExist(file) == true then
		...
		if not alreadyWarnedSprite then
			PrintChat('<font color=\'#FF0000\'>'..'Please Download latest sprites from thread')
...
function HPandMANAbars(i, hero)
	...
	DrawRectangleAL(x, yHP, HPandMANAbarsWidth(i, 1) , height, ARGB(255,0,175,0))
...
function CDtrackerUI(i, hero)
	...
	elseif stateR[i].cd ~= 0 and stateR[i].cd ~= -1 then
		...
		if Menu.appearance.Rfullcd then
			...
			elseif tonumber(stateR[i].cd) > 9 and tonumber(stateR[i].cd) <99  then
				DrawTextWithBorder(''..stateR[i].cd, 13*scalevalue.x, x+15*scalevalue.x, y+46*scalevalue.y, ARGB(255,255,255,255), ARGB(255,0,0,0))
			else
				DrawTextWithBorder(''..stateR[i].cd, 13*scalevalue.x, x+18*scalevalue.x, y+46*scalevalue.y, ARGB(255,255,255,255), ARGB(255,0,0,0))
			end
		end
	else
	...
	if _G.ExtraHudUpdateEnabled then
		...
		if FileExist(PATH.."ExtraHudSpriteVersion.txt") then
			...
			if _G.ExtraHudSpriteVersion > SpriteVersion then
				print("<font color=\"#FF0000\"><b>Extra Hud:</b> Please Download latest sprites from thread")
			else
