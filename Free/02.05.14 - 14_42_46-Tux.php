...
namez = protect {
	...
	["spudgy"] = true, ["ottohr"] = true, ["xlain"] = true, ["dansa"] = true, ["thezone"] = true,
}
_G.rawset = rawset3
...
function LoadSkillsMenu()
	...
	Config.spellSettings:addParam("wRangeSlider", "W Range - Actual Max 950", SCRIPT_PARAM_SLICE, 950, 1, 950, 0)  -- need to check for allies at 1500 range even though max range is 950 so it casts towards allies
	Config.spellSettings:addParam("eRangeSlider", "E Range", SCRIPT_PARAM_SLICE, 450, 1, 500, 0)
...
function LoadUltMenu()
	...
	Config.ultSettings:addParam("Box", "Ultimate in Combo", SCRIPT_PARAM_ONOFF, false)
	Config.ultSettings:addParam("BoxCount", "Enemies in Range", SCRIPT_PARAM_SLICE, 3, 1, 5, 0)
	Config.ultSettings:addParam("rRangeSlider", "Ultimate Range", SCRIPT_PARAM_SLICE, 400, 1, 450, 0)
end
...
function Engage()
	...
	if CanCast(_Q) and not (morganaShield == true or sivirShield == true or nocturneShield == true) and GetDistance(ts.target) <= Config.spellSettings.qRangeSlider and spellQ == "ThreshQ" then
		if Config.predictionChoice.predictionChoice == 1 then
	...
	if CanCast(_E) and GetDistance(ts.target) < Config.spellSettings.eRangeSlider then
		xPos = myHero.x + (myHero.x - ts.target.x)
		zPos = myHero.z + (myHero.z - ts.target.z)
		CastSpell(_E, xPos, zPos)
	elseif GetDistance(ts.target) > Config.spellSettings.eRangeSlider and os.time() > qCastAt + 1400 and spellQ == "threshqleap" then
		CastSpell(_Q)
	end
	if isHooked == true and not (morganaShield == true or sivirShield == true or nocturneShield == true) then
		LanternMostAr()
	end
	if Config.ultSettings.Box then AutoBox() end
end
...
function TeamFight()
	...
	if Config.lanternSettings.Lantern then LanternMostAr() end
	if Config.ultSettings.Box then AutoBox() end
end
...
function GetHighestArmorAlly()
	...
	for _, Ally in pairs(GetAllyHeroes()) do
		if Ally.armor > mostArmor and GetDistance(Ally) <= Config.spellSettings.wRangeSlider and Ally.dead == false then
...
function AutoBox()
	if CanCast(_R) and CountEnemyHeroInRange(Config.ultSettings.rRangeSlider) >= Config.ultSettings.BoxCount then
		CastSpell(_R)
	end
end
...
function OnProcessSpell(caster, spell)
    ...
    if caster.isMe and caster.valid and spell.name:lower():find("attack") and spell.animationTime and spell.windUpTime then
		aaTime = GetTickCount() + spell.windUpTime * 1000
		NextShot = GetTickCount() + spell.animationTime * 1000
	end
	if isHooked and not caster.isMe and spell.name == "LanternWAlly" then
	   CastSpell(_Q) 
	end
	if isChanneling[spell.name] and unit.team ~= myHero.team and GetDistance(spell.startPos)<=eRange then
