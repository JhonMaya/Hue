...
function Variables()
	Spells = {
		...
		["Q"] = {key = _Q, name = "Aqua Prison",           range = 875,  ready = false, dmg = 0, data = myHero:GetSpellData(_Q), speed = 1750, delay = 0.8, width = 180, pos = nil},
...
function NamiCombo()
	if Target and Target.valid then
		local WTarget = WTarget()
		local ETarget = ETarget()
		CastQ(Target)
		if WTarget ~= nil then
			CastW(WTarget)
		end
		if ETarget ~= nil and NamiMenu.combo.comboE then
			CastE(ETarget)
		end
	end
end
...
function CastW(unit)
	if (GetDistanceSqr(unit) > (Spells.W.range * Spells.W.range)) or not Spells.W.ready then
		return false
	else
...
function CastE(unit)
	if (GetDistanceSqr(unit) > (Spells.E.range * Spells.E.range)) or not Spells.E.ready then
		return false
	else
