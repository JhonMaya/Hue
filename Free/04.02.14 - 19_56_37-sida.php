...
function _AutoUpdate:GetCheckFile()
	...
	DownloadFile("https://autocarryreborn.googlecode.com/svn/Scripts/Common/conn.check", self.ConCheckPath, function() self:ConCheckDownloaded() end)
end
...
function _AutoUpdate:GetRequiredFiles()
	...
	-- Sprites
	table.insert(Files, {Name="bar_green.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/Sprites/SidasAutoCarry/bar_green.dds", Type = "Sprite", Required = true})
	table.insert(Files, {Name="bar_red.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/Sprites/SidasAutoCarry/bar_red.dds", Type = "Sprite", Required = true})
	table.insert(Files, {Name="updater_no.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/Sprites/SidasAutoCarry/updater_no.dds", Type = "Sprite"})
	table.insert(Files, {Name="updater_yes.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/Sprites/SidasAutoCarry/updater_yes.dds", Type = "Sprite"})
	table.insert(Files, {Name="updater_viewchanges.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/Sprites/SidasAutoCarry/updater_viewchanges.dds", Type = "Sprite"})
	table.insert(Files, {Name="updater_unselected.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/Sprites/SidasAutoCarry/updater_unselected.dds", Type = "Sprite"})

	-- Libraries
	table.insert(Files, {Name="Prodiction.lua", Path = LIB_PATH, URL = "https://autocarryreborn.googlecode.com/svn/Scripts/Common/Prodiction.lua", Type = "Library", Required = true})
	table.insert(Files, {Name="Collision.lua", Path = LIB_PATH, URL = "https://autocarryreborn.googlecode.com/svn/Scripts/Common/Collision.lua", Type = "Library", Required = true})
	...
	if self.UpdateBoxVisible then
		if Msg == WM_LBUTTONDOWN then
			...
			elseif CursorIsUnder(WINDOW_W / 2 - 4, WINDOW_H / 2 - 7, 135, 20) then
				...
				self.CanInit = true
			elseif CursorIsUnder(WINDOW_W / 2 - 88, WINDOW_H / 2 + 32, 135, 20) then
				RunCmdCommand("start /max https://code.google.com/p/autocarryreborn/source/list?path=/Scripts/Sida%27s+Auto+Carry+-+Reborn.lua")
			end
		end
	end
end
...
function _AutoUpdate:GetLatestVersion()
	...
	DelayAction(function() DownloadFile("https://autocarryreborn.googlecode.com/svn/Scripts/Common/Reborn.version", self.VersionPath, function() self:VersionDownloaded() end) end, 0.5)
end
...
function _AutoUpdate:DownloadLatestScript()
	...
	DelayAction(function() DownloadFile("https://autocarryreborn.googlecode.com/svn/Scripts/Sida's%20Auto%20Carry%20-%20Reborn.lua", self.ScriptPath, function() self:ScriptDownloaded() end) end, 1)
end
...
function RunLoad()
	PrintSystemMessage("Checking license, don't press F9 until complete...")
...
function DoErr(id)
	...
	if id == 1 then
		PrintSystemMessage("This username is already taken.")
	elseif id == 2 then
		PrintSystemMessage("Invalid License.")
	elseif id == 3 then
		PrintSystemMessage("Invalid HWID. Please check Reborn support thread.")
	elseif id == 4 then
		PrintSystemMessage("Your license has been suspended.")
	else
		PrintSystemMessage("Unable to auth, server being DDoS'd by some scrub.")
	end
end
...
function FinishRegister(result)
	if result and string.find(result,"claimed") then
			PrintSystemMessage("Key already claimed")
			if FileExist(tostring(SCRIPT_PATH .. "/Sida/serial.key")) then
	--os.remove(SCRIPT_PATH .. "/Sida/serial.key")
	end
		elseif  string.find(result,"SUCCESS") then
		PrintSystemMessage("Key successfully registered.")
		AddTickCallback(MainTick)
	end
end	
...
function CheckID(User,Pass,Serial,Register)
    local authPath = "Auth/"
    local Host = "sida.botoflegends.com"	
   ...
   -- PrintChat(Host..authPath.."auth.php".."?a=login&user="..User.."&pass="..Pass.."&hwid="..hwid)
	...
	if Register then
		GetAsyncWebResult(Host,authPath.."auth.php".."?a=activateSerial&serial="..Serial.."&user="..User.."&pass="..Pass.."&hwid="..hwid,FinishRegister)
		 ...
		 GetAsyncWebResult(Host,authPath.."auth.php".."?a=login&user="..User.."&pass="..Pass.."&lk=143&hwid="..hwid.."&bol="..GetUser().."&junk="..randomNumber,FinishCheck)
	end
end
