...
--[[
		...
		How to install:
			...
			Name the script EXACTLY "SidasAutoCarryPlugin - Nami.lua" without the quotes.
			Place the plugin in BoL/Scripts/Common folder.
			
		Features:
			Auto updates the script when I push updates
			Q Logic
				-PROdiction/VPrediction Q on SAC Target when in range
			W Logic
				-Heals closest lowest health ally when an ally has less health than toggle (70% default)
				-If there is more than 1 enemy near Nami then find the closest ally champion with the most enemies near them and heal them (2 heal bounces, 1 enemy bounce) 
			...
			E Logic 
				-Auto cast E on Ally with most AD within Nami's range when they begin attacking an enemy champion
			R Logic (experimental)
				-If the closest enemy is within Ultimate range and the ultimate will hit 3 or more enemies and there are 2 or more allies near me then use R
			Auto Logic
				-Uses Q on inconvienant spells (toggle)
				-Uses Q on gapclosers (toggle)
				-Auto exhausts when ally within healing range is taking significant damage (toggle)
			
		Quick rundown:
			-Combo = Q, W, R (SAC Auto Carry mode)
			-Harass = Q, W (SAC Mixed mode)
			-Auto = E, Exhaust, Knock up if Q is ready and casting a bad spell
		...
		Credits: (are commented in the code but I will also mention here)
			-Kain for helping with a few functions
			-Klokje's Power Buffs for the idea to cast E
			-Freeking's Autoupdate
			-Vaynes mighty assistant for gapcloser logic
			-Dienofail for vpred aoeskillshot position
]]
