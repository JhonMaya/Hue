...
function AfterDownload()
	...
	if DOWNLOAD_COUNT == 0 then
		...
		print("<font color=\"#FF0000\"><b> Shen - The Dark Sword of Shadow:</b></font> <font color=\"#FFFFFF\">Required libraries downloaded successfully, please reload (double F9).</font>")
	end
end
...
local UPDATE_NAME = "Shen - The Dark Sword of Shadow"
local UPDATE_HOST = "raw.github.com"
local UPDATE_PATH = "/UglyOldGuy/NintendoBoL/master/Shen%20-%20The%20Dark%20Sword%20of%20Shadow.lua".."?rand="..math.random(1,10000)
...
if _G.ShenUpdate then
	local ServerData = GetWebResult(UPDATE_HOST, UPDATE_PATH)
	if ServerData then
		local ServerVersion = string.match(ServerData, "local ShenVersion = \"%d+.%d+\"")
		ServerVersion = string.match(ServerVersion and ServerVersion or "", "%d+.%d+")
		if ServerVersion then
			ServerVersion = tonumber(ServerVersion)
			if tonumber(ShenVersion) < ServerVersion then
				AutoupdaterMsg("New version available"..ServerVersion)
				AutoupdaterMsg("Updating, please don't press F9")
				DownloadFile(UPDATE_URL, UPDATE_FILE_PATH, function () AutoupdaterMsg("Successfully updated. ("..ShenVersion.." => "..ServerVersion.."), press F9 twice to load the updated version.") end)	 
			else
...
function AutoIgnite()
	if ignitReady then
		for _, enemy in pairs(GetEnemyHeroes()) do
			...
			if enemy.health < igniteDmg and GetDistanceSqr(enemy) < 600*600 then
	...
	if object == myHero then
		if ShenMenu.flashTaunt and Target and flashReady then
    		if spell.name == "ShenShadowDash" then
    			DelayAction(function()CastSpell(flash, Target.x, Target.z) end, 0.3)
    		end
   		end
   	end
