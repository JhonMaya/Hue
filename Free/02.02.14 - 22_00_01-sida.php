...
function _AutoUpdate:GetRequiredDownloads()
	if GetTickCount() > self.LastFileCheck + 5000 then
		if not self:HasRequiredFiles() then
			PrintSystemMessage("Downloading")
			self.Files, self.Directories = self:GetRequiredFiles()
			self:DownloadRequiredFiles()
			self.LastFileCheck = GetTickCount()
			return false
		else
			PrintSystemMessage("Complete")
...
function _AutoUpdate:_OnTick()
	...
	elseif GetTickCount() < self.StartedCheckFile + 5000 and not self:ConCheckDownloaded() then
		return
	elseif GetTickCount() > self.StartedCheckFile then
		print("Unable to get check file!")
		return
	end
...
function _AutoUpdate:GetRequiredFiles()
	...
	-- Sprites
	table.insert(Files, {Name="bar_green.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/Sprites/SidasAutoCarry/bar_green.dds", Type = "Sprite"})
	table.insert(Files, {Name="bar_red.dds", Path = SPRITE_PATH.."SidasAutoCarry/", URL = "https://autocarryreborn.googlecode.com/svn/Sprites/SidasAutoCarry/bar_red.dds", Type = "Sprite"})
	...
	-- Libraries
	table.insert(Files, {Name="Prodiction.lua", Path = LIB_PATH, URL = "https://autocarryreborn.googlecode.com/svn/Scripts/Common/Prodiction.lua", Type = "Library"})
	table.insert(Files, {Name="Collision.lua", Path = LIB_PATH, URL = "https://autocarryreborn.googlecode.com/svn/Scripts/Common/Collision.lua", Type = "Library"})
