...
function CastCombo(Target) 
	...
	if GetDistance(Target) > 450 and Q1Ready and CurrentPassive then
		if PROLeeSin.othersettings.predType == 1 then
			vPred_CastQ(Target)
		else
			ProdictQ:GetPredictionCallBack(Target, prod_CastQ)
		end
	end
...
function LaneClear()
	for _, Enemy in ipairs(enemyMinions.objects) do
		if ValidTarget(Enemy) and GetDistance(Enemy) <= SmiteRange then
			...
			if E1Ready and GetDistance(Enemy) < e1Range then CastSpell(_E)  end
...
function vPred_CastQ(unit)
	if ValidTarget(unit) then
		MinHitChance = 2
		CastPosition, HitChance, HeroPosition = VP:GetLineCastPosition(enemy, qDelay, qWidth, q1Range, qSpeed, myHero, true)
		if HitChance >= MinHitChance and GetDistance(CastPosition) <= q1Range  then
...
function Menu()
    ...
    PROLeeSin:addSubMenu("["..myHero.charName.." - Other Settings]", "othersettings")
		...
		PROLeeSin.othersettings:addParam("predType", "Prediction type:", SCRIPT_PARAM_LIST, 1, { "vPrediction", "Prodiction"  })
