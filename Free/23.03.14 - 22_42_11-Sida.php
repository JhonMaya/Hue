...
function _Minions:GetLaneClearTarget()
	for i, minion in ipairs(self.EnemyMinions.objects) do
		...
		if Orbwalker:CanOrbwalkTarget(minion) and ((minion.health - pdamage) > 2 * VP:CalcDamageOfAttack(myHero, minion, {name = "Basic"}, 0) + self:BonusDamage(minion) or pdamage2 == 0)  then
			return minion
		end
	end
end
...
function _Minions:ShouldWait()
	for i, minion in ipairs(self.EnemyMinions.objects) do
		...
		--if Orbwalker:CanOrbwalkTargetCustomRange(minion, self.AttackRangeBuffer) and (minion.health - 1.5 * VP:GetPredictedHealth2(minion, time * 2)) < (VP:CalcDamageOfAttack(myHero, minion, {name = "Basic"}, 0)) + self:BonusDamage(minion)  then
		if Orbwalker:CanOrbwalkTargetCustomRange(minion, self.AttackRangeBuffer) and (minion.health - 1.5 * VP:GetPredictedHealth2(minion, time)) < (VP:CalcDamageOfAttack(myHero, minion, {name = "Basic"}, 0)) + self:BonusDamage(minion)  then
