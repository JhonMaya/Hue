<?php exit() ?>--by RoachxD 86.127.152.238
function c(bytes)
  s = {}
...
function Variables()
	...
	if VIP_USER then
		...
		ProdQCollision = Collision(Spells.Q.range, Spells.Q.speed, Spells.Q.delay, Spells.Q.width)
	end
		...
		if VIP_USER then
			if MorganaMenu.skills.q.predType == 1 then
				...
				if Spells.Q.pos and Spells.Q.pos ~= nil then
					local willCollide = ProdQCollision:GetMinionCollision(target, Spells.Q.pos)
					if not willCollide then
						if MorganaMenu.skills.q.packetCast then
							Packet("S_CAST", { spellId = _Q, toX = Spells.Q.pos.x, toY = Spells.Q.pos.z, fromX = Spells.Q.pos.x, fromY = Spells.Q.pos.z }):send()
						else
		...
		if VIP_USER then
			if MorganaMenu.skills.w.predType == 1 then
				...
				if Spells.W.pos and Spells.W.pos ~= nil then
					if MorganaMenu.skills.w.packetCast then
						Packet("S_CAST", { spellId = _W, toX = Spells.W.pos.x, toY = Spells.W.pos.z, fromX = Spells.W.pos.x, fromY = Spells.W.pos.z }):send()
					else
...
function OnProcessSpell(unit, spell)
	...
	if unit.type == myHero.type and unit.team ~= myHero.team then
		if SpellToString(unit, spell) ~= nil and spellDmg[unit.charName][SpellToString(unit, spell).."DmgM"] ~= nil then
			for i, ally in pairs(GetAllyHeroes()) do
				if spell.target == ally or spell.target == myHero then
					CastSkill(_E, ally)
				end
			end
		end
