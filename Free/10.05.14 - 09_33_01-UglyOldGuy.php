...
function ETarget()
	...
	if ((ETarget == nil) or (myHero.damage > ETarget)) and (not ZileanMenu.skills.e.eSelf) then
		ETarget = myHero
	end
	return ETarget
end
		...
		for _, enemy in ipairs(GetEnemyHeroes()) do
			if not enemy.dead and source == enemy and buff.name == "timebombenemybuff" then
				...
				if ZileanMenu.skills.ignite.advanced and ignitReady then
					if GetDistanceSqr(enemy) <= 600 * 600 and enemy.health <= (IDmg + QDmg) then
						CastSpell(ignit, enemy)
					end
				end
			end
...
function OnProcessSpell(unit, spell)
	if not isuserauthed then return end
	if ZileanMenu.skills.e.allies and (myHero.mana >= (myHero.maxMana * (ZileanMenu.skills.e.minMana / 100))) then
		if unit.team ~=  myHero.team and unit.type == myHero.type and spell.target.type == myHero.type and spell.target.team == myHero.team then
			CastE(spell.target)
		end
	end
end
