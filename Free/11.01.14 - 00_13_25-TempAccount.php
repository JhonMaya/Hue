<?php exit() ?>--by TempAccount 79.253.204.47
-- Name Check --  
...
function KillSteal()
	if Target then
		if GetDistance(Target) <= eRange and eReady and Target.health <= (eDmg + itemsDmg) then
			if myMana > eMana then
				CastE(Target)
			end
		elseif GetDistance(Target) <= wRange and wReady and Target.health <= (wDmg + itemsDmg) then 
			if myMana > wMana then
				CastW(Target)
			end
		elseif GetDistance(Target) <= wRange and eReady and wReady and Target.health <= (wDmg + eDmg + itemsDmg) then
			if myMana > (wMana + eMana) then
				CastW(Target)
			end
		elseif GetDistance(Target) <= wRange and wReady and rReady and Target.health <= (wDmg + rDmg+ itemsDmg) then
			if myMana > (wMana + rMana) then
				CastW(Target)
			end
		elseif GetDistance(Target) <= eRange and eReady and rReady and Target.health <= (eDmg + rDmg + itemsDmg) then
			if myMana > (eMana + rMana) then
				CastE(Target)
				CastR(Target)
			end
		elseif GetDistance(Target) <= wRange and eReady and wReady and Target.health <= (wDmg + eDmg + rDmg + itemsDmg) then
