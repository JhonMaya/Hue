<?php exit() ?>--by Sida 94.197.121.195
...
function _Minions:DoLastHitTargettedSpells()
	for _, Skill in pairs(Skills:GetLastHitSkills()) do
		if FarmMenu["FarmSkill"..Skill.RawName] then
			local Minion = self:GetLastHitTargettedSpell(Skill)
			if Minion then
...
function _Minions:GetLastHitTargettedSpell(Skill)
	local dmgString = "";
...
function _MenuManager:__init()
	...
	FarmMenu:addParam("farmOffsetDisplay", "No Last Hit Adjustment", SCRIPT_PARAM_INFO, "")
	FarmMenu:addParam("sep", "", SCRIPT_PARAM_INFO, "")
...
function _MenuManager:OnTick()
	...
	if FarmMenu.FarmOffset < 0 then
		FarmMenu._param[8].text = "You will attack earlier"
	elseif FarmMenu.FarmOffset > 0 then
		FarmMenu._param[8].text = "You will attack later"
	else
		FarmMenu._param[8].text = "No last hit adjustment"
	end
end
...
function _Data:__GenerateSkillData()
				  --Name 			  Enabled    Key 	 Range 		Display Name 				Type 			MinMana  AfterAA 	 Require Attack Target 	   Speed 		Delay 	Width 	Collision
