<?php exit() ?>--by Sida 81.170.70.121
		...
		function _Minions:LaneClear()
			...
			-- target = self:FindUnkillable()
			-- if target and myHero.mana / myHero.maxMana * 100 >= LastHitSkillsMenu.MinMana then
			-- 	if target and not Orbwalker:IsShooting() then
			-- 		target, skill = self:GetKillableSkillMinion(false, target)
			-- 		if target then
			-- 			self:CastOnMinion(target, skill)
			-- 		end
			...
			for i, Miniona in ipairs(self.EnemyMinions.objects) do
				...
				if Minion ~= self.LastHitMinion then
					for _, Skill in pairs(Skills:GetLastHitSkills()) do
						...
						if myHero:CanUseSpell(Skill.Key) == READY and ValidTarget(Minion, Skill.Range) and (isLaneClear and LaneClearSkillsMenu["FarmSkill"..Skill.RawName]) or (not isLaneClear and LastHitSkillsMenu["FarmSkill"..Skill.RawName]) then
		...
		function _MenuManager:__init()
			...
			DrawingMenu:addParam("EnemyRanges", "Show Enemy Ranges", SCRIPT_PARAM_ONOFF, true)
		...
		function _Drawing:_OnDraw()
			...
			if Keys.LaneClear or Keys.MixedMode or Keys.LastHit then
			...
			if DrawingMenu.EnemyRanges then
				for _, Enemy in pairs(GetEnemyHeroes()) do
					if ValidTarget(Enemy) then
						local range = Enemy.range + Data:GetGameplayCollisionRadius(Enemy.charName) + Orbwalker:GetScalingRange(Enemy)

						local red = range / GetDistance(Enemy) * 255
						red = red > 255 and 255 or red < 0 and 0 or red

						alpha = red > 0 and red or 10

						if GetDistance(Enemy) <= range then
							Helper:DrawCircleObject(Enemy, range, ARGB(255, 255, 0, 0) , 4)
						else
							if Helper.LowFPSCir then
								Helper:DrawCircleObject(Enemy, range, ARGB(alpha, 0, 255, 0) , 4)
							else
		...
		function _AutoUpdate:__init()
			AutoUpdate = self
			self.vPredVersionRequired = 2.404

			if tonumber(VP.version) < self.vPredVersionRequired then
				PrintSystemMessage("Your VPrediction library is out of date. Please get the latest.")
				return
			end
		...
		function chkUsr()
			...
			for _, usr in pairs(usrs) do
				if GetUser():lower() == usr:lower() then
					PrintSystemMessage("Logged in as "..myHero.name..". Loaded v"..AutoUpdate.CurrentVersion.."_Test as "..(VIP_USER and "VIP" or "Non-VIP").." user. Good luck!")
