<?php exit() ?>--by Sida 94.197.120.147
...
function _Helper:IsEvading()
	return _G.evade or _G.Evadeee or _G.evadeee or _G.Evade or _G.evading
end
...
function _Wards:_OnReceivePacket(p)
	...
	if p.header == 0xB5 then -- create packet
		p.pos = 12
...
function _Wards:_OnCreateObj(Obj)
	if Obj and Data:IsWard(Obj) then
		for i, Inc in pairs(self.IncomingWards) do
			if GetDistance(Inc, Obj) < 30 then
...
function _Wards:GetAttackableWard()
	for _, Ward in pairs(self.EnemyWards) do
		if GetDistance(Ward) < MyHero.TrueRange and Ward.visible then
			return Ward
		end
	end
end
...
function _Vayne:_OnTick()
	if self.bushWardPos then
		if not self.bushWardPos.Target.visible and GetTickCount() < self.bushWardPos.TimeOut then
...
function _Vayne:_OnProcessSpell(Unit, Spell)
    ...
    if Unit.type == 'obj_AI_Hero' and Unit.team == TEAM_ENEMY and GetGapCloser[Unit.charName] and GetDistance(Unit) < 2000 and Spell ~= nil then
        if Spell.name == (type(GetGapCloser[Unit.charName].spell) == 'number' and Unit:GetSpellData(GetGapCloser[Unit.charName].spell).name or GetGapCloser[Unit.charName].spell) then
            if Spell.target ~= nil and Spell.target.name == myHero.name or GetGapCloser[Unit.charName].spell == 'blindmonkqtwo' then
                CastSpell(_E, Unit)
            else
