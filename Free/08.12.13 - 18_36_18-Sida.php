...
function smartW()
	if wUsed() then
		if countEnemiesNearObj(leblancW, 600) < CountEnemyHeroInRange(600) and comboFinished then
			CastSpell(_W)
		elseif comboFinished and not Target then
			CastSpell(_W)
		end
	end
end
...
function OnCreateObj(obj)
	if obj ~= nil then
		if obj.name:find("LeBlanc_Displacement_Yellow_mis.troy") or obj.name:find("leBlanc_displacement_cas.troy") then
			leblancW = obj
		end
			...
			if not _G.evade and not _G.evading then
				if skillshot.skillshot.danger == true then
					if skillshot.skillshot.type == LINE then
						...
						evadingPosition = MyPos - (MyPos - evadingPos):normalized() * wRange
					else
						local MyPos = Vector(myHero.x, myHero.y, myHero.z)
