...
function _Minions:_OnTick()
	...
	for _, Minion in pairs(self.EnemyMinions.objects) do
		if Orbwalker:CanOrbwalkTarget(Minion) then
			...
			if TotalDamage * 2 + MyHero:GetTotalAttackDamageAgainstTarget(Minion) - 2 > Minion.health then
...
-- function _Minions:GetDamageScalePerTurnTick()
-- 	local Scale = 0 --= DebugMenu.Scale
-- 	local Delay = ((Orbwalker.LastWindUp - 250) / 1000) * 1.5
-- 	local Proj = ((10 - (MyHero.ProjectileSpeed * 5))) / 10
-- 	local LastProjectedDamageTick = 0
-- 	local LowestMinionLastHp = 0

-- 	if Delay ~= nil and Proj ~= nil then
-- 		for Minions, in pairs(Minions.EnemyMinions) do
-- 			LastProjectedDamageTick = GetTickCount()
-- 				if Minion then 
-- 					LowestMinionLastHp = Minion.health
-- 				end
-- 		end
-- 	end
	
-- 	for i, in pairs(Minions.EnemyMinions) do
-- 		if Minion.health <= LowestMinionLastHP then
-- 			return Minion.health - LowestMinionLastHP
-- 		end
-- 	end
-- end			
