...
--[[Changelog:
	BETA
		...
		0.5b -> AllyWithMoreAlliesNear and CountAllyHeroInRange reworked, added an option to not insec to allies.
		0.6b -> Fixed QSmiteQ, added whole combo (not implemented), and interrumpt ult.
	]]
...
function OnLoad() --Whatever you load when the game loads should be here.
	...
	CheckFlash()
    Menu()
    PrintChat("<font color='#FFFFFF'> >> PROLeeSin Beta 2.0 loaded << </font>")
end
...
function OnTick() --This function is updated everyframe.
	...
	elseif not ValidTarget(ts.target) then
        ...
        --if PROLeeSin.smartcombosettings.scombo and PROLeeSin.smartcombosettings.UseMoveToMouse then MoveToCursor() end
	...
	--if PROLeeSin.junglesettings.QSmiteQ then QSmiteQ() end
	Resets(ts.target)
end
		...
		if QLanded and Q2Ready and myHero.mana >= q2Mana then
			...
			if RReady and myHero.mana >= (q2Mana + rMana) then 
				...
				elseif iKSDmg > 0 and GetDistance(Target) <= rRange and Target.health <= rKSDmg + iKSDmg + q2ComboDmg(rKSDmg, Target) then
					...
					return true
				elseif iKSDmg > 0 and GetDistance(Target) > rRange and Taget.health <= rKSDmg + iKSDmg + q2ComboDmg(0, Target) then
					CastSpell(IgniteSlot, Target)
...
function Variables()
	...
	--TS
    ts = TargetSelector(TARGET_LESS_CAST_PRIORITY, q2Range, DAMAGE_PHYSICAL)
...
function Checks()
	...
	TrinketReady = CanUseSpell(ITEM_7) == READY
			...
			if QLanded and Q2Ready and myHero.mana >= q2Mana then
				...
				if RReady and myHero.mana >= (q2Mana + rMana) then 
					...
					elseif iKSDmg > 0 and EnemyDmgs.health <= rKSDmg + iKSDmg + q2ComboDmg(rKSDmg, EnemyDmgs) then
						...
						EnemyTable[i].Ready = true
					elseif iKSDmg > 0 and Taget.health <= rKSDmg + iKSDmg + q2ComboDmg(0, EnemyDmgs) then
...
function WardJump()
	...
	if W1Ready and GetDistance(Coordenates) <= WardRange and GotWard and not WardReady then
		...
		elseif TrinketReady and not WardReady then
			CastCoord(ITEM_7, Coordenates.x, Coordenates.z)
			WardReady = true
...
function OnGainBuff(Unit, buff)
	...
	-- function QOnTarget(target)
		-- if Unit == object and buff.name == "BlindMonkQOne" or "blindmonkqonechaos" then
			-- return true
		-- end
	-- end
