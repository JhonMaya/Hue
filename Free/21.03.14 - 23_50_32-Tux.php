...
function OnLoad()
    ...
    ThreshConfig:addParam("qHitChance", "Q Hit Chance Buffer", SCRIPT_PARAM_SLICE, 3, 0, 5, 0)
	ThreshConfig:addParam("BoxCount", "Enemy Count before Using Ulti", SCRIPT_PARAM_SLICE, 3, 0, 5, 0)
	ThreshConfig:addParam("BoxRange", "Use Auto Ultimate at this range", SCRIPT_PARAM_SLICE, 400, 0, 450, 0)
	ts.name = "Thresh"
...
function Engage()
	...
	if CanCast(_Q) and spellQ == "ThreshQ" then
	...
	elseif GetDistance(ts.target) > eRange and os.time() > qCastAt + 1400 and spellQ == "threshqleap" then
		CastSpell(_Q)
	end
	if hooked == true then
		LanternMostAr()
...
function LanternMostAr()
	local allyPos = GetHighestArmorAlly(wRange)
