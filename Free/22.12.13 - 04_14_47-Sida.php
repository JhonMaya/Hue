<?php exit() ?>--by Sida 81.170.95.220
...
elseif not DoneInit then
	...
	if Keys.LaneClear then 
		Skills:CastAll(Crosshair:GetTarget()) 
		if LaneClearMenu.MinionPriority and Orbwalker:CanOrbwalkTarget(Minions.KillableMinion) and not ConfigMenu.SupportMode then
	...
	if Keys.MixedMode then 		
		Skills:CastAll(Crosshair:GetTarget()) 
		if MixedModeMenu.MinionPriority and Orbwalker:CanOrbwalkTarget(Minions.KillableMinion) and not ConfigMenu.SupportMode then
			Orbwalker:Orbwalk(Minions.KillableMinion)
		elseif Orbwalker:CanOrbwalkTarget(Crosshair.Attack_Crosshair.target) then
			Items:UseAll(Crosshair.Attack_Crosshair.target)
			Orbwalker:Orbwalk(Crosshair.Attack_Crosshair.target)
...
function _Orbwalker:_OnProcessSpell(Unit, Spell)
	if Unit.isMe then
		if Data:IsAttack(Spell) then
			--local projSpeed = SpellDatabase[Spell.name]['SpellData:MissileSpeed'] and SpellDatabase[Spell.name]['SpellData:MissileSpeed'] or 0
			--MyHero:SetProjectileSpeed(projSpeed/1000)
...
function _Orbwalker:ResetAttackTimer()
	self.LastAttack = Helper.Tick - GetLatency() / 2 - self.LastAttackCooldown
	MyHero.IsAttacking = false
end
...
-- 		if Target.type == myHero.type then
-- 			local predPos = AutoCarry.GetPrediction({range=MyHero.TrueRange, speed=math.huge, delay=Orbwalker.LastWindUp}, Target)
-- 			if predPos and GetDistance(Position, predPos) - Data:GetGameplayCollisionRadius(Target.charName) + self.FleeingEnemyRangeBuffer > MyHero.TrueRange then
-- 				return false
-- 			end
-- 		end
-- 		local CheckRange
...
-- 			CheckRange = MyHero.TrueRange
-- 		end
-- 		return GetDistance(Target, Position) - Data:GetGameplayCollisionRadius(Target.charName) < CheckRange
-- 	end
-- 	return false
-- end
...
-- 	return self.Stage == STAGE_SHOOT or self.Stage == STAGE_SHOOTING
-- end
	...
	if self:HeroCanMove() and not Helper:IsEvading() and Orbwalker:CanMove() then
		if Helper.Tick > self.LastMoved + self.MoveDelay then
			...
			if self.IsMelee and Crosshair.Attack_Crosshair.target and Crosshair.Attack_Crosshair.target.type == myHero.type and 
				ConfigMenu.MeleeStickyRange > 0 and GetDistance(Crosshair.Attack_Crosshair.target) - Data:GetGameplayCollisionRadius(Crosshair.Attack_Crosshair.target) < ConfigMenu.MeleeStickyRange and 
...
function _Minions:__init()
    ...
    rangePathHero = TargetPredictionVIP(math.huge, MyHero.ProjectileSpeed, 0, 0)
end
rangePathHero = nil 
...
function _Minions:_OnTick()
	...
	Helper:Debug("Number of packets sent: " .. #updates)
	Helper:Debug("Average packet interval: " .. self.AverageUpdate)

-- local found = false
-- 	for _, Minion in pairs(self.EnemyMinions.objects) do
-- 		local SimHealth = Minion.health
-- 		local _, Time = rangePathHero:GetPrediction(Minion)
-- 		Time = Time * 1000

-- 		if self.IncomingDamage[Minion.networkID] then
-- 			for i, Damage in pairs(self.IncomingDamage[Minion.networkID]) do
-- 				Damage:Tick()
-- 				local HitTime = Damage:GetHitTime()
-- 				local SyncTime = Time + (self.FirstUpdate - Time) % 128
-- 				if HitTime and HitTime < self.FirstUpdate + SyncTime and HitTime < MyHero:GetTimeToHitTarget(Minion) then
-- 					SimHealth = SimHealth - Damage:GetDamage()
-- 				end
-- 			end
-- 			--self.IncomingDamage[Minion.networkID] = nil
-- 		end

-- 		local MyDamage = MyHero:GetTotalAttackDamageAgainstTarget(Minion, false)

-- 		if MyDamage > SimHealth and IsKeyDown(string.byte("D")) and Orbwalker:CanShoot() and GetDistance(Minion) <= myHero.range + GetDistance(myHero.minBBox) and SimHealth > 0 then
-- 			self.KillableMinion = Minion
-- 			found = true
-- 			break
-- 		end
-- 	end

-- 	if not found then self.KillableMinion = nil end

-- 	if Orbwalker:CanShoot() and self.KillableMinion and IsKeyDown(string.byte("D")) then
-- 		Orbwalker:Orbwalk(self.KillableMinion)
-- 	elseif Orbwalker:CanMove() and IsKeyDown(string.byte("D")) then
-- 		MyHero:Move()
-- 	end
...
function _Minions:_OnProcessSpell(Unit, Spell)
	...
	-- if Unit and Spell and ValidTarget(Spell.target) and Unit.type == "obj_AI_Minion" and Spell.target.type == "obj_AI_Minion" then
	-- 	if self.IncomingDamage[Spell.target.networkID] == nil then
	-- 		self.IncomingDamage[Spell.target.networkID] = {}
	-- 	end
	-- 	table.insert(self.IncomingDamage[Spell.target.networkID], _Attackk(Unit, Spell))
	-- end
...
function _Minions:_OnRecvPacket(Packet)
	if Packet.header == 0xC4 then
		self.LastUpdate = Helper.Tick - Helper.Latency / 2
		if #updates > 200 then
			table.remove(updates,1)
		end
		table.insert(updates, Helper.Tick)
	end

	self.AverageUpdate = self:GetAverageUpdateTick()
	self.NextUpdate = Helper.Tick + self.AverageUpdate
end

function _Minions:GetAverageUpdateTick()
	local _Total = 0
	if #updates < 2 then return 0 end
...
function _Minions:GetDamageScaling(Minion)
	...
	local Proj = ((MyHero.ProjectileSpeed - 1) / 10) * 1.5
	...
	Scale = (Delay + Proj + AD) / 10
	DebugMenu.Scale = Scale

	--print(Scale)

	local AttackCount = (self:GetNumberOfAttackingMinions(Minion) * 0.5)

	return 1 + (Scale * AttackCount)
end
...
function _Minions:GetTowerMinion()
	for _, Attack in pairs(self.TowerAttacks) do
		...
		local Damage = Attack.Damage * 1.1

		if ValidTarget(Minion) then
			local MyDamage = MyHero:GetTotalAttackDamageAgainstTarget(Minion, true)
...
function _Minions:LaneClear()
	if self.LastHitEnabled then
		if ConfigMenu.SupportMode then
...
function _Minions:GetPredictedDamage(Attack)
	if not ValidTarget(Attack.Source, self.ScanRange, false) or not ValidTarget(Attack.Target) or Helper.Tick > Attack.UpdatesAt then
...
function _Minions:DoMinionCalc(EnemyMinion, Freeze, IsTower)
	...
	if Orbwalker:CanOrbwalkTarget(EnemyMinion) or (MyHero.IsMelee and GetDistance(EnemyMinion) < MyHero.TrueRange + 75) then
		...
		elseif self:GetNumberOfAttackingMinions(EnemyMinion) <= 3 then
			return
		else
...
class '_Attackk'
MINION_MELEE, MINION_RANGE, Hero = 0, 1, 2
function _Attackk:__init(Source, Spell)
	self.Source = Source
	self.Spell = Spell
	self.Type = ((Source.charName:find("Wizard") or Source.charName:find("MechCannon")) and MINION_RANGE or (Source.charName:find("Basic") and MINION_MELEE or (Source.type == myHero.type and 2 or nil)))
	self.Projectile = Vector(Source)
	self.Target = Spell.target
	self.Speed = self:GetProjectileSpeed(Source, Spell)
	self.Delay = Spell.windUpTime * 1000
	self.Pathing = TargetPredictionVIP(math.huge, math.huge, 0, 0)

	if self.Type == MINION_MELEE then
		self.LastTime = GetTickCount()
		self.TimeToHit = GetTickCount() + self.Delay
	end
end

function _Attackk:Tick()
	if self.Type == MINION_RANGE then
		self.Projectile = Vector(self.Projectile) + (Vector(self.Target) - Vector(self.Projectile)):normalized() * self.Speed
	else
		self.TimeToHit = self.TimeToHit - (GetTickCount() - self.LastTime)
	end
end

function _Attackk:GetHitTime()
	if self.Type == MINION_RANGE then
		self.Pathing.Spell.Speed = self.Speed
		self.Pathing.Spell.Delay = self.Delay
		local _, Time = self.Pathing:GetPrediction(self.Target)
		return Time + GetTickCount()
	else
		return self.TimeToHit
	end
end

function _Attackk:GetDamage()
	return self.Source:CalcDamage(self.Target, self.Source.totalDamage)
end
...
function _MenuManager:__init()
	...
	ConfigMenu:addParam("Butcher", "Butcher Mastery", SCRIPT_PARAM_ONOFF, false)
	ConfigMenu:addParam("ArcaneBlade", "Arcane Blade Mastery", SCRIPT_PARAM_ONOFF, false)
	ConfigMenu:addParam("Havoc", "Havoc Mastery", SCRIPT_PARAM_ONOFF, true)
	ConfigMenu:addParam("DoubleEdgedSword", "Double-Edged Sword Mastery", SCRIPT_PARAM_ONOFF, true)
	if VIP_USER then
	...
	ConfigMenu:addParam("SupportMode", "Support Mode", SCRIPT_PARAM_ONOFF, false)
	ConfigMenu:addParam("sep", "", SCRIPT_PARAM_INFO, "")
	ConfigMenu:addParam("MeleeStickyRange", "Stick To Target Range (Melee Only)", SCRIPT_PARAM_SLICE, 0, 0, 300, 0)
	ConfigMenu:addParam("sep", "", SCRIPT_PARAM_INFO, "")
	ConfigMenu:addParam("FarmOffset", "Last Hit Adjustment", SCRIPT_PARAM_SLICE, 0, -50, 50, 0)
	...
	DebugMenu = scriptConfig("Sida's Auto Carry: Debug", "sidasacdebug")
	DebugMenu:addParam("Base", "Base", SCRIPT_PARAM_SLICE, 0, 0, 1, 1)
	DebugMenu:addParam("Scale", "Scale", SCRIPT_PARAM_SLICE, 0, 0, 0.1, 2)
	DebugMenu:permaShow("Scale")
...
function _Drawing:_OnDraw()
	...
	if DrawingMenu.MeleeSticky and ConfigMenu.MeleeStickyRange > 0 and myHero.range < 300 then
		Helper:DrawCircleObject(myHero, ConfigMenu.MeleeStickyRange, Helper:ArgbFromMenu(DrawingMenu.MeleeStickyColour))
	end
end
