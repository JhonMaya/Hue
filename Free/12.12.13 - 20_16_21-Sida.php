...
function _Orbwalker:ResetAttackTimer()
	self.LastAttack = Helper.Tick - Helper.Latency / 2 - self.LastAttackCooldown
	MyHero.IsAttacking = false
end
...
function _Orbwalker:CanMove()
	return (GetTickCount() + GetLatency() / 2 > self.LastAttack + self.LastWindUp + 20)
end

function _Orbwalker:CanShoot()
	return (GetTickCount() + GetLatency() / 2 > self.LastAttack + self.LastAttackCooldown)
end
...
function _Orbwalker:CanOrbwalkTarget(Target)
	if ValidTarget(Target) then
		if Target.type == myHero.type then
			...
			if predPos and GetDistance(predPos) - Data:GetGameplayCollisionRadius(Target.charName) - Data:GetGameplayCollisionRadius(myHero.charName) > myHero.range then
				return false
			end
			return GetDistance(Target) - Data:GetGameplayCollisionRadius(Target.charName) - Data:GetGameplayCollisionRadius(myHero.charName) < myHero.range 
		end
		if GetDistance(Target) - Data:GetGameplayCollisionRadius(myHero.charName) < myHero.range then
			return true
		end
	end
	return false
...
function _MyHero:GetTimeToHitTarget(Target)
	local AttackTime = (GetDistance(Target) / self.ProjectileSpeed) + (Helper.Latency / 2) + Orbwalker.LastWindUp
...
function _Minions:_OnProcessSpell(Unit, Spell)
	...
	if Unit and Spell.target and Unit.team == myHero.team and GetDistance(Unit) <= self.ScanRange then
...
function _Minions:GetDamageScaling(Minion)
	local Scale = 0.05
	local Base = 0.8
	local AttackCount = self:GetNumberOfAttackingMinions(Minion)

	return Base + (Scale * AttackCount)
end
...
function _Minions:DoMinionCalc(EnemyMinion, Freeze)
	...
	if Orbwalker:CanOrbwalkTarget(EnemyMinion) or (MyHero.IsMelee and GetDistance(EnemyMinion) < MyHero.TrueRange + 75) then
		if EnemyMinion.health < MyDamage or self:CanExecuteMinion(EnemyMinion) then
			...
			if EnemyMinion.health - (TotalDamage * 2) < MyDamage then
...
function _Helper:_OnDraw()
	...
	for _, Str in pairs(self.DebugStrings) do
		DrawText(Str, 15, 100, Height, 0xFFFFFF00)
		Height = Height + 20
	end
	self.DebugStrings = {}
end
