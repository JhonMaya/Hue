...
function _MyHero:GetTimeToHitTarget(Target)
	...
	if self.IsMelee then
		return GetTickCount() + self.LastWindUp + GetLatency() / 2
	else
...
function _Minions:_OnTick()
	...
	for _, Minion in pairs(self.EnemyMinions.objects) do
		if Orbwalker:CanOrbwalkTarget(Minion) then
			...
			if PredictedDamage < Minion.health and PredictedDamage + MyHero:GetTotalAttackDamageAgainstTarget(Minion) - 2 > Minion.health then
