...
function Menu()
	...
	Menu:addSubMenu("["..myHero.charName.." Auto Carry: Advanced]", "Advanced")
		Menu.Advanced:addParam("WEDistanceFromEnemy", "W/E Cast Distance from Enemy", SCRIPT_PARAM_SLICE, 125, 10, 250, 0)
		...
		Menu.Advanced:addParam("SatchelJumpDistance", "Satchel Jump Distance", SCRIPT_PARAM_SLICE, 50, 10, 200, 0)
...
function _G.PluginOnLoad()
	...
	if IsSACReborn then
		AutoCarry.Crosshair:SetSkillCrosshairRange(QMaxRange)
	else
...
function CastSkillshotBounce(skill, PredictedPos)
	if PredictedPos and AutoCarry.IsValidHitChance(SkillQ, Target) then
		...
		if GetDistance(EnemyPos) < QRange then
			if not skill.minions or not AutoCarry.GetCollision(skill, myHero, EnemyPos) then
...
function CastW(noTarget)
	if noTarget and WReady then
		...
		local posX = myHero.x + (vectorX * Menu.Advanced.SatchelJumpDistance)
		local posZ = myHero.z + (vectorZ * Menu.Advanced.SatchelJumpDistance)
