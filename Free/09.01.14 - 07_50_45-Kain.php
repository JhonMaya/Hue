...
function Menu()
	...
	Menu:addSubMenu("["..myHero.charName.." Auto Carry: Auto Carry]", "AutoCarry")
		...
		Menu.AutoCarry:addParam("Killsteal", "Ultimate Mega Killsteal", SCRIPT_PARAM_ONOFF, true)
...
function _G.PluginOnTick()
	...
	if Menu.AutoCarry.Killsteal and IsTickReady(60) and not lastAFKStatus then
		KillSteal()
	end
...
function FireW(unit, predic, spell)
	if WReady and ValidTarget(unit, WRange) and predic and GetDistance(predic) < WRange and not unit.dead then
		...
		if CastPos and GetDistance(CastPos) <= WRange then CastSpell(_W, CastPos.x, CastPos.z) end
	end
end
...
function FireE(unit, predic, spell)
	if EReady and ValidTarget(unit, ERange) and predic and GetDistance(predic) < ERange and not unit.dead then
		...
		if CastPos and GetDistance(CastPos) <= ERange then CastSpell(_E, CastPos.x, CastPos.z) end
	end
end
...
function StealJungle()
	...
	if Nashor ~= nil and GetDistance(Nashor) < 1300 and Nashor.visible then KillJungle(Nashor) end
	if Dragon ~= nil and GetDistance(Dragon) < 1300 and Dragon.visible then KillJungle(Dragon) end
	if Golem1 ~= nil and GetDistance(Golem1) < 1300 and Golem1.visible then KillJungle(Golem1) end
	if Golem2 ~= nil and GetDistance(Golem2) < 1300 and Golem2.visible then KillJungle(Golem2) end
	if Lizard1 ~= nil and GetDistance(Lizard1) < 1300 and Lizard1.visible then KillJungle(Lizard1) end
	if Lizard2 ~= nil and GetDistance(Lizard2) < 1300 and Lizard2.visible then KillJungle(Lizard2) end	
end

function KillJungle(object)
	...
	if QReady and GetDistance(object) < QMaxRange and object.health  + 50 < getDmg("Q", object, myHero) then
		CastSkillshotBounce(SkillQ, object)
	elseif RReady and GetDistance(object) < RRange and object.health + 50 < getDmg("R", object, myHero) then
		CastSpell(_R, object.x, object.z)
	elseif object.health + 50 < DmgOnObject and GetDistance(object) < QMaxRange and GetDistance(object) < RRange then
		CastSkillshotBounce(SkillQ, object)
		CastSpell(_R, object.x, object.z)
	end
end
...
function _G.PluginOnDraw()
	...
	if Menu.Draw.DrawDmg and useDrawDamageLib then DrawDamage() end
		...
		for _, enemy in ipairs(GetEnemyHeroes()) do
			if ValidTarget(enemy) then
				if enemy ~= nil and enemy.team ~= myHero.team and not enemy.dead and enemy.visible and GetDistance(enemy) < IgniteRange and enemy.health < getDmg("IGNITE", enemy, myHero) then
					if Main.Advanced.DoubleIgnite and not TargetHaveBuff("SummonerDot", enemy) then
						CastSpell(ignite, enemy)
					elseif not Main.Advanced.DoubleIgnite then
						CastSpell(ignite, enemy)
					end
				end
			end
