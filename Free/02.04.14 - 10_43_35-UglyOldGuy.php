...
function OnLoad()
	...
	--evadeLoad()
	LeblancMenu()
	PrintChat("<font color='#FFFF00'> >> Leblanc - The Deceiver 1.1 Loaded!! <<</font>")
end
...
function LeblancMenu()
	...
	LeblancMenu:addSubMenu("["..myHero.charName.." - Misc Settings]", "misc")
		...
		LeblancMenu.misc:addParam("predType", "Prediction Type", SCRIPT_PARAM_LIST, 1, { "Prodiction", "VPrediction" })
end
...
function CastW(enemy)
	...
	if ValidTarget(enemy) and not wUsed() then
		if VIP_USER then
			if LeblancMenu.misc.predType == 1 then
