<?php exit() ?>--by andreluis034 188.81.133.194
--Encrypt this line and below
-- Auto Updater info
local UPDATE_HOST = 'tarado.no-ip.org'
local UPDATE_PATH = '/andre/AndreBol/raw/master/extrahud.lua'--?rand='..math.random(1,10000) -- 'Disable' caching to prevent older versions from being retrieved
...
local ID = myHero.networkID..myHero.charName.."5"
...
local randomName = "TempFile"..math.random(100000)
--file = io.open(SCRIPT_PATH .. "Movements.txt", "w")
--Main functions

function OnLoad()
	--print((GetGame().map.index == 8 and 4.5) or (GetGame().map.index == 12 and 0) or 8)
	menu()
	...
	if not VIP_USER then
		PrintChat('<font color=\'#'..Chatcolor..'\'>'..'<b>Extra Hud: </b><font color=\'#F8F8F8\'>Version '.._G.ExtraHudVersion..'(Free user) Loaded.'..'</font>')
	else
		PrintChat('<font color=\'#'..Chatcolor..'\'>'..'<b>Extra Hud: </b><font color=\'#F8F8F8\'>Version '.._G.ExtraHudVersion..'(Vip user) Loaded.'..'</font>')
	end
	CheckUpdates()
end
...
function OnTick()	
	...
	if initDone then
		if os.clock() - TickLimit > Menu.performance.OnTickrefreshRate*(1/1000) then
			TickLimit = os.clock()
...
function OnDraw()
	...
	if CanDraw(Menu.performance.refreshRate) then
...
function OnWndMsg(msg,keycode) 
	...
	if Menu.reset.SoftReset then
		if os.clock() - TickLimit3 >= 0.2 then
			TickLimit3 = os.clock()
			--print("hi")
			for i = 1, heroManager.iCount, 1 do
				local enemy = heroManager:getHero(i)
				if enemy.team ~= myHero.team then
					spriteTable[i].sprite:Release()
					enemyTable[i].isRecalling = false
					enemyTable[i].RecallPhase = 0 
			...
			for i=1, #SSpells, 1 do
				local spell = SSpells[i]
				spell.sprite:Release()
				spell.sprite = FindSprite(PATH..spell.Name..'.dds')
				spell.sprite:SetScale(0.28*scalevalue.x, 0.36*scalevalue.y)
			end
			PrintChat('<font color=\'#'..Chatcolor..'\'>'..'<b>Extra Hud: </b><font color=\'#F8F8F8\'>Soft Reset complete, if issue persists Reload(doubl F9).</font>')
		end
	end
--[[elseif Menu.reset.HardReset then
		if os.clock() - TickLimit3 >= 0.2 then
			TickLimit3 = os.clock()
			local UI = SavedPos['UI']
			for i = 1, heroManager.iCount, 1 do
			...
			Menu.appearance.scalebyuser = 100
			UI={x=1502*defaultscalevalue.x, y=114*defaultscalevalue.y}
		end]]
end
...
function OnRecvPacket(p)
	local enemyTable = Saved['eTable']
	if Menu.experimental.FowRecall then
	 	if p.header == 0xD7 then
	 		local pacote = p
	 		--PrintChat("Packet received(Recall)")
	 		pacote.pos = 31
	 		local TimerRec = p:Decode1()
	 		--print(TimerRec)
	  		pacote.pos = 5
	   		...
	   		for i=1, heroManager.iCount, 1 do
				local enemy = heroManager:getHero(i)
				if enemy.team ~= myHero.team and enemy.networkID == networkID then
					if GetGame().map.index == 1 and TimerRec == 2 and enemyTable[i].RecallingTime ~= 7 then
						enemyTable[i].RecallingTime = 7
					end
					if TimerRec == 2 or TimerRec == 3 then
						if enemyTable[i].RecallPhase > 1 then
							---print(enemyTable[i].RecallPhase)
							enemyTable[i].RecallTimer = enemyTable[i].RecallPhase 
							enemyTable[i].RecallPhase = -1
						else
							enemyTable[i].RecallPhase = 0 
							enemyTable[i].isRecalling = false 
						end
						enemyTable[i].isRecalling = false
						--PrintChat(enemy.charName .." Rcall finished")
					else
						enemyTable[i].isRecalling = GetInGameTimer() + enemyTable[i].RecallingTime
						enemyTable[i].RecallPhase = enemyTable[i].RecallingTime
					end
	    			--PrintChat(enemy.charName .." is recalling")
	   			end 
	   		end
	 	end
	else
...
function RecallTimer(i, hero)
	if VIP_USER then
		local enemyTable = Saved['eTable']
		if enemyTable[i].isRecalling then
			if enemyTable[i].RecallPhase == -1 then
				--Canceled
			else
				enemyTable[i].RecallPhase = enemyTable[i].isRecalling - GetInGameTimer()
			end
...
function getDeathTimer(unit)
	if unit.deathTimer < 8 and Menu.experimental.CustomDT then -- check if it is broken or not
		...
		return (GetInGameTimer() < 1800 and ddt) or (GetInGameTimer() <= 3300 and ddt+(ddt*(1/3000*(GetInGameTimer()-1800)))) or ddt+ddt*0.5
	else
...
function DrawRecall(i, hero)
	if VIP_USER then
		local enemyTable = Saved['eTable']
		if enemyTable[i].RecallPhase ~= 0 then
			if enemyTable[i].RecallPhase == -1 then
				--print(enemyTable[i].RecallPhase)
				DrawRectangleAL(enemyTable[i].x+59*scalevalue.x, enemyTable[i].y+52*scalevalue.y, -(59*scalevalue.x)*enemyTable[i].RecallTimer/enemyTable[i].RecallingTime, 14.95*scalevalue.y, ARGB(255,175,0,0))
				someshit = i
				DelayAction(function() --print("hi")
					if enemyTable[someshit].RecallPhase == -1 then 
						--print("hi")
						enemyTable[someshit].RecallPhase = 0 
						enemyTable[someshit].isRecalling = false 
					end 
				end, 2)
			else
				DrawRectangleAL(enemyTable[i].x+59*scalevalue.x, enemyTable[i].y+52*scalevalue.y, -(59*scalevalue.x)*enemyTable[i].RecallPhase/enemyTable[i].RecallingTime, 14.95*scalevalue.y, ARGB(255,0,135,185))
			end
		end
	end
end
...
function SSandDEATHtimers(i, hero)
	...
	if not hero.visible and MissTimer ~= nil and not hero.dead then
		...
		if VIP_USER then
			if not enemyTable[i].isRecalling then
				...
				if enemyTable[i].RecallPhase == -1 then
					DrawText('?',50*scalevalue.x, enemyTable[i].x+17*scalevalue.x, enemyTable[i].y+5*scalevalue.y, ARGB(255,175,0,0))
				else 
					DrawText('?',50*scalevalue.x, enemyTable[i].x+17*scalevalue.x, enemyTable[i].y+5*scalevalue.y, ARGB(255, 0,135,185))
				end
			end
		else
			DrawText('?',50*scalevalue.x, enemyTable[i].x+17*scalevalue.x, enemyTable[i].y+5*scalevalue.y, ARGB(255, 255, 255, 255))
...
function CDtrackerUI(i, hero)
	...
	for j=1, #SSpells, 1 do
		...
		elseif spell.Name == hero:GetSpellData(SUMMONER_2).name then
			spell.sprite:Draw(x+19*scalevalue.x, y+60*scalevalue.y,0xFF)
			if stateS2[i].cd ~= 0 then
				DrawRectangleAL(x+19*scalevalue.x, y+72*scalevalue.y, DrawProgressBar(stateS2[i].cd, stateS2[i].maxCD, 2), 25*scalevalue.y, SScolor)
		...
		for i = 1, heroManager.iCount, 1 do
			...
			if enemy.team ~= myHero.team then
				eTable[i] = 	{x = IconUI.x, y = IconUI.y,													-- Hud
								 ...
								 charName = enemy.charName, alreadyWarnedGult = false,				-- For print alert
								 isRecalling = false, RecallingTime = (GetGame().map.index == 8 and 4.5) or (GetGame().map.index == 12 and 0) or 8, RecallPhase = 0, RecallTimer = 0
								} 
...
function menu()
	...
	--Reset Menu
	Menu:addSubMenu('Reset Settings', 'reset')
		Menu.reset:addParam('SoftReset', 'Soft Reset', SCRIPT_PARAM_ONKEYDOWN,false, 76)
		--Menu.reset:addParam('HardReset', 'Hard Reset', SCRIPT_PARAM_ONKEYDOWN,false, 75)
		--Menu.reset:permaShow("SoftReset")	
	...
	Menu:addSubMenu('Experimental things', 'experimental')
		...
		Menu.experimental:addParam('FowRecall','FOW recall exploit',SCRIPT_PARAM_ONOFF, true)
	...
	--Performance
	Menu:addSubMenu('Performance', 'performance')
	Menu.performance:addParam('refreshRate', 'Drawing Refresh Rate', SCRIPT_PARAM_SLICE, 200, 200, 1000, 0)
	Menu.performance:addParam('OnTickrefreshRate', 'CD tracker refresh Rate', SCRIPT_PARAM_SLICE, 500, 200, 1000, 0)
...
function CheckUpdates()
	if _G.ExtraHudUpdateEnabled then
		DownloadFile("http://tarado.no-ip.org/andre/AndreBol/raw/Versions/ExtraHudVersion", SCRIPT_PATH.."Common\\"..randomName, UpdateCallBack)
--[[		print(GetWebResult("192.168.20.104", "/awabol/bol/raw/master/AWAbolVersion.txt", "", 5))
		GetAsyncWebResult("github.com", "/andreluis034/AndreRepo/blob/master/extrahud.lua", function(d)print(d)end)
  	GetAsyncWebResult(UPDATE_HOST, UPDATE_PATH, 
			function(d) 
				print(UPDATE_HOST .. "got update")
				ServerData = d 
...
GetAsyncWebResult(UPDATE_HOST, UPDATE_PATH, function(a) merda = a end)
file:write(UPDATE_URL.."\n")
file:write(GetWebResult(UPDATE_HOST, UPDATE_PATH, "", 5).."\n")
merda = GetWebResult(UPDATE_HOST, UPDATE_PATH, "", 5)
print (merda)
DownloadFile(UPDATE_URL, UPDATE_FILE_PATH, function ()print("Download")	end)]]
...
function UpdateCallBack()
	DelayAction(function()
		--print("hi0")
		if FileExist(SCRIPT_PATH.."Common\\"..randomName) then
			--print("hi1")
			local latestVersion = readAll(SCRIPT_PATH.."Common\\"..randomName)
			os.remove(SCRIPT_PATH.."Common\\"..randomName)
			if latestVersion and tonumber(latestVersion) ~= nil and tonumber(latestVersion) > tonumber(_G.ExtraHudVersion) then
				--print("hi2")
				DownloadFile(UPDATE_URL, SCRIPT_PATH.."ExtraHud.lua", function() PrintChat('<font color=\'#'..Chatcolor..'\'>'..'<b>Extra Hud: </b><font color=\'#F8F8F8\'>Script successfully updated. Please reload script(Double F9). ('.._G.ExtraHudVersion..' => '..latestVersion..')</font>') end)
			elseif tonumber(latestVersion) == nil then
			    PrintChat('<font color=\'#'..Chatcolor..'\'>'..'<b>Extra Hud: </b><font color=\'#F8F8F8\'>Auto Update failed, please check thread.</font>')
			else
				PrintChat('<font color=\'#'..Chatcolor..'\'>'..'<b>Extra Hud: </b><font color=\'#F8F8F8\'>You have got the latest version: <b>'.._G.ExtraHudVersion..'</b></font>')
			end
		else
			PrintChat('<font color=\'#'..Chatcolor..'\'>'..'<b>Extra Hud: </b><font color=\'#F8F8F8\'>Check for update failed. Check Thread.</font>')
		end
	end,1)
end
