...
---------------------------------- START UPDATE ---------------------------------------------------------------
local versionGOE = 1.01 -- current version
...
local NextAATick = 0
local ts = TargetSelector(TARGET_LESS_CAST_PRIORITY,1000,DAMAGE_PHYSICAL,false)
...
function OnLoad()-- SHEN: 3057, TF: 3078, Gauntlet: 3025
	...
	if VIP_USER and Prodiction ~= nil then RProdict = Prodiction:AddProdictionObject(_R, 1000, 1700, 0.25, 60, myHero) else RProdict = TargetPrediction(1000, 1.7, 250, 60, 0) end
	Menu = scriptConfig("aIrelia v"..versionGOE, "airelia")
	ts.name = "aIrelia"
	Menu:addTS(ts)
	Menu:addSubMenu("General information", "InfoMenu")
	Menu.InfoMenu:addParam("Information1", "aIrelia v"..versionGOE.." created by Anonymous", SCRIPT_PARAM_INFO, "")
	...
	Menu.ComboHelperMenu:addParam("useR","Use R automaticly", SCRIPT_PARAM_ONOFF, false)
	Menu.ComboHelperMenu:addParam("useRks","only when enemy can be killed with R", SCRIPT_PARAM_ONOFF, true)
	...
	Menu.OtherMenu:addParam("autoKS", "Secure kills with Q :P", SCRIPT_PARAM_ONOFF, true)
	Menu.OtherMenu:addParam("autoIgnite", "Autoignite killable enemies", SCRIPT_PARAM_ONOFF, true)
	...
	PrintChat("<font color='#00BFFF'>aIrelia "..versionGOE.." loaded!</font>")
end

function OnUnload()
	PrintChat("<font color='#00BFFF'>aIrelia "..versionGOE.." unloaded :(!</font>")
end
...
function OnTick()
	...
	if Menu.ComboHelperMenu.ComboHelper then
		if sTarget ~= nil then
			if sTarget.valid and ValidTarget(sTarget) then
					...
					if MyDistance > 250 then
							...
							for i, Minion in pairs(enemyMinions.objects) do
								if Minion ~= nil and Minion.valid and not Minion.dead then
									...
									if DMG >= Minion.health then
										if GetDistance(Minion, sTarget) < MyDistance then BestTarget = Minion end
									end
								end
							end
						end
						if BestTarget ~= nil then
							if CanQ and GetDistance(BestTarget) <= 650 then
								...
								if GetDistance(BestTarget, sTarget) > 250 and Menu.ComboHelperMenu.useQ then CastSpell(_Q, sTarget) end
							end
						end
					else
				...
				if Menu.ComboHelperMenu.useR and CanR and GetDistance(sTarget) < 1000 then
					if Menu.ComboHelperMenu.useRks and sTarget.health < getDmg("R", sTarget, myHero) then
						local pos = RProdict:GetPrediction(sTarget)
						--PrintChat("HP: "..sTarget.health..", DMG: "..(getDmg("R", sTarget, myHero)*4)..", After: "..sTarget.health-(getDmg("R", sTarget, myHero)*4))
						if pos ~= nil and GetDistance(pos) < 1000 then
							CastSpell(_R, pos.x, pos.z)
						end					
					elseif not Menu.ComboHelperMenu.useRks then
						local pos = RProdict:GetPrediction(sTarget)
						if pos ~= nil and GetDistance(pos) < 1000 then
							CastSpell(_R, pos.x, pos.z)
						end
	...
	if Menu.OtherMenu.Rmanager and CanR and sTarget ~= nil and Menu.ComboHelperMenu.ComboHelper then
	...
	if Menu.OtherMenu.autoKS and CanQ then
		for i, Enemy in pairs(Enemies) do
			if Enemy ~= nil and Enemy.valid and not Enemy.dead and ValidTarget(Enemy) and GetDistance(Enemy) < 650 then
				if Enemy.health < getDmg("Q", Enemy, myHero) then CastSpell(_Q, Enemy) end
			end
		end
	end
	if Menu.OtherMenu.autoIgnite and iSlot ~= nil and CanIgnite then
		if Menu.OtherMenu.autoKS then
			if not CanQ then
				for i, Enemy in pairs(Enemies) do
					if Enemy ~= nil and Enemy.valid and not Enemy.dead and ValidTarget(Enemy) and GetDistance(Minion, Enemy) < 600 then
						if CanIgnite then
							if Enemy.health < getDmg("IGNITE", Enemy, myHero) then
...
function OnDraw()
	if myHero ~= nil and not myHero.dead and Menu.DrawHelperMenu.drawQrange then DrawCircle(myHero.x, myHero.y, myHero.z, 650, RGB(0,191,255)) end
	if myHero ~= nil and not myHero.dead and Menu.DrawHelperMenu.drawErange then DrawCircle(myHero.x, myHero.y, myHero.z, 425, RGB(0,191,255)) end
	if myHero ~= nil and not myHero.dead and Menu.DrawHelperMenu.drawRrange then DrawCircle(myHero.x, myHero.y, myHero.z, 1000, RGB(0,191,255)) end
	if myHero ~= nil and not myHero.dead and Menu.DrawHelperMenu.drawKillable then
		for i, Minion in pairs(enemyMinions.objects) do
			if Minion ~= nil and Minion.valid and Minion.team ~= myHero.team and not Minion.dead then
				if getDmg("Q", Minion, myHero) >= Minion.health then DrawCircle(Minion.x, Minion.y, Minion.z, 75, RGB(0,191,255)) end
			end
		end
	end
end
