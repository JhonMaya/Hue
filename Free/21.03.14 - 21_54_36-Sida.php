...
function _Orbwalker:_OnSendPacket(p)
 ...
 --  		if Unit.isMe then
 --  			if Helper:GetTime() < Orbwalker.AttackCompletesAt then
	...
	if Unit.isMe then
		if Data:IsAttack(Spell) then
			...
			if self.BaseAttackSpeed == 0.5 then
				self.BaseWindUp = 1000 / (((Spell.windUpTime * 1000) * myHero.attackSpeed))
				self.BaseAttackSpeed = 1000 / ((Spell.animationTime * 1000) * myHero.attackSpeed)
			end
...
function _MyHero:GetTimeToHitTarget(Target)
	if self.IsMelee then
		return Helper:GetTime() + Orbwalker.LastWindUp + GetLatency() / 2
	else
...
function _AntiCancel:_OnAnimation(Unit, Animation)
	if Unit.isMe and self.Animation then
		if Animation == self.Animation then
			self.BlockUntil = Helper:GetTime() + 800
			self.BlockNext = true
...
function _AntiCancel:_OnSendPacket(p)
	if (self.BlockNext or Helper:GetTime() < self.BlockUntil) and p.header == Packet.headers.S_MOVE then
		local packet = Packet(p)
		packet:block()
		self.BlockNext = false
	end
...
function _Minions:__init()
	...
	self.LastAlmostFound = 0

	self.IncomingDetails = {}
	self.LastHittableMinions = {}
	self.AlmostLastHittableMinions = {}
	self.AlmostKillableData = {}
...
function _Minions:_OnTick()
	...
	if not Orbwalker:CanOrbwalkTargetCustomRange(self.KillableMinion, self.AttackRangeBuffer) then
		self.KillableMinion = nil
	end

	if not Orbwalker:CanOrbwalkTargetCustomRange(self.AlmostKillable, self.AttackRangeBuffer) then
		self.AlmostKillable = nil
	-- else
	-- 	local maxTime = Orbwalker:GetNextAttackTime() + Orbwalker.LastWindUp + (Orbwalker.LastAttackCooldown * Orbwalker:GetAttackSlowModifier()) + (Orbwalker.LastWindUp / 2) + 200
	-- 	local MinionHealth = self:GetPredictedHealth(self.AlmostKillable, maxTime / 1000)

	-- 	if MinionHealth > myHero:CalcDamage(self.AlmostKillable, myHero.totalDamage) and Helper:GetTime() > self.LastAlmostFound + 500 then
	-- 		self.AlmostKillable = nil
	-- 	end
	end
	
	if (Keys.MixedMode or Keys.LaneClear or Keys.LastHit) and not Orbwalker:IsShooting() then

		if not Orbwalker:CanOrbwalkTarget(self.KillableMinion) then
			for _, Minion in pairs(self.EnemyMinions.objects) do
				if Orbwalker:CanOrbwalkTargetCustomRange(Minion, self.AttackRangeBuffer) then

					local MinionHealth, MaxDmg, Count = self:GetPredictedHealth(Minion, MyHero:GetTimeToHitTarget(Minion) / 1000)
					local MyDamage = myHero:CalcDamage(Minion, myHero.totalDamage)-- self:myCalcDamage(myHero, Minion)
					MyDamage = MyDamage-- - MaxDmg

					if MinionHealth < MyDamage then
						local row = {Object = Minion, Health = MinionHealth}
						table.insert(self.LastHittableMinions, row)
					end
				end
			end	

			local BestLastHit = nil
			for _, Row in pairs(self.LastHittableMinions) do
				local Minion = Row.Object
				if Orbwalker:CanOrbwalkTarget(Minion) then
					if not BestLastHit or Row.Health < BestLastHit.Health then
						BestLastHit = Row
					end
				end
			end
			...
			if BestLastHit then
				self.KillableMinion = BestLastHit.Object
				return
			end
		end

		if not self.AlmostKillable or #self.AlmostKillableData == 0 then
			for _, Minion in pairs(self.EnemyMinions.objects) do
				if Orbwalker:CanOrbwalkTargetCustomRange(Minion, self.AttackRangeBuffer) then

					local maxTime = (Orbwalker:GetNextAttackTime() - Helper:GetTime()) + 200
					local MinionHealth = self:GetPredictedHealthLaneclear(Minion, maxTime / 1000)

					if MinionHealth < myHero:CalcDamage(Minion, myHero.totalDamage) then
						table.insert(self.AlmostLastHittableMinions, Minion)
					end
				end
			end	

			if #self.AlmostLastHittableMinions > 0 then
				self.AlmostKillable = self.AlmostLastHittableMinions[1]

				self.AlmostKillableData = {}
				local i = 1
				while i <= #VP.ActiveAttacks do
					if VP.ActiveAttacks[i].Attacker and not VP.ActiveAttacks[i].Attacker.dead and VP.ActiveAttacks[i].Target and VP.ActiveAttacks[i].Target.networkID == self.AlmostKillable.networkID then
						table.insert(self.AlmostKillableData, {Attack = VP.ActiveAttacks[i], startPos = (VP.ActiveAttacks[i].Attacker.visionPos and VP.ActiveAttacks[i].Attacker.visionPos or VP.ActiveAttacks[i].Attacker)})
					end
					i = i + 1
				end

				self.LastAlmostFound = Helper:GetTime()
				return
			elseif #self.AlmostKillableData > 0 then
				for _, Data in pairs(self.AlmostKillableData) do
					if Data.Attack.Attacker.dead or GetDistance(Data.Attack.Attacker, Data.startPos) > 3 or Data.Attack.Target.dead then
						self.AlmostKillableData = {}
					end
				end
			end
		...
		if Keys.LaneClear then
			self.LaneClearMinion = self:GetBestLaneClearMinion()
			return
		end	
	elseif Keys.LaneClear and Orbwalker:IsShooting() then
		if self.AttackingLaneClear then
			if self.KillableMinion and Orbwalker.LastEnemyAttacked ~= self.KillableMinion then
				print("Holding")
				myHero:MoveTo(mousePos.x, mousePos.z)
				return
			elseif self.AlmostKillable and Orbwalker.LastEnemyAttacked ~= self.AlmostKillable then
				print("Holding")
				myHero:MoveTo(mousePos.x, mousePos.z)
				return
			end
		elseif self.AlmostKillable and not self.KillableMinion then
			print("Holding")
			myHero:MoveTo(mousePos.x, mousePos.z)
			return
		end
...
function _Minions:_OnProcessSpell(Unit, Spell)
	if Data.UnitInfo[Unit.charName] and not Unit.isMe then
		for _, Data in pairs(self.AlmostKillableData) do
			if Data.Attack.Attacker == Unit and Spell.target and Spell.target.networkID ~= Data.Attack.Target then
				self.AlmostKillableData = {}
				return
			end
		end
		...
		--else
			if Orbwalker:CanShoot() then
				...
				elseif Structures:CanOrbwalkStructure() then
					...
					Orbwalker:Orbwalk(self.LaneClearMinion)
				end
...
function _Minions:GetPredictedHealth(Minion, Time)
	local newHealth, maxDmg, count = VP:GetPredictedHealth(Minion, Time)
	local damage = Minion.health - newHealth
	--local count = self:GetNumberOfAttackingMinions(Minion)

	if count == 0 then
		return Minion.health - damage, maxDmg
	end

	-- local base = 0.75
	-- damage = damage * (base + (0.1 * count))
	local base = -0.5
	maxDmg = maxDmg * (base + (0.15 * count))

	return Minion.health - damage - maxDmg, maxDmg
end

function _Minions:GetPredictedHealthLaneclear(Minion, Time)
	local newHealth, count = VP:GetPredictedHealth2(Minion, Time)
	local damage = Minion.health - newHealth

	-- if count == 0 then
	-- 	return Minion.health - damage
	-- end
...
function _Minions:_PreAttack(Target)
	if Target and Target == self.LaneClearMinion then
		self.AttackingLaneClear = true
	else
		self.AttackingLaneClear = false
	end
end
...
function _Minions:myCalcDamage(source, target, attackCount)
	local armorPen = 0
	local armorPenPercent = 0
	...
	if source.name == myHero.name then
		if FarmMenu.ArcaneBlade then
			magicDamage = myHero.ap * .05
		end
		
		if FarmMenu.DevastatingStrikes then
			armorPenPercent = .02 * FarmMenu.DevastatingStrikes
		end
		
		if FarmMenu.Havoc then
		...
		if FarmMenu.DoubleEdgedSword then
			physDamage = myHero.range < 400 and physDamage*1.02 or (physDamage*1.015)
			magicDamage = myHero.range < 400 and magicDamage*1.02 or (magicDamage*1.015)
		end
		
		if FarmMenu.Butcher then
			physDamage = physDamage + 2
		end
	...
	local totDamage = (physDamage * (100/(100 + target.armor * (1-armorPenPercent)))  
	 + magicDamage * (100/(100 + target.magicArmor * (1-magicPenPercent))) ) * (1-dmgReductionPercent)

	-- if source == myHero and attackCount then
	-- 	if attackCount > 4 and attackCount < 7 then
	-- 		totDamage = totDamage * 1.2
...
function _Minions:checkAttackStillViable (attackDetails)
...
function _Minions:GetNumberOfAttackingMinions(unit)
	local i = 1
	local _Count = 0
	while i <= #VP.ActiveAttacks do
		if VP:GetTime() <= VP.ActiveAttacks[i].hittime and VP.ActiveAttacks[i].Attacker and not VP.ActiveAttacks[i].Attacker.dead and VP.ActiveAttacks[i].Target and VP.ActiveAttacks[i].Target.networkID == unit.networkID then
			_Count = _Count + 1
		end
		i = i + 1
	end
...
function _Minions:RegisterPre()
	if not self.PreRegistered and Plugins then
		Plugins:RegisterPreAttack(function(target) self:_PreAttack(target) end)
		self.PreRegistered = true
	end
end

function _Minions:GetBestLaneClearMinion()
	-- local BestMinion = {}
	...
	-- 		if _Count == 0 then
	-- 			return Minion
	-- 		elseif not BestMinion.Count then
	-- 			BestMinion = {Object = Minion, Count = _Count}
	-- 		elseif BestMinion.Count > _Count then
	-- 			BestMinion = {Object = Minion, Count = _Count}
	-- 		end
	-- 	end
	-- end	
	...
	-- if BestMinion then
	-- 	return BestMinion.Object
	-- else
	-- 	return self:GetSecondLowestHealthMinion()
	-- end
	for _, Minion in ipairs(self.EnemyMinions.objects) do
		if Orbwalker:CanOrbwalkTarget(Minion) then
			if self:GetNumberOfAttackingMinions(Minion) == 0 then
				return Minion
			else
				return self:GetSecondLowestHealthMinion()
			end
...
function _Streaming:EnableStreaming()
	...
	if not self.ChatTimeout then
		self.ChatTimeout = Helper:GetTime() + 3000
	elseif Helper:GetTime() < self.ChatTimeout then
		self:DisableOverlay()
		for i = 0, 15 do
			PrintChat("")
		end
...
function _AutoUpdate:GetRequiredDownloads()
	if GetTickCount() > self.LastFileCheck + 5000 then
		if not self:HasRequiredFiles() then
			...
			self.LastFileCheck = Helper:GetTime()
			return false
		else
...
function _AutoUpdate:_OnTick()
	...
	if self.NextPrint and Helper:GetTime() > self.NextPrint then
...
function _AutoUpdate:ScriptDownloaded()
	self.NextPrint = Helper:GetTime() + 1000	
end
...
function _Riven:ProcSpell(_, Unit, Spell)
	if Unit.isMe and Helper:GetTime() > self.lastQ + 150 then
		if Data:IsAttack(Spell) then
			Orbwalker.LastAttack = Helper:GetTime() - GetLatency() / 2
			...
			MyHero.DonePreAttack = false
			self.StartedOnAttacked = Helper:GetTime()
		elseif Data:IsResetSpell(Spell) then
