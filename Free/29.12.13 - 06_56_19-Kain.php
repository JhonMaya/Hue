...
--[[
        ...
        Auto Carry Plugin - Corki Prodiction Edition
		Author: Kain
		...
		Version History:
			Version: 1.30:
				PROdiction update.
				Updated Q for 3.14 patch.
			Version: 1.29:
				Updated E casting for 3.13 patch.
			Version: 1.28:
				Added PROdiction 2.0.
				Fixed Revamped compatibility for prodiction.
				Reverted: Added PROdiction CanNotMissMode.
			Version: 1.23:
				Fixed Ultimate on single target bug, especially while chasing.
				Fixed bug that sometimes miscounted enemies for Q and R AoE's.
			Version: 1.2:
...
function Vars()
	...
	QNormalRange = 825 -- Old: 600
	QRadius, RRadius = 250, 75
	QRange, WRange, ERange, RRange = ((QNormalRange + QRadius) - 10), 800, 600, 1225
	...
	if VIP_USER then
		if FileExist(SCRIPT_PATH..'Common/Prodiction.lua') then
			...
			rp = SetupPROdiction(_R, SkillR.range, SkillR.speed, SkillR.delay, SkillR.width, myHero,
				function(unit, pos, castSpell)
					if RReady and GetDistance(unit) < SkillR.range then
						FireSpell(unit, pos, castSpell)
					end
				end)

			-- For now, this doesn't seem to be working right.	
			-- for i, enemy in pairs(AutoCarry.EnemyTable) do
			--	rp:CanNotMissMode(true, enemy)
			-- end

			PrintChat("<font color='#CCCCCC'> >> Kain's Corki - PROdiction 2.0 <</font>")
		end
	else
		PrintChat("<font color='#CCCCCC'> >> Kain's Corki - Free Prediction <</font>")
	end
end
...
function Menu()
	...
	AutoCarry.PluginMenu:addParam("HarassE", "Use Gatling Gun", SCRIPT_PARAM_ONOFF, false)
...
function CastQ(enemy)
	...
	if EnemyCount(enemy, QRadius) >= 2 then
		local spellPos, enemyCount = GetAoESpellPosition(QRadius, enemy, SkillQ.delay)
...
function CastR(enemy, minEnemies)
	...
	if enemy and not enemy.dead and ValidTarget(enemy, SkillR.range) then
		if VIP_USER then
			predic = rp:EnableTarget(AutoCarry.GetAttackTarget(), true)
		else
			predic = AutoCarry.GetPrediction(SkillR, enemy)
			if predic then
				FireSpell(enemy, predic, myHero:GetSpellData(_R))
			end
		end
	end
...
function FireSpell(unit, predic, spell)
	if AutoCarry.PluginMenu.HarassR and AutoCarry.MainMenu.MixedMode then
		minEnemies = AutoCarry.PluginMenu.HarassRMinEnemies
	else
		minEnemies = AutoCarry.PluginMenu.ComboRMinEnemies
	end

	if predic and unit and not unit.dead then
		if spell.Name == SkillR.spellKey and RReady and ValidTarget(unit, SkillR.range) then
			local enemyCount = 0
			...
			-- If minEnemies is only 1, then don't waste time getting EnemyCount. It's unnecessary.
			enemyCount = 1
			if minEnemies > 1 then
				enemyCount = 1
				local spellPos, enemyCountAOE = GetAoESpellPosition(splashRadius, unit, SkillR.delay)

				if spellPos and GetDistance(spellPos) <= SkillR.range and enemyCountAOE > 1 then
					enemyCount = enemyCountAOE
				end
			end
			...
			if enemyCount >= minEnemies then
				...
				if ExtraConfig.RSplashDamage then
					local m, minions = GetMinionCollision(SkillR, nil, myHero, unit)

					if m then
						for index, minion in pairs(minions) do
							if GetDistance(minion, unit) > splashRadius then
								collision = true
								break
							end
						end 
					...
					collision = AutoCarry.GetCollision(SkillR, myHero, unit)
				end
				
				if not collision then
					if VIP_USER then
						CastSpell(spell.Name, predic.x, predic.z)
					else
