...
local function CastQ(myTarget)   
    ...
    if useq then
        ...
        if GetDistance(myTarget) <= 500 and GetTickCount() + GetLatency() / 2 - qTick > del then -- (GetTickCount() + GetLatency() / 2) - qTick > del then                      
            DelayAction(function() CastSpell(_Q, myTarget.x, myTarget.z) qTick = GetTickCount() + GetLatency() / 2  end, lastWindUpTime/1000)
        end
...
function CastR(myTarget)   
    ...
    if user then        
        ...
        if myTarget.health <= DamageR or GetTickCount() + GetLatency() / 2 > rTick + 10000 then
