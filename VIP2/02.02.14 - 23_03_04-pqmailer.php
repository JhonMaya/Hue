<?php exit() ?>--by pqmailer 217.95.227.60
--[[
Changelog:
v0.1 - Working combo + menu.]]
...
local Config, rStacks, localVersion, heroTable, enemyTable, allyTable, towerFocusTable, lockedTarget, targetGlobal, targetExpected, IgniteSlot, lastAttack = nil, 0, 0.1, GetHeroTable(1), GetHeroTable(2), GetHeroTable(3), {}, nil, nil, nil, nil, os.clock()
local immuneEffects = {
...
function OnLoad()
	Config = scriptConfig("[SOME GUYS] Akali v."..tostring(localVersion), "sgakali")
	Config:addParam("doCombo", "Combo", SCRIPT_PARAM_ONKEYDOWN, false, GetKey("A"))

	Config:addSubMenu("Combo", "combo")
	Config.combo:addParam("useQ", "Use Q", SCRIPT_PARAM_ONOFF, true)
	Config.combo:addParam("useE", "Use E", SCRIPT_PARAM_ONOFF, true)
	Config.combo:addParam("useR", "Use R", SCRIPT_PARAM_ONOFF, true)
	Config.combo:addParam("Orbwalk", "Use Orbwalk", SCRIPT_PARAM_ONOFF, true)
	...
	print("[SOME GUYS] Akali v."..localVersion.." loaded!")
end
...
function OnTick()
	...
	if Config.doCombo and targetGlobal then
		--if Config.combo.Orbwalk then OrbWalking(targetGlobal) end
	...
	if Config.doCombo and Config.combo.Orbwalk and not targetGlobal then
		--MoveToCursor()
	end
		...
		local rEnergy = myHero:GetSpellData(_R).mana
			if __validTarget(enemy, SpellData.R.range) then
				...
				elseif enemy.health < rKSDmg*2 and rStacks > 1 and myHero.mana > rEnergy*2 then
					CastSpell(_R, enemy)
				elseif enemy.health < rKSDmg*3 and rStacks > 2 and myHero.mana > rEnergy*3 then
					CastSpell(_R, enemy)
				end
			end
		end
...
function OnDraw()
	...
	if Config.draws.circles then
		DrawCircle(myHero.x, myHero.y, myHero.z, SpellData.Q.range, ARGB(255,127,0,110))
		DrawCircle(myHero.x, myHero.y, myHero.z, SpellData.E.range, ARGB(255,95,159,159))
		DrawCircle(myHero.x, myHero.y, myHero.z, SpellData.R.range, ARGB(255,204,50,50))
	end
end
