...
-- Script Name: Renekton - The Real Kaptain Skurvy
-- Script Ver.: 1.1
...
function RenektonMenu()
	...
	RenektonMenu:addSubMenu("[Nintendo "..myHero.charName.."] - Combo Settings", "combo")
		...
		RenektonMenu.combo:addParam("AAs", "Use Skills Smartly with AAs", SCRIPT_PARAM_ONOFF, true)
...
function RenektonCombo()
	if Target and Target.valid then
		...
		if not RenektonMenu.combo.AAs then
			CastSkill(_W, Target)
...
function OnProcessSpell(source, spell)
	...
	if source == myHero then
		if spell.name:lower():find("attack") and spell.target.type == myHero.type then
