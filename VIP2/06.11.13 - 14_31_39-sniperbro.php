<?php exit() ?>--by sniperbro 108.162.221.122
collectgarbage()
---------------------------------- START UPDATE ---------------------------------------------------------------
local versionCASS = 2.331 -- current version
local messageCass = "User Update, Config Fix, E not on Kayle or Tryn" -- message on what was updated
...
function LoadMenu()
	Config = scriptConfig("Cass Whooping", "Cass Whooping Release")
...
function CastItems(target)
	if not ValidTarget(target) or not Config.itemSettings.useItems then 
		...
		for i = 1, enemyHerosCount do
			local Enemy = enemyHeros[i].object
			if Enemy == target and enemyHeros[i].killable ==2 then
...
function OnDraw()
    if not myHero.dead then
        if ValidTarget(newTarget) then
			...
			DrawText("NO TARGET",35,400,50,ARGB(255,255,143,20))
        end
        ...
        for i = 1, enemyHerosCount do
            ...
            if ValidTarget(Enemy) and Config.drawSettings.DrawText then
                ...
                elseif killable == 2 then
                    DrawText3D(tostring("Combo killable"),Enemy.x,Enemy.y, Enemy.z,16,ARGB(255,248,255,20), true)
