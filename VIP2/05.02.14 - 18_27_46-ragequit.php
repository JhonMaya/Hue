...
function HandleR()
	if Menu.UseR then
		...
		if Target and ValidTarget(Target) and not AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target) and AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target, GetEDashPos(Target)) then
			if not HasUlt and myHero:CanUseSpell(_E) == READY and myHero:CanUseSpell(_R) then
				CastSpell(_R, Target.x, Target.z)
				CastSpell(_E, Target.x, Target.z)
			end
		end
...
function SetupMenus()	
	...
	MenuUlt = scriptConfig("The Wonderful Riven WindSlash Config", "NyanUlt")
	MenuUlt:addParam("Expire", "Use Windslash if it will expire", SCRIPT_PARAM_ONOFF, true)
	MenuUlt:addParam("Kill", "Use Windslash if it will kill Target", SCRIPT_PARAM_ONOFF, true)
	MenuUlt:addParam("Range", "Use Windslash if it Target becomes out of range", SCRIPT_PARAM_ONOFF, true)
