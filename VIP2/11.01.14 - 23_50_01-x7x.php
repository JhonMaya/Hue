...
function OnTick()
				...
				if DownloadedTable then
					for i, x in pairs(tabZ) do
						if tostring(x) == GetUser() then
							...
							Menu.ComboMenu:addParam("useMEC","Use Q to land good R", SCRIPT_PARAM_ONOFF, true)
							...
							Menu.ComboMenu:addParam("RcatchAmount","- it can catch X enemies:", SCRIPT_PARAM_SLICE, 2, 1, 5, 0)
							Menu.ComboMenu:addParam("RkillAmount","- it can kill X enemies:", SCRIPT_PARAM_SLICE, 1, 1, 5, 0)
							...
							Menu.OtherMenu:addParam("RcatchAmount","- it can catch X enemies:", SCRIPT_PARAM_SLICE, 4, 1, 5, 0)
							Menu.OtherMenu:addParam("RkillAmount","- it can kill X enemies:", SCRIPT_PARAM_SLICE, 2, 1, 5, 0)
	...
	elseif scriptReady and not varFirstLoad and varUpdated then
		...
		--Combo Stuff
		if ts.target ~= nil and not BallMoving and Menu.ComboMenu.ComboKey then
			if Menu.ComboMenu.useQ and CanQ then
				if CountEnemyHeroInRange(1025) >= 2 and Menu.ComboMenu.useMEC and CanR then
					local Pos = GetAoESpellPosition(400, ts.target, 250)
			...
			if Menu.ComboMenu.useR and CanR then
				if Menu.ComboMenu.RcatchAmount ~= 0 then
					...
					if n >= Menu.ComboMenu.RkillAmount then CastSpell(_R) end
				end
		...
		--AuHRS Stuff
		if ts.target ~= nil and not BallMoving and Menu.HarrasMenu.HarrasKey then
		...
		if BallMoving == false then
			if Menu.OtherMenu.RcatchAmount ~= 0 then
				...
				if n >= Menu.OtherMenu.RkillAmount then CastSpell(_R) end
			end
...
function OnDraw()
	if scriptReady then
		...
		if Menu.DrawMenu.drawRrange and Ball ~= nil and CanR then DrawCircle(Ball.x, Ball.y, Ball.z, 400, RGB(255,0,255)) end
