<?php exit() ?>--by pqmailer 217.82.15.34
...
function OnLoad()		
    ...
    qp = pd:AddProdictionObject(_Q, RangeQ, 1200, 0.245, 50, myHero, function(unit, pos, castSpell) if QReady and GetDistance(unit) < RangeQ then CastSpell(_Q, pos.x, pos.z) end end)
    ep = pd:AddProdictionObject(_E, RangeE, 1400, 0.245, 0, myHero, function(unit, pos, castSpell) if EReady and GetDistance(unit) < RangeE then CastSpell(_E, pos.x, pos.z) end end)
    rp = pd:AddProdictionObject(_R, RangeR, math.huge, 0.245, 50, myHero, function(unit, pos, castSpell) if RReady and GetDistance(unit) < RangeR then CastSpell(_R, pos.x, pos.z) end end)

	PrintChat("<font color='#CCCCCC'>>> Lux by PQMailer loaded <<</font>")
	PrintChat("<font color='#CCCCCC'>>> THIS IS A BETA; REPORT AND BUGS <<</font>")
end
...
function Harass()
	if ValidTarget(ts.target) then
		...
		if Distance <= RangeE and EReady then
			ep:EnableTarget(ts.target, true)
			ETriggerHero = true
		end
...
function Combo()
	if ValidTarget(ts.target) then
		...
		if Distance <= RangeQ and QReady then
			qp:EnableTarget(ts.target, true)
		end
		if Distance <= RangeE and EReady then
		...
		if Distance <= RangeR and RReady and (EnemyTable[index]["killable"] == true or Config.general.comboRalways) and Config.general.comboR then
			rp:EnableTarget(ts.target, true)
		end
		if Distance <= RangeAD and Orbwalking.CanAttack() and (TargetHaveBuff("luxilluminatingfraulein", ts.target) or ts.target.armor < 80 or ((getDmg("AD", ts.target, myHero)*2) >= ts.target.health)) then
			Orbwalking.Attack(ts.target)
		end
