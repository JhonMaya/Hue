<?php exit() ?>--by sniperbro 173.245.50.157

collectgarbage()
---------------------------------- START UPDATE ---------------------------------------------------------------
local versionCASS = 2.325 -- current version
local messageCass = "User Update Only" -- message on what was updated
local SCRIPT_NAME_CASS = "CassWhoopingRelease"
---------------------------------------------------------------------------------------------------------------
local needUpdate_CASS = false
local needRun_CASS = true
local URL_CASS = "http://dlr5668.cuccfree.org/"..SCRIPT_NAME_CASS..".lua"
local PATH_CASS = BOL_PATH.."Scripts\\"..SCRIPT_NAME_CASS..".lua"
function CheckVersionCASS(data)
	local onlineVerCASS = tonumber(data)
	if type(onlineVerCASS) ~= "number" then return end
	if onlineVerCASS and onlineVerCASS > versionCASS then
		print("<font color='#e0f900'>"..SCRIPT_NAME_CASS..": </font>".."<font color='#e0f900'> There is a new version. Auto Update. Don't F9 till done...</font>") 
		needUpdate_CASS = true
	else
		print("<font color='#00FF00'>"..SCRIPT_NAME_CASS.." : "..versionCASS.." </font><font color='#00FF00'> loaded. You have the most recent version.</font>")  
	end
end
function UpdateScriptCASS()
	if needRun_CASS then
		needRun_CASS = false
		if _G.UseUpdater == nil or _G.UseUpdater == true then 
			GetAsyncWebResult("dlr5668.cuccfree.org", SCRIPT_NAME_CASS.."Ver.lua", CheckVersionCASS) 
		else
			print("<font color='#ff0000'> Cass Whooping: Auto-updating off. You are using version : "..versionCASS.."</font>")
		end
	end

	if needUpdate_CASS then
		needUpdate_CASS = false
		DownloadFile(URL_CASS, PATH_CASS, function()
                if FileExist(PATH_CASS) then
                    print("<font color='#00FF00'>"..SCRIPT_NAME_CASS..": </font>".."<font color='#00FF00'> Updated, Reload script for new version.</font>")
					print("<font color='#00FF00'> Update Notes: "..messageCass.."</font>") 
                end
		end)
	end
end
AddTickCallback(UpdateScriptCASS)
---------------------------------- END UPDATE -----------------------------------------------------------------
...
if _G.GetAsyncWebResult == nil or _G.GetUser == nil or _G.CastSpell == nil then print("Cass Whooping: Unauthorized User") return end
...
if abs(a2-a1) > 500000 and abs(a3-a2) > 500000 then print("Cass Whooping: Unauthorized User") return end
if abs(a2-a1) < 500 and abs(a3-a2) < 500 then print("Cass Whooping: Unauthorized User") return end
_G.rawset = rawset3
namez = protect {
	...
	["andysmalll"] = true, ["jakdo"] = true, ["paradoxel"] = true, ["sillyfang"] = true, ["lumi"] = true, ["h4ck"] = true, ["jcannon"] = true,
	...
	["anubis342"] = true, ["grimmerz"] = true, ["badstrip"] = true, ["magmia"] = true, ["avi8856455"] = true, ["polycarbonite"] = true,
	...
	["derpherp"] = true, ["likagangsta"] = true, ["tmsjah"] = true, ["rfx"] = true, ["fatfurball"] = true, ["bercley"] = true,
...
function OnUnload()
	PrintFloatText(myHero,2,"Cass Whooping Release "..versionCASS.." UnLoaded!")
end
...
function LoadMenu()
	Config = scriptConfig("Cass Whooping", "Cass Whooping Release"..versionCASS)
...
function assistedUltCheck()
	if Config.ultSettings.smartUlt and RREADY and Config.ultSettings.manualR then
		if ValidTarget(ts.target) then
			local vec = GetCassMECS(75, rangeR, 1, false) -- 80 degree R
			if vec ~= nil and GetDistance(vec) < rangeR then
				CastSpell(_R, vec.x, vec.z)
			end
		end	
...
function OnSendPacket(p)
	...
	if packet:get('name') == 'S_CAST' then
		if packet:get('spellId') == _R then
			local vec = GetCassMECS(75, rangeR, 1, false) -- 80 degree R
			if not ValidTarget(ts.target) and Config.ultSettings.smartUlt or (vec and GetDistance(vec) > rangeR) then
				packet:block()
			end
		end
	end
...
function Target()
	...
	for i = 1, enemyHerosCount do
		...
		if ValidTarget(Enemy) then
			...
			if killHim >= Enemy.health and killMana<= myHero.mana then
				enemyHeros[i].killable = 3
				
			elseif comboKiller >= Enemy.health then
	...
	if ValidTarget(ts.target) then				
		CastSkills(ts.target)					
	end
	if ValidTarget(targetSelected) then
		if ValidTarget(ts.target) then
			ts.target = targetSelected
			for i = 1, enemyHerosCount do
			local Enemy = enemyHeros[i].object
				if ValidTarget(Enemy) then 
					if ts.target ~= Enemy then
						local edamage = getDmg("E", ts.target, myHero, 3)
						if Config.combatKeys.tSettings.autoE then 
							if edamage/3<ts.target.health then
								CastE(ts.target)
							end
						else
    ...
    if ValidTarget(target) then
	    ...
	    for i = 1, target.buffCount do
	        ...
	        if BuffIsValid(tBuff) and (tBuff.name == "cassiopeianoxiousblastpoison" or tBuff.name == "cassiopeiamiasmapoison"
	        	or tBuff.name == "toxicshotparticle" or tBuff.name == "bantamtraptarget"
	        	or tBuff.name == "poisontrailtarget" or tBuff.name == "deadlyvenom") and tBuff.endT - delay - GetGameTimer() > GetLatency() / 1000 then
...
function CastItems(target)
	if not ValidTarget(target) or not Config.itemSettings.useItems then 
		...
		for i = 1, enemyHerosCount do
			local Enemy = enemyHeros[i].object
			if Enemy == target and enemyHeros[i].killable ==3 then
...
function orbWalk() 
	local range = myHero.range + GetDistance(myHero.minBBox)
	if GetTickCount() > NextShot and ValidTarget(ts.target) and GetDistance(ts.target)<=range and Config.combatKeys.tSettings.teamFight and Config.orbwalkSettings.OrbWalkAA and not QREADY and not WREADY and not EREADY then
...
function OnDraw()
    if not myHero.dead then
        if ValidTarget(ts.target) then
        	DrawCircle2(ts.target.x, ts.target.y, ts.target.z, 150, ARGB(150,183,60,244), 3)
        	DrawText("Targetting: "..ts.target.charName,35,400,50,ARGB(255,255,143,20))
        end
