...
local versionSPIDER = 0.010 -- current version
...
namez = protect {
	...
	["igotcslol"] = true, ["kyk"] = true, ["errinqq"] = true, ["clg aphromoo"] = true, ["codex1337"] = true, ["ztempaccount"] = true,
	["andyi"] = true, ["surrealpower"] = true, ["ahkasha"] = true, ["toxicteddy"] = true, ["ms el jefe"] = true, ["phebos"] = true,
	["h4ck"] = true, ["nakeshima"] = true, ["xnsm"] = true, ["volkanyx"] = true, ["volkanyx"] = true, ["pumadanny"] = true,
	["huminzi123"] = true, ["enzyme"] = true, ["nixxor"] = true, ["xthanhz"] = true, ["lastchicken"] = true, ["lordsn4ke"] = true,
	["technox"] = true, ["shaunyboi"] = true, ["swain"] = true, ["hassliebe"] = true, ["listerkeler"] = true, ["baccano"] = true,
	["12122215"] = true, ["xin265755"] = true, ["yigoyar"] = true, ["blackhype"] = true, ["107220663"] = true, ["vapingpbr"] = true,
	["hellfish117"] = true, ["madgavis"] = true, ["ace1369"] = true, ["jdogjackson"] = true, ["myroomun"] = true, ["kaoshan"] = true,
	["dunkin"] = true, ["xll.de"] = true, ["mma4tw"] = true, ["rtstrauma"] = true, ["lexxes"] = true, ["toxic 123"] = true,
	["frozer"] = true, ["bloodyfinale"] = true, ["remus3"] = true, ["remus3"] = true, ["god"] = true, ["bawgroup"] = true,
	["kazie"] = true, ["che"] = true, ["shzimba"] = true, ["xlain"] = true,  ["crushertheking94"] = true, ["refleckz45"] = true,
	["poindexter"] = true, ["thechancelor"] = true, ["138618"] = true, ["rosenkrantz"] = true,  ["znudee"] = true, ["jokerad123"] = true,
	["muffinzz"] = true, ["himizu"] = true, ["damein1781"] = true, ["anik"] = true,  ["purem5"] = true, ["jackypaul"] = true, 
	["prasinos"] = true, ["echomango"] = true, ["shtevenish"] = true, ["fabolous1"] = true,  ["792007467"] = true, ["63777377"] = true,
	["anekraf"] = true, ["thenitrozyniak"] = true, ["nab423"] = true, ["tongamo"] = true,  ["yrmom3141"] = true, ["varunen"] = true,
	["shortyboy"] = true,
}
_G.rawset = rawset3
...
function LoadSkillsMenu()
	...
	Config.skillActivation:addParam("humanErange", "Human E Max Range", SCRIPT_PARAM_SLICE, 900, 100, 1075, 0)
	...
	Config.skillActivation:addParam("spiderErange", "Spider E Min Range", SCRIPT_PARAM_SLICE, 500, 100, 1075, 0)

	Config.skillActivation:addParam("UseR", "Use R", SCRIPT_PARAM_ONOFF, true)
	
	rangeE = Config.skillActivation.humanErange
end
function LoadKSMenu()
...
function LoadSkillRanges()
	rangeQ = 625
	rangeW = 1000
	rangeE = 1100 --Config.skillActivation.humanErange
	rangeQS = 475
	rangeWS = 475
	rangeES = 925 --Config.skillActivation.spiderErange
	killRange = 1100
end
...
function LoadVIPPrediction()
	if not predictionActivated then
		if Config.predictionChoice.useVpred then
			VP = VPrediction()
			predictionActivated = true
...
function OnTick()
	if not myHero.dead then
		print(Config.skillActivation.spiderErange)
...
function CastQUniversal(target)
	if SMITEREADY then	
		if ValidTarget(target) and Config.Farm.jungleFarm.jungleFarmQ then
	...
	if Config.rappel.rappelSmite then
		...
		if not rappel then
			...
			if next(jungleMinions)~=nil then
				for i, obj in pairs(jungleMinions) do
					if obj and obj.valid then
						if GetDistance(obj)<= rangeES and GetDistance(obj)>=Config.skillActivation.spiderErange then
	...
	if Config.rappel.rappelSteal and SPIDEREREADY then
		...
		if not rappel then
				...
				for i, obj in pairs(jungleMinions) do
					if obj and obj.valid and EpicCreep(obj) then
						if SmiteDmg>=obj.health and SMITEREADY and EpicCreep(obj) then 
							...
							if GetDistance(obj)<= rangeES and smiteRappel and EpicCreep(obj) and GetDistance(obj)>=Config.skillActivation.spiderErange then
								...
								if EpicCreep(obj) then
									if SmiteDmg>=obj.health and smiteRappel and SMITEREADY then
										if SPIDEREREADY and GetDistance(obj)<=rangeES and GetDistance(obj)>=Config.skillActivation.spiderErange and GetDistance(obj)>=smiteRange then
											CastSpiderEDefensive()
										end
										CastQUniversal(obj)
			...
			if next(jungleMinions)~=nil then
				for i, obj in pairs(jungleMinions) do
					if obj and obj.valid then
						if GetDistance(obj)<= rangeES and GetDistance(obj)>=Config.skillActivation.spiderErange and smiteRappel then
							local spiderQAvailable = GetTickCount() > QSCDTimer and SPIDERQLEARNED
							local humanQAvailable = GetTickCount() > QCDTimer and HUMANQLEARNED
							if spiderQAvailable then
								local mobHealth = ((obj.maxHealth-obj.health)/100)
	...
	for i, minion in pairs(enemyMinions.objects) do
		if ValidTarget(minion) and GetDistance(minion)<=rangeE and HUMANEREADY and ValidTarget(newTarget) and (GetDistance(minion)<=GetDistance(newTarget)) and newTarget.type ~= "obj_AI_Minion" and SMITEREADY then
			...
			elseif Config.predictionChoice.useVpred then
				local pred = VP:GetCircularCastPosition(newTarget, math.huge, 90, rangeE, 1400)
				if pred and GetDistance(pred)<=rangeE then
					local hitMinion = checkhitlinepass(myHero, pred, 100, rangeE, minion, 50)
					if hitMinion and GetDistance(minion)<=smiteRange and SmiteDmg>=minion.health then
						Count = Count + 1
...
function checkParamInfo()
	...
	if not predictionCheck and not predictionActivated then
		if Config.predictionChoice.useVpred then
			require "VPrediction"
	...
	elseif predictionCheck and not predictionActivated then
		LoadVIPPrediction()
	end
end
			...
			if GetDistance(newTarget)<=killRange then
		     	if isHuman() then
		       		...
		       		elseif SPIDERQREADY and GetDistance(newTarget)<=rangeQS then
		       			CastR(newTarget)
		       			CastQ(newTarget)
		       		elseif SPIDEREREADY and GetDistance(newTarget)<=rangeES and GetDistance(newTarget)>=Config.skillActivation.spiderErange and champSelected then
		       			CastR(newTarget)
    	...
    	CastItems(target)
     	if isHuman() then
       		...
       		elseif SPIDERQREADY and GetDistance(target)<=rangeQS then
       			CastR(target)
       			CastQ(target)
       		elseif SPIDEREREADY and GetDistance(target)<=rangeES and GetDistance(target)>=Config.skillActivation.spiderErange then
       			CastR(target)
       			CastSpiderE(target)
       		end
       	else
	...
	if ValidTarget(target) and Config.combatKeys.teamFight then
    	...
    	if isHuman() then
       		...
       		elseif SPIDERQREADY and GetDistance(target)<=rangeQS then
       			CastR(target)
       			CastQ(target)
       		elseif SPIDEREREADY and GetDistance(target)<=rangeES and GetDistance(target)>=Config.skillActivation.spiderErange and ((GetTickCount()>eStunTimer and not HUMANEREADY and hasLessDamage(target)) or not target.canMove or HUMANEREADY) then
       			CastR(target)
       			CastSpiderE(target)
       		end
       	else
...
function CastSpiderE(target)
	...
	if ValidTarget(target) then
		if GetDistance(target) <= rangeES and GetDistance(target)>=Config.skillActivation.spiderErange then
			if not rappel then
				CastSpell(_E, target)
			else
				CastSpell(_E, target)
		...
		elseif GetTickCount() > aaTime then
			if ValidTarget(minion) then
				if isHuman() then
					...
					if SPIDERQREADY and GetDistance(minion)<=rangeQS and Config.Farm.laneFarm.farmQ and (Config.Farm.laneFarm.farmSpider or Config.Farm.laneFarm.farmMix) then
						local qmDmg = getDmg("QM", minion, myHero, 3)
						if qmDmg>=minion.health and (Config.Farm.laneFarm.farm or Config.Farm.laneFarm.farmToggle or Config.Farm.laneFarm.laneClear or Config.Farm.laneFarm.laneClearToggle) then
							CastQ(minion)
						elseif (Config.Farm.laneFarm.laneClear or Config.Farm.laneFarm.laneClearToggle) and (Config.Farm.laneFarm.farmSpider or Config.Farm.laneFarm.farmMix) then
					...
					elseif HUMANWREADY and GetDistance(minion)<=rangeW and Config.Farm.laneFarm.farmW and not wBuffOn and (Config.Farm.laneFarm.farmHuman or Config.Farm.laneFarm.farmMix) then
						local wDmg = getDmg("W", minion, myHero, 3)
						if wDmg>=minion.health and (Config.Farm.laneFarm.farm or Config.Farm.laneFarm.farmToggle or Config.Farm.laneFarm.laneClear or Config.Farm.laneFarm.laneClearToggle) then
							CastR(minion)
							CastHumanW(minion)
		...
		if ValidTarget(minion) then
			...
			elseif GetTickCount() > aaTime then
				if haveAggro and spiderlings and ValidTarget(aggroUnit) and aggroUnit == minion and not isHuman() and Config.Farm.jungleFarm.jungleFarmAggro then
					...
					local HeroPos = Vector(myHero.x, myHero.y, myHero.z)
					local Pos = mobPos +(mobPos-HeroPos)*(-700/GetDistance(aggroUnit))
					...
					if ValidTarget(minion) then
						...
						if isHuman() then
							...
							if SPIDERQREADY and GetDistance(minion)<=rangeQS and Config.Farm.jungleFarm.jungleFarmQ and (Config.Farm.jungleFarm.jungleSpider or Config.Farm.jungleFarm.jungleMix) then
								CastQ(minion)
							elseif SPIDERWREADY and GetDistance(minion)<=rangeWS and Config.Farm.jungleFarm.jungleFarmW and (Config.Farm.jungleFarm.jungleSpider or Config.Farm.jungleFarm.jungleMix) then
								CastSpiderW(minion)
							elseif HUMANWREADY and GetDistance(minion)<=rangeW and not wBuffOn and Config.Farm.jungleFarm.jungleFarmW and (Config.Farm.jungleFarm.jungleHuman or Config.Farm.jungleFarm.jungleMix) and ((not spiderlings and Config.Farm.jungleFarm.jungleFarmAggro) or not Config.Farm.jungleFarm.jungleFarmAggro) then
								CastR(minion)
								CastHumanW(minion)
...
function orbWalk()	
	...
	elseif GetTickCount() > aaTime and Config.OrbWalk.champOrbWalk.moveToMouse and (Config.combatKeys.teamFight or Config.combatKeys.harass or Config.combatKeys.harassToggle) then
		MoveToMouse()
	end
end
