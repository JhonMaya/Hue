...
function AutoSkills()
	if NamiMenu.skills.q.autoQ and Spells.Q.ready then
		for _, enemy in pairs(GetEnemyHeroes()) do
			if not enemy.dead and enemy.visible and (GetDistanceSqr(enemy) < (Spells.Q.range * Spells.Q.range)) then
	...
	if NamiMenu.skills.w.autoW and Spells.W.ready then
		for _, ally in pairs(GetAllyHeroes()) do
			...
			if (ally.health <= (ally.maxHealth * MenuHealth)) and (myHero.mana >= (myHero.maxMana * MenuMana)) and (CountEnemyHeroInRange(Spells.W.range, ally) < 2) then
				CastW()
