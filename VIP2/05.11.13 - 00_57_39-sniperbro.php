...
function OnSendPacket(p)
	...
	if packet:get('name') == 'S_CAST' then
		if packet:get('spellId') == _R then
			local vec = GetCassMECS(75, rangeR, 1, false) -- 80 degree R
			if not ValidTarget(newTarget) and Config.ultSettings.smartUlt or (vec and GetDistance(vec) > rangeR) then
				packet:block()
			end
		end
	end
...
function OnDraw()
    if not myHero.dead then
        if ValidTarget(newTarget) then
        	DrawCircle2(newTarget.x, newTarget.y, newTarget.z, 150, ARGB(150,183,60,244), 3)
        	DrawText("Targetting: "..newTarget.charName,35,400,50,ARGB(255,255,143,20))
        end
