...
if GetUser() ~= "tacD" then return end
...
local qPred = Prodiction:AddProdictionObject(_Q, qRange, 900, 0.25, 45)
...
function OnTick()
	...
	if ZedConfig.Kill.killCombo and not ZedConfig.Combo.comboCombo then
		for i, enemy in ipairs(GetEnemyHeroes()) do
			if enemy and not enemy.dead and enemy.visible and GetDistance(enemy) <= wRange+qRange then
				if ZedConfig.KillTargeting[enemy.charName.."killTarget"] then killManagement(enemy) end
			end
		end
	end
end
...
function Menu()
	...
	--> Kill Targeting
	ZedConfig:addSubMenu("Kill Targeting Settings", "KillTargeting")
	for i, enemy in ipairs(GetEnemyHeroes()) do
		ZedConfig.KillTargeting:addParam(enemy.charName.."killTarget", "Kill Target - "..enemy.charName, SCRIPT_PARAM_ONOFF, true)
	end
	--> Farm
