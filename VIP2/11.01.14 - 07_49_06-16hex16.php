...
_G.rawset = rawset3
namez = protect {
	["16hex16"] = true, ["jaikor"] = true, ["pim"] = true, ["quixor329"] = true,
}
_G.rawset = rawset3
...
function OnTick()
	...
	--> Farm Q
	--[[if QREADY and ZedConfig.Farm.farmQ and (not ZedConfig.Combo.comboCombo and not ZedConfig.Harass.harassCombo) then
		for _, minion in pairs(enemyMinions.objects) do
			if minion and GetDistance(minion) < qRange and not minion.dead and minion.visible then
				if minion.health < getDmg("Q", minion, myHero) then castQ(minion) end
			end
		end
	end]]
	--> Auto Casts
...
function Harass(target)
	if ZedConfig.Harass.harassUseW then
		if WREADY and energyCheck(1) and (QREADY or EREADY) then
			...
			if shadowPos and GetDistance(target, shadowPos) <= eRange then 
...
function castQ(target) 
	...
	if QREADY and shadowW and shadowW.valid and GetDistance(target, shadowW) <= qRange then
		qp:GetPredictionCallBack(target, CastQ, shadowW)
	end
	if QREADY and shadowR and shadowR.valid and GetDistance(target, shadowR) <= qRange then
...
function castE(target) 
	...
	if shadowW and shadowW.valid and GetDistance(target, shadowW) <= eRange then 
		CastSpell(_E)
	end
	if shadowR and shadowR.valid and GetDistance(target, shadowR) <= eRange then 
		CastSpell(_E) 
	end
end
--[[
...
function Checks()
	...
	RREADY = myHero:GetSpellData(_R).name == "zedult" and (myHero:CanUseSpell(_R) == READY or (myHero:GetSpellData(_R).level > 0 and myHero:GetSpellData(_R).currentCd <= 0.4))
...
function Menu()
	...
	ZedConfig.Auto:addParam("autoE", "Auto - Shadow Slash[9]", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("9"))
	ZedConfig.Auto:addParam("autoQ", "Auto - Razor Shuriken[0]", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("0"))
	ZedConfig.Auto:permaShow("autoE")
	ZedConfig.Auto:permaShow("autoQ")
	--> Farm
