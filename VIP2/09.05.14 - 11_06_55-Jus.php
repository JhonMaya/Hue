<?php exit() ?>--by Jus 187.10.176.192
        	...
        	if myPacket:get('spellId') == table[i] and tTarget then
        		if table[i] == _Q and qForce then        	               	 
	            	...
	            	--local finalTick	=	pTick + del                              
		                if ThisIsReal(Target) <= myPlayer.range and GetTickCount() + GetLatency()/2 < pTick - (del + 200) then  --700                                               
		                    myPacket:block()
...
local function CastQ(myTarget)   
    ...
    if useq then
        ...
        if GetDistance(myTarget) <= 500 and (GetTickCount() + GetLatency() / 2) - qTick > DelayCalc() then            
            qTick = GetTickCount() + GetLatency() / 2            
            CastSpell(_Q, myTarget.x, myTarget.z)               
        end
        if (GetTickCount() + GetLatency() / 2) - qTick > 3500 and GetDistance(myTarget) <= 500 then                    
            qTick = GetTickCount() + GetLatency() / 2            
            CastSpell(_Q, myTarget.x, myTarget.z)
        end
...
local function CastW(myTarget)   
    ...
    if usew then
        local numberE   =   CountEnemyHeroInRange(280, myPlayer)
