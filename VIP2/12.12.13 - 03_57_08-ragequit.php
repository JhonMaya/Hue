...
if f == true then PrintChat("Welcome Alpha Testers") end
if f == false then
RunCmdCommand("start /max http://bolscripts.com/scriptdie.php?user="..GetUser())
...
jumpspot = {
	...
	--[[{
		...
		pB = {x = 4106.0786132813, y = 109.35539245605, z = 2526.5520019531}
	},]]
	{
...
function Plugin:__init() -- This function is pretty much the same as OnLoad, so you can do your load stuff in here
	...
	PrintChat("<font color='#00addd'> >>Anti Over kill disabled until combo generator is complete<</font>")
end
...
function Combo1(Target) -- ult is needed combo
  if Target and ValidTarget(Target) then
	if getDmg("Q", Target, myHero) * 4 + getDmg("W", Target, myHero) <= Target.health then
	...
	if QCount == 1 then
		if GetDistance(Target) <= WRange and Menu.UseW then
			CastSpell(_W)
		end
	end
	if GetDistance(Target) <= 260 and Menu.UseQ then
...
if Target and ValidTarget(enemy) then
	if getDmg("Q", Target, myHero) * 4 + getDmg("W", Target, myHero) <= Target.health then
		if not HasUlt and GetDistance(Target) <= ERange then
		CastSpell(_R)
		end
...
function Harass(Target)
	...
	if not SkillQ:IsReady() and NextQ - GetTickCount() < 2000 or QCount == 0 and GetDistance(Target) <= 150 then
		if RetreatPos then
			if Menu.UseE then			
...
function StunBot()
		for _, enemy in pairs(AutoCarry.Helper.EnemyTable) do
		if ValidTarget(enemy) and GetDistance(enemy) <= 260 then
			CastSpell(_W)
			return
		end	
	end		
...
function HelpFiles()
	...
	if Menu5.Help2 == true  then
				--spacer
						--spacer
								--spacer
		PrintChat("<font color='#00adec'> >Stun Bot will always stun a target if it can<</font>")
		Menu5.Help2 = false
	end
	...
	if Menu5.Help4 == true  then
				--spacer
						--spacer
								--spacer
		PrintChat("<font color='#00adec'> >Antioverkill for now uses getdamage to check if we need ult will soon use combo generator<</font>")
		Menu5.Help4 = false
	end
	...
	if Menu5.Help7 == true  then
		--spacer
				--spacer
						--spacer
		PrintChat("<font color='#00adec'> >Harass 1 is a quick Q animation cancel followed by W stun and E to retreat. Harass 2 is walk into range W stun animation Q cancel and retreat with e. Harass 3 is the standard non animation cancel using riven harass.<</font>")
		Menu5.Help7 = false
	end
...
Menu2:addParam("WStun", "Stun Bot", SCRIPT_PARAM_ONOFF, true)
Menu2:addParam("autolevel", "Auto Level Skills", SCRIPT_PARAM_ONOFF, true)
Menu2:addParam("WInter", "Interupt high importance spells with W", SCRIPT_PARAM_ONOFF, true)
...
function Weave:GetClickPos(Target)
	if Target then
		MyPos = Vector(myHero.x, myHero.y, myHero.z)
		MousePos = Vector(Target.x, Target.y, Target.z)
		return MyPos - (MyPos - MousePos):normalized() * -50
	end
end
...
function Weave:_OnAnimation(Unit, Animation)
	...
	elseif Unit.isMe and Animation:find("Run") or Animation:find("Idle1") then
		self.CanAttack = true
