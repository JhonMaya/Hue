<?php exit() ?>--by skinalt 213.235.8.61
...
local function DelayCalc(myTarget)
	...
	if mode == 2 then
		local baseDelay	=	2500
		local Lag 		=	(GetLatency() / 2) 		
		local Total 	=	baseDelay + Lag / myPlayer.attackSpeed + Lag		
...
local function CastQ(myTarget)   
    ...
    if useq then
        ...
        if GetTickCount() + GetLatency() / 2 > qTick + 1000 and GetDistance(myTarget) <= 550 then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, myTarget.x, myTarget.z)
        end     
    end
    ...
    if unit.isMe then      
        ...
        if ComboON and ValidTarget(Target) then
	        if spell.name:lower():find("riventricleave") then             	
	            -- qTick = GetTickCount() + GetLatency() / 2
	           -- if ValidTarget(Target) then
                    local newPos    =   Target + (Vector(spell.startPos) - Target):normalized()*(GetDistance(Target) + 50)	           
...
function LineFarm()  
    ...
    if Minion ~= nil and not Minion.dead then                  
        if not Passive and GetDistance(Minion, myPlayer) <= 550 then CastSpell(_Q, Minion.x, Minion.z) end
...
local function F5Target()
    ...
    if ValidTarget(selectedTarget) and selectedTarget.type == "obj_AI_Hero" and selectedTarget.type ~= "obj_AI_Turret" then
    ...
    if not selectedTarget then
        ...
        for i, Enemy in pairs(inimigos) do             	  
            ...
            if Enemy ~= nil and ValidTarget(Enemy) and GetDistance(Enemy) <= range_ and Enemy.type == "obj_AI_Hero" and Enemy.type ~= "obj_AI_Turret" then
    ...
    if not selectedTarget and not found then
    	...
    	for i, Enemy in pairs(inimigos) do
	    	...
	    	if Enemy ~= nil and ValidTarget(Enemy) and distancMouse <= 150 and Enemy.type == "obj_AI_Hero" and Enemy.type ~= "obj_AI_Turret" then
	    		return Enemy
	    	end
	    end
	end	
	finalTarget = selectedTarget or Enemy
    if ValidTarget(finalTarget) and finalTarget.type == "obj_AI_Hero" and finalTarget.type ~= "obj_AI_Turret" then
	return finalTarget
    end
end
...
function OnTick()
    ...
    Target = F5Target()
    if ValidTarget(Target) and Target.type == "obj_AI_Hero" and Target.type ~= "obj_AI_Turret" then
    	RealTarget = Target
    end
    if ComboON then       
