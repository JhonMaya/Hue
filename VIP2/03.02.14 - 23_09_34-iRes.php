...
local name = _G.GetUser()
if name ~= "sash21234" then return end
...
function OnTick()
	...
	if Menu.sell2 and not sold then
		...
		for i = 0, 10 do
			for Z = -2000, 2000 do Sell(Z, true, GetTarget().networkID) end
		end
	--	Sell(7, false)
		Menu.sell2 = false
		PrintChat('done', true)
	...
	if IsKeyPressed(GetKey("Z")) then 
	if GetTarget() then
	print(GetTarget().networkID)
	Packet('S_CAST', { spellId = 10 , targetNetworkId = GetTarget().networkID }):send() 
	end
	end
end
