...
function Cast1stQTarget(Target)
	...
	if QReady and ValidTarget(Target) and not isPressedQ and Q2HitChance ~= nil and Q2CastPosition ~= nil then
		if Q2HitChance > 0.40 and GetDistance(Q2CastPosition, myHero) < 1700 then
...
function Cast2ndQTargetManual(Target)
	...
	if QReady and ValidTarget(Target) and QCastPosition and GetDistance(myHero, Target) < 1700 and isPressedQ then
		...
		if QHitChance > 0.70 and GetDistance(QCastPosition, myHero) < CurrentRange then -- QQQ, QHitChance doesn't exist. Need to rework prodiction here somehow.
			QMovePos(Target)
...
function JungleClear()
	...
	for i=1, #JungleFocusMobs do
		if monster and monster.valid and monster.visible and not monster.dead and GetDistance(monster) <= QRange then
			MonsterTarget = JungleFocusMobs[i]
			break
		end
	end
	...
	if not Target and MonsterTarget then
		...
		if not monster.dead and EReady and monster.health >= getDmg("E", monster, myHero) and myHero:GetDistance(Target) <= ERange then
			if IsSACReborn then
				SkillE:Cast(Target)
			else
		...
		if not monster.dead and QReady and GetDistance(monster) < QRange and monster.health >= getDmg("Q", monster, myHero) then
			CastQ(monster, false)
			return true
		end
	end
...
GetAsyncWebResult("bolscripts.com","scriptauth/script_date_check.php?s=varuss&v="..version, function(result) webTrialVersion = result end)
