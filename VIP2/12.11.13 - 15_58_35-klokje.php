...
class 'TProdiction' -- { 
    ...
    function TProdiction:Check()
        self.checked = true
        if debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 83 then 
            AdvancedCallback:bind('OnDash', function(unit, dash) self:OnDash(unit, dash) end)
            AdvancedCallback:bind('OnGainBuff', function(unit, buff) self:OnGainBuff(unit, buff) end)
        end 
        ...
        if self.value and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 83 then 
            self.canNotMiss = self.value
        end 
    end 

    function TProdiction:CanNotMissMode(enable)
        if self.checked and debug.getinfo(ProdictManager.GetInstance().extra, "S").linedefined == 83 then 
   ...
   -- end
    ...
    function TProdiction:OnDash(unit, dash)
        ...
        if unit.networkID == self.target.networkID and self:Valid() and GetDistance(dash.startPos, dash.endPos) > 100 then 
            local travelTimeA = 0
            dista =  (self.prodict.Spell.Delay - (((GetLatency()) / 1000) +0.05)) * self.prodict.Spell.Speed
           ...
           -- print(t)
            if travelTimeA <= t and t <= travelTimeB then
                ...
                if distance*distance <= self.prodict.Spell.RangeSqr then 
                    ...
                    time2 = GetDistance(dash.startPos, dash.endPos) / dash.speed
                    if time <= time2 then 
