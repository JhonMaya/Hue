...
local function MenuExtraCombo()
    ...
    menu.combo.extracombo:addParam("targetrange", "Range to Auto Target", SCRIPT_PARAM_SLICE, 800, 550, 1000, -1)
        	...
        	if myPacket:get('spellId') == table[i] and tTarget then
        		if table[i] == _Q and qForce then        	               	 
	            	...
	            	--local finalTick	=	pTick + del                              
		                if ThisIsReal(Target) <= myPlayer.range and GetTickCount() + GetLatency()/2 < pTick then  --700                                               
		                    myPacket:block()
...
local function CastQ(myTarget)   
    ...
    if useq then
        ...
        if aa and GetDistance(myTarget) <= 500 and (GetTickCount() + GetLatency() / 2) - qTick > 1500 then
            qTick = GetTickCount() + GetLatency() / 2            
            CastSpell(_Q, myTarget.x, myTarget.z)            
        end
        if (GetTickCount() + GetLatency() / 2) - qTick > 2500 and GetDistance(myTarget) <= 500 then                    
            qTick = GetTickCount() + GetLatency() / 2            
            CastSpell(_Q, myTarget.x, myTarget.z)
        end
...
function LineFarm()  
    ...
    if Minion ~= nil and not Minion.dead then                  
        if aa and GetDistance(Minion, myPlayer) <= 550 then CastSpell(_Q, Minion.x, Minion.z) end
