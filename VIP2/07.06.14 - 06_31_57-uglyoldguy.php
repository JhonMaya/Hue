...
local UPDATE_URL = "http://www.dienofail.com/LuluNightly.lua" .. "?rand=" .. math.random(1,100000)
function Download()
...
_ENV[c({115,99,114,105,112,116,118,101,114})] = 0.071
...
function OnTick()
    if initDone and _G[c({106,100,107,110,109,118})] then
        ...
        if Config.Support then
          Support()
          if target and ValidTarget(target) and GetDistance(target) < 550 then
              OrbWalking(target)
          else
              moveToCursor()
          end
...
function CastW(Target)
    if Target == nil or Target.dead or not ValidTarget(Target) or GetDistance(Target) > SpellW.Range then return end
    ...
    elseif WReady and GetDistance(Target) < 1500  and GetDistance(Target) > Config.Extras.WChaseDistance and not IsMyManaLow() then
      ...
      if PredictedPos ~= nil then
        if GetDistance(Target) - GetDistance(PredictedPos) > 150 then
          CastSpell(_W, myHero)
        end
      end
    elseif WReady and GetDistance(Target) < 1100 and not IsMyManaLow() then
      local PredictedPos, _ = CombinedPos(Target, 0.500, math.huge, myHero, false)
      if PredictedPos ~= nil then
        if GetDistance(Target) > GetDistance(PredictedPos) + 150  and GetDistance(PredictedPos) > Config.Extras.WChaseDistance then
