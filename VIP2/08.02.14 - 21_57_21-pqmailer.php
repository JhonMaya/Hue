...
local PSAuth = ProSeriesAuth("VarusAuth")
local lastKSAttempt = 0
local DebuffTable = {}
local target
local QData = {last = 0, nextRelease = 0, haveToRelease = false, target = nil, manualAim = false}
...
function Init()
	...
	Config.combo:addParam("useQstacks", "Min. stacks to use Q", SCRIPT_PARAM_SLICE, 3, 0, 3, 0)
	...
	Config.combo:addParam("useEstacks", "Min. stacks to use E", SCRIPT_PARAM_SLICE, 3, 0, 3, 0)
	...
	Config.harass:addParam("useQstacks", "Min. stacks to use Q", SCRIPT_PARAM_SLICE, 2, 0, 3, 0)
	Config.harass:addParam("useE", "Use E", SCRIPT_PARAM_ONOFF, false)
	Config.harass:addParam("useEstacks", "Min. stacks to use E", SCRIPT_PARAM_SLICE, 2, 0, 3, 0)
	...
	Config.ult:addParam("aimRmouse", "Aim to best target", SCRIPT_PARAM_ONKEYDOWN, false, GetKey("T"))
	Config.ult:addParam("useRinRange", "Use R in AA range", SCRIPT_PARAM_ONOFF, false)
	Config.ult:addParam("useRoutRange", "Use R out of AA range", SCRIPT_PARAM_ONOFF, false)
	...
	Config.extra:addParam("aimQmouse", "Aim to nearest mouse target", SCRIPT_PARAM_ONKEYDOWN, false, GetKey("W"))
	...
	PSUpdate = ProSeriesUpdate("VarusUpdate")
	PSUpdate.LocalVersion = Version
	PSUpdate.SCRIPT_NAME = "PSVarus"
	PSUpdate.SCRIPT_URL = "http://pqbol.de/scripts/release/proseries/adc/varus/PSVarus.lua"
	PSUpdate.PATH = BOL_PATH.."Scripts\\".."PSVarus.lua"
	PSUpdate.HOST = "pqbol.de"
	PSUpdate.URL_PATH = "/scripts/release/proseries/adc/varus/revision.lua"
	PSUpdate:Run()
...
function OnTick()
	...
	_CheckQRelease()
	_ResetSettings()
	if Config.extra.aimQmouse then _CastQtoMouse() end
	if Config.ult.aimRmouse then _CastRtoBestTarget() end
	...
	if ValidTarget(target) then
		...
		if Config.combo.teamfight then
			if myHero:CanUseSpell(_R) == READY then
				...
				if enemy ~= nil and count ~= nil then
					...
					if ((Config.ult.useRinRange and distance <= trueRange) or (Config.ult.useRoutRange and distance > trueRange)) and count ~= nil and count >= Config.ult.bounceCount then
						tpR:GetPredictionCallBack(target, _CastR)
					end
				end
			elseif distance > target.trueRange and DebuffTable[target.networkID] ~= nil then
		...
		elseif Config.harass.poke or Config.harass.pokeToggle then
			if distance > target.trueRange and DebuffTable[target.networkID] ~= nil then
...
function OnSendPacket(p)
	if p.header == 0xE5 then
		...
		if sourceNetworkId == myHero.networkID and spellId == 128 and not QData.manualAim then
			p:Block()
		end
	end
end
...
function OnWndMsg(Msg, Key)
	if Key == 81 then
		if Msg == 256 then
			QData.manualAim = true
		elseif Msg == 257 then
			DelayAction(function()
				QData.manualAim = false
			end, 1)
		end
	end
end
...
function _CastQ(unit)
	if ValidTarget(unit) and myHero:CanUseSpell(_Q) == READY then
		...
		if pos ~= nil and GetDistance(pos) <= SpellData.Q.Range and _StartQ() then
			QData.last = os.clock()
			QData.nextRelease = os.clock() + _GetQHoldTime(GetDistance(pos))
			QData.target = unit
			QData.haveToRelease = true
		end
	end
end

function _CheckQRelease()
	if QData.haveToRelease and myHero:CanUseSpell(_Q) == READY and os.clock() > QData.nextRelease then
		if ValidTarget(QData.target) then
			local qPredPos = tpQ:GetPrediction(QData.target)

			if qPredPos ~= nil then
				_ReleaseQ(qPredPos)
			end
		elseif _CountTable(DebuffTable) > 0 then
			table.sort(DebuffTable, function(x, y) if x > y then return true end end)

			for id, count in pairs(DebuffTable) do
				local unit = objManager:GetObjectByNetworkId(id)

				if ValidTarget(unit) then
					local predPos = tpQ:GetPrediction(unit)

					if predPos ~= nil and GetDistance(predPos) <= SpellData.Q.Range then
						QData.nextRelease = os.clock() + _GetQHoldTime(GetDistance(predPos)) - (QData.nextRelease - os.clock())
						QData.target = unit
						break
					end
				end
			end
		elseif ValidTarget(target) and target.qPredPos ~= nil and GetDistance(target.qPredPos) <= SpellData.Q.Range then
			QData.nextRelease = os.clock() + _GetQHoldTime(GetDistance(target.qPredPos)) - (QData.nextRelease - os.clock())
			QData.target = unit
		elseif os.clock() > QData.last + (SpellData.Q.ChargetimeMax - 0.2) then
			_ReleaseQ(mousePos)
		end
	end
end

function _CastQtoMouse()
	local bestT = nil
