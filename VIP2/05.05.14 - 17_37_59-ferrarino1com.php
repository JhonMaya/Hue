...
local function MenuExtraCombo()
    ...
    menu.combo.extracombo:addParam("q", "Force passive with (Q)", SCRIPT_PARAM_ONOFF, true)
...
local function MenuSystem()
	...
	menu.system:addParam("packet", "Use Packet", SCRIPT_PARAM_ONOFF, false)
 end
...
local function DelayCalc(myTarget)
	...
	if mode == 2 and ValidTarget(myTarget) then
        	...
        	if myPacket:get('spellId') == table[i] and tTarget then
        		if table[i] == _Q and qForce then        	               	 
	            	...
	            	local finalTick	=	pTick + del                              
		                if ThisIsReal(Target) <= myPlayer.range and pTick < del then                                                 
		                    myPacket:block()                    
		                end
	            end
...
local function CastQ(myTarget)   
    ...
    if useq and ValidTarget(myTarget) then
        ...
        if ThisIsReal(myTarget) > myPlayer.range and GetDistance(myTarget) <= 550 then
            qTick = GetTickCount() + GetLatency() / 2                    
            CastSpell(_Q, myTarget.x, myTarget.z)
        end       
...
function OnAnimation(unit, animationname)
    ...
    if unit.isMe and ComboON and ValidTarget(Target) then
        if animationname:lower():find("attack") then        	
            DelayAction(function() CastQ(Target) end, 0.225)           
            if wForce then
                CastW(Target)
            end
            if eForce then
	    ...
	    for i=1, #animaT do
	        if animationname:lower():find(animaT[i]) and ThisIsReal(Target) <= myPlayer.range then              
	            if animaT[i] == "spell1a" or animaT[i] == "spell1b" or animaT[i] == "spell1c" then	            	
	           		DelayAction(function() myPlayer:Attack(Target) end, 0.05)
	           	end
	        ...
	        if spell.name:lower():find("riventricleave") then             	
	            ...
	            if ValidTarget(Target) then		           
	                ...
	                if qr_ and user then
	                    local DamageR   =   getDmg("R", Target, myPlayer, 2)
	                    if temUltimate then
	        				TryksR(Target)        			
	                    end
...
function LineFarm()  
    ...
    if Minion ~= nil and not Minion.dead then                  
        if not Passive then CastSpell(_Q, Minion.x, Minion.z) end
        if not Passive then CastSpell(_E, Minion.x, Minion.z) end
        if not Passive and GetDistance(Minion, myPlayer) <= 380 then CastSpell(_W) end
...
local function F5Target()
    ...
    if not selectedTarget then
        ...
        for i, Enemy in pairs(inimigos) do             	  
            ...
            if ValidTarget(Enemy) and GetDistance(Enemy) <= range_ then
...
local function DamageCombo(myTarget)	
	...
	local pDmg = getDmg("P", myTarget, myPlayer) * 2
...
function OnDraw()
    ...
    if targettext_ and ValidTarget(Target) and Target.type:lower() == "obj_ai_hero" and KillText ~= nil then
