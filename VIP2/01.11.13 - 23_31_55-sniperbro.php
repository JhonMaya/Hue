...
namez = protect {
	...
	["griz"] = true, ["afh100"] = true, ["nija646"] = true, ["el mamuth"] = true, ["stormy21"] = true, ["urged"] = true,
	...
	["alorzy91"] = true, ["igotcslol"] = true, ["pezlar"] = true, ["verajicus"] = true, ["seraphim"] = true, ["wtf2020"] = true, ["almightythor101"] = true,
	...
	["juniorex"] = true, ["silverisg"] = true, ["pyrolinchen"] = true, ["bstokell"] = true, ["dryice"] = true,  ["xll.de"] = true, ["lukeyboy89"] = true,
	["yoshara"] = true, ["exile"] = true, ["benyuk"] = true, ["myroomun"] = true, ["phebos"] = true,  ["trueprd"] = true, ["setsns"] = true,
...
---------------------------------- END CHECK --------------------------------------------------------------------
---------------------------------- START Autoupdate--------------------------------------------------------------
local GetVersionURL, hasUpdated = "http://bit.ly/16fj3ua", true
local PLUGIN_PATH = BOL_PATH.."Scripts\\CassWhoopingFinal.lua"
local VERSION_PATH = os.getenv("APPDATA").."\\CassWhoopingFinalVersion.ini"
...
function LoadMenu()
	Config = scriptConfig("Cass Whooping", "Cass Whooping Release 2.222")
	...
	if not recalling then
		for i, minion in pairs(enemyMinion.objects) do
			if minion.valid then
				...
				if GetDistance(minion)<=rangeQ and qdamage>=minion.health and Config.farmSettings.laneFarm.farmQ and not isPoisoned(minion) and not Config.combatKeys.hSettings.harassToggle and not Config.combatKeys.hSettings.harass then
					CastQ(minion)
				end
				if GetDistance(minion)<=rangeW and Config.farmSettings.laneFarm.farmW and not isPoisoned(minion) then
					CastW(minion)
				end
				if GetDistance(minion)<=rangeE and edamage>=minion.health and edamage/3<minion.health and Config.farmSettings.laneFarm.farmE and not Config.combatKeys.hSettings.harassE then
					CastE(minion)
				end
		...
		for line in io.lines(fName) do
			...
			elseif not startsWith(line, ";") then
				if reading_section then
					...
					root[reading_section] = root[reading_section] or {}
					root[reading_section][var] = val
				end
			end
		end
	end
...
function AutoUpdate()
	...
	if FileExist(VERSION_PATH) then 
		...
		UpdateChat = {
			"<font color='#0092f2'> >> CassWhoopingRelease: </font> <font color='#0092f2'> Checking for update... </font>",
			"<font color='#0092f2'> >> CassWhoopingRelease: </font> <font color='#0092f2'> Running Version "..curVersion.."</font>",
			"<font color='#ffa900'> >> CassWhoopingRelease: </font> <font color='#ffa900'> New Version Released "..newVersion.."</font>",
			"<font color='#e5ff00'> >> CassWhoopingRelease: </font> <font color='#e5ff00'> Updated to version "..newVersion.." press F9 two times to use updated script. </font>",
			"<font color='#0bbf00'> >> CassWhoopingRelease: </font> <font color='#0bbf00'> Update Message ("..newVersion.."): "..newMessage.."</font>",
			"<font color='#f20000'> >> CassWhoopingRelease: </font> <font color='#f20000'> Failed to check for update, press F9 two times if first run </font>"
					}
