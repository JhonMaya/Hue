...
function OnTick()
	...
	if Config.combo.teamfight then
		if ValidTarget(Target) then
			if Config.combo.useQ and myHero:CanUseSpell(_Q) == READY and GetDistance(Target) <= SpellData.Q.Range then
	...
	if Config.harass.poke and ManaCheck() then
		if ValidTarget(Target) then
			if Config.harass.useQ and myHero:CanUseSpell(_Q) == READY and GetDistance(Target) <= SpellData.Q.Range then
...
function CastQ(unit, pos, spell)
	if ValidTarget(unit) and pos ~= nil and GetDistanceSqr(pos) <= SpellData.Q.Range*SpellData.Q.Range and myHero:CanUseSpell(_Q) == READY then
		CastSpell(_Q, pos.x, pos.z)
	end
end

function UseQReturn(unit)
	if unit and GetDistance(unit) >= (SpellData.Q.Range*Config.harass.qreturn.minRange/100) and GetDistance(unit) <= (SpellData.Q.Range*Config.harass.qreturn.maxRange/100) then
		return true
	end
