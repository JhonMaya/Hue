...
--[[Changelog:
	ALPHA
		...
		1.19b -> Fixed typos, movetomouse options, disable QSMITEQ due to the release, added hydra to jungle farming.
		1.19c -> Modified KillDraws, better ordered, priority changed. Added circle around targets.
	]]
...
function OnTick() --This function is updated everyframe.
	...
	elseif not ValidTarget(ts.target) then
		...
		if PROLeeSin.harasssettings.harass and PROLeeSin.harasssettings.UseMoveToMouse then MoveToCursor() end
	end
...
function OnDraw() --Draws, updated everyframe
	...
	if PROLeeSin.drawsettings.circles then
		if ValidTarget(ts.target, q2Range) then DrawCircle(ts.target.x, ts.target.y, ts.target.z, 35, 0xC6EFF7) end
		if ValidTarget(tsInsec.target, q2Range) then DrawCircle(tsInsec.target.x, tsInsec.target.y, tsInsec.target.z, 35, 0xC6EFF7) end
	end
end
...
function GetDamages()
    for i = 1, heroManager.iCount do
        local EnemyDmgs = heroManager:getHero(i)
		if ValidTarget(EnemyDmgs) then
			q1Dmg = getDmg("Q", EnemyDmgs, myHero, 1)
			q2Dmg = getDmg("Q", EnemyDmgs, myHero, 2)
    ...
    Combo2 = q1Dmg + q2Dmg + eDmg + rDmg + IgniteDmg + ItemsDmg
    Combo1 = q1Dmg + q2Dmg + eDmg + AADmg*2 + IgniteDmg + ItemsDmg
end
...
function KillDraws()
    for i = 1, heroManager.iCount do
        ...
        if ValidTarget(EnemyDraws) then
			if (EnemyDraws.health < IgniteDmg) and IgniteReady then
                PrintFloatText(EnemyDraws, 0, "Ignite!")
            elseif EnemyDraws.health < (rDmg) then
...
function Menu()
    ...
    PROLeeSin:addSubMenu("["..myHero.charName.." - Carry Peeler Settings]", "peelersettings")
        PROLeeSin.peelersettings:addParam("pcombo", "Peel Combo Key", SCRIPT_PARAM_ONKEYDOWN, false, HKZoningCombo)
