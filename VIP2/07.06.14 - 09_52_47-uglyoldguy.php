...
local VERSION_PATH = "/Dienofail/BoL/master/versions/LuluNightly.version" .."?rand="..math.random(1,10000)
local UPDATE_FILE_PATH = SCRIPT_PATH.."LuluNightly.lua"
...
_ENV[c({115,99,114,105,112,116,118,101,114})] = 0.074
...
function OnLoad()
	if GetUser() == 'BurningSchnitzel' then
...
function Menu()
	  ...
	  for idx, enemy in ipairs(Enemies) do
	      Config.ComboSub:addParam("W"..tostring(enemy.networkID), "Enable W usage on ".. enemy.charName, SCRIPT_PARAM_ONOFF, true) 
	  end
	  ...
	  Config.Extras:addParam("WDistance", "Minimum Distance to Enemy for W Enemy", SCRIPT_PARAM_SLICE, 500, 100, 650, 0)
...
function OnTick()
	if initDone and _G[c({106,100,107,110,109,118})] then
		...
		if Config.Support then
			Support()
			if target and ValidTarget(target) and GetDistance(target) < 550 and Config.SupportSub.Orbwalk then
			    OrbWalking(target)
...
function Combo(Target)
	if Target ~= nil and ValidTarget(Target, 1700) and not Target.dead  then
		...
		if Config.ComboSub.useW and not IsMyManaLow() and Config.ComboSub["W" .. tostring(Target.networkID)] then
		  	CastW(Target)
		end
...
function CastW(Target)
	...
	elseif WReady and GetDistance(Target) < 1350  and GetDistance(Target) > Config.Extras.WChaseDistance and CheckAngle(Target) then
		...
		if PredictedPos ~= nil then
			if GetDistance(PredictedPos) - GetDistance(Target) > 100 then
				CastSpell(_W, myHero)
			end
		end
	end
...
function AoEQ(Target)
	...
	local function ComputeHits(Pos1, Pos2, Pos3, Pos4) 
		...
		for idx, enemy in ipairs(EnemyHeroes) do
			if ValidTarget(enemy) and not enemy.dead and enemy ~= nil then
				...
				if GetDistance(pointSegment1, pointLine1) < SpellQ.Width + 50 then
					NumHit1 = NumHit1 + 1
				else
					...
					if GetDistance(pointSegment2, pointLine2) < SpellQ.Width + 50 then
					  NumHit1 = NumHit1 + 1
					end
				end
			end 
	...
	local function GenerateTwoVectors(Target, ExtendObject)
	  --Given Pos1-->4 (corresponding to extended vectors + hero vector, compute enemies hit)
		...
		if Target ~= nil and ExtendedObject ~= nil and ValidTarget(Target) and not Target.dead then
			...
			if Position ~= nil and Hitchance ~= nil and ExtendObject ~= nil then
			  	if GetDistance(Target) < SpellQ2.Range + 100 then
				    ...
				    if Config.Extras.Debug then
				      print('End position generated ' .. tostring(EndPos1) .. "\t" .. tostring(EndPos2))
				    end
			  	end
			end
		end
	...
	if QReady and Pix ~= nil and GetDistance(Target) < SpellQ1.Range - 20 and CountEnemyHero(Target, SpellQ1.Range+25) == 0 then
		...
		elseif CastPos1 ~= nil and Hit1 ~= nil and Hit1 >= Config.Extras.Hitchance and GetDistance(CastPos1, Pix) < SpellQ1.Range and GetDistance(CastPos1) < SpellQ2.Range then
			CastSpell(_Q, CastPos1.x, CastPos1.z)
		  	if Config.Extras.Debug then
		    	print('Extend Cast still best option')
		  	end
		end
	elseif QReady and EReady and GetDistance(Target) < SpellQ2.Range - 20 and CountEnemyHero(Target, SpellQ1.Range+25) == 0 then
	  	ExtendedQ(Target)
	    if Config.Extras.Debug then
	...
	elseif EReady and QReady and myHero.mana > myHero:GetSpellData(_Q).mana + myHero:GetSpellData(_E).mana and GetDistance(Target) < SpellQ2.Range + 25  and CountEnemyHero(Target, SpellQ1.Range+25) > 0 then
		...
		if #ExtendedEnemyTable >= 1 then
			for idx, val in ipairs(ExtendedEnemyTable) do
				...
				if CurrentHits ~= nil and CurrentHits >= SingleCaseHit + Config.Extras.MinPix and CurrentHits > BestHits then
		...
		if Config.Extras.Debug then
			print('Besthits' .. tostring(BestHits))
		end
	...
	elseif Pix ~= nil and not EReady and QReady and GetDistance(Target) < SpellQ2.Range then
		...
		if CastPos2 ~= nil and Hit2 ~= nil and Hit2 >= Config.Extras.Hitchance and GetDistance(CastPos2) < SpellQ1.Range then
			CastSpell(_Q, CastPos2.x, CastPos2.z)
		elseif CastPos1 ~= nil and Hit1 ~= nil and Hit1 >= Config.Extras.Hitchance and GetDistance(CastPos1) < SpellQ2.Range and GetDistance(CastPos1, Pix) < SpellQ2.Range then
			CastSpell(_Q, CastPos1.x, CastPos1.z)
		end
	end
end
...
function ExtendedQ(Target)
	...
	local function WillHit(Target, ExtendObject)
		if ExtendObject ~= nil and Target ~= nil and GetDistance(Target) < SpellQ2.Range + 50 and GetDistance(ExtendObject) < SpellE.Range and GetDistance(ExtendObject, Target) < SpellQ1.Range then
			local Position, Hitchance = VP:GetPredictedPos(Target, 0.500, math.huge, myHero, false)
	  ...
	  --Single Case First
		...
		elseif NumHit ~= nil and FirstPosition ~= nil then
			...
			elseif GetTickCount() - LastSpellTick > 300 then
				...
				if NumHit1 == NumHit2 then
					CastSpell(_Q, FirstPosition1.x, FirstPosition1.z)
				elseif NumHit1 > NumHit2 then
					CastSpell(_Q, FirstPosition1.x, FirstPosition1.z)
				elseif NumHit1 < NumHit2 then
		...
		if Config.SupportSub["support"..tostring(idx)] then
			...
			if val ~= nil and not val.dead and GetDistance(val) < 1000 then 
				...
				if ClosestEnemy ~= nil and CloseDistance ~= nil then
					if Config.Extras.Debug then
						print('Supporting Closest Enemy' .. tostring(ClosestEnemy.charName))
					end
					if val.health*100 / val.maxHealth < Config.SupportSub.MinEHealth and Config.SupportSub.useE and EReady and GetDistance(val) < SpellE.Range and GetDistance(ClosesetEnemy, val) < 1000 then
						CastSpell(_E, val)
						if Config.Extras.Debug then
					...
					if val.health*100 / val.maxHealth < Config.SupportSub.MinRHealth and Config.SupportSub.useR and RReady and GetDistance(val) < SpellR.Range and GetDistance(ClosesetEnemy, val) < 1000 then
						CastSpell(_R, val)
						if Config.Extras.Debug then
					...
					elseif Pix ~= nil and QReady and EReady and Config.SupportSub.useQ and Config.SupportSub.useE then
						Combo(target)
						if Config.Extras.Debug then
							print('Supporting Closest Enemy Combo' .. tostring(ClosestEnemy.charName))
						end
					end 
				end
			end
		...
		if Config.SupportSub["support"..tostring(idx)] and val ~= nil and GetDistance(val) < 1000 and not val.dead then
			...
			for idx, enemy in ipairs(Enemies) do
			    ...
			    if not enemy.dead and ValidTarget(enemy) and GetDistance(enemy) < SpellW.Range and Config.Extras.WGapClosers then
			        local IsDashing, CanHit, Position = VP:IsDashing(enemy, SpellW.Delay, 0, SpellW.Speed, myHero)
			        if Position ~= nil and IsDashing ~= nil and CanHit ~= nil then
						if IsDashing and CanHit and GetDistance(Position) < SpellW.Range and WReady and GetDistance(val, Position) < 350 then
							if Config.Extras.Debug then
