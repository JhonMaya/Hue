...
local function MenuExtraCombo()
    ...
    menu.combo.extracombo:addParam("targetrange", "Range to Auto Target", SCRIPT_PARAM_SLICE, 820, 500, 1000, 0)
...
function OnLoad()
    menu = scriptConfig("Riven by Jus", "RivenBunny")
        	...
        	if myPacket:get('spellId') == table[i] and tTarget then
        		if table[i] == _Q and qForce then        	               	 
	            	...
	            	--local finalTick	=	pTick + del                              
		                if ThisIsReal(Target) <= myPlayer.range and GetTickCount() + GetLatency()/2 < pTick - (del + 250) then  --700                                               
		                    myPacket:block()
...
local function CastQ(myTarget)   
    ...
    if useq then
        ...
        if GetDistance(myTarget) <= 550 and GetTickCount() + GetLatency() / 2 - qTick > del then -- (GetTickCount() + GetLatency() / 2) - qTick > del then                      
            if mode_ == 1 then delayType = ( (del/1000) ) else  delayType = ( del/1000 - 0.125 ) end
            if delayType < 1 then delayType = 0.22 end
            --print(tostring((lastWindUpTime/1000) - ((GetLatency())/2000)))
        ...
        if GetDistance(myTarget) <= 550 and GetTickCount() + GetLatency() / 2 - qTick > 3000 then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, myTarget.x, myTarget.z)
        end 
    end
    ...
    for i, Enemys in pairs(GetEnemyHeroes()) do
        ...
        local rDmg = getDmg("R", Enemys, myPlayer,2)           
            if Enemys.health <= rDmg then
                ...
                if temUltimate and Enemys.health <= rDmg or GetTickCount() + GetLatency() / 2 > rTick + 10000 then
...
local function TargetIsValid(myTarget)
    return myTarget ~= nil and myTarget.type == "obj_AI_Hero" and myTarget.type ~= "obj_AI_Turret" and myTarget.type ~= "obj_AI_Minion" and not TargetHaveBuff("UndyingRage", myTarget) and not TargetHaveBuff("JudicatorIntervention", myTarget)
end
...
function F5Target()
    ...
    if not selectedTarget and not found then
    	...
    	for i, Enemy in pairs(inimigos) do
	    	...
	    	if Enemy ~= nil and ValidTarget(Enemy) and TargetIsValid(Enemy) and distancMouse <= 150 then
	    		return Enemy
	    	end
	    end
	end	
	finalTarget = selectedTarget or Enemy
    --if ValidTarget(finalTarget) and TargetIsValid(finalTarget) or ValidTarget(Enemy) and TargetIsValid(Enemy) then
	return finalTarget
    --end
end
	...
	for i=1, #needTable do		
		if needTable[i] ~= 0 then			
			...
			if needTable[i] == wDmg then
				if GetDistance(myTarget) <= 282 then CastSpell(_W) end
			end			
...
function OnDraw()
    ...
    if passive_ then
        DrawText3D("Passive Counter: "..tostring(PassiveIndicator), myPlayer.x, myPlayer.y, myPlayer.z, 20, ARGB(255,255,255,255), true)
    end
    if spots_ then
        local From, To, CastPos = ReturnDrawPoint()
