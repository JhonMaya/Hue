<?php exit() ?>--by Hellsing 91.96.208.184
...
function AfterDownload()
	...
	if DOWNLOAD_COUNT == 0 then
		...
		print("<font color=\"#FF794C\">[".._G.IsLoaded.."]:</font><font color=\"#FFDFBF\"> Required libraries downloaded successfully, please reload (double [F9]).</font>")
	end
end
for DOWNLOAD_LIB_NAME, DOWNLOAD_LIB_URL in pairs(REQUIRED_LIBS) do
	if FileExist(LIB_PATH .. DOWNLOAD_LIB_NAME .. ".lua") then
		...
		print("<font color=\"#FF794C\">[".._G.IsLoaded.."]:</font><font color=\"#FFDFBF\"> Not all required libraries are installed. Downloading: <b><u><font color=\"#73B9FF\">"..DOWNLOAD_LIB_NAME.."</font></u></b> now! Please don't press [F9]!</font>")
...
-- Vars for Abilitys -- 
	...
	local QREADY, WREADY, EREADY, RREADY, Q2READY, W2READY, E2READY = false
-- Vars for TargetSelector --
	local ts
	local ts = TargetSelector(TARGET_LESS_CAST_PRIORITY, 1200, DAMAGE_PHYSICAL, true)
	ts.name = "Rengar"
...
-- Vars for Damage Calculations and Drawing --
	...
	local liandrysDmg = 0
	local KillText = {}
	local KillTextColor = ARGB(255, 255, 38,0)
	local KillTextList = {		
							...
							"Kill! - (Q)+(Q2)+(E)",			-- 13
							"Kill! - (Q)+(Q2)+(W)+(E)",		-- 14
							"Kill! - Use all to burst him!" -- 15
						}
-- Vars for Autolevel --
	local levelSequence = 	{
								startQ = { 1,2,3,1,1,4,1,3,1,3,4,3,3,2,2,4,2,2 },
								startQ2 = { 1,3,2,1,1,4,1,3,1,3,4,3,3,2,2,4,2,2 },
...
-- Misc Vars --
	...
	local VP = nil
	local RengarMenu
	local OrbwalkDetectionDone = false
	local SAC = false
	local MMA = false
	local SiOrbWalker = false
	local trippleQtargetChange = false
	local jungleTargetChange = false
	local targetModeSOW
	local targetModeSOWclearKey
	local ultJumpCasted = false
---------------------------------------------------------------------
--- OnLoad ----------------------------------------------------------
---------------------------------------------------------------------
function OnLoad()
...
function OrbwalkDetection()
	if _G.AutoCarry
		then
			print("<font color=\"#FF794C\">[".._G.IsLoaded.."]:</font><font color=\"#FFDFBF\"> Sidas Auto Carry: Reborn (<b><font color=\"#cd57c5\">SAC:R</font></b>) detected! <font color=\"#6699ff\"><b>SOW</b></font> disabled! <u><font color=\"#5CFF26\">No reassign for the Keys needed!</font></u></font>")
			SAC = true
			rSOW.Menu.Enabled = false
			if _G.AutoCarry and _G.AutoCarry.Keys
				then reborn = _G.AutoCarry
			end
	elseif _G.MMA_Loaded 
		then
			print("<font color=\"#FF794C\">[".._G.IsLoaded.."]:</font><font color=\"#FFDFBF\"> Marksman's Mighty Assistant (<b><font color=\"#FFFF00\">MMA</font></b>) detected! <font color=\"#6699ff\"><b>SOW</b></font> disabled! Please <u><font color=\"#FF2626\">reassign the MMA-keys to the Script-Keys</font></u> if not done yet!</font>")
			MMA = true
			rSOW.Menu.Enabled = false
	else
			print("<font color=\"#FF794C\">[".._G.IsLoaded.."]:</font><font color=\"#FFDFBF\"> No other Orbwalker detected! Using Simple Orbwalker (<b><font color=\"#6699ff\">SOW</font></b>)! Please <u><font color=\"#FF2626\">reassign the SOW-keys to the Script-Keys</font></u> if not done yet!</font>")
			SiOrbWalker = true
			rSOW.Menu.Enabled = true
	end
	OrbwalkDetectionDone = true
	-- Disables SOW's inbuild autoreset by clearing the SOW-table
	rSOW.AttackResetTable = {}
end
---------------------------------------------------------------------
--- Menu ------------------------------------------------------------
---------------------------------------------------------------------
function AddMenu()
	...
	-- Extra --
	RengarMenu.Extra:addSubMenu("(Q) Settings", "Qspell")
	RengarMenu.Extra.Qspell:addParam("qModeSwitch", "Switch Q-AttackMode: ", SCRIPT_PARAM_LIST, 1, { "Cast before AA", "Cast after AA", "Standard Q-Cast"})
	RengarMenu.Extra.Qspell:addParam("bushQ", "AA-reset with Q after a jump from bush: ", SCRIPT_PARAM_ONOFF, true)
	RengarMenu.Extra:addParam("AutoLevelSkills", "Auto Level Skills (Reload Script!)", SCRIPT_PARAM_LIST, 1, { "No Autolevel", "QWEQ - R>Q>E>W", "QEWQ - R>Q>E>W", "WQEQ - R>Q>E>W", "EQWQ - R>Q>E>W", "EQWE - R>E>Q>W"})
	RengarMenu.Extra:addSubMenu("Packet Settings", "Packets")
	RengarMenu.Extra.Packets:addParam("packetUsage", "Use packets to cast spells: ", SCRIPT_PARAM_ONOFF, true)
	RengarMenu.Extra:addSubMenu("TrippleQ Settings", "TrippleQ")
	RengarMenu.Extra.TrippleQ:addParam("trippleQDelay", "Delay for Itemusage: ", SCRIPT_PARAM_SLICE, 0.1, 0, 1, 1)
	
	-- SOW-Orbwalking --
	RengarMenu.Orbwalk:addParam("switchTargetMode", "Switch targetmode while Tripple-Q: ", SCRIPT_PARAM_ONOFF, true)
	RengarMenu.Orbwalk:addParam("jungleToTarget", "Switch targetmode while JungleClear: ", SCRIPT_PARAM_ONOFF, true)
	RengarMenu.Orbwalk:addParam("OrbwalkInfo", "", SCRIPT_PARAM_INFO, "")
	...
	RengarMenu.Draw:addParam("drawKillText", "Draw KillText: ", SCRIPT_PARAM_ONOFF, false)
	RengarMenu.Draw:addParam("drawDamageOnHPBar", "Draw Damage on Healthbar: ", SCRIPT_PARAM_ONOFF, true)
	RengarMenu.Draw:addParam("drawTarget", "Draw current target: ", SCRIPT_PARAM_ONOFF, false)
		-- Hp Bar Damagedrawings --
		RengarMenu.Draw:addSubMenu("Healthbar Damage Drawins: ", "HealthbarDamage")
		RengarMenu.Draw.HealthbarDamage:addParam("drawSmartDmg", "Draw smart (Q)+(W)+(E)-Damage: ", SCRIPT_PARAM_ONOFF, true)
		RengarMenu.Draw.HealthbarDamage:addParam("drawTrippleQDmg", "Draw TrippleQ-Damage: ", SCRIPT_PARAM_ONOFF, true)
		RengarMenu.Draw.HealthbarDamage:addParam("drawInfo", "", SCRIPT_PARAM_INFO, "")
		RengarMenu.Draw.HealthbarDamage:addParam("drawQDmg", "Draw (Q)-Damage: ", SCRIPT_PARAM_ONOFF, false)
		RengarMenu.Draw.HealthbarDamage:addParam("drawQ2Dmg", "Draw Emp(Q)-Damage: ", SCRIPT_PARAM_ONOFF, false)
		RengarMenu.Draw.HealthbarDamage:addParam("drawWDmg", "Draw (W)-Damage: ", SCRIPT_PARAM_ONOFF, false)
		RengarMenu.Draw.HealthbarDamage:addParam("drawW2Dmg", "Draw Emp(W)-Damage: ", SCRIPT_PARAM_ONOFF, false)
		RengarMenu.Draw.HealthbarDamage:addParam("drawEDmg", "Draw (E)-Damage: ", SCRIPT_PARAM_ONOFF, false)
		RengarMenu.Draw.HealthbarDamage:addParam("drawE2Dmg", "Draw Emp(E)-Damage: ", SCRIPT_PARAM_ONOFF, false)
		RengarMenu.Draw.HealthbarDamage:addParam("drawIDmg", "Draw Ignite-Damage: ", SCRIPT_PARAM_ONOFF, false)
...
function OnTick()
	...
	if OrbwalkDetectionDone == false then OrbwalkDetection() end
...
function KeyBindings()
	...
	-- Automatically calls the SAC functions while pressing Key's
	if SAC
		then
			if reborn.Keys.LaneClear then JungleClear() LaneClear() end
			if reborn.Keys.MixedMode then Harass() end
			if reborn.Keys.AutoCarry then SBTW() end
	end
	-- Tripple Q --
	-- Sets our current Targetmode to 'to target' in SOW while pressing the TrippleQKey
	if TrippleQKey and RengarMenu.Orbwalk.switchTargetMode and rSOW.Menu.Mode == 1 and trippleQtargetChange
		then
			-- Saves the original mode in a var --
			targetModeSOW = 1
			rSOW.Menu.Mode = 2
	end
	-- Resets the changed Targetmode back to the originial setting
	if not TrippleQKey and RengarMenu.Orbwalk.switchTargetMode and rSOW.Menu.Mode == 2 and targetModeSOW == 1
		then
			rSOW.Menu.Mode = 1
			targetModeSOW = 2
	end
	if not trippleQtargetChange and targetModeSOW == 1
		then 
			rSOW.Menu.Mode = 1
			targetModeSOW = 2
	end
	-- JungleClear w/o stuttering --
	if ClearKey and RengarMenu.Orbwalk.jungleToTarget and rSOW.Menu.Mode == 1 and jungleTargetChange
		then
			-- Saves the original mode in a var --
			targetModeSOWclearKey = 1
			rSOW.Menu.Mode = 2
	end
	if not ClearKey and rSOW.Menu.Mode == 2 and targetModeSOWclearKey == 1 and RengarMenu.Orbwalk.jungleToTarget
		then 
			rSOW.Menu.Mode = 1
			targetModeSOWclearKey = 2
	end
	if not jungleTargetChange and targetModeSOWclearKey == 1 and RengarMenu.Orbwalk.jungleToTarget
		then 
			rSOW.Menu.Mode = 1
			targetModeSOWclearKey = 2
	end
...
---------------------------------------------------------------------
--- Custom MMA/SAC/Targetselector Target ----------------------------
---------------------------------------------------------------------
...
function Check()
	...
	Q2READY = (myHero:CanUseSpell(_Q) == READY and Ferocity == 5)
	W2READY = (myHero:CanUseSpell(_Q) == READY and Ferocity == 5)
	E2READY = (myHero:CanUseSpell(_Q) == READY and Ferocity == 5)
	...
	-- Set the slots for item --
		...
		leapRangeSlot	= GetInventorySlotItem(3452) -- Item ID for 6 trophies
	
	-- Check for Leaprangeenhancer --
	if leapRangeSlot ~= nil 
		then leapRange = 725
	end
			
	-- SOW Callbacks Reset --
	if not SBTWKey and not ClearKey and not HarassKey and not TrippleQKey then
		for i, cb in ipairs(rSOW.AfterAttackCallbacks) do
			table.remove(rSOW.AfterAttackCallbacks, i)
		end
		for i, cb in ipairs(rSOW.BeforeAttackCallbacks) do
			table.remove(rSOW.BeforeAttackCallbacks, i)
		end
	end	
	-- Tripple Q --
...
function UseItems(enemy)
	...
	if ValidTarget(enemy) then
		...
		if sotdReady	and GetDistance(enemy) <= 320 then CastSpell(sotdSlot) end
	end
end
---------------------------------------------------------------------
...
function CastTheQ()
	if SiOrbWalker
		then
			if RengarMenu.Extra.Qspell.qModeSwitch == 1
				then rSOW:RegisterBeforeAttackCallback(function() CastSpell(_Q) end)
			elseif RengarMenu.Extra.Qspell.qModeSwitch == 2
				then rSOW:resetAA() rSOW:RegisterAfterAttackCallback(function() CastSpell(_Q) end)
			elseif RengarMenu.Extra.Qspell.qModeSwitch == 3
				then CastSpell(_Q)
			end
	end
	if MMA
	...
	if SAC then
		if RengarMenu.Extra.Qspell.qModeSwitch == 1
			then reborn.Plugins:RegisterPreAttack(function() CastSpell(_Q) end)
		else CastSpell(_Q)
		end
	end
end
...
function CastTheW(enemy)
	if (not WREADY or (GetDistanceSqr(enemy, myHero) > wRange*wRange))
		...
		if not RengarMenu.Extra.Packets.packetUsage
			then CastSpell(_W)
		elseif RengarMenu.Extra.Packets.packetUsage
...
function SBTW()
	if ValidTarget(Target) and allowSpells
		then
			if Ferocity <= 4
				then
					if RengarMenu.SBTW.sbtwQ and GetDistanceSqr(Target, myHero) < qRange*qRange and QREADY
						then
							CastTheQ()
					end
					if RengarMenu.SBTW.sbtwW and GetDistanceSqr(Target, myHero) < wRange*wRange and WREADY
						then CastTheW(Target)
					end
					if RengarMenu.SBTW.sbtwE and GetDistanceSqr(Target, myHero) < eRange*eRange and EREADY
						then AimTheE(Target)
					end
			end
			if Ferocity == 5
				then
					if HealthCheck(RengarMenu.SBTW.sbtwHeal)
						...
						if RengarMenu.SBTW.empPrioritySBTW == 2
							then
								if GetDistanceSqr(Target) < wRange*wRange
									then CastTheW(Target)
								end
						end
						if RengarMenu.SBTW.empPrioritySBTW == 3
							then
			...
			if RengarMenu.SBTW.sbtwItems then UseItems(Target) end
		end
end
---------------------------------------------------------------------
--- Functions for KS ------------------------------------------------
---------------------------------------------------------------------
function smartKS()
	if not TrippleQKey then -- prevents TrippleQ stuttering
		for _, enemy in pairs(enemyHeroes) do
			if enemy ~= nil and ValidTarget(enemy) and allowSpells then
			local distance = GetDistance(enemy)
			local hp = enemy.health
				if hp <= qDmg and QREADY and (distance <= qRange*qRange)
					then CastSpell(_Q)
				elseif hp <= wDmg and WREADY and (distance <= wRange*wRange) 
					then CastTheW(enemy)
				elseif hp <= eDmg and EREADY and (distance <= eRange*eRange) 
					then AimTheE(Target)
				elseif hp <= (qDmg + wDmg) and QREADY and WREADY and (distance <= qRange*qRange)
					then CastTheW(enemy)
				elseif hp <= (qDmg + eDmg) and QREADY and EREADY and (distance <= qRange*qRange)
					then AimTheE(Target)
				elseif hp <= (wDmg + eDmg) and WREADY and EREADY and (distance <= wRange*wRange)
					then AimTheE(Target)
				elseif hp <= (qDmg + wDmg + eDmg) and QREADY and WREADY and EREADY and (distance <= qRange*qRange)
					then AimTheE(Target)
				end
			end
		end
...
function JungleClear()
	...
	if JungleMob ~= nil
		then
			if GetDistanceSqr(JungleMob, myHero) <= 200*200
				then
					jungleTargetChange = true 
			else jungleTargetChange = false 
			end
			...
			if Ferocity <= 4 then
				...
				if RengarMenu.Jungle.jungleW and GetDistanceSqr(JungleMob, myHero) <= wRange*wRange
					then CastTheW(JungleMob)
				end
				if RengarMenu.Jungle.jungleE and GetDistanceSqr(JungleMob, myHero) <= eRange*eRange
			...
			if Ferocity == 5 then
				if HealthCheck(RengarMenu.Jungle.jungleHeal) then
					if RengarMenu.Jungle.empPriorityJungle == 1 and GetDistanceSqr(JungleMob, myHero) <= qRange*qRange
						then CastTheQ()
					end
					if RengarMenu.Jungle.empPriorityJungle == 2 and GetDistanceSqr(JungleMob, myHero) <= wRange*wRange
						then CastTheW(JungleMob)
					end
...
function OnCreateObj(obj)
	if obj ~= nil then
		...
		if obj.name == "Rengar_LeapSound.troy" and TrippleQKey
			then
				ultJumpCasted = true
		end
		if obj.name == "Rengar_LeapSound.troy" and not TrippleQKey and RengarMenu.Extra.Qspell.bushQ
			then
				if ValidTarget(Target)
					then
						DelayAction(function()
							if GetDistanceSqr(Target, myHero) <= ((qRange*2)*(qRange*2))
								then
									rSOW:resetAA() Packet("S_CAST", {spellId = _Q}):send()
							end		
						end, 0.4)
				end
		end
			...
			then
				...
				if Ferocity <= 4
					then
						...
						if RengarMenu.Farm.farmW and GetDistanceSqr(minion, myHero) <= wRange*wRange
							then CastTheW(minion)
						end
						if RengarMenu.Farm.farmE and GetDistanceSqr(minion, myHero) <= eRange*eRange
				...
				if Ferocity == 5
					then
						if HealthCheck(RengarMenu.Farm.farmHeal)
							then 
								if RengarMenu.Farm.empPriorityFarm == 1 and GetDistanceSqr(minion, myHero) <= qRange*qRange
									then CastTheQ()
								end
								if RengarMenu.Farm.empPriorityFarm == 2 and GetDistanceSqr(minion, myHero) <= wRange*wRange
									then CastTheW(minion)
								end
...
function OnGainBuff(unit, buff)
	if unit.isMe and buff.name == "RengarR"
		then	allowSpells = false
				ultActive = true
	end
	if unit.isMe and buff.name == "rengarrspeed"
		then	allowSpells = true
	end
end
	...
	for i=1, heroManager.iCount do
		local enemy = heroManager:GetHero(i)
			if ValidTarget(enemy) and enemy ~= nil
				...
				qDmgPre 	= ((getDmg("Q", enemy, myHero)) or 0)
				qDmgEPre	= ((getDmg("Q", enemy, myHero, 2)) or 0) -- qEmp
			...
			-- Set Kill Text --	
				-- Harass your enemy! -- 
				if enemy.health > totalDmg then KillText[i] = 1
					-- "Kill! - Ignite" --
					...
					-- "Kill! - (Q)+(Q2)" --
					elseif enemy.health <= (qDmg + qDmgE)
						then 
							if QREADY and Ferocity == 4 then KillText[i] = 7
							else KillText[i] = 2
							end
					-- "Kill! - (Q)+(W)" --
					...
					-- "Kill! - (Q)+(Q2)+(W)" --
					elseif enemy.health <= (qDmg + qDmgE + wDmg)
						then
							if QREADY and WREADY and Ferocity == 4 then KillText[i] = 12
							else KillText[i] = 2
							end
					-- "Kill! - (Q)+(Q2)+(E)" --
					elseif enemy.health <= (qDmg + qDmgE + eDmg)
						then
							if QREADY and EREADY and Ferocity == 4 then KillText[i] = 13
							else KillText[i] = 2
							end
					-- "Kill! - (Q)+(Q2)+(W)+(E)" --
					elseif enemy.health <= (qDmg + qDmgE + wDmg + eDmg)
						then
							if QREADY and WREADY and EREADY and Ferocity == 4 then KillText[i] = 14
							else KillText[i] = 2
							end
				end	
		end
...
function AutoLevelMySkills()
		...
		elseif RengarMenu.Extra.AutoLevelSkills == 3 then
			autoLevelSetSequence(levelSequence.startQ2)
		...
		elseif RengarMenu.Extra.AutoLevelSkills == 6 then
...
if Target
	...
	if TrippleQCombo == 2
		then
			if GetDistance(Target) <= leapRange
				then
					trippleQtargetChange = true
				--	CastSpell(_Q)
	...
	if TrippleQCombo == 3 and ultJumpCasted
		then 
	...
	if TrippleQCombo == 5
		then
			if GetDistance(Target) <= qRange and SecondQUsed == true and Ferocity >= 4
				then
					CastSpell(_Q)
					rSOW:RegisterAfterAttackCallback(function()DelayAction(function()
								trippleQtargetChange = false
							end, 0.7) end)
			end
	end
...
function OnProcessSpell(object, spell)
	if ValidTarget(Target)
		then
			if spell.name == 'RengarQ' and TrippleQCombo >= 2
				then
					DelayAction(function()
					local delay = RengarMenu.Extra.TrippleQ.trippleQDelay
					if sotdReady then CastSpell(sotdSlot) end
					if youReady	then CastSpell(youSlot) end
					if dfgReady	then CastSpell(dfgSlot, Target) end
							if hxgReady	then CastSpell(hxgSlot, Target) end
							if bwcReady	then CastSpell(bwcSlot, Target) end
							if botrkReady then CastSpell(botrkSlot, Target) end
...
---------------------------------------------------------------------
--- Damagedrawing on Lifebar ----------------------------------------
---------------------------------------------------------------------
function GetHPBarPos(unit)
	unit.barData = {PercentageOffset = {x = -0.05, y = 0}}
...
function DrawLineHPBar(damage, line, text, unit)
	...
	local transparency = 350 - math.round(255*((unit.health-thedmg)/unit.maxHealth))
	if transparency >= 255 then transparency=254 end
	
	local unitCurrentHPpercentAfterDmg = ((unit.health-thedmg)/unit.maxHealth)
	if unitCurrentHPpercentAfterDmg  <= 0.5
		then 
			redcolor = 255
			greencolor = math.round((unitCurrentHPpercentAfterDmg/0.5)*255)
	else
		redcolor = math.round(((1-unitCurrentHPpercentAfterDmg)/0.5)*255)
		greencolor = 255
	end
	if Offs_X < Real_X
		then
			Offs_X = Real_X	--RGB(239,255,191)
			DrawLine(Offs_X-150, StartPos.y-(30+(line*12)), Offs_X-150, StartPos.y-10, 1, ARGB(transparency, 255,223, 191))
			DrawText(tostring("Kill him with: "..text),15,Offs_X-148,StartPos.y-(30+(line*13)),ARGB(transparency, 239,255, 191))
	else
			DrawLine(Offs_X-150, StartPos.y-(30+(line*12)), Offs_X-150, StartPos.y-10, 1, ARGB(transparency, redcolor,greencolor, bluecolor))
			DrawText(tostring(text),15,Offs_X-148,StartPos.y-(30+(line*13)),ARGB(transparency, redcolor,greencolor, bluecolor))
	end
end
function DrawMyDmgOnHpBar()
	if RengarMenu.Draw.drawDamageOnHPBar
		then
			local currLine = 1
			for i, enemy in ipairs(GetEnemyHeroes()) do		
				if enemy~=nil and not enemy.dead and enemy.visible and ValidTarget(enemy) then		
						-- Q --
						if QREADY and RengarMenu.Draw.HealthbarDamage.drawQDmg and not Q2READY
							then
							DrawLineHPBar(dmgQ(enemy), currLine, "Q: "..dmgQ(enemy), enemy)
							currLine = currLine + 1
						end
						-- Q2 --
						if Q2READY and RengarMenu.Draw.HealthbarDamage.drawQ2Dmg
							then
							DrawLineHPBar(dmgQ2(enemy), currLine, "Q2: "..dmgQ2(enemy), enemy)
							currLine = currLine + 1
						end
						-- W --
						if WREADY and RengarMenu.Draw.HealthbarDamage.drawWDmg and not W2READY
							then
							DrawLineHPBar(dmgW(enemy), currLine, "W: "..dmgW(enemy), enemy)
							currLine = currLine + 1
						end
						-- W2 --
						if W2READY and RengarMenu.Draw.HealthbarDamage.drawW2Dmg
							then
							DrawLineHPBar(dmgW2(enemy), currLine, "W2: "..dmgW2(enemy), enemy)
							currLine = currLine + 1
						end
						-- E --
						if EREADY and RengarMenu.Draw.HealthbarDamage.drawEDmg and not E2READY
							then
							DrawLineHPBar(dmgE(enemy), currLine, "E: "..dmgE(enemy), enemy)
							currLine = currLine + 1
						end
						-- E2 --
						if E2READY and RengarMenu.Draw.HealthbarDamage.drawE2Dmg
							then
							DrawLineHPBar(dmgE2(enemy), currLine, "E2: "..dmgE2(enemy), enemy)
							currLine = currLine + 1
						end
						-- Ignite --
						if IREADY and RengarMenu.Draw.HealthbarDamage.drawIDmg
							then
							DrawLineHPBar(dmgI(enemy), currLine, "Ignite: "..dmgI(enemy), enemy)
							currLine = currLine + 1
						end
						
					if RengarMenu.Draw.HealthbarDamage.drawSmartDmg
						then
						-- Q --
						if QREADY and not WREADY and not EREADY and not Q2READY
							then
							DrawLineHPBar(dmgQ(enemy), currLine, "Q: "..dmgQ(enemy), enemy)
							currLine = currLine + 1
						-- W --
						elseif WREADY and not EREADY and not QREADY and not W2READY
							then
							DrawLineHPBar(dmgW(enemy), currLine, "W: "..dmgW(enemy), enemy)
							currLine = currLine + 1
						-- E --
						elseif EREADY and not QREADY and not WREADY and not E2READY
							then
							DrawLineHPBar(dmgE(enemy), currLine, "E: "..dmgE(enemy), enemy)
							currLine = currLine + 1
						-- Q+W --
						elseif QREADY and WREADY and not EREADY
							then
							DrawLineHPBar(dmgQW(enemy), currLine, "Q+W: "..dmgQW(enemy), enemy)
							currLine = currLine + 1
						-- Q+E --	
						elseif QREADY and not WREADY and EREADY
							then
							DrawLineHPBar(dmgWE(enemy), currLine, "Q+E: "..dmgQE(enemy), enemy)
							currLine = currLine + 1
						-- W+E --
						elseif not QREADY and WREADY and EREADY
							then
							DrawLineHPBar(dmgWE(enemy), currLine, "W+E: "..dmgWE(enemy), enemy)
							currLine = currLine + 1
						-- Q+W+E --
						elseif QREADY and WREADY and EREADY 
							then
							DrawLineHPBar(dmgQWE(enemy), currLine, "Q+W+E: "..dmgQWE(enemy), enemy)
							currLine = currLine + 1
						end
					end
						-- TrippleQ --
						if (RREADY or ultActive or TrippleQKey) and RengarMenu.Draw.HealthbarDamage.drawTrippleQDmg
							then 
							DrawLineHPBar(dmgTrippleQ(enemy), currLine, "TrippleQ: "..dmgTrippleQ(enemy), enemy)
							currLine = currLine + 1
						end
					end
				end	
...
function dmgQ(unit)
    local myQDmg = getDmg("Q", unit, myHero, 1)
	local myAADmg = getDmg("AD", unit, myHero)
	local myQ1Dmg = myQDmg + myAADmg 
    return math.round(myQ1Dmg)
end
function dmgQ2(unit)
    local myQDmg = getDmg("Q", unit, myHero, 2)
	local myAADmg = getDmg("AD", unit, myHero)
	local myQ2Dmg = myQDmg + myAADmg
    return math.round(myQ2Dmg)
end
function dmgW(unit)
    local myWDmg = getDmg("W", unit, myHero)
    return math.round(myWDmg)
end
function dmgW2(unit)
    local myW2Dmg = getDmg("W", unit, myHero, 2)
    return math.round(myW2Dmg)
end
function dmgE(unit)
    local myEDmg = getDmg("E", unit, myHero)
    return math.round(myEDmg)
end
function dmgE2(unit)
    local myE2Dmg = getDmg("E", unit, myHero, 2)
    return math.round(myE2Dmg)
end
function dmgI(unit)
...
function dmgQW(unit)
	local qwDmg = dmgQ(unit)+dmgW(unit)
	return math.round(qwDmg)
end
function dmgQE(unit)
	local qeDmg = dmgQ(unit)+dmgE(unit)
	return math.round(qeDmg)
end
function dmgWE(unit)
	local weDmg = dmgW(unit)+dmgE(unit)
	return math.round(weDmg)
end
function dmgQWE(unit)
	local qweDmg = dmgQ(unit)+dmgW(unit)+dmgE(unit)
	return math.round(qweDmg)
end
function dmgTrippleQ(unit)
	local i1Dmg = ((IREADY and getDmg("IGNITE", unit, myHero)) or 0)
	local bwc1Dmg = ((bwcReady and getDmg("BWC", unit, myHero)) or 0) -- Bilgewater Cutlass
	local botrk1Dmg 	= ((botrkReady and getDmg("RUINEDKING", unit, myHero)) or 0) -- Blade of the Ruined King
	local sheen1Dmg	= ((sheenReady and getDmg("SHEEN", unit, myHero)) or 0) -- Sheen
	local trinity1Dmg = ((trinityReady and getDmg("TRINITY", unit, myHero)) or 0) -- Trinity Force
	local TrippleQDmg = dmgQ(unit)+dmgQ(unit)+dmgQ2(unit)+dmgW(unit)+dmgE(unit)+bwc1Dmg+botrk1Dmg+sheen1Dmg+trinity1Dmg
	return math.round(TrippleQDmg)
end
