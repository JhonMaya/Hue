...
if myHero.charName ~= "Nidalee" and myHero.charName ~= "Kayle" and myHero.charName ~= "Akali" and myHero.charName ~= "Talon" and myHero.charName ~= "Leona" then return end
...
class 'mostEnemies'
    ...
    function mostEnemies:GetCircleMEC(radius, main_target, delay)
            ...
            local position = self:GetCenter(targets)
            ...
            while not best_pos_found do
                    ...
                    position = self:GetCenter(targets)
                    circle.center = position
...
class 'Collision' 
    ...
    -------------------------------------------------------
    --[[
        Class: Collision with VPrediction
        Version: 0.01
    ]]
    -------------------------------------------------------
...
HERO_ALLY = 3
    ...
    function VCollision:__init(sRange, projSpeed, sDelay, sWidth)
        uniqueId = uniqueId + 1
        self.uniqueId = uniqueId
    ...
    function VCollision:GetMinionCollision(pStart, pEnd)
        self.enemyMinions:update()
    ...
    function VCollision:GetHeroCollision(pStart, pEnd, mode)
        if mode == nil then mode = HERO_ENEMY end
        local heros = {}
    ...
    function VCollision:GetCollision(pStart, pEnd)
        local b , minions = self:GetMinionCollision(pStart, pEnd)
        local t , heros = self:GetHeroCollision(pStart, pEnd, HERO_ENEMY)
...
SAUpdate.URL_PATH = "/BotHappy/bhscripts/raw/master/REV/LibRev.lua" 
SAUpdate:Run()