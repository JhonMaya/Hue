...
_ENV[c({115,99,114,105,112,116,118,101,114})] = 1.31
...
function SendQ()
        ...
        --   print("me")
        if Target ~= nil and GetDistance(Target) <= GetCustomRange(_Q) then
    ...
    if myPacket:get('name') == 'S_CAST' then
      if myPacket:get('spellId') == _Q then
        ...
        if pType_ == 3 then
          ...
          Packet('S_MOVE', {toX = newPos.x, toY = newPos.z, fromX = newPos.y, fromY = newPos.z }):send() --spell.startPos.x 
...
["Hg"]          =       {ready = false, range = 400, SlotId = 3146, slot = nil},
...
function CastQ(myTarget)  
  ...
  if useq and ThisIsReal(myTarget) > myPlayer.range and GetDistance(myTarget) <= GetCustomRange(_Q) then       
    --if GetDistance(myTarget) <= GetCustomRange(_Q) then
      --if menu.comboextra.range.useNew == 2 or 3 then
        --qTick = GetTickCount() + GetLatency() / 2        
  ...
  --------use---------
    ...
    if unit.isMe then      
        ...
        --print("PROCESS: AA")          
            ...
            DelayAction(function() if spell.Target ~= nil then CastSpell(_Q, spell.Target.visionPos.x, spell.Target.visionPos.z) end end, spell.animationTime - GetLatency() / 2000)
          --print("3) AA")
        ...
        if ComboON and Target ~= nil then
          ...
          if spell.name:lower():find("riventricleave") then
            ...
            --if temSlow and ThisIsReal(Target) <= myPlayer.range then myPlayer:Attack(Target) end          
                ...
                --   Packet('S_MOVE', {toX = spell.endPos.x, toY = spell.endPos.z, fromX = spell.endPos.x, fromY = spell.endPos.z}):send()
                -- end
                -- if menu.system.pType == 3 then
