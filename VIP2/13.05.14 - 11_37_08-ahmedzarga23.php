...
function getVersion()
        GetAsyncWebResult("dl.dropboxusercontent.com","/s/amosylb7142jxfs/Irelia2.txt",getDownloadVersion)
end

function update()
   if updateCheck == false then
       local PATH = BOL_PATH.."Scripts\\Irelia2.lua"
       local URL = "https://dl.dropboxusercontent.com/s/glnjvr083ipx9zo/Irelia2.lua/Irelia2.lua"
       if latestVersion~=nil and latestVersion ~= _G.IreliaVersion then
           updateCheck = true
           PrintChat("UPDATING Irelia - "..SCRIPT_PATH:gsub("/", "\\").."Irelia2.lua")
           DownloadFile(URL, PATH,function ()
            PrintChat("UPDATED - Please Reload (F9 twice)")
            end)
        elseif latestVersion == _G.IreliaVersion then
            updateCheck = true
            PrintChat("Irelia is up to date")
        end
   end
end
     ...
     if (myHero.health / myHero.maxHealth) <= (Config.AdvOpt.AutoRLow / 100) then
        ...
        for i, minion in pairs(EnemyMinions.objects) do
            if minion ~= nil  and  not Rfound and Config.AdvOpt.useR2  then
             ...
             if Rfound and Config.AdvOpt.useR3 then 
                R:Cast(MinionObj2.x,MinionObj2.z)
                                end 
            end 
        end 
