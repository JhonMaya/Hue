...
-------Variables-------
local testTick, WhenAAAgain, testTick3 = 0,0,0
-------BENCHMARK Variables-------
...
local function MenuExtraCombo()
    ...
    menu.combo.extracombo:addParam("targetrange", "Range to Auto Target", SCRIPT_PARAM_SLICE, 800, 550, 1000, 1000)
        	...
        	if myPacket:get('spellId') == table[i] and tTarget then
        		if table[i] == _Q and qForce then        	               	 
	            	...
	            	--local finalTick	=	pTick + del                              
		                if ThisIsReal(Target) <= myPlayer.range and GetTickCount() + GetLatency()/2 < del then  --700                                               
		                    myPacket:block()
...
local function CastQ(myTarget)   
    ...
    if useq then
        ...
        if (GetTickCount() + GetLatency() / 2) - qTick > lastAttack - lastWindUpTime and GetDistance(myTarget) <= 500 then
            qTick = GetTickCount() + GetLatency() / 2           
        ...
        --     CastSpell(_Q, myTarget.x, myTarget.z)
        -- end
        if (GetTickCount() + GetLatency() / 2) - qTick > 2000 - lastWindUpTime and GetDistance(myTarget) <= 500 then
            qTick = GetTickCount() + GetLatency() / 2            
            CastSpell(_Q, myTarget.x, myTarget.z)
        end
    end
...
local function CastW(myTarget)   
    ...
    if usew then
        if ThisIsReal(myTarget) > myPlayer.range and GetDistance(myTarget) <= 282 then            
        	CastSpell(_W)            
        elseif GetDistance(myTarget) <= 282 and myPlayer:CanUseSpell(_Q) ~= READY then
            CastSpell(_W)
        end
    end
