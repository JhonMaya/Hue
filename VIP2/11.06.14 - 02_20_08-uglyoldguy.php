...
_ENV[c({115,99,114,105,112,116,118,101,114})] = 0.14
...
function Menu()
    ...
    Config.FarmSub:addParam("LastHit", "Minion Limit only for killable minions", SCRIPT_PARAM_ONOFF, false)
    --KS
...
function AoEFarm()
  ...
  local function countminionshitQ(pos, from)
      ...
      for i, minion in ipairs(EnemyMinions.objects) do
          ...
          if MinionIsOnSegment and GetDistance(MinionPointSegment, MinionPointLine) < SpellQ.Width+40 and (Config.FarmSub.LastHit and getDmg("Q", minion, myHero) > minion.health) then
              n = n +1
              -- if Config.Extras.Debug then
  ...
  local function ComputeHits(Pos1, Pos2, Pos3, Pos4) 
    ...
    for idx, enemy in ipairs(EnemyMinions.objects) do
      if enemy ~= nil and ValidTarget(enemy) and not enemy.dead and (Config.FarmSub.LastHit and getDmg("Q", enemy, myHero) > enemy.health) then
