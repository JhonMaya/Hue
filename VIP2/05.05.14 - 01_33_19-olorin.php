<?php exit() ?>--by olorin 94.121.165.63
...
function OnLoad()
        ...
        AUConfig:addParam("sacmma", "Using SAC/MMA [Recommended use]", SCRIPT_PARAM_ONOFF, true)
...
function OnDraw()
        ...
        local function dtext(text, person)
                ...
                elseif person == nil then
                        if ts.target ~= nil then PrintFloatText(ts.target, 0, text) end
                elseif person ~= nil then
...
function Combo()
        ...
        if AUConfig.combosettings.comboActive then
                if (qready or wready) and AUConfig.orbwalker.Enabled and AUConfig.screwAA then
                        SOWi:DisableAttacks()
                elseif not (qready or wready) and AUConfig.orbwalker.Enabled and SOWi:CanMove() and AUConfig.combosettings.movetomouse and AUConfig.combosettings.comboActive then
                        myHero:MoveTo(mousePos.x, mousePos.z)
                        SOWi:EnableAttacks()
                elseif not (qready or wready) and AUConfig.orbwalker.Enabled and not SOWi:CanMove() then
                        --Return
                elseif not AUConfig.sacmma and AUConfig.combosettings.movetomouse and AUConfig.combosettings.comboActive then
                        myHero:MoveTo(mousePos.x, mousePos.z)
                elseif AUConfig.combosettings.movetomouse and AUConfig.combosettings.comboActive and _G.AutoCarry and _G.AutoCarry.MyHero and AUConfig.sacmma and AUConfig.screwAA and (qready or wready) then
        ...
        if ts.target ~= nil and not (string.find(ts.target.type, "obj_AI_Minion") and ts.target.team ~= myHero.team and (not string.find(ts.target.name, "Worm"))) then
                if AUConfig.combosettings.comboActive then
                        ...
                        if canKill(ts.target, 3) and hasMana(3) and ValidTarget(ts.target, 620) and qready then
                                doCombo(3, ts.target)
                        ...
                        elseif canKill(ts.target, 7) and hasMana(8) and qready and not wready and ultAvailable and not ts.target.canMove and ValidTarget(ts.target, 620) then doCombo(8, ts.target)
                        elseif canKill(ts.target, 8) and wready and hasMana(11) and not qready and ultAvailable and not ts.target.canMove and ValidTarget(ts.target, 620) then doCombo(11, ts.target)
                        elseif canKill(ts.target, 1.1) and hasMana(7) and qready and wready and ultAvailable and not ts.target.canMove and ValidTarget(ts.target, 620) then doCombo(7, ts.target)
                        ...
                        elseif canKill(ts.target, 1) and qready and wready and rready and ValidTarget(ts.target, 620) and ultAvailable then
                                if pyroStack == 3 and eready and hasMana(1) then
                                        doCombo(1, ts.target)
                                        elseif stunReady and hasMana(7) then
                                        doCombo(7, ts.target)
                                end
                        elseif canKill(ts.target, 1) and qready and wready and rready and (GetDistanceSqr(myHero.visionPos, ts.target.visionPos) > 562500 and GetDistanceSqr(myHero.visionPos, ts.target.visionPos) < 990025) and ValidTarget(ts.target, 995) and not tibbersAlive and AUConfig.combosettings.useflash and flashready then
                        ...
                        if ValidTarget(ts.target, 620) then
                                ...
                                if ultAvailable and hasMana(12) and ((stunReady or (pyroStack == 3 and eready))) and not didCombo then
                                        ...
                                        if not wready and hasMana(12) then doCombo(12, ts.target) end
                                end
...
function Harass() -- or AUConfig.orbwalker.Enabled
        if AUConfig.harasssettings.harass then
                if (qready or wready) and AUConfig.orbwalker.Enabled and AUConfig.harasssettings.screwAA then
                        SOWi:DisableAttacks()
                else
                        SOWi:EnableAttacks()
                end
                if AUConfig.harasssettings.movetomouse and ((_G.AutoCarry and (not _G.AutoCarry.Orbwalker ~= nil or not  _G.AutoCarry.Crosshair:GetTarget())) or AUConfig.orbwalker.Enabled and SOWi:CanMove()) then
                        myHero:MoveTo(mousePos.x, mousePos.z)
                end
                if ts.target ~= nil and _G.AutoCarry and _G.AutoCarry.Orbwalker then _G.AutoCarry.Orbwalker:Orbwalk(ts.target) end
                if ts.target ~= nil and AUConfig.orbwalker.Enabled then SOWi:OrbWalk(ts.target) end
...
function CommandBear()
        if ts.target ~= nil and tibbersAlive and AUConfig.tibbers.directTibbers then
                ...
                local hitboxSqr = VP:GetHitBox(target)
                hiboxSqr = hitboxSqr * hitboxSqr --since we are using GetDistanceSqr
                if GetDistanceSqr(tibbersObject.visionPos, target.visionPos) < 42025 + hitboxSqr then
                ...
                elseif GetDistanceSqr(tibbersObject.visionPos, target.visionPos) > 42025 + hitboxSqr then
        ...
        elseif tibbersAlive and AUConfig.tibbers.followTibbers and not myHero.dead then
                if GetDistanceSqr(tibbersObject.visionPos, myHero.visionPos) > 15600 then Packet("S_MOVE", {type = 6, dwArg1 = 1, dwArg2 = 0, sourceNetworkId = tibbersObject.networkID, unitNetworkId = tibbersObject.networkID, targetNetworkId = 0, waypoint = 1, x = myHero.visionPos.x, y = myHero.visionPos.z}):send() end
        end
end
 
 
function CastExploit(spell, target)
        if target ~= nil and (((ValidTarget(ts.target, 4000) and target.type == myHero.type)) or target.type ~= myHero.type) then
                CastSpell(_Q, target)
        end
end
