<?php exit() ?>--by pqmailer 141.101.99.210
class 'Plugin'
...
function Plugin:OnTick()
	...
	if Cards.Tracker then
		...
		if Keys.MixedMode then
			if Cards.Blue and Menu.farm.mixedmode.useBlueMM and not self:CheckMana(Menu.farm.mixedmode.useBlueMMSlider) and ValidTarget(Minions.AlmostKillable) then
				CastSpellEx(_W)
				return
			end
			if Cards.Red and Menu.farm.mixedmode.useRedMM then
				if Menu.farm.mixedmode.useBlueMM and not self:CheckMana(Menu.farm.mixedmode.useBlueMMSlider) then
					return
				end
				if ValidTarget(Minions.AlmostKillable) and self:CountMinions(Minions.AlmostKillable, 100) >= Menu.farm.mixedmode.useRedMMSlider then
					CastSpellEx(_W)
					return
				end
			end
		...
		if Keys.LaneClear then
			if Cards.Blue and Menu.farm.laneclear.useBlueLC and not self:CheckMana(Menu.farm.laneclear.useBlueLCSlider) and ValidTarget(Minions.AlmostKillable) then
				CastSpellEx(_W)
				return
			end
			if Cards.Red and Menu.farm.laneclear.useRedMM then
				...
				if ValidTarget(Minions.AlmostKillable) and self:CountMinions(Minions.AlmostKillable, 100) >= Menu.farm.laneclear.useRedLCSlider then
					CastSpellEx(_W)
					return
				end
			end
...
function Plugin:CountMinions(Point, Range)
	local Count = 0
	for _, Minion in pairs(Minions.EnemyMinions.objects) do
		if ValidTarget(Minion) and GetDistance(Minion, Point) <= Range then
			Count = Count + 1
		end
	end
...
Menu.farm.mixedmode:addParam("useRedMM", "Pick red card in mixed mode", SCRIPT_PARAM_ONOFF, true)
Menu.farm.mixedmode:addParam("useRedMMSlider", "Minion count for red card", SCRIPT_PARAM_SLICE, 3, 1, 10, 0)
Menu.farm.mixedmode:addParam("useBlueMM", "Pick blue card in mixed mode", SCRIPT_PARAM_ONOFF, true)
Menu.farm.mixedmode:addParam("useBlueMMSlider", "Min. mana % for blue card", SCRIPT_PARAM_SLICE, 70, 0, 100, 0)
...
Menu.farm.laneclear:addParam("useRedLCSlider", "Minion count for red card", SCRIPT_PARAM_SLICE, 2, 1, 10, 0)
