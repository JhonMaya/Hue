...
function Vars()
	...
	minionArray = minionManager(MINION_ALL, GetTrueRange(), myHero, MINION_SORT_HEALTH_ASC)
...
function Menu()
	...
	Menu:addSubMenu("["..myHero.charName.." Auto Carry: Info]", "ScriptInfo")
		Menu.ScriptInfo:addParam("sep", myHero.charName.." Auto Carry: Version "..version..releaseSubVersion, SCRIPT_PARAM_INFO, "")
		Menu.ScriptInfo:addParam("sep1","Created By: Kain", SCRIPT_PARAM_INFO, "")
		Menu.ScriptInfo:addParam("sep2","With help from Dienofail and pqmailer", SCRIPT_PARAM_INFO, "")
...
function _G.PluginOnLoad()
	...
	PrintChat(" >> Varus: The Arrow of Retribution by Kain loaded!")
end
...
function _G.PluginOnTick()
	...
	-- Hack for lane clear in Dominion with Revamped
	if not IsSACReborn and AutoCarry.MainMenu.LaneClear and not Target and SCMAP then
		local minion = findWeakMinion()
...
--return lowest HP minion
function findWeakMinion()
	minionArray:update()
	tmp = nil
	for i,minionObject in ipairs(minionArray.objects) do
		if ValidTarget(minionObject, GetTrueRange()) then
			tmp = minionObject
			break
		end
	end
...
function Cast2ndQTargetManual(Target)
	...
	if QReady and ValidTarget(Target) and QCastPosition and GetDistance(myHero, Target) < 1700 and isPressedQ and IsKeyDown(KeyQ) then
...
function _G.PluginOnSendPacket(packet)
	...
	--if p:get("spellId") == _E and not (AutoCarry.MainMenu.AutoCarry or AutoCarry.MainMenu.LaneClear or AutoCarry.MainMenu.MixedMode or Menu.AutoCarry.SlowE) then
		--p:block()
	--end
    if packet.header == 0xE5 and isPressedQ and IsKeyDown(KeyQ) then --and Cast then -- 2nd cast of channel spells packet2
        ...
        if spelltype == 0x80 then -- 0x80 == Q
            packet.pos = 1
            packet:Block()
            PrintChat("(Debug) Packet blocked 1")
        end
    end
    if packet.header == 0x99 and isPressedQ and IsKeyDown(KeyQ) then --and Cast then -- 2nd cast of channel spells packet2
        packet:Block()
		PrintChat("(Debug) Packet blocked 2")
	end
end
--]]
