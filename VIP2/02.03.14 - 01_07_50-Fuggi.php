...
function Init()
    ...
    Minions = minionManager(MINION_ENEMY, q.Range, player, MINION_SORT_HEALTH_ASC)
    FarmMinions = minionManager(MINION_ENEMY, w.Range, player, MINION_SORT_HEALTH_ASC)
    JungleFarmMinions = minionManager(MINION_JUNGLE, w.Range, player, MINION_SORT_HEALTH_ASC)
...
function sendMousePacket()
    if Target == nil or (Target ~= nil and Target.dead == true) then
        ultUp = false
        sbtwUlt = false        
    end
    if Target ~= nil then
		...
		if ultUp == false or sbtwUlt == false then
            ...
            if Config.SMsbtw.useQ and QREADY and firstQ == false and TargetDistance < q.Range then
                ...
                if not GetMinionCollision(myHero, pPosQ, q.Width, Minions.objects) then 
                    ...
                    local i = 0
                    local mDist = 0
                    local line = Vector(myHero.x - Target.x, myHero.z - Target.z)
                    for index, minion in pairs(Minions.objects) do
                        if ValidTarget(minion) then
                            check = GetDistance(minion)
                            if check < q.Range and GetDistance(minion, Target) < q.Range then 
                                mVec = Vector(minion)
                                if line:dist(mVec) > mDist then
                                    mDist = line:dist(mVec)
                                    dPos = Vector(minion)
                                end
                            end
                        end
