...
local function MenuExtraCombo()
    ...
    menu.combo.extracombo:addParam("smart", "Triple Q-AA Delay", SCRIPT_PARAM_SLICE, 1000, 100, 3000, -1)   
...
local function MenuExtra()
    ...
    menu.extra:addParam("jump", "Jump Key", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("T"))
end
...
local function MenuSystem()
	...
	menu.system:addParam("orb", "Enable orbwalk", SCRIPT_PARAM_ONOFF, true)
...
local function DelayCalc(myTarget)
	...
	if mode == 2 then
		...
		local Lag 		=	(GetLatency() / 2) 		
		local Total 	=	GetDistance(myTarget, myPlayer) + 825 / myPlayer.attackSpeed + Lag
...
function OnSendPacket(p)
    ...
    for i=1, #table do
        if myPacket:get('name') == 'S_CAST' and myPacket:get('spellId') == _Q then
            if tTarget then 
            	local del 	=	DelayCalc(Target)
            	local finalTick	=	pTick + del                              
                if Passive and ThisIsReal(Target) <= myPlayer.range and pTick < del then                                                 
                    myPacket:block()                    
                end
            end
        -- p = CLoLPacket(71)
...
local function CastQ(myTarget)
    ...
    local useq      =   menu.combo.skills.q
    --local qReady    =   myPlayer:CanUseSpell(_Q) == READY    
    if useq and ValidTarget(myTarget) then
        if not Passive and GetDistance(myTarget) <= 550 then
        ...
        --     CastSpell(_Q, myTarget.x, myTarget.z)
        local del 	=	DelayCalc(Target)       
        if GetTickCount() + GetLatency() / 2 > qTick + del and GetDistance(myTarget) <= 550 then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, myTarget.x, myTarget.z)
        end
    end
...
local function CastW(myTarget)
   ...
   -- local wReady    =   myPlayer:CanUseSpell(_W) == READY
    ...
    if usew and ValidTarget(myTarget) then
        if GetDistance(myTarget) <= 280 then            
        	CastSpell(_W)            
        end
    end
...
local function CastE(myTarget)
    ...
    if usee and ValidTarget(myTarget) then
        if GetDistance(myTarget, myPlayer) <= 410 then
...
function CastR(myTarget)   
    ...
    if user and ValidTarget(myTarget) then     
        local rlvl      =   myPlayer:GetSpellData(_R).level
        local DamageR   =   getDmg("R", myTarget, myPlayer)
...
-- 	local objTable	=	{"missile", "ChaosTurretFire2_mis.troy", "ChaosTurretFire2_tar.troy"}
-- 	--print(tostring(object.type))
-- 	if object.type == "obj_AI_Turret" and GetDistance(myHero) <= 900 then
-- 		print(object.name)
-- 		-- for i=1, #objTable do			
-- 		-- 	if object.name:find(objTable[i]) then
    ...
    if unit.isMe then
        ...
        if spell.name:lower():find("riventricleave") then             	
            ...
            if ValidTarget(Target) then
             ...
             --    if movePos then
                    Packet('S_MOVE', {x = spell.startPos.x, y = spell.startPos.z}):send() --spell.startPos.x
                --end


                --local rlvl      =   myPlayer:GetSpellData(_R).level
                if qr_ and user then
                    local DamageR   =   getDmg("R", Target, myPlayer)
                    if temUltimate then
        				TryksR(Target)        			
                    elseif temUltimate and Target.health <= DamageR then
        ...
        if spell.name:lower():find("rivenfeint") then
            ...
            if ValidTarget(Target) then
                ...
                if er_ and user then --and temUltimate and Target.health <= DamageR then
                --     CastSpell(_R, Target.x, Target.z)
                -- elseif user and temUltimate and qr_ and GetTickCount() + GetLatency() / 2 > rTick + 14000 then
                --     CastSpell(_R, Target.x, Target.z) 
                	CastR(Target)       
                elseif usew and ew_ then
                    CastW(Target)
                    if useItems_ then CastCommonItem() end
                end
            end
...
function JumpQ()
	if CastSpell(_Q, mousePos.x, mousePos.z) then	
		myPlayer:MoveTo(mousePos.x, mousePos.z)
	elseif CastSpell(_E, mousePos.x, mousePos.z) then
		myPlayer:MoveTo(mousePos.x, mousePos.z)
	else
		myPlayer:MoveTo(mousePos.x, mousePos.z)
	end
...
function LineFarm()      
   ...
   -- local items_0   =       menu.combo.extra.items  
    ...
    if Minion ~= nil and not Minion.dead then       
        ...
        OrbWalk(Minion)                    
        if CastSpell(_Q, Minion.x, Minion.z) then
        elseif CastSpell(_E, Minion.x, Minion.z) then
        elseif GetDistance(Minion, myPlayer) <= 380 then
            CastSpell(_W)
        end
...
function JungleBitch()
    ...
    if Minion ~= nil and not Minion.dead then       
        ...
        if usew and GetDistance(Minion, myPlayer) <= 380 then
            CastSpell(_W)
        end
...
function SidaMMA(myTarget)
	...
	if ComboON and myOrb then OrbWalk(myTarget) end
end
...
local function F5Target()
    ...
    if ValidTarget(selectedTarget) and selectedTarget.type:lower() == "obj_ai_hero" then
    ...
    if not selectedTarget then
        ...
        for i, Enemy in pairs(inimigos) do
        	for a, Compare in ipairs(GetEnemyHeroes()) do
        		if Enemy ~= Compare then       	  
		            local basedmg   = 100
		            --local myDmg     = (myPlayer:CalcDamage(Enemy, 200) or 0)            
		            if ValidTarget(Enemy) and GetDistance(Enemy) <= range_ and Enemy.health <= Compare.health then
		            	--local finalDmg	=	Enemy.health
...
function OnTick()
    ...
    Target = F5Target()
    if ValidTarget(Target) and Target.type:lower() == "obj_ai_hero" then
    	RealTarget = Target
    end
    if ComboON then       
...
function moveToCursor()	 
    if ValidTarget(myTarget) and GetDistance(myTarget, myPlayer) > 1 and GetDistance(mousePos, myTarget) > 1 then
        local moveToPos = myPlayer + (Vector(mousePos) - myPlayer):normalized()* 200
...
function OnDraw()
    ...
    if targetDraw and ValidTarget(Target) and Target.type:lower() == "obj_ai_hero" then
        for i=0, 3, 1 do
