...
function OnProcessSpell(Object, Spell)
	if Object.isMe then
		...
		if Spell.name:find("Attack") or Spell.name == GetSpellData(_Q).name then
...
function PassiveExploit()
	if NWID and objManager:GetObjectByNetworkId(NWID) then
			Object = objManager:GetObjectByNetworkId(NWID)
			NWID = nil
	end
	if attackTarget and attackTarget.valid and Object and Object.valid then
			if GetDistance(attackTarget, Object) < 300  and attackTarget.health < myHero.totalDamage then
					CastSpell(_Q, attackTarget)
					CastSpell(_W, attackTarget)
					CastSpell(_E, attackTarget)
			end
			
			QDmg = getDmg("Q", attackTarget, myHero, 3)
			if myHero:CanUseSpell(_Q) ~= READY and GetDistance(attackTarget, Object) < 200 and attackTarget.health < QDmg then
					CastSpell(_E, attackTarget)
					CastSpell(_W, attackTarget)
			end    
			Object = nil
