...
local versionGOE = 0.97
local nameGOE = "Ahri - Flying Balls"
local fnameGOE = "aAhri"
local needUpdate_GOE = false
local needRun_GOE = true
local needRun_GOE_2 = true
local bbba = false -- auth successful
local URL_GOE = "http://anony4bol.comoj.com/Ahri.lua"
...
local tabZ = {}
local function encryptWWW(str)
	ae = {}
	for i=1, str:len() do
		table.insert(ae, bit32.bxor(string.byte(string.sub(str, i, i)), 4186503819))
	end
	bc = ""
	for x, char in pairs(ae) do
		if x == 1 then
			bc = bc..ae[x]
		else
			bc = bc.."//"..ae[x]
...
local function decryptWWW(str)
	local bb = DecodeB64(str)
	local ae = SplitString(bb, "//")
	bc = ""
	for i, char in pairs(ae) do
		if char ~= nil and tostring(char):len() > 0 then
			local x1 = bit32.bxor(char, 5481052194)
			bc = bc..string.char(tonumber(x1))
		end
	end
	return bc
end
function decGOE(data)
	if data ~= nil then
		local nString = decryptWWW(tostring(data))
		local username = GetUser()
		if nString == username then
			bbba = true
		end
	end
end
...
function UpdateScriptGOE()
	if needRun_GOE then
		needRun_GOE = false
		if debug.getinfo(GetAsyncWebResult).what == "C" then GetAsyncWebResult("anony4bol.comoj.com", "AhriVer.lua", CheckVersionGOE) end
	end
	if needRun_GOE_2 and not needRun_GOE then
		needRun_GOE_2 = false
		if debug.getinfo(GetAsyncWebResult).what == "C" then
			
			GetAsyncWebResult("anony4bol.comoj.com", "vipusers.php?user="..encryptWWW(GetUser()), decGOE)
		end
	end
