...
local Error_calcs = 0
local file2 = SCRIPT_PATH .. "ANN_data.txt"
		...
		for enemy_networkID, visible_time in pairs(visibleDB) do
			...
			elseif dataDB[enemy_networkID] ~= nil and current_tick - visible_time >= 2100 and not dataDB[enemy_networkID]["has_learned"] and #dataDB[enemy_networkID]["waypoint_table"] >= 11 and GetTickCount() - dataDB[enemy_networkID]["last_stable_time"] >= 0 and #dataDB[enemy_networkID]["movement_table"] >= 22 then
				...
				for idx, value in ipairs(copy_table) do
					...
					copy_table[idx] = value/15000
				end

				table.insert(temp_db, normalize_value(x))
				table.insert(temp_db, normalize_value(y))
				table.insert(temp_db, normalize_value(dataDB[enemy_networkID]["waypoint_table"][6].x))
				table.insert(temp_db, normalize_value(dataDB[enemy_networkID]["waypoint_table"][6].y))
...
function CalculateError(vpred_table, ANN_table, real_table)
	...
	for i = 1, #vpred_table, 2 do
		...
		local ann_point = {x = denormalize(ANN_table[i]), y = denormalize(ANN_table[i+1])}
		local real_point = {x = denormalize(real_table[i]), y = denormalize(real_table[i+1])}
...
function OnDraw()
	...
	if Config.Draw then
		...
		if VPred_total_error ~= 0 and ANN_total_error ~= 0 and Error_calcs ~= 0 then
			local VPred_average_error = VPred_total_error / Error_calcs
			local ANN_average_error = ANN_total_error / Error_calcs
			DrawText3D("VPred error: " .. tostring(round(VPred_average_error, 2)) .. " ANN error: " .. tostring(round(ANN_average_error, 2)), myHero.x+250, myHero.y+250, myHero.z+250, 15,  ARGB(255,255,0,0), true )
