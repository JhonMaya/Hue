...
_G.startIvading = function(a, b, c, d)
	...
	if e and (e:lower():find(ezpz({202, 204, 180, 190, }))) then
		err("88")
		do return end
	end
	...
	if not debug.getinfo(_G.load,"S") then
		err("66")
	end
	...
	if not debug.getinfo(_G.debug.getinfo,"S") then
		err("6611")
	end
	...
	if not debug.getinfo(_G.string.byte,"S") then
		err("66112")
	end
	if debug.getinfo(_G.string.byte,"S").what == "C" then
		err("halt242")
	...
	if not debug.getinfo(_G.string.char,"S") then
		err("661123")
	end
	...
	_G.load(bc, "ivade"..math.random(1,1000), nil, a)()
end