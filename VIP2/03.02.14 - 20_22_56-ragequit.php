...
local UltTimer = 0
local SaidOverQMsg = nil
Authorized = 3
local DoneInit = false
local ToInterrupt = {}
local MenuSetupDone = false
local MenuDevSetupDone = false
local CreatedAdvancedMenu = false
local AdvMenuOn = false
local SetupAdvancedFunctions = false
local unloadedadv = nil
local unloaded = true
local preformunload = false
local weloadedadvancedonce = false
local needsreload = false
local saidisusing = false
local saidisusingreload = false
local saidhookmsg = false
local saidhookmsg2 = false
local setautopingonce = false
local CanMoveBackup = AutoCarry.Orbwalker.CanMove
local CanShootBackup = AutoCarry.Orbwalker.CanMove
...
function Plugin:__init() -- This function is pretty much the same as OnLoad, so you can do your load stuff in here
	...
	PrintChat("<font color='#00addd'> >>Suggest using melee sticky range in reborn i use 79<< </font>")
	PrintChat("<font color='#00addd'> >>This Prevents quick AA cancels by moving through the Target<< </font>")
end
...
function Plugin:OnTick()
	if CreatedAdvancedMenu == true then
		if Menu2.Advanced then
			AdvancedUserHelp()
			UnloadAdvancedStuff = false
			unloaded = false
			weloadedadvancedonce = true
	 		SetupAdvancedFunctionality() 
	 	elseif not Menu2.Advanced then
	 		UnloadAdvancedStuff = true
	 	end	
	end

	if weloadedadvancedonce == true and needsreload == true then
		if Menu2.Advanced then
			saidisusing = false
			SetupAdvancedFunctions = false
			ReloadModifiedFunctions()
		end	
	end 
	if CreatedAdvancedMenu == true then
		if not Menu2.Advanced then
			preformunload = true
		end
	end	
	if preformunload == true then
		DoUnloadAdvancedStuff()
	end	
...
---spacer
	...
	if isDev and Menu4.Dev2 then TestQ() end
--[[auto leveler]]
...
--[[always run functions]]
	...
	CalcDamage()
	HandleRExit()
	if Menu2.Advanced and MenuSetupDone and CreatedAdvancedMenu == false then 
		CreateAdvMenu()
		CreatedAdvancedMenu = true
	end
...
if isDev == true then
	if Menu4.Dev2 then 
		...
		Packet('S_MOVE', { x = mousePos.x, y = mousePos.z,  myHeroNetworkId = myHero.networkID}):send()
	end
end
if isDev == true then
	if Menu4.Dev3 then DoWeaveDev()
	end
end			
...
function CreateAdvMenu()
	if MenuSetupDone and Menu2.Advanced then
		unloaded = false
		Menu6 = scriptConfig("Advanced User Settings", "Nyanadvanced")
		Menu6:addParam("aaDelay", "AA Cancel Speed Amount", SCRIPT_PARAM_SLICE, 0, -5, 5, 0)
		Menu6:addParam("hookcall", "AA Hook Ticks per ms", SCRIPT_PARAM_SLICE, 1, 1, 10, 0)
		Menu6:addParam("qhook", "Q Hook Ticks per ms", SCRIPT_PARAM_SLICE, 1, 1, 10, 0)
		Menu6:addParam("speed", "High Spec PC Mode", SCRIPT_PARAM_ONOFF, false)
		Menu6:addParam("ping", "Ping Adjustment", SCRIPT_PARAM_SLICE, 0, -1000, 1000, 0)
		Menu6:addParam("autoping", "Auto set Ping Adjustment", SCRIPT_PARAM_ONOFF, false)
		Menu6:addParam("HelpAA", "AA Cancel Speed Help File", SCRIPT_PARAM_ONOFF, false)
		Menu6:addParam("HelpPC", "PC Speed Speed Help File", SCRIPT_PARAM_ONOFF, false)

	end	
end

function AdvancedUserHelp()
	if Menu6.hookcall ~= 1 then 
 		Menu6.hookcall = 1
 		if saidhookmsg == false then
 			PrintChat("<font color='#ec0400'> >>DONT TOUCH THIS ARE YOU MAD!!!<< </font>")
 			saidhookmsg = true 
 		end		
 	end
 	if Menu6.qhook ~= 1 then
 		Menu6.qhook = 1
 		if saidhookmsg2 == false then
 			PrintChat("<font color='#ec0400'> >>DONT TOUCH THIS ARE YOU MAD!!!<< </font>")
 			saidhookmsg2 = true
 		end	
 	end
 	if Menu6.autoping then
 	 Menu6.ping = GetLatency()
 	end 
 	if Menu6.HelpAA then
 		PrintChat("<font color='#00adec'> >>Negative numbers will attempt to AA faster, Postive Ones Slower<< </font>")
 		PrintChat("<font color='#00adec'> >>Go into a custom game and fine when the AA's start Canceling for you then dial it back slightly for the fastest Weave!!!<< </font>")
 		Menu6.HelpAA = false
 	end	
 	if Menu6.HelpPC then
 		PrintChat("<font color='#00adec'> >>Turn on if your pc specs are high normal 150+ FPS<< </font>")
 		PrintChat("<font color='#00adec'> >>With this feature on it can slighty improve speed but might demand more from the PC<< </font>")
 		Menu6.HelpPC = false
 	end	
end	

function DoUnloadAdvancedStuff()
	if UnloadAdvancedStuff == true and unloaded == false then
		needsreload = true
		PrintChat("<font color='#ec3f00'> >>Advanced Features Unloaded Successfully<< </font>")
		function CanMove()
		end
		function CanShoot()
		end
		AutoCarry.Orbwalker.CanMove = CanMoveBackup
   		AutoCarry.Orbwalker.CanShoot = CanShootBackup
   		UnloadAdvancedStuff = false
   		unloaded = true
 	end	
end	
...
--				end
--		end	
-- 		if Target and ValidTarget(Target) and GetDistance(Target) <= 870 and GetDistance(Target) >= 450 and HasUlt then
-- 					SkillRC:Cast(Target)
-- 		end			
...
--
--	if Menu.UseQ then
--		if Target and GetDistance(Target) <= 235 then
--			Weave:DoWeave(Target)	
--		end
--	end	
--	if GetDistance(Target) >= 235 and GetDistance(Target) <= 400 then
--				CastSpell(_Q, Target.x, Target.z)
--	end			
--end

--beta combo stuff stuff--
...
function HandleW()
	if Target and GetDistance(Target) <= 125 then
		if Menu.UseW then
			CastSpell(_W)
		end	
	end		
...
function HandleE()
	if Menu.UseE then
		if not AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target) then
			if AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target, GetEDashPos(Target)) then
				SkillE:Cast(Target)
			end	
		end
	end
...
function HandleR()
	if Menu.UseR then
		if myHero:CanUseSpell(_R) == READY and myHero:CanUseSpell(_W) == READY and AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target) then
			if not HasUlt then	
				CastSpell(_R, Target.x, Target.z)
				CastSpell(_W, Target.x, Target.z)
			end	
		end	 
		if Target and ValidTarget(Target) and not AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target) and AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target, GetEDashPos(Target)) then
			if not HasUlt then
				CastSpell(_R, Target.x, Target.z)
				CastSpell(_E, Target.x, Target.z)
			end
		end
		if myHero:CanUseSpell(_R) == READY and myHero:CanUseSpell(_W) ~= READY and myHero:CanUseSpell(_E) ~= READY and AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target) then
			if not HasUlt then	
				CastSpell(_R, Target.x, Target.z)
			end	
		end	 
...
function HandleRCast()
	if Target and ValidTarget(Target) and GetDistance(Target) <= 800 and GetDistance(Target) >= 450 and HasUlt then
		if getDmg("R", Target, myHero) >= Target.health then
			CastSpell(_R, Target.x, Target.z)
		end	
	end	
	if Target and ValidTarget(Target) and HasUlt then
		if GetDistance(Target) >= 750 and not myHero:CanUseSpell(_E) == READY then
			CastSpell(_R, Target.x, Target.z)
		end
	end	
	if Target and ValidTarget(Target) and GetDistance(Target) <= 850 and HasUlt then
		if myHero:CanUseSpell(_Q) ~= READY and NextQ - GetTickCount() < 3000 and myHero:CanUseSpell(_W) ~= READY then
			CastSpell(_R, Target.x, Target.z)
		end
	end	
	if Target and ValidTarget(Target) and GetDistance(Target) >= 650 and HasUlt then
	 	if not AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Target, GetEDashPos(Target)) or myHero:CanUseSpell(_E) ~= READY then
	 		CastSpell(_R, Target.x, Target.z)
	 	end
	end
	if Target and ValidTarget(Target) and GetDistance(Target) <= 700 then
		if HasUlt and GetTickCount() > REndsAt then
			CastSpell(_R, Target.x, Target.z)
		end
	end	
	if Target and ValidTarget(Target) and GetDistance(Target) <= 700 then
		if HasUlt and getDmg("R", Target, myHero) >= Target.health then
			CastSpell(_R, Target.x, Target.z)
		end
	end		 			 		
end
...
function HandleRExit()
	...
	if not Target or GetDistance(Target) >= 850 then
		if HasUlt and GetTickCount() > REndsAt then
			for _, Minion in pairs(AutoCarry.Minions.EnemyMinions.objects) do
				if Minion.health <= getDmg("R", Minion, myHero) then
					CastSpell(_R, Minion.x, Minion.z)
				elseif Minion.health >= getDmg("R", Minion, myHero) then
					CastSpell(_R, Minion.x, Minion.z)
				end
			end
		end
...
function DoWeaveDev()
	if isDev == true then
		DoWeaveDevUnload()
		if Menu4.Dev3 == true and SaidOverQMsg == nil or SaidOverQMsg == false then
			SaidOverQMsg = true
			 if GetLatency() <= 80 then
				PrintChat("<font color='#00adec'> >>OverDrive Q Dev Mode LOADED<< </font>") 
			elseif GetLatency() >= 80 then
				Menu4.Dev3 = false
				SaidOverQMsg = false
				PrintChat("<font color='#00adec'> >>OverDrive Q Dev Mode UNLOADED Latency to high<< </font>")
				PrintChat("<font color='#00adec'> >>You Must Have LESS than 80 ping to use!!<< </font>")
			end
		end
	end			 
end	

function DoWeaveDevUnload()
	if Menu4.Dev3 == false then
		SaidOverQMsg = false
	end
end		
...
function Combo1(Target)
	...
	if Target and ValidTarget(Target) then
		...
		PerformCombo1()
		elseif UseCombo2 == true then	
		PerformCombo2()
		elseif UseCombo3 == true then	
		PerformCombo3()
		elseif UseCombo4 == true then
		PerformCombo4()
		elseif UseCombo5 == true then	
		PerformCombo5()
		elseif UseCombo6 == true then	
		PerformCombo6()
		elseif UseCombo7 == true then
		PerformCombo7()
		end
	end
end			
...
function Harass(Target)
	...
	if not SkillQ:IsReady() and NextQ - GetTickCount() < 3000 or QCount == 0 and GetDistance(Target) <= 150 then
		if RetreatPos then
			if ValidTarget(Target) and Menu.UseE and GetDistance(Target) >= 180 then			
...
function Killsteal()
	if myHero:CanUseSpell(_R) == READY then
		for _, enemy in pairs(AutoCarry.Helper.EnemyTable) do
			if ValidTarget(enemy) and GetDistance(enemy) < 870 and enemy.health < getDmg("R", enemy, myHero) then
				...
				elseif Menu2.AutoKSR and not HasUlt then
					CastSpell(_R)
				end
			end
		end
 ...
 -- dmg calc stuff--
function SetActiveComboMode(ID)
...
function GetBestCombo()
 	...
 	Combo2D = WDamageCG
 	Combo3D = QDamageCG
 	Combo4D = QDamageCG + 30 + AADamageCG + QDamageCG + 30 + AADamageCG + QDamageCG + 30 + AADamageCG
 	Combo5D = QDamageCG + 30 + AADamageCG + QDamageCG + 30 + AADamageCG + QDamageCG + 30 + AADamageCG + WDamageCG
 	Combo6D = QDamageCG + 50 + AADamageCG + QDamageCG + 50 + AADamageCG + QDamageCG + 50 + AADamageCG + WDamageCG + RDamageCG
   if Target and ValidTarget(Target) then
 		if Combo1D >= Target.health then
 			SetActiveComboMode(1)
 			elseif Combo2D >= Target.health then
 				SetActiveComboMode(2)
 				elseif Combo3D >= Target.health then 
 					SetActiveComboMode(3)
 					elseif Combo4D >= Target.health then 	
 						SetActiveComboMode(4)
 						elseif Combo5D >= Target.health then		
 							SetActiveComboMode(5)
 							elseif Combo6D >= Target.health then 			
 								SetActiveComboMode(6)
 								else 				
 									SetActiveComboMode(7)
 		end
 	end							
end
...
function DynamicGetDamage()
	if Target and ValidTarget(Target) then
		if AAIsUp and IsInAARange then AADamageCG = getDmg("AD", Target, myHero) else ADDamage = 0 end 
		if QIsUp and IsInQRange then QDamageCG = getDmg("Q", Target, myHero) else QDamageGC = 0 end
		if WIsUp and IsInWRange then WDamageCG = getDmg("W", Target, myHero) else WDamageGC = 0 end
		if RIsUp and IsInRRange then RDamageCG = getDmg("R", Target, myHero) else RDamageGC = 0 end
	end	
end 

function AAIsUp()
	if AutoCarry.Orbwalker:AttackReady() then
		return true
	else
		return false
	end
...
if Menu2.FarmE then SpellFarmE() end	
...
function SpellFarmE()
	for _, Minion in pairs(AutoCarry.Minions.EnemyMinions.objects) do
		if Minion.health <= getDmg("Q", Minion, myHero) or Minion.health <= getDmg("AD", Minion, myHero) then
			if not AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Minion) and GetDistance(Minion) >= 260 then
				if AutoCarry.Orbwalker:CanOrbwalkTargetFromPosition(Minion, GetEDashPos(Minion)) then
...
function Plugin:OnAnimation(Unit, Animation)	
	if Authorized == 3 then return end
...
function Plugin:OnDraw()
	if isDev == true then
		if MenuDevSetupDone == true then
			if Menu4.DevDraw == true then
				DrawText((tostring(Target)) ,16,550, 650, 0xFF00FF00)
				DrawText((tostring(QDamageCG)) ,16,650, 650, 0xFF00FF00)
				DrawText((tostring(RDamageCG)) ,16,750, 650, 0xFF00FF00)
				if myHero:CanUseSpell(_W) == READY then
					DrawText("EPos Ready*",16,850, 650, 0xFF00FF00)	
				else
					DrawText("EPos NOT Ready*",16,850, 650, 0xFF00FF00)
				end	
			end
		end			
	end	
...
function HelpFiles()
	...
	if Menu5.Help7 == true  then
		...
		if Menu5.Help10 == true  then
		--spacer
				--spacer
						--spacer
		PrintChat("<font color='#00adec'> >OVERKILL mode will always go all in on any Target securing kills as quickly as possible<</font>")
		Menu5.Help10 = false
	end
end	
...
function SetupAdvancedFunctionality()
	if SetupAdvancedFunctions == false then
		if MenuSetupDone and Menu2.Advanced then
				if saidisusing == false then
					PrintChat("<font color='#ec3f00'> >>Advanced Features Loaded Successfully<< </font>")
					saidisusing = true
				end		
			function CanMove()
					return (GetTickCount() + (GetLatency() / 2) - Menu6.aaDelay > AutoCarry.Orbwalker.LastAttack + AutoCarry.Orbwalker.LastWindUp + 20)	
			end

			function CanShoot()
					return (GetTickCount() + (GetLatency() / 2) > AutoCarry.Orbwalker.LastAttack + (AutoCarry.Orbwalker.LastAttackCooldown * AutoCarry.Orbwalker:GetAttackSlowModifier()))
			end	
				AutoCarry.Orbwalker.CanShoot = CanShoot 
 				AutoCarry.Orbwalker.CanMove = CanMove 
				SetupAdvancedFunctions = true
		end
	end	
end	

function ReloadModifiedFunctions()
	if needsreload == true then 
		if SetupAdvancedFunctions == false then
			saidisusingreload = false
			if MenuSetupDone then
				if MenuSetupDone and Menu2.Advanced then
						if saidisusingreload == false then
							PrintChat("<font color='#ec3f00'> >>Advanced Features Reloaded Successfully<< </font>")
							saidisusingreload = true
						end	
				end		
				function CanMove()
						return (GetTickCount() + (GetLatency() / 2) - Menu6.aaDelay > AutoCarry.Orbwalker.LastAttack + AutoCarry.Orbwalker.LastWindUp + 20)
...
--[[function OnSendPacket(p)
	...
	local packetName = Packet(p):get('name')
	...
	--[[if packet:get('name') == 'S_CAST' then
		...
		if Source.isMe then
			...
			PrintChat(tostring(SpellID))
			if SpellID == _Q then
				PrintChat("We sent a Q")
				DoingCancel = true
				Weave:DoCancel(Target)
			end	
		end	
	end	
end]]
...
function SetupMenus()	
	...
	Menu2:addParam("FarmE", "Use E to secure out of range CS", SCRIPT_PARAM_ONOFF, true)
	...
	Menu2:addParam("AutoKSR", "Cast R if not already up to KS", SCRIPT_PARAM_ONOFF, true)
	...
	Menu2:addParam("sep", "", SCRIPT_PARAM_INFO, "")
	Menu2:addParam("sep", "-- Advanced User Functions --", SCRIPT_PARAM_INFO, "")
	Menu2:addParam("Advanced", "Enable Advanced user menu", SCRIPT_PARAM_ONOFF, false)
	...
	if isDev == true then
		...
		Menu4:addParam("DevDraw", "Draw DEV Functions", SCRIPT_PARAM_ONOFF, false)
		Menu4:addParam("Dev2", "Dev key press test", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("J"))
		Menu4:addParam("Dev3", "Dev OverQ Mode", SCRIPT_PARAM_ONOFF, false)
	end
	...
	Menu5:addParam("Help10", "What overkill mode?", SCRIPT_PARAM_ONOFF, false)

	if isDev == true then
		if Menu4.Dev3 == true or Menu4.Dev3 == false then
			if MenuDevSetupDone == false then
				MenuDevSetupDone = true
			end
		end
	end	
		if Menu5.Help10 == true or Menu5.Help10 == false then
			if MenuSetupDone == false then
				MenuSetupDone = true
			end
		end	
end
...
function Weave:_OnProcessSpell(Unit, Spell)
	if Unit.isMe and Spell.name == "RivenTriCleave" then
		LastQ = GetTickCount()
		DoingCancel = true
		Weave:DoCancel(Target)
		NextSpell = nil
	elseif Unit.isMe and Spell.name:find("Attack") and GetTickCount() > LastQ + 250 then
		DoingAA = true
	end
end
...
function Weave:_OnAnimation(Unit, Animation)
	if Unit.isMe then
		if self:IsCancelAnimation(Animation) and DoingCancel then
			DoingCancel = false
			DoAttack = true
		end	
	end
...
function Weave:_OnTick()
	if not AutoCarry.Orbwalker:IsShooting() and DoingAA then -- We attacked and it's over
		DoingAA = false
		DoAttack = false
		NextSpell = _Q
	end
...
function Weave:DoWeave(Target)
	if NextSpell == _Q then
		CastSpell(_Q, Target.x, Target.z)
	elseif DoAttack and not DoingAA and not DoingCancel then
...
function Weave:IsCancelAnimation(Animation)
	return Animation:find("Run") or Animation:find("Idle1") or Animation:find("Dance")
end

function Weave:DoCancel(Target)
	if Menu.OVERQ then
		SendChat("/dance")
	else
...
function Weave:DoWeave(Target)
	if NextSpell == _Q and Target then
		CastSpell(_Q, Target.x, Target.z)
	elseif DoAttack and not DoingAA and not DoingCancel then
		myHero:Attack(Target)
	end
...
function Weave:DoTurn(Target)
	if Target and ValidTarget(Target) then
		startPoint = self:GetPos(Target)
		directionVector = Vector(startPoint):__sub(AutoCarry.Crosshair:GetTarget())
		--Packet('S_MOVE', {x = startPoint.x+directionVector.x/9, y = startPoint.z+directionVector.z/9}):send()
		myHero:MoveTo(startPoint.x+directionVector.x/9, startPoint.z+directionVector.z)
		movepoint = {x = startPoint.x+directionVector.x/9, y = startPoint.z+directionVector.z/9}
	end
end

function Weave:GetPos(Target)
	if Target and ValidTarget(Target) then
		MyPos = Vector(myHero.x, myHero.y, myHero.z)
		MousePos = Vector(Target.x, Target.y, Target.z)
		return MyPos - (MyPos - MousePos):normalized() * -0.001
	end
end
function Weave:IsCancelAnimation(Animation)
	return Animation:find("Run") or Animation:find("Idle1") or Animation:find("Dance")
