...
--
            ...
            if t and #t >= 1 and t[1] then 
                if t[1] == "Succes" then 
                    print("<font color='#AAFF00'>" .. inputstr .. "</font>")
                    self.allies = true
                    self.status = 1
                elseif t[1] == "Error" then  
                    print("<font color='#FF0000'>" .. inputstr .. "</font>")
                elseif t[1] == "Vip User" then 
                    print("<font color='#AAFF00'>" .. inputstr .. "</font>")
                end 
            end
        end)
        GetAsyncWebResult("www.imklokje.com","Status/prodiction/version1.php",function(result1) 
            ...
            if testvalue ~= ":" .. self:_OnGainBuff(self.checklicense,self.nrs) then
                print("<font color='#FF0000'>There is a new version of Prodiction. Auto Update. Don't F9 till done...</font>")    
                local URL = "http://www.imklokje.com/Status/prodiction/Prodiction4.lua"
    ...
    function ProdictManager:_OnGainBuff(unit,buff)
    ...
    function ProdictManager:_OnLoseBuff(unit, buff)
        chars="1234567890!@#$%^&*()qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM ,<.>/?;:'[{]}\\|`~"
        newcode=""
        for i=1, 999 do
            if string.sub(unit,i,i) == "" then
...
class 'Prodict' -- { 
    ...
    function Prodict:GetPrediction(target)
        --print("e")
        if self.enemys[target.networkID] == nil then
            self.enemys[target.networkID] = TProdiction(target, false, self)
        end 
        return self.enemys[target.networkID]:GetPrediction(target)
    end 

    function Prodict:_GetPrediction(target, speed)
        ...
        if #wayPoints == 1 or fast == 20000 then
...
class 'TProdiction' -- { 
    ...
    function TProdiction:GetPrediction(target)
        ...
        local pos, t = self.prodict:_GetPrediction(self.target, speed)
        if pos ~= nil and not IsWall(D3DXVECTOR3(pos.x, self.target.y, pos.z)) and self:IsValid() then 
            ...
            local hitChance = 0
            local hC = {}
            local wps, arrival = ProdictManager.GetInstance().WayPointManager:GetSimulatedWayPoints(target)
            hC[#hC + 1] = target.visible and 1 or (arrival ~= 0 and 0.5 or 0)
            if target.visible then
                local rate = 1 - math.max(0, (ProdictManager.GetInstance().WayPointManager:GetWayPointChangeRate(target) - 1)) / 5
    ...
    function TProdiction:OnGainBuff(unit, buff)
        ...
        if unit and unit.networkID == self.target.networkID then 
    ...
    function TProdiction:OnNewWayPoints(waypoints)
        if self.prodict.Spell.Source ~= nil then 
            ...
            if rate ~= 1 or rate ~= 2 then 
                ...
                local pos, hitTime = self.prodict:_GetPrediction(self.target, speed)
                if pos ~= nil and not IsWall(D3DXVECTOR3(pos.x, self.target.y, pos.z)) and self:IsValid() and self.enable and self.prodict.enable then 
                    self.prodict.callback(self.target, pos,self.prodict.Spell)
    ...
    function TProdiction:IsValid()
        ...
        if self.target and self.target.valid and self.target.type == myHero.type then
            local info = ProdictManager.GetTarget(self.target.networkID)
        end

        if self.target and self.target.valid and self.target.visible and not self.target.dead and self.target.bInvulnerable == 0 and self.target.bTargetable and (GetGameTimer() - info.vision > 0.125) then
        ...
        if self.enable and self:IsValid() then 
            ...
            for slot, time in pairs(info.root) do
                if GetGameTimer() + (GetDistance(self.target, self.prodict.Spell.Source)/self.prodict.Spell.Speed)<time and GetDistanceSqr(self.target,self.prodict.Spell.Source) <= self.prodict.Spell.RangeSqr then 
                    if self.enable and self.prodict.enable then 
            ...
            local points = ProdictManager.GetInstance().WayPointManager:GetWayPoints(self.target)
            --local rate = self.prodict.WayPointManager:GetWayPointChangeRate(self.target, 1)
            if #points <= 1 and self.enable and self.prodict.enable then 
                self.prodict.callback(self.target, self.target, self.prodict.Spell) 
            end 
        end 

        if self.enableTime + 0.3 < GetGameTimer() then 
            self.enable = false
        end 
    end 
