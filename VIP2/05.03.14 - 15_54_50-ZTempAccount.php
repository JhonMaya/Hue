...
local cVersion = "0.5"
local cAutoupdate = true
local cURL1 = "raw.github.com"	
local cURL2 = "/Andyi19/Github/master/Ziggs.lua"
local CURL3 = "https://"..cURL1 ..cURL2
local cPath = BOL_PATH.."Scripts\\0xZiggs.lua"

local Variable
if cAutoupdate then GetAsyncWebResult(cURL1,cURL2, function(K)Variable=K end)

function Update()
	if Variable ~= nil then
		local cGetVersion
		local Shit1, Shit2, Shit3 = nil, string.find(Variable, "local version = \"")
		if Shit3 then
			Shit1, Shit2 = string.find(Variable, "\"", Shit1+1)
		end

		if Shit1 then 
			cGetVersion=string.sub(Variable, Shit3+1, Shit1-1)
		end

		if cGetVersion ~= nil and tonumber(cGetVersion) ~= nil and tonumber(cGetVersion) > tonumber(cVersion) then
			DownloadFile(cURL3.."?nocache".. myHero.charName..os.clock(), cPath, function() print("<font color=\"#FF0000\"><b>Ziggs Combo:</b> successfully updated. ("..cVersion.." => "..cGetVersion..")</font>")end)
			elseif cGetVersion then
				print("<font color=\"#FF0000\"><b>Ziggs Combo:</b> You have got the latest version: <u><b>"..cGetVersion.."</b></u></font>")
			end
			Variable = nil
		end
	end
