...
-- Script Name: Morgana - The Fallen Angel 
-- Script Ver.: 1.2
...
_G.MorganaUpdate = true -- change to false to turn off auto updater

if myHero.charName ~= "Morgana" then return end

local MorganaVersion = 1.2
local UpdateHost  = "raw.github.com"
local VersionLink = "/UglyOldGuy/NintendoBoL/master/Support%20Bundle/Versions/Morgana.ver"
local ScriptLink  = "/UglyOldGuy/NintendoBoL/master/Support%20Bundle/Morgana%20-%20The%20Fallen%20Angel.lua"
...
if MorganaUpdate then
  local UpdateData = GetWebResult(UpdateHost, VersionLink)
  if UpdateData then
    if tonumber(UpdateData) > tonumber(MorganaVersion) then
      ...
      DownloadFile(DownLoad, ScriptPath, function () DownloadingFile = false PrintChat("<font color='#99CC33'> Updated "..tonumber(MorganaVersion).." > "..tonumber(UpdateData).." Sucessfully please double F9</font>") end)
    else
...
function OnLoad()
  ...
  Variables()
  PrintChat("<font color='#990099'> >> Morgana - The Fallen Angel Version: "..MorganaVersion.."<<</font>")
end
...
function MorganaMenu()
    ...
    MorganaMenu.drawing:addParam("target", "Draw Selected Target", SCRIPT_PARAM_ONOFF, true) -- Done
...
function OnTick()
	if not isuserauthed or Recalling then return end
	Checks()
	AutoSkills()
...
function AutoSkills()
	if MorganaMenu.skills.q.autoQ then
		for i, enemy in pairs(GetEnemyHeroes()) do
			if not MorganaMenu.skills.q[enemy.hash] and not enemy.dead and enemy.visible and GetDistance(enemy) <= MorganaMenu.skills.q.maxRange then 
        CastSkill(_Q, enemy)
      end
		end
	end
	...
	if MorganaMenu.skills.w.autoW then
		for i, enemy in pairs(GetEnemyHeroes()) do
			if not enemy.canMove and not enemy.dead and enemy.visible then
				CastSkill(_W, enemy)
			end
		end
	end
...
function CastSkill(skill, target)
	...
	if skill == _Q then
        ...
        local CollisionQ =  Collision(Spells.Q.range, Spells.Q.speed, Spells.Q.delay, Spells.Q.width)
				if Spells.Q.pos and Spells.Q.pos ~= nil and not CollisionQ:GetMinionCollision(myHero, Spells.Q.pos)  then
...
function OnWndMsg(Msg, Key)
  if Msg == WM_LBUTTONDOWN then
    ...
    if starget and minD < 500 then
      if Target and starget.charName == Target.charName then
        Target = nil
      else
        Target = starget
        print("<font color=\"#FF0000\">"..myHero.charName..": New target selected: "..starget.charName.."</font>")
      end
    end
  end
end

function OnRecall(player)
  if player.isMe then
    Recalling = true
  end
end

function OnAbortRecall(player)
  if player.isMe then
    Recalling = false
  end
end

function OnFinishRecall(player)
  if player.isMe then
    Recalling = false
  end
end
...
function OnDraw()
    ...
    if Target ~= nil and MorganaMenu.drawing.target then
