<?php exit() ?>--by Jus 187.10.183.71
if myHero.charName ~= "Riven" or not VIP_USER then return end

local version = "2.44"

local function _AutoupdaterMsg(msg) 
    print("<font color=\"#6699ff\"><b>Riven, I'm not a Bunny:</b></font> <font color=\"#FFFFFF\">"..msg..".</font>") 
end
...
local function MenuExtraCombo()
    ...
    menu.combo.extracombo:addParam("manager", "Try Speed up Q-AA", SCRIPT_PARAM_ONOFF, false)   
...
local function MenuSystem()
    ...
    menu.system:addParam("", "[Others Settings]", SCRIPT_PARAM_INFO, "")
    menu.system:addParam("update", "Auto Update", SCRIPT_PARAM_ONOFF, true)
    AUTOUPDATE = menu.system.update
 end

local function PermaMenu()
...
local function DelayCalc()
	...
	if mode == 2 then
		local Total = (1/myPlayer.attackSpeed*1000) - GetLatency()/2 --ms
...
local function CastQ(myTarget)   
    ...
    local mode_     =   menu.combo.extracombo.mode
    local delayType =   nil  
    if useq then
        local del   =   DelayCalc()
        ...
        if GetDistance(myTarget) <= 500 and GetTickCount() + GetLatency() / 2 - qTick > del then -- (GetTickCount() + GetLatency() / 2) - qTick > del then                      
            if mode_ == 1 then delayType = ( (del/1000) - 0.05 ) else  delayType = ( (lastWindUpTime/1000) - ((GetLatency())/2000) ) end
        ...
        if GetDistance(myTarget) <= 500 and ThisIsReal(myTarget) > myPlayer.range and GetTickCount() + GetLatency() / 2 - qTick > 1000 then
            qTick = GetTickCount() + GetLatency() / 2
            CastSpell(_Q, myTarget.x, myTarget.z)
        end 
    end
...
local function UltimateKS()
    ...
    for i, Enemys in pairs(GetEnemyHeroes()) do
        if ValidTarget(Enemys) and GetDistance(Enemys) < 900 and Enemys.type == "obj_AI_Hero" and Enemys.type ~= "obj_AI_Turret" and Enemys.type ~= "obj_AI_Minion" and not TargetHaveBuff("UndyingRage", Enemys) and not TargetHaveBuff("JudicatorIntervention", Enemys) then 
        local rDmg = getDmg("R", Enemys, myPlayer,2)           
            if Enemys.health <= rDmg then
                CastSpell(_R)
                rTick = GetTickCount() + GetLatency() / 2
                if temUltimate and Enemys.health <= rDmg or GetTickCount() + GetLatency() / 2 > rTick + 10000 and GetDistance(Enemys) <= 900 then
...
function CastR(myTarget)   
    ...
    if user then        
        ...
        if ValidTarget(myTarget) and GetDistance(myTarget) <= rRange and enemyHea <= HealthE and nEnemys >= rEnemys and not temUltimate then
            rTick = GetTickCount() + GetLatency() / 2 
            CastSpell(_R)
        end
        if temUltimate and ValidTarget(myTarget) and  myTarget.health <= DamageR or GetTickCount() + GetLatency() / 2 > rTick + 10000 and GetDistance(myTarget) <= 900 then
            CastSpell(_R, myTarget.x, myTarget.z)
        end
    end
end
...
function OnAnimation(unit, animationname)
    ...
    local eForce		=	menu.combo.extracombo.e 
    local mode_         =   menu.combo.extracombo.mode
    local speed_        =   menu.combo.extracombo.manager
    local delayType     =   nil    
...
function JumpQ()
    ...
    if qCount == 0 then   
       ...
       qTick = GetTickCount() + GetLatency()/2
    elseif (From and To and CastPos ~= nil) and GetDistance(myPlayer, CastPos) <= 80 then
...
local function F5Target()
    ...
    if ValidTarget(selectedTarget) and selectedTarget.type == "obj_AI_Hero" and selectedTarget.type ~= "obj_AI_Turret" and selectedTarget.type ~= "obj_AI_Minion" and not TargetHaveBuff("UndyingRage", selectedTarget) and not TargetHaveBuff("JudicatorIntervention", selectedTarget) then
    ...
    if not selectedTarget then
        ...
        for i, Enemy in pairs(inimigos) do             	  
            ...
            if Enemy ~= nil and ValidTarget(Enemy) and GetDistance(Enemy) <= range_ and Enemy.type == "obj_AI_Hero" and Enemy.type ~= "obj_AI_Turret" and Enemy.type ~= "obj_AI_Minion" and not TargetHaveBuff("UndyingRage", Enemy) and not TargetHaveBuff("JudicatorIntervention", Enemy) then
    ...
    if not selectedTarget and not found then
    	...
    	for i, Enemy in pairs(inimigos) do
	    	...
	    	if Enemy ~= nil and ValidTarget(Enemy) and distancMouse <= 150 and Enemy.type == "obj_AI_Hero" and Enemy.type ~= "obj_AI_Turret" and Enemy.type ~= "obj_AI_Minion" and not TargetHaveBuff("UndyingRage", Enemy) and not TargetHaveBuff("JudicatorIntervention", Enemy) then
	    		return Enemy
	    	end
	    end
	end	
	finalTarget = selectedTarget or Enemy
    if ValidTarget(finalTarget) and finalTarget.type == "obj_AI_Hero" and finalTarget.type ~= "obj_AI_Turret" and finalTarget.type ~= "obj_AI_Minion" and not TargetHaveBuff("UndyingRage", finalTarget) and not TargetHaveBuff("JudicatorIntervention", finalTarget) then
	return finalTarget
    end
end
...
function OnTick()
    ...
    Target = F5Target()
    if ValidTarget(Target) and Target.type == "obj_AI_Hero" and Target.type ~= "obj_AI_Turret" and Target.type ~= "obj_AI_Minion" and not TargetHaveBuff("UndyingRage", Target) and not TargetHaveBuff("JudicatorIntervention", Target) then
    	RealTarget = Target
    end
...
function OnDraw()
    ...
    if targettext_ and ValidTarget(Target) and KillText ~= nil and Target.type == "obj_AI_Hero" and Target.type ~= "obj_AI_Turret" and not TargetHaveBuff("UndyingRage", Target) and not TargetHaveBuff("JudicatorIntervention", Target) then
    ...
    if spots_ then
        ...
        if from_ and To ~= nil then
            DrawCircle2(To.x, To.y, To.z, 120, ARGB(255, 255, 255, 000))            
        end
