...
class 'Caster' -- {
    ...
    function Caster:__init(spell, range, spellType, speed, delay, width, useFastCollision)
		...
		self.cast_function = nil 

		self.secondary_cast_method = nil 
		self.secondary_cast_condition = nil
    ...
    function Caster:Cast(target)
		...
		if self.cast_function ~= nil then 
			self.cast_function(target)
			return true 
		end 
	...
	function Caster:UpdateCast(func)
		self.cast_function = func 
	end 

	function Caster:GetCast()
		return self.cast_function
	end

	function Caster:RegisterSecondaryCast(cast, condition) 
		self.secondary_cast_method = cast 
		self.secondary_cast_condition = condition 
		AddTickCallback(function() self:OnTick_SecondaryCast() end)
	end 

	function Caster:OnTick_SecondaryCast() 
		Target = iTarget.Instance():GetTarget() 
		if Target then 
    		if self.secondary_cast_condition ~= nil and self.secondary_cast_condition(Target) then 
    			self.secondary_cast_method(Target) 
    		end 
    	end
	end 
...
class 'CustomizableCombo' -- {
	...
	function CustomizableCombo:GetCasters()
		return self.casters
	end 

	function CustomizableCombo:SetCaster(raw_cast_variable, new_caster)
		for i=1, #self.casters do 
			local caster = self.casters[i]
			if caster.spell == raw_cast_variable then 
				self.casters[i] = new_caster 
				return true
			end 
		end 
		return false 
...
class 'AutoFunctions' -- {
	...
	function AutoFunctions:__init(...)
		...
		self.enable_autoshield = false
		self.jungleMinions = {}
		self.heal_conditions = {}
		self.heal_percentages = {}
		self.auto_shield = nil 
			...
			if caster:Ready() and Menu.Get(SpellStrings[caster.spell]).killsteal then 
				for _, enemy in pairs(GetEnemyHeroes()) do 
					if ValidTarget(enemy, caster.range) then 
						if getDmg(SpellStrings[caster.spell], enemy, myHero) > enemy.health and caster:Condition(enemy) then 
							return caster, enemy 
						end 
					end 
				end 
	...
	function AutoFunctions:OnTick_LastHit() 
		...
		if minion and caster:Ready() and caster:Condition(minion) and Menu.Get("AutoFunctions").LastHit then 
			caster:Cast(minion)
		end 
	end 
	...
	function AutoFunctions:GetCaster_BuffSteal(Target) 
		...
		for _, c in ipairs(temp) do 
			if c:Ready() and c:Condition(Target) and GetDistance(Target) < c.range and c:GetDamage(Target) > Target.health then 
				return c 
			end 
		end 
		return nil 
	...
	function AutoFunctions:OnTick_Heal() 
		...
		if caster then 
			if ally_use then 
				local caster_type = caster.spellType 
				if caster_type == SPELL_TARGETED or caster_type == SPELL_LINEAR or caster_type == SPELL_LINEAR_COL or caster_type == SPELL_CIRCLE then 
					local target = self:GetTarget_Heal(caster) 
					if target then 
						return caster:Cast(target) 
					end 
				elseif caster_type == SPELL_TARGETED_FRIENDLY then 
	...
	function AutoFunctions:GetTarget_Heal(caster) 
		for i, enemy in pairs(GetEnemyHeroes()) do 
			if enemy and not enemy.dead and ValidTarget(caster.range, enemy) then 
				return enemy 
			end 
		end 
		for i, minion in pairs(self.enemyMinions.objects) do 
			if minion and not minion.dead and GetDistance(minion) < caster.range then
				 return minion 
			end
		end 
	end 
	...
	function AutoFunctions:OnTick_AutoShield() 
		if not self.enable_autoshield then return end
	...
	function AutoFunctions:GetCasters()
		return self.casters
	end 

	function AutoFunctions:SetCaster(raw_cast_variable, new_caster)
		for i=1, #self.casters do 
			local caster = self.casters[i]
			if caster.spell == raw_cast_variable then 
				self.casters[i] = new_caster 
	...
	function AutoFunctions:Enable_AutoShield(recommended_caster) 
		local recommended = recommended_caster or self.casters[1]
		self.auto_shield = AutoShield() 
		self.auto_shield.caster = recommended
		self.enable_autoshield = true 

		for i, v in pairs(self.casters) do 
			Menu.Get(SpellStrings[v.spell]):addParam("autofunc1", "-- Auto-Functions: AutoShield --", SCRIPT_PARAM_INFO, "")
			Menu.Get(SpellStrings[v.spell]):addParam("autoshield", "Use for auto shield", SCRIPT_PARAM_ONOFF, false)
		end 
	end 
...
class 'Calculation' -- {
   ...
   function Calculation.DrawCircleNextLvl(x, y, z, radius, width, color, chordlength)
	    radius = radius or 300
	  	quality = math.max(8,Calculation.round(180/math.deg((math.asin((chordlength/(2*radius)))))))
	...
	function Calculation.round(num) 
		if num >= 0 then return math.floor(num+.5) else return math.ceil(num-.5) end
	end

	function Calculation.DrawCircle2(x, y, z, radius, color)
	    local vPos1 = Vector(x, y, z)
	    local vPos2 = Vector(cameraPos.x, cameraPos.y, cameraPos.z)
	    local tPos = vPos1 - (vPos1 - vPos2):normalized() * radius
	    local sPos = WorldToScreen(D3DXVECTOR3(tPos.x, tPos.y, tPos.z))
	    if OnScreen({ x = sPos.x, y = sPos.y }, { x = sPos.x, y = sPos.y }) then
	       Calculation.DrawCircleNextLvl(x, y, z, radius, 1, color, 75) 
	    end
	end
...
class 'iDraw' -- {
	...
	_G.iDraw_HPBar_DisplayMode_Text = {"Individual", "Enemy"}

	function iDraw:__init() 
		Menu.Get():addSubMenu("Drawing", "Drawing")
		Menu.Get("Drawing"):addParam("Disable", "Disable Drawing", SCRIPT_PARAM_ONOFF, false)

		Menu.Get("Drawing"):addSubMenu("HP Bars", "HPBar")
		Menu.Get("Drawing").HPBar:addParam("Draw", "Enable", SCRIPT_PARAM_ONOFF, false)
		Menu.Get("Drawing").HPBar:addParam("Mode", "Draw Mode", SCRIPT_PARAM_SLICE, 1, 0, 2, 0)
		Menu.Get("Drawing").HPBar:addParam("DisplayMode", "", SCRIPT_PARAM_INFO, "")

		Menu.Get("Drawing"):addSubMenu("Float Text", "FloatText") 
		Menu.Get("Drawing").FloatText:addParam("Draw", "Enable", SCRIPT_PARAM_ONOFF, false)
		Menu.Get("Drawing").FloatText:addParam("TTK", "Enable Time To Kill (seconds)", SCRIPT_PARAM_ONOFF, false)

		Menu.Get("Drawing"):addSubMenu("Range Circles", "RangeCircles")
		Menu.Get("Drawing").RangeCircles:addParam("Draw", "Enable", SCRIPT_PARAM_ONOFF, false)
		Menu.Get("Drawing").RangeCircles:addParam("DrawIndividual", "Draw Individual spells", SCRIPT_PARAM_ONOFF, false)
		Menu.Get("Drawing").RangeCircles:addParam("NoLagCircles", "Enable no lag circles (requires reload)", SCRIPT_PARAM_ONOFF, false)

		self.spell_table = {"Q", "W", "E", "R"}

		for i, spell in ipairs(self.spell_table) do 
			Menu.Get("Drawing").RangeCircles:addParam("sep"..spell, "", SCRIPT_PARAM_INFO, "")
			Menu.Get("Drawing").RangeCircles:addParam("Draw"..spell, "Draw "..spell, SCRIPT_PARAM_ONOFF, false)
			Menu.Get("Drawing").RangeCircles:addParam("ColorReady"..spell, "Ready Color", SCRIPT_PARAM_COLOR, {40, 71,149,38})
			Menu.Get("Drawing").RangeCircles:addParam("ColorNotReady"..spell, "Not Ready Color", SCRIPT_PARAM_COLOR, {40, 177, 0, 0})
		end
	...
	function iDraw:OnTick() 
		self:Update_Menu_HPBar() 
		self:Update_Circles()
	end 

	function iDraw:OnDraw() 
		if Menu.Get("Drawing").Disable then return end 

		if Menu.Get("Drawing").HPBar.Draw then 
			self:DrawEnemies_HPBar()
		end 

		if Menu.Get("Drawing").FloatText.Draw then 
			self:DrawEnemies_FloatText() 
		end 

		if Menu.Get("Drawing").RangeCircles.Draw then 
			self:DrawCircles() 
		end
	end 

	function iDraw:Update_Circles() 
		if Menu.Get("Drawing").RangeCircles.NoLagCircles then 
			self.old = _G.DrawCircle
			_G.DrawCircle = Calculation.DrawCircle2
		end 
	end 

	function iDraw:DrawCircles() 
		if Menu.Get("Drawing").RangeCircles.DrawIndividual then 
			for i, spell in ipairs(self.spell_table) do 
				if Menu.Get("Drawing").RangeCircles["Draw" .. spell] then 
					local range = myHero:GetSpellData(SpellString[spell]).range
					local color = _ColorARGB.FromTable(Menu.Get("Drawing").RangeCircles["ColorNotReady" .. spell])
					if myHero:CanUseSpell(SpellString[spell]) == READY then 
						color = _ColorARGB.FromTable(Menu.Get("Drawing").RangeCircles["ColorReady" .. spell])
					end 
					DrawCircle(myHero.x, myHero.y, myHero.z, range, color)
				end 
			end 
		else 
			local longest_range = 0 
			local color = _ColorARGB.FromTable({40, 177, 0, 0})
			for i, spell in ipairs(self.spell_table) do 
				local range = myHero:GetSpellData(SpellString[spell]).range
				if range > longest_range then 
					longest_range = range
					local color = _ColorARGB.FromTable(Menu.Get("Drawing").RangeCircles["ColorNotReady" .. spell])
					if myHero:CanUseSpell(SpellString[spell]) == READY then 
						color = _ColorARGB.FromTable(Menu.Get("Drawing").RangeCircles["ColorReady" .. spell])
			...
			DrawCircle(myHero.x, myHero.y, myHero.z, longest_range, color)
		end 
	end 

	function iDraw:DrawEnemies_FloatText() 
		for k, v in pairs(GetEnemyHeroes()) do 
			if v then
				self:DrawEnemy_FloatText(v)
			end 
		end 
	end 

	function iDraw:DrawEnemy_FloatText(enemy)
		if enemy and enemy.visible and not enemy.dead and GetDistance(enemy, cameraPos) < 3000 then  
			local Text = ""
			local KillText = self:GetData_Enemy(enemy)

			Text = KillText

			local dps = myHero:CalcDamage(enemy, myHero.damage) * myHero.attackSpeed
			local ttk = (enemy.health - DamageCalculation.CalculateRealDamage(enemy)) / dps 
			local TimeToKill = string.format("%4.1f", ttk)

			if Menu.Get("Drawing").FloatText.TTK then 
				Text = KillText .. " (" .. TimeToKill .. " /s Time to Kill)"
			end 

			PrintFloatText(enemy, 0, Text)
		end  
	end 

	function iDraw:Update_Menu_HPBar()
		local Mode = Menu.Get("Drawing").HPBar.Mode 
		Menu.Get("Drawing").HPBar.DisplayMode = iDraw_HPBar_DisplayMode_Text[Mode] 
	end 

	function iDraw:DrawEnemies_HPBar()
		for k, v in pairs(GetEnemyHeroes()) do 
			if v then 
				self:DrawEnemy_HPBar(v)
			end 
		end 
	end 

	function iDraw:DrawEnemy_HPBar(enemy)
		if enemy and enemy.visible and not enemy.dead and GetDistance(enemy, cameraPos) < 3000 then 
			...
			if Mode == 2 then -- Combo Draw
				local Text, IndicatorPos = self:GetData_Enemy(enemy)
			...
			elseif Mode == 1 then -- Individual spells
				local spell_table = self:GetData_Spells(enemy)
				local height_modifier = 0 
				local indicator_table = {}
				for i, spell in pairs(spell_table) do 
	...
	function iDraw:GetData_Spells(enemy) 
		local damage_table = DamageCalculation.GetSpellData(enemy)
		local result = {}
		for i, spell in pairs(damage_table) do 
			if spell.ready then 
				local health_left = math.round(enemy.health - spell.damage)
				local percent_left = math.round(health_left / enemy.maxHealth * 100)
				local percent_bar = percent_left / 103 * 100
				result[i] = {damage_percent = percent_left, IndicatorPos = percent_bar}
			end 
		end 
		return result 
	end 

	function iDraw:GetData_Enemy(enemy) 
		local real_damage = DamageCalculation.CalculateRealDamage(enemy)
		local burst_damage = DamageCalculation.CalculateBurstDamage(enemy)
		if real_damage > enemy.health then 
			return "KILLABLE!", 0
		elseif enemy.health > real_damage and enemy.health < burst_damage then 
			return "Full Combo", 0
		else 
			local health_left = math.round(enemy.health - real_damage)
			local percent_left = math.round(health_left / enemy.maxHealth * 100)
			local percent_bar = percent_left / 103 * 100 
			return (percent_left .. "% Possible"), percent_bar
		end  
	end 
...
class "DamageCalculation" -- {
    ...
    function DamageCalculation:_GetSpellData(Target) 
    	local damage_table = {}
    	for i, spell in pairs(self.spells) do 
    		damage_table[spell.name] = {damage = self:_GetDamage(spell.name, Target), ready = (myHero:CanUseSpell(spell.spell) == READY), mana = myHero:GetSpellData(spell.spell).mana}
    	end
    	return damage_table
    end 

    function DamageCalculation.GetSpellData(Target)
    	return DamageCalculation.Instance():_GetSpellData(Target) 
    end 

    function DamageCalculation:_GetSpells_Killable(Target)
    	if self:CalculateBurstDamage(Target) < Target.health then return nil end

    	local killable_spell = self:_GetSpell_Killable(Target)
    	if killable_spell ~= nil then 
    		return killable_spell.name
    	end 
    	...
    	-- Check for 2 combo 
    	for i, spell in pairs(self.spells) do 
    		for _i, _spell in pairs(self.spells) do 
    			if spell ~= _spell then 
    				if damage_table[spell.name].damage + damage_table[_spell.name].damage > Target.health then 
    					return {spell.name, _spell.name}
    				end 	
    			end 
    		end 
    	end  
    	...
    	for i, spell in pairs(self.spells) do 
    		for _i, _spell in pairs(self.spells) do 
    			if spell ~= _spell then 
    				...
    				for __i, __spell in pairs(self.spells) do 
		    			if spell ~= __spell and _spell ~= __spell then 
		    				if damage_table[spell.name].damage + damage_table[_spell.name].damage + damage_table[__spell.name].damage > Target.health then 
		    					return {spell.name, _spell.name, __spell.name}
		    				end 	
		    			end 
		    		end 
    			end 
    ...
    function DamageCalculation:_GetSpell_Killable(Target)
    	for i, spell in pairs(self.spells) do 
    		if self:_GetDamage(spell.name, Target) > Target.health then 
    			return spell 
    		end 
    	end 
    	return nil 
...
class 'iTarget' -- {
	...
	function iTarget.Instance()
		if iTarget.instance == "" then iTarget.instance = iTarget() end return iTarget.instance
	end 

	function iTarget:__init() 
		self.ts = TargetSelector(TARGET_PRIORITY, 2000, DAMAGE_PHYSICAL, true)
	end 

	function iTarget:GetTarget() 
		if _G.MMA_Target ~= nil and _G.MMA_Target.type:lower() == 'obj_ai_hero' then return _G.MMA_Target end
		if _G.AutoCarry and _G.AutoCarry.Crosshair and _G.AutoCarry.Attack_Crosshair ~= nil and _G.AutoCarry.Attack_Crosshair.target ~= nil and _G.AutoCarry.Attack_Crosshair.target.type:lower() == 'obj_ai_hero' then return _G.AutoCarry.Attack_Crosshair.target end
		self.ts:update()
