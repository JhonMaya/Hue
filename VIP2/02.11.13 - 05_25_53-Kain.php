<?php exit() ?>--by Kain 173.245.48.215
...
function Vars()
	...
	if VIP_USER then
		if FileExist(SCRIPT_PATH..'Common/Prodiction.lua') then
			...
			ProdictQ = SetupPROdiction(_Q, 1520, 1300, 0.125, 60, myHero, FireQ)
			ProdictQCollision = Collision(1520, 1300, 0.9, 60)

			ProdictW = SetupPROdiction(_W, 900, math.huge, 0.9, 80, myHero, FireW)

			PrintChat("<font color='#CCCCCC'> >> Nidalee - PROdiction 2.0 <</font>")
		end
	else
		PrintChat("<font color='#CCCCCC'> >> Nidalee - Free Prediction <</font>")
	end
end
...
function PluginOnTick()
	...
	if AutoCarry.Keys.AutoCarry and (Target ~= nil or LastTarget ~= nil) then
		SwitchFormIfNeeded()
	end

	if AutoCarry.Keys.AutoCarry and Target ~= nil then
		if currentForm then
	...
	if (AutoCarry.Keys.MixedMode or AutoCarry.Keys.LaneClear) and currentForm and Target ~= nil then
		-- if Menu.mixedmode.CastW and CheckMana() then CastW(Target) end
		if CheckMana() then CastQ(Target) end
	end

	if AutoCarry.Keys.LaneClear and not currentForm then
		if Menu.laneclear.CastE then
			SmartFarm()
			FarmMinion()
		end
		
		local JungleTarget = AutoCarry.Jungle:GetAttackableMonster()
...
function KillstealJavelin(switchNeeded, mainTarget)
	if IsSpellReady("Q") then
		if mainTarget and mainTarget.valid and not mainTarget.dead and GetDistance(mainTarget) <= 1520 then
		...
		for _, enemy in pairs(GetEnemyHeroes()) do
			if enemy and enemy.valid and not enemy.dead and GetDistance(enemy) < QRange then
				...
				if (enemy.health <= qDmg or (not QReady and not WReady and not EReady)) and GetDistance(enemy) > PounceMaxRange and GetDistance(enemy) <= 1520 then
...
function FireQ(unit, pos, spell)
	local unitDirection = GetDirection(unit)
	local unitDistance = GetDistance(pos) - (getHitBoxRadius(unit)/2)
	local maxQRange = 1520
	local awayQDiff = .95

	if ((unitDirection ~= DIRECTION_AWAY and unitDistance < maxQRange) or (unitDirection == DIRECTION_AWAY and unitDistance < (maxQRange * awayQDiff))) and
		myHero:GetSpellData(_Q).name == "JavelinToss" and not ProdictQCollision:GetMinionCollision(pos, myHero) then
		CastSpell(_Q, pos.x, pos.z)
	end
end

function FireW(unit, pos, spell)
...
function PluginOnDraw()
	...
	if Target and not Target.dead and (AutoCarry.Keys.AutoCarry or AutoCarry.Keys.MixedMode) then
	...
	if QReady and ValidTarget(unit) then
		if currentForm then
			if GetDistance(unit) <= QRange then
				ProdictQ:EnableTarget(unit, true)
			end
		else
			if GetDistance(unit) <= QRange then
				CastSpell(_Q, unit)
	...
	if WReady and ValidTarget(unit) then
		if currentForm then
			if GetDistance(unit) <= WRange then
				ProdictW:EnableTarget(unit, true)
			end
		else
...
function Heal()
	if CheckMana() then
		if not currentForm and Menu.healing.ForceHeal and not AutoCarry.Keys.AutoCarry and CheckHealth(myHero) and RReady and IsSpellReady("E") then
			CastSpell(_R)
...
--UPDATEURL=
--HASH=B521C1E1338BE67373A2FACF48AD2147
