...
function CastQ(myTarget)  
  ...
  if useq then
    ...
    if ThisIsReal(myTarget) > myPlayer.range and GetDistance(myTarget) <= GetCustomRange(_Q) then       
    --if GetDistance(myTarget) <= GetCustomRange(_Q) then
      --if menu.comboextra.range.useNew == 2 or 3 then
        --qTick = GetTickCount() + GetLatency() / 2        
  ...
  --------use---------
    ...
    if unit.isMe then      
        ...
        --print("PROCESS: AA")          
            ...
            DelayAction(function() if spell.Target ~= nil then SendQ() end end, spell.animationTime - GetLatency() / 2000)
          --print("3) AA")
